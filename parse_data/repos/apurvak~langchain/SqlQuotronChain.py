    ...
    def _call(
        ...
        try:
            ...
            if self.return_intermediate_steps:
                chain_result[VisualizationChain.INTERMEDIATE_STEPS_KEY] = intermediate_steps
            return chain_result
...
class SQLDatabaseSequentialChain(Chain):
    ...
    quotron_chain: VisualizationChain.VisualizationChain
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        quotron_chain = VisualizationChain.VisualizationChain.from_llm(llm,database,**kwargs)

        return cls(sql_chain=sql_chain, decider_chain=decider_chain, quotron_chain=quotron_chain, **kwargs)
    ...
    def _call(
        ...
        sql_chain_output = self.sql_chain(
            new_inputs, callbacks=_run_manager.get_child(), return_only_outputs=True
        )
        visualization_inputs = {
            "query": inputs[self.input_key]
        }
        visualization_output = self.quotron_chain(
            new_inputs, callbacks=_run_manager.get_child(), return_only_outputs=True
        )

        chain_result: Dict[str, Any] = {self.output_key: visualization_output}
