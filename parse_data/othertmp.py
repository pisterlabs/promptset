...
class Summary:
    ...
    def init(CONFIG):
        Summary.useGPU = (
            CONFIG.get("DEVICE", "cpu") == "gpu"
            or CONFIG.get("DEVICE", "cpu") == "cuda"
        )
            ...
            if Summary.summarizer == None:
                ...
                Summary.summarizer = [
                    pipeline(
                        ...
                        device=0 if Summary.useGPU else -1,
                    )
    ...
    def summarizeMarkdown(
        ...
        contentLen = length
        if contentLen == None:
            contentLen = Summary.getLength(content, fast=fast)
        ...
        content = Summary.summarizeHTML(
            content, url, min_length, max_length, withCodeBlocks, fast=fast
        )
    ...
    def summarizeText(content, min_length=10, max_length=100, length=None, fast=False):
        contentLen = length
        if contentLen == None:
            contentLen = Summary.getLength(content)
        if contentLen < min_length:
            return content
        if max_length > contentLen:
            max_length = contentLen
    ...
    def summarizeHTML(
        ...
        try:
            ...
            text_summary = " ".join(t.strip() for t in texts)
            text_summary = Summary.summarizeText(
                text_summary, min_length, max_length, fast=fast
            )
...
zz("HIthere")
zz(prompt, temp=0.4)
zz(prompt=prompt, temp=0.4)
zz(prompt="Hi", temp=0.4)
zz("promptinghere", temp=0.4)

from openai import OpenAI

client = OpenAI("SDf")

client.chat.completions.create([asdf], [asdf])
client.completions.create([], [asdf2])
