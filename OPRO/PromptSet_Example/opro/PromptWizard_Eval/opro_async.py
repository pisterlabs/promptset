...
async def get_dataset(CHOSEN_PROMPT, sample_size=40):
    # Check if the synthetic data already exists
    SYNTHETIC_DATA_FILEPATH_ASYNC = f"{PWD}dataset.json"
    assert os.path.exists(SYNTHETIC_DATA_FILEPATH_ASYNC), "dataset not found."
...
async def score(prompts, dataset):
    ...
    for prompt in tqdm(prompts, desc="Scoring"):
        accuracy = 0
        prompt_interpolated = [prompt.format(TEXT=data_pair["question"]) for data_pair in dataset]
        generated_answers = await run_llm_coroutine(prompt_interpolated, temperature=0.0, msg="Scoring - 30 calls mostly")
        assert len(generated_answers) == len(dataset)
        for i in range(len(generated_answers)):
            accuracy += score_helper(generated_answers[i], dataset[i]["answer"])
        prompt_score_pairs[prompt] = accuracy / len(dataset) * 100
...
async def eval_opro(prompt, cache_dir="0", TRAINING_SAMPLE_SIZE=30, TESTING_SAMPLE_SIZE=200, PROMPTS_PER_STEP=20, STEP_COUNT=15, MAX_PROMPT_SCORE_PAIRS=10):
    ...
    datset = await get_dataset(
        CHOSEN_PROMPT, sample_size=TRAINING_SAMPLE_SIZE + TESTING_SAMPLE_SIZE
    )

    # Train-Test Split
    training_sample = datset[:TRAINING_SAMPLE_SIZE]
    testing_sample = datset[
