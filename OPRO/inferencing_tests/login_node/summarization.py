...
"""
    ...
    while not has_correct_keywords(response):
        response = llm.generate_response(
            model="llama3-70b",
            prompt=prompt.format(pairs_str=pairs_str),
            temperature=1.0,
        )
    return response
    ...
    with tqdm(sample, desc=prompt, position=1, leave=False) as pbar:
        for idx, data_pair in enumerate(pbar):
            res = llm.generate_response(
                model="llama3-8b",
                prompt=prompt.format(TEXT=data_pair["text"]),
                temperature=0.0,
            )
...
def opro(CHOSEN_PROMPT, training_sample):
    ...
    if os.path.exists(SAVE_PATH):
        with open(SAVE_PATH, "r") as f:
            results = json.load(
                f
            )  # {step1: {prompt1: score1, prompt2: score2, ...}, step2: {...}, ...}
        return results
    else:
        prompt_score_pairs = {
        ...
        prompt_score_pairs = dict(
            sorted(prompt_score_pairs.items(), key=lambda x: x[1], reverse=True)[
    ...
    for i in range(1, STEP_COUNT + 1):
        ...
        while True:
            try:
                ...
                prompt_score_pairs = dict(
                    sorted(
                        combined_ins_score_pairs.items(),
                        key=lambda x: x[1],
                        reverse=True,
                    )[:MAX_PROMPT_SCORE_PAIRS]
                )
...
def summarization_opro(prompt, TRAINING_SAMPLE_SIZE=10, TESTING_SAMPLE_SIZE=30):
    ...
    best_prompt = max(
        ...
        key=opro_results[str(len(opro_results))].get,
    )
