...
"""
    ...
    while len(new_prompts) < request_count:
        responses = await run_llm_coroutine([prompt for _ in range(request_count)], temperature=1.0, model="gpt-4o")
        for res in responses:
            try:
...
async def score(prompts, testing_sample):
    ...
    for prompt in tqdm(prompts, desc="Scoring"):
        ...
        generated_translation = await run_llm_coroutine(prompt_interpolated, temperature=0.0, model="gpt-4o-mini")
