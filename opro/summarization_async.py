...
async def generate_synthetic_data(CHOSEN_PROMPT, sample_size=40):
    ...
    """
        ...
        while len(data_pairs) < request_count:
            ...
            temperatures = [i/len(prompts) for i in range(len(prompts), 0, -1)]  # Varying temperature to diversify responses (decreasing order so that temp is high when polling)
            response = await run_llm_coroutine(prompts, temperature=temperatures, model="llama3-70b")
            for res in response:
                try:
...
async def summarization_opro(prompt, cache_dir="0", TRAINING_SAMPLE_SIZE=10, TESTING_SAMPLE_SIZE=30, PROMPTS_PER_STEP=20, STEP_COUNT=10, MAX_PROMPT_SCORE_PAIRS=10):
