...
async def translation_opro(prompt, cache_dir="0", TRAINING_SAMPLE_SIZE=10, TESTING_SAMPLE_SIZE=30, PROMPTS_PER_STEP=8, STEP_COUNT=5, MAX_PROMPT_SCORE_PAIRS=20):
    ...
    print("Calculating Test Scores...", flush=True)
    result = {
    ...
    print("Printing Test Scores:", flush=True)
    print(f"Initial Prompt Score: {result['initial_prompt']}", flush=True)
    print(f"Optimized Prompt Score: {result['optimized_prompt']}", flush=True)
    return result
