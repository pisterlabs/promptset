...
def get_content_extracts(results):
    ...
    for url in urls:
        ...
        try:
            content_extracts.append(doc.summary()[:1000])
        except:
            content_extracts.append(None)
        print(f"Extracted content: {content_extracts[-1][:1000]}")
...
def get_genwebsearch_response(question: str) -> str:
    ...
    # Get the contents in a batch
    content_extracts = get_content_extracts(results[:limit])
    ...
    for n, result in enumerate(results[:limit]):
        ...
        try:
            result_content_extract = content_extracts[n]
        except IndexError:
            result_content_extract = None
        if not result_content_extract:
            result_content_extract = result["snippet"]
        summary = summarize_content(
            result_content_extract, result["link"], result["title"], search_query
        )
...
@provider(
    name="SerpAPI (gpt-3.5-turbo)",
    emoji="🪁",
    rate_limit="sequential",
    settings_schema={},
)
def SerpAPIProvider(prompt: str, **kwargs) -> str:
    """ChainForge custom provider that uses SerpAPI for search results & Open AI for generating responses."""
