...
from transformers import OpenAIGPTLMHeadModel, OpenAIGPTTokenizer, OPTForCausalLM
...
def main():
    ...
    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_path,  truncation = True, fast = False)
    ...
    #if '_t' in args.input_type:
    #    special_tokens['additional_special_tokens'].append('<PT>')
    #    special_tokens['additional_special_tokens'].append('<PC>')
    #    special_tokens['additional_special_tokens'].append('<CT>')
    #    special_tokens['additional_special_tokens'].append('<CC>')
    ...
    with open(f"{input_path}/{args.split}.source", "r") as source_data, open(f"{input_path}/{args.split}.target", "r") as dataset_target:
        for sample, target in zip(source_data, dataset_target):
            ...
            #assert len(target_lst) == len(output_lst), print(len(target_lst), len(output_lst), target_lst, output_lst)
