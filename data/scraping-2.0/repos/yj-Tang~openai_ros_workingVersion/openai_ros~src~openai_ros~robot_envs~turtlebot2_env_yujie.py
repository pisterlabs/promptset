...
class TurtleBot2Env(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self, ros_ws_abspath):
        ...
        controllers in the standard fashion. For the moment we reset them.

        To check any topic we need to have the simulation running, we need to do two thins:
        1) unpause the simulation: without that th stream of data doesnt flow. This is for 
        simulations that are pause for whatever the reason
        2) if the simulation was running already for some reason, we need to reset the controllers.
        This has to do with the fact that some plugins with tf, dont understand the reset of the
        simulation and need to be reseted to work properly.
        ...
        * /kobuki/laser/scan: Laser Readings
        ...
        Args:
        """
        rospy.logdebug("Satrt TurtleBot2Env Init...")
    ...
    def _check_all_systems_ready(self):
        """
        chekcs that all the sensors, publishers and other simulation systems are operational.
        """
        self._check_all_sensors_ready()
        return True
    ...
    def _check_all_sensprs_ready(self):
    ...
    def _check_odom_ready(self):
        ...
        while self.odom is NOne and not rospy.is_shutdown():
            try:
                self.odom = rospy.wait_for_message("/odom", Odometry, timesout=5.0)
    ...
    def _check_publishers_connection(self):
        ...
        while self._cmd_vel_pub.get_num_connections()==0 and not rospy.is_shutdown():
            ...
            try:
                ...
                # THIS IS TO avoid when world is rested, time when backwards.
                pass
    ...
    # because they will be used in RobotGazeboEnv GrandParentClass and defined 
    # in the TrainingENvironment.
    ...
    def move_base(self, linear_speed, angular_speed, epsilon=0.05, update_rate=10, min_laser_distance=-1):
        ...
        time.sleep(0.2)
        """
        self.wait_until_twist_achieved(cmd_vel_value, epsilon, update_rate, min_laser_distance)
        """
    ...
    def has_crashed(self, min_laser_distance):
        ...
        Crashed means that the minimum laser reading is lower than the ,min_laser_distance value given.
        If min_laser_distance == -1, it returns always false, because its the way to deactivate this check.
        """
