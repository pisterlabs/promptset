...
AZURESEARCH_DIMENSIONS = int(os.environ.get('AZURESEARCH_DIMENSIONS', 1536))
...
def get_search_client(endpoint: str,
                      key: str,
                      index_name: str,
...
class AzureSearch(VectorStore):
    'Helper Class to support Azure Search vector store.'
    def __init__(
        self,
        ...
        **kwargs: Any,
        ...
        self.client = get_search_client(
            self.azure_cognitive_search_name,
            self.azure_cognitive_search_key,
            self.index_name,
    ...
    def similarity_search_with_score(
        self, query: str, k: int = 4, filters: str = ''
...
class AzureSearchVectorStoreRetriever(BaseRetriever, BaseModel):
    'Helper class to retrieve documents from Azure Search'
