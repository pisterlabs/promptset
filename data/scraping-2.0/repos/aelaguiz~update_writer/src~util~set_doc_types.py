...
if __name__ == "__main__":
    ...
    for idx, (doc_id, metadata, doc_contents, embeddings) in enumerate(docdb_docs):
        ...
        if metadata.get('type'):
            # logger.debug(f"Type is alerady set on document {doc_id} {idx}/{len(docdb_docs)}: {metadata.get('type')}")
            continue

        if metadata.get('tweet_id'):
            metadata['type'] = 'tweet'
        elif metadata.get('email_id'):
            metadata['type'] = 'email'
        elif metadata.get('channel'):
            metadata['type'] = 'slack'
        else:
            logger.error(f"Could not determine type for document {doc_id} {idx}/{len(docdb_docs)}: {metadata}")
            continue

        logger.debug(f"Updating document {doc_id} {idx}/{len(docdb_docs)}: {metadata['type']}")

        docdb._collection.update(
            ids=[doc_id],
            metadatas=[metadata],
        )