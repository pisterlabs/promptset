...
class OpenRouterEvalArguments(argparse.Namespace):
    completion_fn: str
    eval: str
...
def run(args: OpenRouterEvalArguments, registry: Optional[Registry] = None) -> str:
    if args.debug:
    ...
    completion_fn_instances = [registry.make_completion_fn(url, api_key=args.api_key, model=args.model) for url in completion_fns]
    run_config = {
...
def main() -> None:
    parser = get_parser()
    args = cast(OpenRouterEvalArguments, parser.parse_args(sys.argv[1:]))
    logging.basicConfig(
