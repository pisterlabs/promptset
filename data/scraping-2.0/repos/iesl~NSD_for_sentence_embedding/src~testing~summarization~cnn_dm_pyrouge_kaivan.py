...
def greedy_selection(sent_words_sim, top_k_max, sent_lens=None, sent_embs_words_sim=None):
    ...
    #if word_norm_emb is not None:
    #    matching_dist = (-sent_words_sim.sum(dim=1) * word_norm_emb.sum(dim=1))
    #if sent_embs_words_sim is not None and word_norm_emb is not None:
    #    matching_dist = (-word_norm_emb.sum(dim=1) * sent_embs_words_sim.sum(dim=1))
    else:
...
def select_by_topics(basis_coeff_list, all_words_tensor, w_freq_tensor, top_k_max, device, sent_lens=None,
                     freq_w_tensor=None, sent_embs_tensor=None, second_sent_emb_format = 'None'):
...
def rank_sents(basis_coeff_list, article, citations, word_norm_emb, word_d2_idx_freq, top_k_max, device):
    ...
    if 'embs' in args.method_set:
        ...
        m_d2_sent_ranks['norm_w_in_sent_freq_4+sent_emb'] = select_by_topics(cit_w_emb_tensors_list, all_words_tensor,
                                                                    w_freq_tensor, top_k_max, device, cit_sent_lens,
                                                                    freq_w_tensor=freq_w_4_tensor, 
                                                                    sent_embs_tensor=cit_sent_embs_w_tensor, second_sent_emb_format = 'Single')
    ...
    if basis_coeff_list is not None:
        ...
        m_d2_sent_ranks['ours_freq_4+norm_w'] = select_by_topics(basis_coeff_list, all_words_tensor, w_freq_tensor, top_k_max,
                                                          device, sent_lens=None, freq_w_tensor=freq_w_4_tensor,
                                                          sent_embs_tensor=cit_w_emb_tensors_list, second_sent_emb_format = 'Multi')
