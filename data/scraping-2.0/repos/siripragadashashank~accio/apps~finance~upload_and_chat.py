...
def get_vectorstore(text_chunks=None):
    ...
    collection = client.create_collection(f"accio_financial_temp_{unq_id}")
    ...
    vectorstore = Chroma(
        client=client,
        collection_name=f"accio_financial_temp_{unq_id}",
...
def app():
    ...
    user_question = st.text_input("Ask a question about your documents: (Model: Llama2-7b · "
    ...
    with st.sidebar:
        ...
        temperature = st.slider("Temperature", min_value=0.01, max_value=1.0, step=0.1, key="temperature_uc")
        max_new_tokens = st.slider("Max Tokens", min_value=100, max_value=500, step=50, key="max_new_tokens_uc")
