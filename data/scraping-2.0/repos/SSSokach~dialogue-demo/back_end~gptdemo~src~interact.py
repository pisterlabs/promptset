...
def run():
    ...
    emotion_map = {"n": 13081, "h": 13082, "s": 13083, "a": 13084, "d": 13085}
    emotion_map_ = {13081: "[neutral]", 13082: "[happiness]", 13083: "[sadness]", 13084: "[anger]", 13085: "[disgust]"}
    ...
    while True:
        ...
        if raw_text.strip() == 'clear':
            history = [[emotion]]
            print(emotion_map_[history[0][0]], history[1:])
            continue
        elif raw_text.strip() in ["n", "h", "s", "a", "d"]:
            emotion = emotion_map[raw_text.strip()]
            history = [[emotion]] + history[1:]
            print(emotion_map_[history[0][0]], history[1:])
            continue
        raw_text = " ".join(list(raw_text.replace(" ", "")))
        history.append(tokenize(raw_text))
        history = history[0:1] + history[1:][-(2 * args.max_history + 1):]
        print(emotion_map_[history[0][0]], history[1:])
        with torch.no_grad():
