...
# Modified date : 2019-03-23 18:27
# Author : DARREN
# Describe : not set
...
from openai_gpt2 import GPT2Tokenizer
...
def _tokenize_and_encode(obj, tokenizer):
    ...
    if isinstance(obj, str):
        ret = tokenizer.tokenize(obj)
        return tokenizer.convert_tokens_to_ids(ret)
    elif isinstance(obj, int):
        return obj
    return list(_tokenize_and_encode(o, tokenizer) for o in obj)

def _gpt2_tokenize_and_encode(obj,tokenizer):
    """ Tokenize and encode a nested object """
    if isinstance(obj, str):
        ret = tokenizer.encode(obj)
        return ret
    elif isinstance(obj, int):
        return obj
    return list(_gpt2_tokenize_and_encode(o, tokenizer) for o in obj)
...
#       pylog.info(len(tensor_datasets[1]))
#       for i in range(len(tensor_datasets[1])):
#           item = tensor_datasets[1][i]
#           pylog.info(item.size())
#           pylog.info(item)
        return tensor_datasets

    elif config["model_name"] == "gpt2":
        special_tokens_lt = config["special_tokens_lt"]
        tokenizer = GPT2Tokenizer.from_pretrained(config=config)
#       for token in special_tokens_lt:
#           pylog.info(token)
#           ret = tokenizer.decode(token)
#           pylog.info(ret)
#       special_tokens_ids = [tokenizer.encoder(token) for token in special_tokens_lt]
#       pylog.info(special_tokens_ids)
        # encode the datasets
        encoded_datasets = _get_encoded_datasets(tokenizer, config)
        # Compute the max input length for the Transformer
        max_length, input_length = _get_max_and_input_length(model, encoded_datasets)
        # Prepare inputs tensors and dataloaders
        tensor_datasets = _gpt2_pre_process_datasets(encoded_datasets, input_length, max_length)
#       pylog.info(len(tensor_datasets[1]))
#       for i in range(len(tensor_datasets[1])):
#           item = tensor_datasets[1][i]
#           pylog.info(item.size())
...
def _gpt2_pre_process_datasets(encoded_datasets, input_len, cap_length):
    """ Pre-process datasets containing lists of tuples(story, 1st continuation, 2nd continuation, label)
