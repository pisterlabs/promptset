...
data_file = './data/ComplexWebQuestions/ComplexWebQuestions_dev.json.pkl' # input the file path
# data_file = './data/ComplexWebQuestions/ComplexWebQuestions_test.json.pkl'
if 'dev' in data_file:
...
for item in tqdm(data):
    ...
    if '%s_sub_sentences' % (key) not in item:
        try:
            sub_sentences = query_gpt3(item[key])
        except:
            time.sleep(60)
            sub_sentences = query_gpt3(item[key])
        if sub_sentences is None:
            sub_sentences = []
        item['%s_sub_sentences' % (key)] = sub_sentences

    if item['%s_sub_sentences' % (key)] != []:
        num_save_examples += 1
    ...
    print('-' * 20)
    print('sentence:', item[key])
    print('')
    for sub_sentence in item['%s_sub_sentences' % (key)]:
        print(sub_sentence)
