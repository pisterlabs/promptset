...
import numpy as np
from gym3.types import DictType, Discrete, TensorType, Real
...
class ActionMapping(abc.ABC):
    ...
    def __init__(self, n_camera_bins: int = 11):
        ...
        self.stats_ac_space = DictType(
            **{
                "buttons": TensorType(shape=(len(Buttons.ALL),), eltype=Real(4)),    ###### shape=(len(Buttons.ALL),), eltype=Real(4)),    ###### Discrete(2)),
...
class CameraHierarchicalMapping(ActionMapping):
    ...
    def _precompute_to_factored(self):
        ...
        for jnt_ac, button_comb in self.BUTTONS_IDX_TO_COMBINATION.items():
            ...
            #print("len(new_button_ac): ", len(new_button_ac))
    ...
    def get_action_space_update(self):
        print("action_mapping.py get_action_space_update: self.camera_combinations: ", len(self.camera_combinations))
        #print("action_mapping.py get_action_space_update: self.camera_combinations: ", self.camera_combinations) 
        print("action_mapping.py get_action_space_update: self.BUTTONS_COMBINATIONS: ", len(self.BUTTONS_COMBINATIONS))
        return {
            "camera": TensorType(shape=(1,), eltype=Discrete(len(self.camera_combinations))),
            "buttons": TensorType(shape=(1,4), eltype=Real(4)), ###### changed len(self.BUTTONS_COMBINATIONS) to 4, and eltype=Discrete(4)) to Real 
        }
