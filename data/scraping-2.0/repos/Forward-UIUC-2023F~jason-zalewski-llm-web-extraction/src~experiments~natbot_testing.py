...
import natbot_prompt_program # For allowing custom generated code with natbot

import sys
sys.path.append('/Users/jasonz/forward_data_lab_llmie/forward_data-llm_ie/')
from src import main # For allowing custom extract text to json output to json file
from src import web_extractor # might not need to import
...
if (
	...
	_crawler = Crawler()
	openai.api_key =  openai_api_key= openai.api_key
...
try:
        while True:
            ...
            if command == "r" or command == "":
                ...
                _crawler.click(id)
                time.sleep(1)
            elif command == "program":  # custom command to execute code 
                prompt_text = input("What Task do you want to be solved by a generated program?: ")
                print(natbot_prompt_program.natbot_generate_code(prompt_text, _crawler.page.url))
                time.sleep(1)
            elif command == "extract_text": # custom command to extract text and put into json file in json format
                prompt_text = input("What do you want to be extracted from the webpage?: ")
                prompt_text = "You have to output your response in JSON format and make sure its valid JSON: " + prompt_text
                main.ExtractAndAppendToJSON(_crawler.page.url, prompt_text, "natbot_extract_text_json")
                time.sleep(1)
            elif command == "o":
