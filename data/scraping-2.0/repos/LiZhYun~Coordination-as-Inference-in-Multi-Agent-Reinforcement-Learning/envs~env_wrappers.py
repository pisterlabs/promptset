...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            all_last_actions = []
            ob, s_ob, reward, done, info, available_actions, all_last_actions = env.step(data)
            if 'bool' in done.__class__.__name__:  # 类名
                if done:
                    ob, s_ob, available_actions, all_last_actions = env.reset()
            else:
                if np.all(done):
                    ob, s_ob, available_actions, all_last_actions = env.reset()
            remote.send((ob, s_ob, reward, done, info, available_actions, all_last_actions))
        elif cmd == 'reset':
            ob, s_ob, available_actions, all_last_actions = env.reset()
            remote.send((ob, s_ob, available_actions, all_last_actions))
        elif cmd == 'reset_task':
            ob = env.reset_task()
            remote.send(ob)
        elif cmd == 'render':
...
class ShareSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        self.action_dim = self.remotes[0].recv()
    ...
    def reset(self):
        for remote in self.remotes:
            remote.send(('reset', None))
        results = [remote.recv() for remote in self.remotes]
        obs, share_obs, available_actions, all_last_actions = zip(*results)
        return np.stack(obs), np.stack(share_obs), np.stack(available_actions), np.stack(all_last_actions)
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        results = [env.step(a) for (a, env) in zip(self.actions, self.envs)]
        obs, share_obs, rews, dones, infos, available_actions, all_last_actions = map(
            np.array, zip(*results))

        for (i, done) in enumerate(dones):
            if 'bool' in done.__class__.__name__:
                if done:
                    obs[i], share_obs[i], available_actions[i], all_last_actions[i] = self.envs[i].reset()
            else:
                if np.all(done):
                    obs[i], share_obs[i], available_actions[i], all_last_actions[i] = self.envs[i].reset()
        self.actions = None

        return obs, share_obs, rews, dones, infos, available_actions, all_last_actions

    def reset(self):
        results = [env.reset() for env in self.envs]
        obs, share_obs, available_actions, all_last_actions = map(np.array, zip(*results))
        return obs, share_obs, available_actions, all_last_actions
    ...
    def render(self, mode="human"):
        if mode == "rgb_array":
            ...
            for env in self.envs:
                env.render(mode=mode)
                # env.sim.render(mode='window', camera_name='first-person', width=16, height=16, depth=False)
                # env.render(mode=mode)
        else:
            raise NotImplementedError
