# this is a copy of pbrain/eegview/utils.py
...
def read_cohstat(fh):
    ...
    while 1:
        ...
        cxy[(i,j)] = array(list(map(float, vals[1:])))
    ...
    while 1:
        ...
        phases[(i,j)] = array(list(map(float, vals[1:])))
...
def ij_across_eois(eoi1, eoi2, amp):
    ...
    if len(ind1)<len(ind2):
        for i in ind1:
	    seen[i]=1
        ind2uniq=[j for j in ind2 if j not in seen]
        indeoi1=ind1
        indeoi2=ind2uniq
    else:
	for i in ind2:
	    seen[i]=1
        ind1uniq=[j for j in ind1 if j not in seen]
        indeoi1=ind1uniq
        indeoi2=ind2
    ij = []
    for i in indeoi1:
...
def electrode_pairs_across_eois(eoia, eoib, amp):
    ...
    def smaller_one_first(eoi1, eoi2):
        ...
        eoi2u = [ e for e in eoi2 if e not in seen]
...
def convert_ebersole(filein, fileout):
    ...
    fo = open(fileout, 'wb')
    for line in fi:
        a = array('f', list(map(float, line.split(',')[1:])))
        fo.write(a.tostring())
...
def gen_surrogate_data(eeg, tmin, tmax, eoi, filters, numSurrs) :
    ...
    for i, pair in enumerate(randInds) :
        ...
        for j, tup in enumerate(filters.items()) :
            ...
            fsurr1 = list(filter(surr1))
            fsurr2 = list(filter(surr2))
