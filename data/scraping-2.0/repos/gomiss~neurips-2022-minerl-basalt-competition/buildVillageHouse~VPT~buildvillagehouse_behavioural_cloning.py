...
from openai_vptV3.agent import PI_HEAD_KWARGS, MineRLAgent
from data_loaderV3 import DataLoader
from openai_vptV3.lib.tree_util import tree_map
...
EPOCHS = 15 if USING_FULL_DATASET else 2
...
MAX_BATCHES = 50000 if USING_FULL_DATASET else int(1e9)
...
def behavioural_cloning_train(data_dir, in_model, in_weights, out_weights, new_output=True):
    ...
    if new_output:
        old_pi_head_buttons_model_key = [
            'pi_head.buttons.linear_layer.weight',
            'pi_head.buttons.linear_layer.bias'
        ]
        model_weights = th.load(in_weights, map_location=DEVICE)
        new_model_weights = OrderedDict({k: v for k, v in model_weights.items() if k not in old_pi_head_buttons_model_key})
        agent.load_weights(path=None, weights = new_model_weights)

    else:
        agent.load_weights(path=in_weights)
    ...
    if new_output:
        original_agent.load_weights(path=None, weights = new_model_weights)
    else:

        original_agent.load_weights(path=in_weights)
    ...
    for batch_i, (batch_images, batch_actions, batch_episode_id) in tqdm(enumerate(data_loader)):
        # print(batch_i)
        batch_loss = 0
        batch_false = 0
        batch_true = 0
        for image, action, episode_id in zip(batch_images, batch_actions, batch_episode_id):
            ...
            if batch_i % LOSS_REPORT_RATE == 0 and batch_i > 0:
                pred_action = agent.policy.pi_head.sample(pi_distribution, deterministic=not True)
                if (pred_action['buttons'].squeeze() == agent_action['buttons'].squeeze()):
                    batch_true += 1
                else:
                    batch_false += 1
        ...
        loss_sum += batch_loss
        if batch_i % LOSS_REPORT_RATE == 0 and batch_i > 0:
            acc = batch_true / (batch_true + batch_false)
            time_since_start = time.time() - start_time
            print(f"Time: {time_since_start:.2f}, Batches: {batch_i}, Avrg loss: {loss_sum / LOSS_REPORT_RATE:.4f}, Acc:{acc:.2f}")
            loss_sum = 0
            if batch_i>1000:
