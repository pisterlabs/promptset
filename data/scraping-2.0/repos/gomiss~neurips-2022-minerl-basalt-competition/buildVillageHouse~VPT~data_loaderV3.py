...
from openai_vptV3.agent import ACTION_TRANSFORMER_KWARGS, resize_image, AGENT_RESOLUTION
...
def json_action_to_env_action_moving(json_action):
    """
    Converts a json action into a MineRL action.
    Returns (minerl_action, is_null_action)
    """
    ...
    for key in keyboard_keys:
        ...
        #      "inventory" action if GUI was open.
        #      Not doing it here, as BASALT uses ESC to quit the game.
        if key in KEYBOARD_BUTTON_MAPPINGMoving:
            env_action[KEYBOARD_BUTTON_MAPPINGMoving[key]] = 1
            is_null_action = False
...
def data_loader_worker(tasks_queue, output_queue, quit_workers_event):
    ...
    while True:
        ...
        for i in range(len(json_data)):
            ...
            action, is_null_action = json_action_to_env_action_moving(step_data)
...
class DataLoader:
    ...
    """
    def __init__(self, dataset_dir_list, n_workers=8, batch_size=8, n_epochs=1, max_queue_size=16):
        assert n_workers >= batch_size, "Number of workers must be equal or greater than batch size"
        self.dataset_dir = dataset_dir_list
        ...
        for dataset_dir in dataset_dir_list:
            ...
            for unique_id in unique_ids:
                ...
                self.json_path.append(json_path)
...
def actionData_loader_worker(tasks_queue, output_queue, quit_workers_event):
    """
    Worker for the data loader.
    """
    cursor_image = cv2.imread(CURSOR_FILE, cv2.IMREAD_UNCHANGED)
    ...
    while True:
        ...
        for i in range(video_length):
            ...
            reward = i/video_length * 10 + 1
            reward = np.array([reward])
            reward = np.expand_dims(reward,axis=1)
            # reward = th.tensor([reward]).unsqueeze(1)
            ...
            if ret:
                ...
                frame = np.asarray(np.clip(frame, 0, 255), dtype=np.uint8)
                frame = resize_image(frame, AGENT_RESOLUTION)
                output_queue.put((trajectory_id, frame, reward), timeout=QUEUE_TIMEOUT)
            else:
                print(f"Could not read frame from video {video_path}")
        video.release()
        # Signal that this task is done
...
class ActionsDataLoader(DataLoader):
    """
    Generator class for loading batches from a dataset
    ...
    def __init__(self, dataset_dir, n_workers=8, batch_size=8, n_epochs=1, max_queue_size=8):
        ...
        for unique_id in unique_ids:
            video_path = os.path.abspath(os.path.join(dataset_dir, unique_id + ".mp4"))
            # json_path = os.path.abspath(os.path.join(dataset_dir, unique_id + ".jsonl"))
            demonstration_tuples.append(video_path)
        ...
        self.processes = [
            Process(
                target=actionData_loader_worker,
                args=(
                    self.task_queue,
                    output_queue,
                    self.quit_workers_event,
    ...
    def __next__(self):
        ...
        for i in range(self.batch_size):
            workitem = self.output_queues[self.n_steps_processed % self.n_workers].get(timeout=QUEUE_TIMEOUT)
            # workitem = self.output_queues[0].get(timeout=QUEUE_TIMEOUT)
            if workitem is None:
                # Stop iteration when first worker runs out of work to do.
                # Yes, this has a chance of cutting out a lot of the work,
                # but this ensures batches will remain diverse, instead
                ...
                # one worker outputs all samples to the same batch.
                raise StopIteration()
            trajectory_id, frame, reward = workitem
            batch_frames.append(frame)
            batch_rewards.append(reward)
            batch_episode_id.append(trajectory_id)
            self.n_steps_processed += 1
        return batch_frames, batch_rewards, batch_episode_id
...
if __name__=="__main__":
    ...
    测试use动作的dataloader
    """
    # data_dir = "/tmp/actionsVideo"
    # N_WORKERS = 8
    # BATCH_SIZE = 8
    # EPOCHS = 1
    
    # action_data_loader = ActionsDataLoader(
    ...
    # for batch_i, (batch_images, batch_rewards, batch_episode_id) in enumerate(action_data_loader):
    #     batch_loss = 0
    #     for image, reward, episode_id in zip(batch_images, batch_rewards, batch_episode_id):
    #         print(batch_i, reward, episode_id)
    ...
    data_dir = "/tmp/MineRLBasaltBuildVillageHouse-v0"


    env = gym.make("MineRLBasaltFindCave-v0")
    agent = MineRLAgent(env, device='cuda', policy_kwargs=None, pi_head_kwargs=None)
    env.close()

    buttons_actions_num_dict = defaultdict(int)
    json_path_list = glob.glob(os.path.join(data_dir, "*.jsonl"))
    ...
    for json_path in tqdm(json_path_list):
        ...
        for i in range(len(json_data)):
            ...
            buttons_actions_num_dict[int(agent_action["buttons"].squeeze())] += 1
            # print(agent_action,buttons_actions_num_dict)

    with open("action_stat.json", 'w') as f:
        json.dump(buttons_actions_num_dict, f)
