...
class CameraHierarchicalMapping(ActionMapping):
    ...
    def __init__(self, *args, **kwargs):
        self.legal_action = self.get_legal_mask() if kwargs.pop('use_legal_action', False) else None
    ...
    def get_legal_mask(self):
        legal_actions = np.ones(len(self.BUTTONS_COMBINATIONS), dtype=np.int)
        remove_buttons = OrderedDict(
            hotbar=[f"hotbar.{i}" for i in range(1, 10)],
            fore_back=[],
            left_right=[],
            sprint_sneak=["sprint", "sneak"],
            use=['use'],
            drop=["drop"],
            attack=["attack"],
            jump=[],
        ...
        remove_buttons_list = list(remove_buttons.values())
        button_len = len(self.BUTTONS_COMBINATIONS[0])
        for i, button in enumerate(self.BUTTONS_COMBINATIONS):
            remove = False
            for j in range(button_len):
                if button[j] in remove_buttons_list[j]:
                    remove = True
                    break
