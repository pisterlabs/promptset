...
def write_to_csv(processed_data, error_indices):
    ...
    data.to_csv(f"{checkpoint_dir}/data_checkpoint{len(processed_data)}.csv", index=False)
    error_df.to_csv(f"{checkpoint_dir}/error_checkpoint{len(processed_data)}.csv", index=False)
