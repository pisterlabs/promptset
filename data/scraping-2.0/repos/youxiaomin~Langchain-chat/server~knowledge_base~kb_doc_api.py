...
from configs import (DEFAULT_VS_TYPE, EMBEDDING_MODEL,
                    ...
                    logger, log_verbose,)
from server.utils import BaseResponse, ListResponse, run_in_thread_pool, ChatMessage
...
def search_role_docs(role_name: str = Body(..., description="role_name", example="kb1"),
        blogerid: str = Body(..., description="Knowledge Base Name", example="kb1"),
        taskid: str = Body(..., description="Knowledge Base Name", example="kb1"),
        blog: str = Body(..., description="Knowledge Base Name", example="kb1"),
        comment: str = Body(..., description="Knowledge Base Name", example="kb1"),
	) -> ChatMessage:
    doc_res = []
    doc_res_str = []
    juese_name = role_name#role_config.roleId2Name.get(blogerid, "")
    kb_id = role_config.role2kb.get(juese_name, '')
    kb = KBServiceFactory.get_service_by_name(kb_id)
    if kb is None:
        return []

    #juben_words = role_config.role2keywords.get(juese_name, [])
    #for word in juben_words:
    #    jieba.add_word(word, freq=None, tag=None)
    #seg_list = jieba.cut(blog + ' ' + comment, cut_all=False)
    #seg_list = set([word for word in seg_list if word not in stop_words])
    comment = re.sub(r'#.*?#', '', comment)
    blog = re.sub(r'#.*?#', '', blog)

    if len(blog) > 4:
        docs = kb.search_docs(blog, VECTOR_SEARCH_TOP_K, SCORE_THRESHOLD)
        relate_doc = [DocumentWithScore(**x[0].dict(), score=x[1]) for x in docs]
        doc_content = [doc.page_content for doc in relate_doc]
        doc_res += [doc_content]
        doc_res_str += "".join(doc_content)
        print(blog, doc_content, [doc.score for doc in relate_doc])

    comment=comment.replace("你", role_name)
    sent_list=re.split(r"[,，。；：！？ \[\]\…]", comment)
    sent_list=[sent for sent in sent_list if sent not in ',，。；：！？ \[\]\…']
    for sent in sent_list:
        if len(sent) < 5: continue
        #inter = [word for word in juben_words if word in sent] if len(inter)<1: continue
        docs = kb.search_docs(sent, VECTOR_SEARCH_TOP_K, SCORE_THRESHOLD)
        relate_doc = [DocumentWithScore(**x[0].dict(), score=x[1]) for x in docs]
        doc_content = [doc.page_content for doc in relate_doc]
        doc_res += [doc_content]
        doc_res_str += "".join(doc_content)
        print(sent, doc_content, [doc.score for doc in relate_doc])

    rank_res=[]
    for rank in range(VECTOR_SEARCH_TOP_K):
        for alist in doc_res:
            if len(alist) > rank:
                if alist[rank] not in rank_res:
                    rank_res.append(alist[rank])

    return ChatMessage(
            question=blog + ' ' + comment,
            response=''.join(doc_res_str),
            history=[],
            source_documents=rank_res,
        )
