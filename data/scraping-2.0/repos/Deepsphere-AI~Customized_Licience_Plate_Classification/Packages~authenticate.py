...
def authenticate_check():
    ...
    with open("C:/Users/ds_007/anaconda3/envs/VehClass/prompt_two.txt",'r') as f:
        prompt_two = f.read()
    ...
    with c13:
        if len(file_uploaded)!=0:
            Name = st.text_input("Enter Name of owner")
    with c11:
        st.write("")
    with c14:
        st.write("")
    ...
    with c12:
        ...
        st.write("")
    with c13:
        if pb_select  in st_list1 and len(file_uploaded)!=0 and Name!="":
            Check_button = st.button("Authenticate")
            if Check_button == True:
                text = text_extraction(image)
                # for text_list in Nested_list:
                st.success(text_list)
                try:
                    response = Ceck(Name,text) # Sending text to GPT3 for response
                    st.markdown(sp)
                    response = response.replace("\n","") # Removing \n from response
                    sp = response.split("|") # Apply split with respect to |
                    if  '' in sp: # Removing empty spaces
                        sp.remove('')
                    if len(sp)>=6: # To display a table(dataframe) we should atleast have 21 elements(strings) 7 rows * 3 Columns
                        category = []
                        probability = []
                        reason = []
                        i = 0
