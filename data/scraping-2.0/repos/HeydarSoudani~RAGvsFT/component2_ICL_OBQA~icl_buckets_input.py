...
def main():
    ...
    parser.add_argument('--output_resutls_dir', type=str)
    parser.add_argument('--output_resutls_filename', type=str)
    ...
    resutls_path = os.path.join(args.output_resutls_dir, args.output_resutls_filename)
    
    with open(resutls_path, 'w', newline='') as file:
        tsv_writer = csv.writer(file, delimiter='\t')
        tsv_writer.writerow(["Title", "accuracy"])
        ...
        for relation_name, relation_data in query_data.items():
            for bk_name, bk_data in relation_data.items():
                ...
                eval_res = [relation_name+'_'+bk_name] + [sample.is_correct.mean()]
