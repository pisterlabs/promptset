...
from boxes.utils import num_tokens_from_string
...
def get_with_completion_api(engine, prompt, temperature, max_length):
    ...
    response = openai.Completion.create(
        ...
        max_tokens=max_length - num_tokens,
    )
    return response.choices[0].text.strip()


def get_with_chat_completion_api(engine, messages, temperature):
        ...
        if not output_path.is_file():
            ...
            try:
                if chat_completion:
                    ...
                    output = get_with_chat_completion_api(engine, messages, temperature)
                else:
                    prompt = system_msg + "\n\n" + 'Now give me a Statement for below description:\n' \
                             + f'Description: {sentence}'
                    output = get_with_completion_api(engine, prompt, temperature, max_length)
            except openai.error.OpenAIError as e:
                print(e)
                if e.args[0] == 'This is not a chat model and thus not supported in ' \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_base_path', type=str,
                        default='results/complex-boxes-dataset/plaintext/ada/',
                        help='Base path for prediction outputs.')
    parser.add_argument('--engine', type=str, default="ada", help='OpenAI engine to use.')
    parser.add_argument('--temperature', type=float, default=0, help='Temperature setting for OpenAI model.')
    parser.add_argument('--max_length', type=int, default=2049, help='Model context max length.')
