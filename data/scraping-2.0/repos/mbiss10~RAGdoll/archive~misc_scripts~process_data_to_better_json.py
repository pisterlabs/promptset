...
for section in cs_courses:
    ...
    if course_name not in courses_processed:
        doc = Document(page_content=section["descriptionSearch"], metadata={
            "course_prefix": "CSCI",
            "course_number": course_num,
            "course_title": section["titleLong"],
            "source_url": course_url})
        cs_docs.append(doc)
        courses_processed.add(course_name)


# with open(f"{DEV_SET_PATH}/temp/cs_course_descriptions_docs.txt", "w") as f:
#     f.write(str(cs_docs))
