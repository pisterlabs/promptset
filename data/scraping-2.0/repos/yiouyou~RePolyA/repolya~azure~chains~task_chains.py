...
from repolya.azure import utils
from repolya.azure.chains import prompts
...
class TaskChains:
    ...
    def uiInputText(cls, task, code_snippets):
        ...
        return utils.refine(code), _token_cost
    ...
    def promptTemplate(cls, task, code_snippets):
        ...
        return json.loads(res), _token_cost
...
with st.chat_message("assistant"):
    ...
    def chat(cls, task):
        ...
        res = res.replace("'''",'"""')
        res = res[res.find("{") : res.rfind("}") + 1]
        return json.loads(res,strict=False), _token_cost
