...
path = os.path.dirname("C:\Users\Admin\Desktop\Medico\MDxApp\01_🏥_Diagnosis_Assistant.py")
...
if 'lang_select' in st.session_state:
    lang = st.sidebar.selectbox(transl[st.session_state['lang_select']]
                                ["language_selection"], options=list(transl.keys()), key='lang_select')
else:
    lang = st.sidebar.selectbox(transl[st.session_state['lang_tmp']]["language_selection"], options=list(
        transl.keys()), key='lang_select')
...
# Declare lists
genders_list = ["{}".format(transl[lang]['male']),
                "{}".format(transl[lang]['female'])]
pregnant_list = ["{}".format(transl[lang]['no']),
                 "{}".format(transl[lang]['yes'])]
##
...
with col1:
    ...
    st.radio("**{}**".format(transl[lang]
             ['gender']), genders_list, key='gender')
# Age selector
with col2:
    st.number_input(
...
with col3:
    ...
    st.radio("**{}**".format(transl[lang]['pregnant']), pregnant_list,
             disabled=st.session_state.disabled, key='pregnant')
...
submit_button = st.button(
    '**{}**'.format(transl[lang]['submit']), help=":green[**{}**]".format(transl[lang]['submit_help']))
st.write('')

st.subheader(
    ":computer: :speech_balloon: :pill: **{}**".format(transl[lang]['diagnostic']))
if submit_button:
    if report_list[1] == "{}".format(transl[lang]['none']):
        ...
        with st.spinner('{}'.format(transl[lang]['submit_wait'])):
            try:
                st.session_state.diagnostic = openai_create(
                    prompt=question_prompt)
                st.write('')
                st.write(st.session_state.diagnostic.replace(
                    "<|im_end|>", ""), unsafe_allow_html=True)
                st.markdown(
                    """
