...
for location in locations[1:]:
    ...
    # for image_index in range(len(new_images), image_count):
        # # Generate new image URL
        # new_image_url = generate("Picture of a happy guy with a vape")

    # # Check if the new image URL is already in new_images
        # if new_image_url not in [image['src'] for image in new_images]:
            # # Append the new image URL to new_images
            # new_images.append({'src': new_image_url, 'name': 'Happy guy with a vape.'})

    # # Assign new_images to product['images']
