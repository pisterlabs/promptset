...
# submit the answer to part 1 [2023-11-27 17:21] this should be implemented next
# open a browser with the scoreboard to screenshot

# fetch part 2 (a few retries with a little time in between if it fails the first time)
# make the api call to openai to get the python code (the call should have examples) (need different gpt_messages.json)
# run the code (with eval?) on the input and get the answer
# submit the answer to part 2
# open a browser with the scoreboard to screenshot

# make it so we can switch to a different model (most capable GPT4 model) and make a bigger context (more examples)

class AutoSolver:
    def __init__(self, year, day):
        self.prevent_calls_to_aoc_website = False
