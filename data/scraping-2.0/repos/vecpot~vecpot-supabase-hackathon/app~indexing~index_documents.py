...
from models import openai_embedding
...
class Indexer:
    ...
    def index_documents(documents: List[BasicDocument]):
        ...
        with Session() as session:
            ...
            if documents_to_delete:
                ...
                Indexer.remove_documents(documents_to_delete, session, documents[0].namespace)
        ...
        embeddings = []
        for para in paragraph_contents:
            emb_vector = openai_embedding(para)
            embeddings.append(emb_vector.tolist())
        embeddings = torch.Tensor(embeddings)
        
        # Add the embeddings to the index
        logger.info(f"Updating Pinecone index...")
        PineconeIndex.get().update(paragraphs, embeddings, documents[0].namespace)
    ...
    @staticmethod
    def remove_documents(documents: List[Document], session=None, namespace:str = None):
        ...
        logger.info(f"Removing documents from pinecone index...")
        PineconeIndex.get().remove(paragraph_ids, namespace)
