...
def soft_pair(candidate):
  ...
  valid_len_list = []
  for v in valid_tag_list:
  # for item in candidate[valid_tag_list]:
    valid_len_list.append(len(candidate[v].label))
...
def lf_mixup(mixdata, sentence1_idx, start_idx1, sentence2_idx, start_idx2, hyper_lambda):
  ...
  # mixup
  for i in range(min(mix_len,len(new_seq1)-start_idx1,len(new_seq2)-start_idx2)):
    # print(len(new_seq1),start_idx1,i,flush=True)
  ...
  # substitution
  for i in range(min(mix_len,len(new_seq1),len(new_seq2))):
    try:
        ...
        for ii in range(len(sub_sequence_i)):
          for jj in range(len(sub_sequence_j)):
            if sub_sequence_i[ii] == sub_sequence_j[jj]:
              ...
              new_seq1_tag = []
              new_seq2_tag = []
              for i, item in enumerate(newseq1_tag):
                  new_seq1_tag.append(tag2onehot(item))
              for i, item in enumerate(newseq2_tag):
                  new_seq2_tag.append(tag2onehot(item))                   
                # padding
              while len(new_seq1_tag) < args.max_seq_length:
                  new_seq1_tag.append(np.zeros(len(label_map)+1))
              while len(new_seq2_tag) < args.max_seq_length:
                  new_seq2_tag.append(np.zeros(len(label_map)+1))
              # add newseq1
...
def slack_mixup(mixdata, sentence1_idx, sentence2_idx, start_idx1, start_idx2, hyper_lambda):
  ...
  # print(new_seq1_tag, new_seq2_tag)
...
def soft_augment(candidate_data=None, num_mixup=None, hyper_alpha=8, score_limit_upper=500, score_limit_low=0):
  ...
  for index in pair_index_list:
    ...
    print("hq flag",high_quality_flag,flush=True)
    if high_quality_flag:
      ...
      new_sample_count += 1
      # case_util(score, candidate_data[i].text_a, candidate_data[j].text_a, candidate_data[i].label, candidate_data[j].label,new_seq, new_seq_tag, prefix='Soft_case')
    if new_sample_count >= num_mixup:
      break
...
def active_augment_learn(init_flag=None, train_data=None, unlbl_data=None, num_initial=200, 
              ...
              Epochs=100, score_limit_low=0, score_limit_upper=500, fit_only_new_data=False, 
  ...
  init_data_loader, query_idx = get_tr_set(size=num_initial, train_examples=train_data, label_list=label_list,args=args,tokenizer=tokenizer,logger=logger)
  ...
  if init_flag:
    ...
    model = active_train(init_data_loader, None, Epochs,args=args,num_labels=num_labels,device=device,label_list=label_list,n_gpu=n_gpu,processor=processor,tokenizer=tokenizer,logger=logger)

  # report
  report = evaluate('Intialization', model, args=args, eval_on='test', processor=processor, label_list=label_list, tokenizer=tokenizer, logger=logger, device=device)
  ...
  '''
    Uncomment the following block for original seqmix method
  '''

  # if augment_method == slack_augment or augment_method == soft_augment:
  ...
  #     soft_loader = get_tr_set(train_examples=soft_data, soft_labels=soft_labels, label_list=label_list,args=args,tokenizer=tokenizer,logger=logger)
  # else:
  #     print(augment_method,flush=True)
  #     mix_data, new_sample_count = augment_method(train_data, num_augment, hyper_alpha, score_limit_upper, score_limit_low)
  #     soft_loader = None

  # aug_data_loader = get_tr_set(train_examples=train_data, label_list=label_list,args=args,tokenizer=tokenizer,logger=logger)
  # model = active_train(data_loader=aug_data_loader, model=model, Epochs=Epochs, soft_loader=soft_loader,args=args,num_labels=num_labels,device=device,label_list=label_list,n_gpu=n_gpu,processor=processor,tokenizer=tokenizer,logger=logger)
  # #return model
  # report = evaluate('SeedSetAug', model, args=args, eval_on='test', processor=processor, label_list=label_list, tokenizer=tokenizer, logger=logger, device=device)
  ...
  # print(print_table)
  # save_result(prefix=prefix, func_paras=func_paras, report=report, table=print_table,output_dir=args.output_dir)
  ...
  for idx in range(num_query):
    ...
    query_idx, query_instance = active_policy(model, pool, num_sample[idx],args,tokenizer,logger,label_list)
    ...
    aug_total_count += new_sample_count
    aug_data_loader = get_tr_set(train_examples=train_data,label_list=label_list,args=args,tokenizer=tokenizer,logger=logger)
    model = active_train(data_loader=aug_data_loader, model=model, Epochs=Epochs, soft_loader=soft_loader,num_labels=num_labels,args=args,device=device,label_list=label_list,n_gpu=n_gpu,processor=processor,tokenizer=tokenizer,logger=logger)
    if single_use:
      train_data = train_data[:-new_sample_count]
      aug_total_count = new_sample_count
    report = evaluate('SeqMixAug', model, args, eval_on='test', processor=processor, label_list=label_list, tokenizer=tokenizer, logger=logger, device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prefix", type=str, default='lc_slack')
    parser.add_argument("--active_policy", type=str, default='lc')
    parser.add_argument("--augment_method", type=str, default='slack')
    ...
    soft_model = active_augment_learn(init_flag=False, train_data=train_examples, unlbl_data=unlbl_examples ,augment_rate=args.augment_rate, hyper_alpha=args.hyper_alpha, active_policy=active_policy, augment_method=augment_method, prefix=args.prefix, Epochs=100)