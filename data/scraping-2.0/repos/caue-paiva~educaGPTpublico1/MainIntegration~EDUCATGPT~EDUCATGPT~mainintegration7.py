...
from . import qdran_query as QD
from . import intent_identi as int_iden
from . import learningSaveFunction3 as saveProgress
from dotenv import load_dotenv


load_dotenv("/home/kap/Desktop/pythonGPT/keys.env")

#materiaChat = "historia e geografia"
...
total_tokens, completion_tokens,prompt_tokens, total_cost = 0, 0, 0, 0 #salvar o gasto de tokens da API do chatGPT
#current_subject = ""

subjectPrompt= "você é um tutor que explica de forma didática as matérias "

prompt_body = """de forma precisa e profunda para os alunos. Pense passo-a-passo no melhor jeito de responder uma pergunta ou resolver algo e apenas traga resposta corretas que o aluno aprecie
...
def classifica(UserInput: str):
        ...
        print("resultado da ferramenta: " + resultado) #problemas no NLU com o modelo sendo meio bugado em pesquisa_web, as vezes fala que é web quando não é 
...
class chatBot():
    ...
    def chatModel(self):
        input = self.input
        global total_cost, total_tokens, prompt_tokens, completion_tokens, subjectPrompt
        
        if self.classificar == 1: #chain de pesquisa na web e para trazer links
        ...
        elif self.classificar == 2:  # chain de pesquisa de questões em docs
           ...
           #print("retorno doc: " + retornoDOC + "\n\n")
            ...
            return  retu_dict
             ...
             print(f"subject antes da template: {subjectPrompt}")
             template = subjectPrompt + prompt_body
             print("fez template \n")
             print(f"template global do chat: {template}")
             prompt = PromptTemplate(
