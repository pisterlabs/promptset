...
json_prompt_template = """Calculate the sum of values for given key {key}:

Key To List of Values as JSON:

{data}

Format output as JSON as given format below:
{{ "sum": value_of_sum }}

Output:"""
json_prompt = PromptTemplate(template=json_prompt_template, input_variables=["key", "data"])

# Create an LLMChain with the chat model and custom prompt mapping
json_llm_chain = LLMChain(llm=llm, prompt=json_prompt, verbose=False)

# Function to ask GPT to calculate the sum of values for a given key
def calculate_json_sum(key):
    if key not in random_dict:
        return {"success": False, "message": f"Key '{key}' not found in the dictionary."}
    
    values = random_dict[key]
    expected_sum = sum(values)
    json_input = json.dumps(random_dict)
    
    start_time = time.time()
    response = json_llm_chain.run({"key": key, "data": json_input})
    end_time = time.time()
...
def perform_calculations(num_iterations, sleep_duration):
    ...
    for _ in range(num_iterations):
        for key_to_query in random_dict.keys():
            result = calculate_json_sum(key_to_query)
