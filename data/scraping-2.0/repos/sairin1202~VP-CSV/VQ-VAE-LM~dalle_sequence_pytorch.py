...
class DALLE(nn.Module):
    def __init__(
        ...
        *,
        ...
        # should be changed for different task
        # image_fmap_size = 16
        image_seq_len = image_fmap_size ** 2 * IMG_SEQ_NUM
    ...
    def generate_images(
        ...
        *,
        ...
        for cur_len in range(out.shape[1], total_len):
            ...
            logits = self.generate_forward(text, image, mask = mask)[:, -1, :]

            filtered_logits = top_k(logits, thres = filter_thres)
            probs = F.softmax(filtered_logits / temperature, dim = -1)
            sample = torch.multinomial(probs, 1)
            # sample = torch.argmax(probs, dim=-1, keepdim=True)
        ...
        images = []
        for _ in range(IMG_SEQ_NUM):
            img_seq = out[:, text_seq_len+8*8*(_):text_seq_len+8*8*(_+1)]
            images.append(vae.decode(img_seq))
        

        # if exists(clip):
        #     scores = clip(text_seq, images, text_mask = mask, return_loss = False)
    ...
    def forward(
        ...
        if exists(image) and not is_empty(image):
            for _ in range(IMG_SEQ_NUM):
                img = image[:,_,:,:,:]
                is_raw_image = len(img.shape) == 4

                if is_raw_image:
                    image_size = self.vae.image_size
                    # print(img.shape[1:], (3, image_size, image_size))
                    assert tuple(img.shape[1:]) == (3, image_size, image_size), f'invalid image of dimensions {image.shape} passed in during training'
                    img = self.vae.get_codebook_indices(img)
            ...
            # print(imgs.size())
            img_emb = self.image_emb(imgs)
            img_emb += self.image_pos_emb(img_emb)
            tokens = torch.cat((tokens, img_emb), dim = 1)
            if exists(mask):
                mask = F.pad(mask, (0, img_emb.shape[1]), value = True)
            # print(tokens.size())
        # when training, if the length exceeds the total text + image length
        # remove the last token, since it needs not to be trained
        # print(tokens.shape[1], total_seq_len)
        ...
        offsetted_image = imgs + self.num_text_tokens
