...
def run():
    ...
    # data = pd.read_csv("../asr_manual_gpt2_test.csv")
#    history = []
    while True:
        history = []
        raw_text = input("Q>>> ")
...
#        history = []
#        raw_text = row[1]['question']
#        raw_ans = row[1]['answer']
#        print (raw_text, '\n', raw_ans)
#        if ":" in raw_ans:
#            print ('************')
#            continue
        history.extend([tokenizer.encode(raw_text),tokenizer.encode(raw_ans)])
        with torch.no_grad():
...
#        fqs.append(out_text)
#    data["followUp"] = fqs
#    data.to_csv("asrmanual_gpt_test_results.csv")
