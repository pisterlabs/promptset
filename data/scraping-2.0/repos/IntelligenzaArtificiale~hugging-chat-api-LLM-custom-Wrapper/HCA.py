...
class HCA(LLM):
    ...
    def create_chatbot(self) -> None:
        if not any([self.email, self.password, self.cookie_path]):
            raise ValueError("email, password, or cookie_path is required.")
        
        try:
            if self.email and self.password:
                # Create a ChatBot using email and password
...
# llm = HCA(email="YOUR_EMAIL", password="YOUR_PASSWORD", log=True , model=1, temperature=0.9, top_p=0.95, repetition_penalty=1.2, top_k=50, truncate=1024, watermark=False, max_new_tokens=1024, stop=["</s>"], return_full_text=False, stream=True, use_cache=False, is_retry=False, retry_count=5)
