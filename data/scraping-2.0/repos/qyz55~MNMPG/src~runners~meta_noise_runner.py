...
class MetaNoiseRunner:
    ...
    def reset_first(self, test_mode=False):
        ...
        if test_mode or not self.args.noise_bandit:
            self.noise = self.noise_distrib.sample(self.batch['state'][:,0], test_mode)[0:1,:]
        else:
            self.noise = self.noise_distrib.sample(self.batch['state'][:,0], test_mode)
        self.batch.update({"noise": self.noise}, ts=0)
        self.t = 0
    def run(self, test_mode=False, meta_mode=False, test_uniform=False, use_rode=False):
        ...
        while True:
            if meta_mode:
                action, logp = chosen_mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, test_mode=test_mode, need_log_p=meta_mode)
                log_ps.append(logp)
            else:
                action = chosen_mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, test_mode=test_mode, need_log_p=meta_mode)
        ...
        self._update_noise_returns([episode_return], self.noise, final_env_infos, test_mode)
        self.noise_distrib.update_returns(self.batch['state'][:,0], self.noise, [episode_return], test_mode, self.t_env)
    ...
    def run_meta(self, test_mode=False, meta_mode=False, test_uniform=False, use_rode=False):
        ...
        while True:
            if meta_mode:
                actions, logp = chosen_mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode, need_log_p=meta_mode)
                log_ps.append(logp)
            else:
                actions = chosen_mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode, need_log_p=meta_mode)
