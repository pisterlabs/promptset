...
#################################################################################
# Test the Retrieval system, have OpenAI define if an answer is suitable or not #
#################################################################################
test_queries = [
...
def main():
    # Load the database collection and matched embeddings to use
    ...
    collection = client.get_collection("chars1000-50_MiniLM-L6", embedding_function=embedding_function)
    ...
    for query in test_queries:
        ...
        print(i,test_results[i])
        print(query)
        print(response.choices[0].message.content)
        print("#########################################")
    #    print(i, response.choices[0].message.content, test_results[i])
        for book in books:
            if book['id'] == response.choices[0].message.content:
                print("GoodReads suggests ", book['title'], book['url'])
                break
        i += 1
