...
"""
This approach (20) is based on approach 15, but with the following changes:
- Implict actions are tested with GPT 4.0
...
model_engine = "text-davinci-003"  # does not work via this api
...
def preprocess_text_with_LLM(doc):
    ...
    def generate_response_3_instruct(prompt) -> str:
        try:
            if debug_mode: print(f"*** Prompt: *** len: {len(prompt).__str__()} \n {prompt} \n")
...
def write_to_file(number: int, nlp):
    ...
    output_path = f"/Users/vincentderekheld/PycharmProjects/bachelor-thesis/Evaluation/Approach-20/Text{number.__str__()}-{model_engine}.txt"
    with open(output_path, 'w') as file:
        file.write(content)
