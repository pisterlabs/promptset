...
import crystalai
from crystalai import error, util, version
...
def _make_session() -> requests.Session:
    if crystalai.requestssession:
        if isinstance(crystalai.requestssession, requests.Session):
            return crystalai.requestssession
        return crystalai.requestssession()
    if not crystalai.verify_ssl_certs:
        warnings.warn("verify_ssl_certs is ignored; openai always verifies.")
    s = requests.Session()
    proxies = _requests_proxies_arg(crystalai.proxy)
    if proxies:
        s.proxies = proxies
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or crystalai.api_base
        ...
        self.api_version = api_version or crystalai.api_version
        self.organization = organization or crystalai.organization
    ...
    def request_headers(
        ...
        if crystalai.app_info:
            user_agent += " " + self.format_app_info(crystalai.app_info)
        ...
        if crystalai.app_info:
            ua["application"] = crystalai.app_info
    ...
    def _prepare_request_raw(
        ...
        if method == "get" or method == "delete":
            ...
            raise error.APIConnectionError(
                ...
                "OpenAI bindings. Please contact us through our help center at help.crystalcomputing.com for "
                "assistance." % (method,)
            )
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_kwargs = {
            ...
            "headers": headers,
            "data": data,
            "proxy": _aiohttp_proxies_arg(crystalai.proxy),
            "timeout": timeout,
        }
        try:
...
class AioHTTPSession(AsyncContextManager):
    ...
    async def __aenter__(self):
        self._session = crystalai.aiosession.get()
