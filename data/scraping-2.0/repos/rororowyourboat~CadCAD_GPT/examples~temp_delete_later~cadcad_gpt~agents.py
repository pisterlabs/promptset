...
class PlannerAgent:
    ...
    def __call__(self, prompt: str) -> openai.openai_object.OpenAIObject:
        ...
        messages=[
            ...
            assistant: Since the user query is asking to analyze the column in the current dataframe I should use the analysis_agent function in one step.  ``` [use the function analysis_agent to fetch the columns in the dataframe.] ```
            user: What would happen to the A column at the end of the simulation if my xyz param was 20?
            assistant: Since the user query has a multistep question here which involves changing a param and then analyzing the dataframe column I should plan 2 steps for this. ``` [we use function change_param to change the xyz parameter to 20, use function analysis_agent to get the A at the end of the simulation.] ```
