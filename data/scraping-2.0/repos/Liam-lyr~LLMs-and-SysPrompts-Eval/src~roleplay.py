...
def get_sys_prompt(language: str) -> str:
    ...
    if language == "en":
        return "You could be any character, as decided by the user. You need to be very creative, your role could come from any time period, it could be from literature, or it could be a professional position, etc. Please play the role assigned by the user, conform to their character, and communicate with the user accordingly."
    if language == "zh":
        return "你可能是任何角色，由用户决定。你需要非常具有创造性，你的角色可能来自古今中外，可能来自文艺作品，也可能是专业的职位等。请你扮演好用户指派的角色，符合其人设，并于用户交流。"
    else:
        raise ValueError("Language must be one of 'en' or 'zh'.")


def get_raw_response(sys_prompt: str, model: str, language: str, prompt: tuple[int, list[str]], seed: int, task: str = "roleplay"):
    r"""
...
def get_raw_response_of_all_questions_of_task(model: str, prompts: list[tuple[int, list[str]]], raw_response_dir: str, seed: int, task: str = "roleplay") -> None:
    r"""
...
def get_gpt_raw_eval_of_all_questions_of_task(sys_prompt: str, model: str, prompts_for_eval: list[tuple[int, list[str]]], raw_eval_dir: str, seed: int, task: str = "roleplay", rate_limit: int = 3, wait_time: int = 60) -> None:
    r"""
