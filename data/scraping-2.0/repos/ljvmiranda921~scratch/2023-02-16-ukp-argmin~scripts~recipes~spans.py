...
@dataclass
class SpansPromptExample(PromptExample):
...
def openai_correct_spans(
    ...
    prompt_path: Path = OPENAI_DEFAULTS.SPAN_PROMPT_PATH,
    max_examples: int = 2,
    ...
    """
    log("RECIPE: Starting recipe spans.openai.correct", locals())
    examples = read_prompt_examples(examples_path, example_class=SpansPromptExample)
    nlp = spacy.blank(lang)
    if segment:
    ...
    openai = OpenAISuggester(
        ...
        openai_n_retries=10,
        prompt_example_class=SpansPromptExample,
    )
    for eg in examples:
    ...
    return {
        ...
        "config": {
            ...
            "blocks": [
                ...
                {
                    "view_id": "html",
                    "html_template": OPENAI_DEFAULTS.SPANS_HTML_TEMPLATE,
                },
            ],
            "show_flag": True,
...
def openai_fetch_spans(
    ...
    """
    Get bulk Span suggestions from the OpenAI API, using zero-shot or
    few-shot learning. The results can then be corrected using the
    `spans.manual` recipe.  This approach lets you get OpenAI queries upfront,
    which can help if you want multiple annotators or reduce the waiting time
    between API calls.
    """
    log("RECIPE: Starting recipe spans.openai.fetch", locals())
    api_key, api_org = get_api_credentials(model)
    examples = read_prompt_examples(examples_path, example_class=SpansPromptExample)
    nlp = spacy.blank(lang)
    if segment:
