...
# Env Variables
ui_api_key=gr.Textbox(placeholder="Huggingface API Key - https://huggingface.co/settings/tokens, hf-XXX",label="Huggingface Write API Key")
...
def update_global_variables(ui_api_key, ui_weaviate_url, ui_product_name, ui_product_description, ui_product_prompt):
    ...
    try:
        # Setting g_hf_api_key
        if ui_api_key != "":
            print('Setting g_hf_api_key - '+ui_api_key)
            g_hf_api_key=ui_api_key
            g_output=g_output+'Setting g_hf_api_key - '+ui_api_key
        else:
            print("exception in function - update_global_variables")
            raise ValueError('Required Huggingface API Key')
...
def create_hf_embeddings(text):
    ...
    headers = {"Authorization": f"Bearer {g_hf_api_key}"}
...
# -- Create embedding from dict to numpy array
def convert_embedding_to_tensor(embedding):
    if isinstance(embedding, dict):
        tensor = np.zeros((1,))
        for key, value in embedding.items():
            tensor = np.concatenate([tensor, np.expand_dims(value, axis=-1)])
    else:
        tensor = np.expand_dims(np.asarray(embedding), axis=-1)
    return tensor
            ...
            for page_no, page in enumerate(pdf.pages, start=1):
                ...
                if text:
                    ...
                    for chunk in chunks:
                        item =  {
                                    'text': chunk,
                                    'embedding': convert_embedding_to_tensor(create_hf_embeddings(chunk)),
                                    'page_no': page_no
                                }
    ...
    try:
        ...
        for key, value in combined_values.items():
            ...
            item= {
                        'key':key,
                        'description': value,
                        'embedding': convert_embedding_to_tensor(create_hf_embeddings(value))
                }
