...
Responses = [openai.Completion.create(engine="text-davinci-002", prompt=prompt, max_tokens=60).choices[0].text.strip() for prompt in Prompts]
Modified_Responses = [openai.Completion.create(engine="text-davinci-002", prompt=prompt, max_tokens=60).choices[0].text.strip() for prompt in Modified_Prompts]
