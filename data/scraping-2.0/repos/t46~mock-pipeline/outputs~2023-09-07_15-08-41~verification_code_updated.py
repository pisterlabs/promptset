...
for prompt, expected_response in test_set.items():
    original_responses[prompt] = openai.Completion.create(engine="text-davinci-002", prompt=prompt, max_tokens=60).choices[0].text.strip()
    specific_responses[specific_prompts[prompt]] = openai.Completion.create(engine="text-davinci-002", prompt=specific_prompts[prompt], max_tokens=60).choices[0].text.strip()
