...
for general, specific in questions:
    general_output = openai.Completion.create(engine="text-davinci-003", prompt=general, max_tokens=5)
    specific_output = openai.Completion.create(engine="text-davinci-003", prompt=specific, max_tokens=5)
    
    general_responses.append(general_output.choices[0].text.strip())
    specific_responses.append(specific_output.choices[0].text.strip())

# Categorize responses
general_numerical = [1 if response.isdigit() else 0 for response in general_responses]
specific_numerical = [1 if response.isdigit() else 0 for response in specific_responses]
