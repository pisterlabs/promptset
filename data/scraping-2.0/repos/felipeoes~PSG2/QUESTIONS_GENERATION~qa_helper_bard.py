...
class QAHelper:
    ...
    def save_checkpoint(self, file_index: int, file_name: str):
        ...
        dest_folder_name = self.folder_name.split("/")[-1]
        checkpoint_path = (
            CHECKPOINTS_DIR
    ...
    def upload_dataframe(self, csv_path: Path):
        ...
        folders = self.folder_name.split("/")
    ...
    def load_dataframe(self, csv_path: Path):
        ...
        if not csv_path.exists():
            return pd.DataFrame(
                columns=["file_index", "file_name", "context", "prompt", "question", "answer"]
            )
    ...
    def run(self):
        ...
        for folder in self.folder_name.split("/"):
        ...
        """   for file_index, file_name, file_content in contents:
            try:
                text = self.extract_text(file_content=file_content)

                if not self.check_text(text):
                    continue

                for part in self.text_splitter.split_text(text):
                    if self.check_text(part):
                        context, questions, answers = self.generator.generate_qa(
                            part)
                ...
                print(
                    f"KeyboardInterrupt | Dataframe length: {len(df)} | Csv path: {csv_path} | File index: {file_index}"
                )
                break
