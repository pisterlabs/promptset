...
def get_f1_metrics(real_preds, sample_preds, pred_label):
    ...
    f1_score = f1_score(pred_label, real_preds + sample_preds, pos_label=None, average=None)
    return f1_score.tolist()

    
def get_accuracy(real_preds, sample_preds, pred_label):
    #_, threshold_val, pred_label = get_threshold(real_preds, sample_preds, target_fpr)
    return accuracy_score(pred_label, real_preds + sample_preds)
    

def get_confusion_matrix(real_preds, sample_preds, pred_label):
    #_, threshold_val, pred_label = get_threshold(real_preds, sample_preds, target_fpr)
    confusion_m = confusion_matrix(pred_label, real_preds + sample_preds, normalize='true')
    return confusion_m
