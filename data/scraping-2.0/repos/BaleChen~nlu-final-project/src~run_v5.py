...
def save_roc_curves(experiments):
    ...
    for experiment, color in zip(experiments, COLORS):
        ...
        plt.plot(metrics["fpr"], metrics["tpr"], label=experiment['name'], color=color)
        # print roc_auc for this experiment
        print(f"{experiment['name']} roc_auc: {metrics['roc_auc']}")
        if 'roc_auc_score' in metrics:
            print(f"{experiment['name']} roc_auc_score: {metrics['roc_auc_score']}")
...
def get_perturbation_results(span_length=10, n_perturbations=1, n_samples=500, perturb = True):
    ...
    if perturb:
        ...
        dataset_name_list = ['xsum','squad','writing']
        all_dataset_results = {}
        for dataset_name in dataset_name_list:
            with open(f'results/detectgpt-{base_model_name}-t5-3b-temp/{dataset_name}-perturbation_50_z_results.json', 'r') as f:
                data = json.load(f)
            data = data['raw_results']
            perturbated_data =[]
            for i in range(len(data)):
                perturbated_data.append({
                    'original':data[i]['original'],
                    'sampled':data[i]['sampled'],
                    'perturbed_sampled':data[i]['perturbed_sampled'],
                    'perturbed_original':data[i]['perturbed_original']
                })
            all_dataset_results[dataset_name] = perturbated_data
...
def run_perturbation_experiment(scoring_model, results, criterion, span_length=10, n_perturbations=1, n_samples=500,target_fpr=0.01):
    ...
    if ensemble:
        name = f'perturbation_{scoring_model}_{n_perturbations}_{criterion}'
    else: 
...
def load_base_model_and_tokenizer(name):
    if args.openai_model is None:
        ...
        if 'gpt-j' in name or 'neox' in name:
            ...
            base_model = transformers.OPTForCausalLM.from_pretrained(checkpoint_dir,cache_dir=cache_dir)
        else:
            base_model = transformers.AutoModelForCausalLM.from_pretrained(name, **base_model_kwargs, cache_dir=cache_dir)
    else:
        base_model = None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ensemble_scoring', type=str, default="") # expect the argument to be a list of score model, must odd number of model 
    args = parser.parse_args()

    API_TOKEN_COUNTER = 0
    
    ensemble = False # flag 
    ensemble_scoring_list = []
    print('>>>>>>>To adjust the ensemble flag')
    print('Before:', ensemble)
    if args.ensemble_scoring:
        print(f'Switch to Ensemble Scoring')
        ensemble_scoring_list = [str(x) for x in args.ensemble_scoring.split(",")]
        ensemble = True
    ...
    if 'opt' in args.scoring_model_name:
        SAVE_FOLDER = f"tmp_results/{output_subfolder}{base_model_name}{scoring_model_string}-load_perturbed-{sampling_string}/{START_DATE}-{START_TIME}-{precision_string}-{args.pct_words_masked}-{args.n_perturbation_rounds}-{args.dataset}-{args.n_samples}-{args.ft_epoch}"
    ...
    outputs = []
    nofModels = len(ensemble_scoring_list)
    if not args.baselines_only:
        # run perturbation experiments
        for n_perturbations in n_perturbation_list:
            if 'opt' in args.scoring_model_name:
                all_dataset_results = get_perturbation_results(args.span_length, n_perturbations, n_samples, False)
            else:
                perturbation_results = get_perturbation_results(args.span_length, n_perturbations, n_samples, True)
            for perturbation_mode in ['d', 'z']:
                if 'opt' in args.scoring_model_name:
                    for dataset_name,perturbation_results in all_dataset_results.items():
                        output = run_perturbation_experiment(base_model_name,
                        ...
                        OPT_FOLDER = SAVE_FOLDER+f'/{dataset_name}'
                        if not os.path.exists(OPT_FOLDER):
                            os.makedirs(OPT_FOLDER)
                        with open(os.path.join(OPT_FOLDER, f"perturbation_{n_perturbations}_{perturbation_mode}_results.json"), "w") as f:
                            json.dump(output, f)
                        
  
    if ensemble:
        # run perturbation experiments
        print('Enter Ensemble Experiments')
        for n_perturbations in n_perturbation_list:
            for perturbation_mode in ['d', 'z']:
                prediction_label_list =dict()
                prediction_score_list = dict()
                for score_model in ensemble_scoring_list:
                    del base_model
                    del base_tokenizer
                    torch.cuda.empty_cache()
                    scoring_model_name= score_model
                    print(f'>>>>>>Ensemble: Loading scoring model here {scoring_model_name}')
                    ...
                    output = run_perturbation_experiment(score_model,
                        perturbation_results, perturbation_mode, span_length=args.span_length, n_perturbations=n_perturbations, n_samples=n_samples)
                    prediction_label_list[score_model] = output['pred_label']
                    prediction_score_list[score_model] = output['predictions']['real']+output['predictions']['samples']
                # majority vote to get the final prediction
                final_pred_list = []
                final_score_list = []
                zipped = zip(*prediction_label_list.values())
                for sample in list(zipped):
                    final_pred = mode(sample)
                    final_pred_list.append(final_pred)
                zipped_score = zip(*prediction_score_list.values())
                for model_name, val in prediction_score_list.items():
                    fpr_,tpr_,_ = roc_curve([0]*len(perturbation_results)+[1]*len(perturbation_results),val)
                    roc_auc_ = float(auc(fpr_, tpr_))
                    print(f'When score model is {model_name}, roc_auc:', roc_auc_)
                for scores in list(zipped_score):
                    final_score = sum(scores)/len(scores)
                    final_score_list.append(final_score)
                print('>>>>>Calculating Evaluation Metric>>>>>')
                print('the length of perturbation_result',len(perturbation_results))
                print('*********Final Prediction:', final_pred_list)
                print('*********Combined Score:', final_score_list)
                f1_ = f1_score([0] * len(perturbation_results) + [1] * len(perturbation_results), final_pred_list)
                fpr, tpr,_ = roc_curve([0] * len(perturbation_results) + [1] * len(perturbation_results), final_pred_list)
                roc_auc = auc(fpr, tpr)
                roc_auc = float(roc_auc)
                fpr_score, tpr_score,_ = roc_curve([0]*len(perturbation_results)+[1]*len(perturbation_results),final_score_list)
                roc_auc_score =float(auc(fpr_score, tpr_score))

                
                
                name = f'perturbation_ensemble_{n_perturbations}_{perturbation_mode}'
                output_ensemble = {
                    'name': name,
                    'pred_label':final_pred_list,
                    'info': {
                        'pct_words_masked': args.pct_words_masked,
                ...
                outputs.append(output_ensemble)
                with open(os.path.join(SAVE_FOLDER, f"perturbation__ensemble_{n_perturbations}_{perturbation_mode}_results.json"), "w") as f:
                    json.dump(output, f)
