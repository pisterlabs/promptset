...
if __name__ == '__main__':
    ...
    if ensemble:
        # run perturbation experiments
        print('Enter Ensemble Perturbation Experiments')
        for n_perturbations in n_perturbation_list:
            for perturbation_mode in ['d', 'z']:
                prediction_label_list =dict()
                for score_model in ensemble_scoring_list:
                    print(f'Loading SCORING model {score_model}...')
                    del base_model
                    del base_tokenizer
                    torch.cuda.empty_cache()
                ...
                print('>>>>>Calculating Evaluatio Metric>>>>>')
                print('the length of perturbation_result',len(perturbation_results))
                print('the final prediction:', final_pred_list)
                f1_ = f1_score([0] * len(perturbation_results) + [1] * len(perturbation_results), final_pred_list)
                fpr, tpr, roc_auc = roc_curve([0] * len(perturbation_results) + [1] * len(perturbation_results), final_pred_list)
