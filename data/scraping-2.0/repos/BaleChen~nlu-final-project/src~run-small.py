...
if __name__ == '__main__':
    ...
    if not args.baselines_only and not args.random_fills and not "noise" in args.perturb_method:
        int8_kwargs = {}
        half_kwargs = {}
        if args.int8:
        ...
        if not args.perturb_method == "one-model":
            ...
            mask_model = transformers.AutoModelForCausalLM.from_pretrained(mask_filling_model_name, **int8_kwargs, **half_kwargs, cache_dir=cache_dir)
        try:
    ...
    # mask_tokenizer.add_special_tokens({'pad_token': '<pad>'}) # TODO Only GPT needs it?
    ...
    if args.scoring_model_name:
        ...
        if not args.perturb_method == "one-model":
            ...
            base_model, base_tokenizer = mask_model, mask_tokenizer
...
########### DETECT GPT EXPERIMENTS ##################
    if not args.baselines_only:
        if not "noise" in args.perturb_method:
            load_mask_model()
        # run perturbation experiments
        for n_perturbations in n_perturbation_list:
            perturbation_results = get_perturbation_results(args.span_length, n_perturbations, n_samples, args.perturb_method)
