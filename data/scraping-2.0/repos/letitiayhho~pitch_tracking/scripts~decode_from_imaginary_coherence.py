...
#SBATCH --output=logs/decode_from_imaginary_coherence_%j.log
...
from util.io.imaginary_coherence import *
...
def main(SUB, TASK, RUN):
    ...
    for epoch in range(n_epochs):
        ...
        for channel in range(n_chans):
            ...
            # compute coherence
            f, Cxy = imaginary_coherence(current_epoch, current_stim, FS)
    ...
    save_fp = f'../data/bids/derivatives/coherence/sub-{SUB}_task-{TASK}_run-{RUN}_imcoh-by-epoch.pkl'
    np.save(save_fp, X)
    ...
    save_fp = f'{DECODING_ROOT}/sub-{SUB}/sub-{SUB}_task-{TASK}_run-{RUN}_desc-imcoh_scores.npy'
