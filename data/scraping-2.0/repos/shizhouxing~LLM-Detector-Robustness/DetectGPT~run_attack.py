...
from openai_perturbations import get_perturbations, save_perturbations, parse_response
from llama_adapters import llama_parse_response
...
import string
from attackers import DipperParaphraser, genetic_attack_agent
...
def load_mask_model():
    ...
    if args.openai_model is None and 'llama' not in str(type(base_model)):
        base_model.cpu()
...
def sample_from_model(texts, min_words=55, prompt_tokens=30):
    ...
    if args.chatgpt:
        ...
        for idx, prefix in tqdm(enumerate(prefixes), total=len(prefixes), desc='sample chatgpt results'):
            res = get_chatgpt_response('Please complete the following paragraph: ' + prefix)

            decoded[idx] = prefix.strip() + ' ' + res['choices'][0]['message']['content'].strip()
        decoded = [x[:255] if len(x.split()) > 255 else x for x in decoded]
    else:
...
def get_ranks(texts):
    if not args.openai_model:
        return [-get_rank(text) for text in texts]
    else:
        global API_TOKEN_COUNTER
        total_tokens = sum(len(GPT2_TOKENIZER.encode(text)) for text in texts)
        API_TOKEN_COUNTER += total_tokens * 2  # multiply by two because OpenAI double-counts echo_prompt tokens
        ...
        pool = ThreadPool(args.batch_size)
        return pool.map(get_rank, texts)
...
def get_roc_metrics(real_preds, sample_preds):
    ...
    print()
    return fpr.tolist(), tpr.tolist(), float(roc_auc), float(tpr_at_fpr)
...
def attack_ll(args, texts, attack_method='random'):
    base_model.to(DEVICE)
    agent = genetic_attack_agent(base_tokenizer, base_model, DEVICE)
    random = attack_method == 'random'
    if not os.path.exists(f'results_gpt2'):
        os.mkdir(f'results_gpt2')
    if not os.path.exists(f'results_gpt2/chatgpt_seed_{args.dataset}_{args.seed}'):
        os.mkdir(f'results_gpt2/chatgpt_seed_{args.dataset}_{args.seed}')
    if not os.path.exists(f'results_gpt2/random_results_seed_{args.seed}_{args.dataset}'):
        os.mkdir(f'results_gpt2/random_results_seed_{args.seed}_{args.dataset}')
    if not os.path.exists(f'results_gpt2/genetic_results_seed_{args.seed}_{args.dataset}'):
        os.mkdir(f'results_gpt2/genetic_results_seed_{args.seed}_{args.dataset}')

    for idx, text in enumerate(texts):
        if not os.path.exists(f'results_gpt2/chatgpt_seed_{args.dataset}_{args.seed}/{idx}.json'):
            results = get_perturbations(text)
            save_perturbations(results, idx, path=f'results_gpt2/chatgpt_seed_{args.dataset}_{args.seed}/{idx}.json')
        mapping = parse_response(f'results_gpt2/chatgpt_seed_{args.dataset}_{args.seed}/{idx}.json')
    ...
    samples = []
    for idx, text in enumerate(texts):
        if os.path.exists(f'results_gpt2/{attack_method}_results_seed_{args.seed}_{args.dataset}/{idx}.txt'):
            with open(f'results_gpt2/{attack_method}_results_seed_{args.seed}_{args.dataset}/{idx}.txt', 'r+', encoding="utf-8") as f:
                text = f.readline()
                samples.append(text.strip())
                continue
        if args.attack_model == 'chatgpt':
            mapping = parse_response(f'results/{args.attack_model}_seed_{args.dataset}_{args.seed}/{idx}.json')
        elif args.attack_model == 'llama':
            mapping = llama_parse_response(f'results/{args.attack_model}_seed_{args.dataset}_{args.seed}/{idx}.json')
        else:
        ...
        for key in mapping.keys():
            if len(mapping[key]) == 1 and '/' in mapping[key][0]:
                mapping[key] = mapping[key][0].split('/')
        if random:
            elite = agent.random_replacement(text, mapping)
        else:
            elite = agent.attack(text, mapping)

        samples.append(' '.join(elite[0]))
        with open(f'results_gpt2/{attack_method}_results_seed_{args.seed}_{args.dataset}/{idx}.txt', 'w') as f:
            f.write(' '.join(elite[0]))
            f.write('\n')
            f.write(str(get_ll(' '.join(elite[0]))))
    return samples


def paraphrase_ll(args, sampled_text):
    dp = DipperParaphraser()
    if not os.path.exists(f'{args.dataset}_results_paraphrase_{args.seed}'):
        os.mkdir(f'{args.dataset}_results_paraphrase_{args.seed}')
    for idx, text in enumerate(sampled_text):
        prompt = ' '.join(text.split()[:30])
        sampled_text[idx] = dp.paraphrase(text, lex_diversity=80, order_diversity=40, prefix=prompt, do_sample=True, top_p=0.95, top_k=None, max_length=512)
        with open(f'{args.dataset}_results_paraphrase_{args.seed}/{idx}.txt', 'w') as f:
            f.write(sampled_text[idx])
    return sampled_text
...
def get_perturbation_results(args, span_length=10, n_perturbations=1, n_samples=500):
    ...
    if args.attack:
        if args.paraphrase:
            sampled_text = paraphrase_ll(args, sampled_text)
        else:
            sampled_text = attack_ll(args, sampled_text, attack_method=args.attack_method)
    ...
    for res in tqdm.tqdm(results, desc="Computing log likelihoods"):
        p_sampled_ll = get_ranks(res["perturbed_sampled"])
        p_original_ll = get_ranks(res["perturbed_original"])
        res["original_ll"] = -get_rank(res["original"])
        res["sampled_ll"] = -get_rank(res["sampled"])
...
def run_perturbation_experiment(results, criterion, span_length=10, n_perturbations=1, n_samples=500):
    ...
    fpr, tpr, roc_auc, tpr_at_fpr = get_roc_metrics(predictions['real'], predictions['samples'])
...
def generate_data(dataset, key, args):

    data = load_data(dataset)
    data = [d['prompt'] + ' ' + d['human_text'] for d in data]
...
def load_base_model_and_tokenizer(name):
    if args.openai_model is None:
        ...
        if 'llama' in name:
            ...
            base_model = transformers.LlamaForCausalLM.from_pretrained(name, device_map='auto', load_in_8bit=True, max_memory=max_memory_mapping)
        else:
            base_model_kwargs = {}
    ...
    if 'llama' in name:
        ...
        base_tokenizer = tokenizer_class.from_pretrained(name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack_method', type=str, default="genetic")
    parser.add_argument('--attack_model', type=str, choices=['chatgpt', 'llama'], default='chatgpt')
    ...
    data = generate_data(args.dataset, args.dataset_key, args)
    ...
    if not args.baselines_only:
        ...
        for n_perturbations in n_perturbation_list:
            perturbation_results = get_perturbation_results(args, args.span_length, n_perturbations, n_samples)
