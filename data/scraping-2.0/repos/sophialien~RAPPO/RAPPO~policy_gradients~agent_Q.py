...
from ._auto_LiRPA.bound_general import BoundedModule
from ._auto_LiRPA.eps_scheduler import LinearScheduler
from ._auto_LiRPA.bounded_tensor import BoundedTensor
from ._auto_LiRPA.perturbations import PerturbationLpNorm
from .models import *
...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        def env_constructor():
            ...
            return Env(self.GAME, norm_states=self.NORM_STATES,
                       ...
                       friction_ratio=self.params.FRICTION_RATIO,
                       gravity_ratio=self.params.GRAVITY_RATIO)

        self.envs = [env_constructor() for _ in range(self.NUM_ACTORS)]

        envs = self.envs
        self.body_mass = copy.deepcopy(envs[0].env.model.body_mass.copy())
        self.geom_friction = copy.deepcopy(envs[0].env.model.geom_friction.copy())
        self.opt_gravity = copy.deepcopy(envs[0].env.model.opt.gravity.copy())
        self.opt_wind = copy.deepcopy(envs[0].env.model.opt.wind.copy())

        
        print("body mass")
        print(self.body_mass)

        for idx in range(len(self.body_mass)):
            self.body_mass[idx] = self.body_mass[idx] * self.params.MASS_RATIO
        for idx in range(len(self.geom_friction)):
            self.geom_friction[idx] = self.geom_friction[idx] * self.params.FRICTION_RATIO
        for idx in range(len(self.opt_gravity)):
            self.opt_gravity[idx] = self.opt_gravity[idx] * self.params.GRAVITY_RATIO
        for idx in range(len(self.opt_wind)):
            self.opt_wind[idx] = self.opt_wind[idx] * self.params.WIND_RATIO
        ...
        self.policy_model_old = policy_net_class(self.NUM_FEATURES, self.NUM_ACTIONS,
                                             self.INITIALIZATION,
                                             time_in_state=time_in_state,
                                             activation=self.policy_activation)

        # Instantiate convex relaxation model when mode is 'robust_ppo'
        if self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'advQ_robust_ppo' or self.MODE == 'advV_robust_ppo':
        ...
        if self.MODE == 'advQ_ppo' or self.MODE == 'advQ_robust_ppo' or self.MODE == 'advV_robust_ppo':
            self.val_model_Q = value_net_class(self.NUM_FEATURES+self.NUM_ACTIONS, self.INITIALIZATION)
            self.val_opt_Q = optim.Adam(self.val_model_Q.parameters(), lr=self.VAL_LR, eps=1e-5)
        ...
        if self.MODE == 'advV_ppo_meta' or self.MODE == 'advV_robust_ppo_mata':
            self.params.ADV_DELTA = torch.tensor(0.5, requires_grad=True)
            self.params.ADV_DELTA_beta = torch.tensor(0.5, requires_grad=True)
    ...
    def create_relaxed_model(self, time_in_state=False):
        ...
        if isinstance(self.policy_model, CtsPolicy):
            if self.ROBUST_PPO_METHOD == "convex-relax":
                ...
                #self.relaxed_policy_model.ptb = PerturbationLpNorm(norm=np.inf, eps=self.sarsa_eps_scheduler.get_eps())
            else:
    ...
    def apply_advQ_attack(self, last_states, action=None):
        attack_adv_eps = self.params.ROBUST_PPO_EPS * self.params.ADV_DELTA_BALL

        step_eps = attack_adv_eps / 10

        clamp_min = last_states - attack_adv_eps
        clamp_max = last_states + attack_adv_eps
        # Random start.
        noise = (torch.rand(last_states.shape)*2-1)*attack_adv_eps
        states = last_states + noise
        
        if action is not None:
            action_min = action*0 - 1
            action_max = action*0 + 1
        ...
        with torch.enable_grad():
            for i in range(10):
                ...
                if action is not None:
                    action = action.clone().detach().requires_grad_()
                    value = self.val_model_Q(torch.cat((states.float(), action), 1)).mean()
                    #print(states.shape)
                    ...
                    update = action.grad.sign()
                    # Clamp to +/- eps.
                    action.data = torch.min(torch.max(action.data - update, action_min), action_max)
                else:
                    value = self.val_model(states.float()).mean()
                    #print(states.shape)
            ...
            if action is not None:
                self.val_opt_Q.zero_grad()
            else:
                action = states
                self.val_opt.zero_grad()
        return states.detach().float(), action.detach().float()
    ...
    def setup_stores(self, store):
        ...
        if self.params.MODE == 'robust_ppo' or self.params.MODE == 'adv_sa_ppo' or self.MODE == 'advQ_robust_ppo' or self.MODE == 'advV_robust_ppo':
            robust_cols ={
                'eps': float,
                'beta': float,
                'kl': float,
    ...
    def advantage_and_return(self, rewards, values, not_dones, adv_values=None):
        """
        ...
        if adv_values is not None:
            #values = ( (1 - self.params.ADV_DELTA) *values + self.params.ADV_DELTA*adv_values)
            V_s_tp1 = ch.cat([values[:,1:], values[:, -1:]], 1) * not_dones
            deltas = rewards + self.GAMMA * V_s_tp1 - self.params.ADV_DELTA * self.params.ADV_DELTA_beta - adv_values
        else:
            V_s_tp1 = ch.cat([values[:,1:], values[:, -1:]], 1) * not_dones
    ...
    def reset_envs(self, envs, danger_state=None, first_time=False):
        '''
        Resets environments and returns initial state with shape:
        (# actors, 1, ... state_shape)
	    '''
        if self.CPU:
            return cpu_tensorize([env.reset(danger_state, self.params.GAME,mass=self.body_mass, friction=self.geom_friction, gravity=self.opt_gravity,  wind=self.opt_wind) for env in envs]).unsqueeze(1)
        else:
            return cu_tensorize([env.reset(danger_state, self.params.GAME,mass=self.body_mass, friction=self.geom_friction, gravity=self.opt_gravity,  wind=self.opt_wind) for env in envs]).unsqueeze(1)
    ...
    def multi_actor_step(self, actions, envs):
        ...
        for action, env in zip(actions, envs):
            ...
            current_state = envs[0].env.sim.get_state()
            posafter, height, ang = current_state.qpos[0:3]
            s = envs[0].env.state_vector()
            if is_done:
        ...
        data = list(map(tensor_maker, [normed_rewards, states, not_dones, posafter, height, ang, s]))
        return [completed_episode_info, *data, info]
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        for t in iterator:
            ...
            done_info, next_rewards, next_states, next_not_dones, posafter, height, ang, s, info = ret
    ...
    def collect_saps(self, num_saps, should_log=True, return_rewards=False,
        ...
        with torch.no_grad():
            ...
            if not test:
                ...
                #print(trajs.rewards.shape)
                # Calculate advantages and returns
                #if self.MODE == 'advV_ppo_meta' or self.MODE == 'advV_robust_ppo_mata':
                #    adv_states, actions = self.apply_advQ_attack(trajs.states)
                #    adv_values =  self.val_model(adv_states)
                #    
                #    advantages, returns = self.advantage_and_return(trajs.rewards,
                #                                values, trajs.not_dones, adv_values)
                #else:
                advantages, returns = self.advantage_and_return(trajs.rewards,
            ...
            if should_log:
                ...
                if not test:
                    ...
                    self.plot_info['mean_reward'] = avg_ep_reward
    ...
    def sarsa_steps(self, saps):
        ...
        for i in range(self.params.VAL_EPOCHS):
            ...
            for selected in splits:
                ...
                bounded_inputs.ptb = PerturbationLpNorm(norm=np.inf, eps=self.sarsa_eps_scheduler.get_eps())
    ...
    def take_steps(self, saps, logging=True, value_only=False, adversary_step=False, increment_scheduler=True):
        ...
        val_grad = 0
        if not self.SHARE_WEIGHTS:
            if self.MODE == 'advQ_ppo' or self.MODE == 'advQ_robust_ppo':
                adv_states, actions = self.apply_advQ_attack(saps.states, saps.actions)
                advQs = self.val_model_Q(torch.cat([adv_states, actions], dim=-1))
                Vs = self.val_model(saps.states)
                Qs = self.val_model_Q(torch.cat([saps.states, saps.actions], dim=-1))
                Q_target = ( (self.params.ADV_DELTA) *Vs + (1-self.params.ADV_DELTA)*advQs).squeeze()

                V_advantages, V_returns = self.advantage_and_return(saps.rewards.unsqueeze(0),
                                            Q_target.unsqueeze(0), saps.not_dones.unsqueeze(0))

                
                
                #print(Q_target.shape)
                val_loss = value_step(saps.states, V_returns.squeeze(0),
                                        V_advantages.squeeze(0), saps.not_dones, val_model,
                                        val_opt, self.params, store_to_pass,
                                        old_vs=Vs.squeeze().detach()).mean()


                Q_loss = value_step(torch.cat([saps.states, saps.actions], dim=-1), V_returns.squeeze(0),
                                        V_advantages.squeeze(0), saps.not_dones, self.val_model_Q,
                                        self.val_opt_Q, self.params, store_to_pass,
                                        old_vs=Q_target.detach()).mean()

            elif self.MODE == 'advV_ppo' or self.MODE == 'advV_robust_ppo':
                adv_states, actions = self.apply_advQ_attack(saps.states)
                advQs = self.val_model(adv_states)
                Vs = self.val_model(saps.states)
                ...
                val_loss = value_step(saps.states, V_returns.squeeze(0),
                                        ...
                                        val_opt, self.params, store_to_pass,
                                        old_vs=Vs.squeeze().detach()).mean()
            elif self.MODE == 'advV_ppo_meta' or self.MODE == 'advV_robust_ppo_mata':
                adv_states, actions = self.apply_advQ_attack(saps.states) 
                advQs = self.val_model(adv_states)
                Vs = self.val_model(saps.states)
                self.params.ADV_DELTA_beta = (1/self.params.ADV_DELTA)*(-advQs + Vs)
                ...
                #print(Q_target.shape)
                val_loss, val_grad = value_step(
                                        saps.states, V_returns.squeeze(0),
                                        V_advantages.squeeze(0), saps.not_dones, val_model,
                                        val_opt, self.params, store_to_pass,
                                        old_vs=Vs.squeeze().detach(),
                                        meta=True
                                        )
                val_loss = val_loss.mean()
                delta_grad = (Vs - advQs).detach()
        ...
        if logging:
            ...
            self.plot_info['value_loss'] = val_loss.cpu().detach().numpy()


        if (self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'advQ_robust_ppo' or self.MODE == 'advV_robust_ppo') and not adversary_step and logging:
        ...
        if self.MODE == 'advV_ppo_meta' or self.MODE == 'advV_robust_ppo_mata':
            ...
            args = [saps.states, saps.actions, saps.action_log_probs,
                    ...
                    V_advantages.squeeze(0), policy_model, policy_params,
        ...
        if (self.MODE == 'robust_ppo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'advQ_robust_ppo' or self.MODE == 'advV_robust_ppo') and isinstance(self.policy_model, CtsPolicy) and not adversary_step:
        ...
        if adversary_step:
            ...
            policy_loss, surr_loss, entropy_bonus, policy_grad = self.policy_step(*args)
        ...
        if (self.params.MODE == 'robust_ppo' or self.params.MODE == 'adv_sa_ppo' or self.MODE == 'advQ_robust_ppo' or self.MODE == 'advV_robust_ppo') and not adversary_step:
        ...
        return policy_loss, surr_loss, entropy_bonus, val_loss, policy_grad, val_grad, delta_grad
    ...
    def train_step(self):
        ...
        self.n_steps += 1
        print()
        return avg_ep_reward, self.plot_info
    ...
    def train_step_impl(self, adversary_step=False, increment_scheduler=True):
        ...
        policy_loss, surr_loss, entropy_bonus, val_loss, policy_grad, val_grad, delta_grad = self.take_steps(saps, adversary_step=adversary_step, increment_scheduler=increment_scheduler)
        if self.MODE == 'advV_ppo_meta' :
            self.val_model.zero_grad()
            self.params.POLICY_ADAM.zero_grad()

            saps_meta, avg_ep_reward_meta, avg_ep_length_meta = self.collect_saps(num_saps, collect_adversary_trajectory=adversary_step)
            
            mean_old, std_old = self.policy_model_old(saps_meta.states)
            mean, std = self.policy_model(saps_meta.states)
            dist = mean, std
            new_log_ps = self.policy_model.get_loglikelihood(dist, saps_meta.actions)
            ratio_new_old = ch.exp(new_log_ps - saps_meta.action_log_probs)
            
            h = torch.bmm(torch.transpose((mean_old-mean).unsqueeze(2),1,2),(torch.inverse(torch.diag((std_old+std)/2))).repeat(mean.shape[0],1,1))
            h = torch.bmm(h, (mean_old-mean).unsqueeze(2))
            h = torch.exp(-1/8*h)
            h1 = torch.det(torch.diag(std_old))**(1/4)*torch.det(torch.diag(std))**(1/4)
            h1 = h1/torch.det(torch.diag((std_old+std)/2))
            h = 1 -h1.repeat(mean.shape[0],1,1)*h
            surrogate = -(ratio_new_old * saps_meta.advantages) + self.params.THIRD_W*h*(((2-h)**2)**(1/2))
            
            J_grad = ch.autograd.grad(surrogate.mean(), self.policy_model.parameters())

            J_grad = [item.view(-1) for item in J_grad]
            J_grad = ch.cat(J_grad) 

            
            self.params.ADV_DELTA.data -= self.params.META_LR * (delta_grad.mean() * (policy_grad.detach() * (J_grad.detach() ) ).sum() +self.params.SECOND_W)
            self.params.ADV_DELTA.data = torch.clamp(self.params.ADV_DELTA.data, 0, 1)
            

            self.plot_info['adv_delta'] = self.params.ADV_DELTA.data.cpu().detach().numpy()
        ...
        self.plot_info['policy_loss'] = policy_loss
    ...
    def run_test(self, max_len=2048, compute_bounds=False, use_full_backward=False, original_stdev=None, danger_state=None):
        print("-" * 80)
        start_time = time.time()
        ...
        with torch.no_grad():
            output = self.run_test_trajectories(max_len=max_len, danger_state=danger_state)
            ep_length, ep_reward, actions, action_means, states, behaviour = output
            ...
            # Unroll the trajectories (actors, T, ...) -> (actors*T, ...)
        return ep_length, ep_reward, actions.cpu().numpy(), action_means.cpu().numpy(), states.cpu().numpy(), kl_upper_bound, behaviour

    def run_test_trajectories(self, max_len, should_tqdm=False, danger_state=None):
        # Arrays to be updated with historic info
        envs = self.envs
        initial_states = self.reset_envs(envs, danger_state, first_time=True)
        ...
        #current_state = envs[0].env.sim.get_state()
        #pre_posafter, pre_height, pre_ang = current_state.qpos[0:3]


        dict_list = [
                        'eval_rewards_steps',
                        'eval_rewards_sqr_steps',
                        'eval_rewards_posa_steps',
                        'eval_rewards_alive_steps',
                        'eval_rewards_quad_ctrl_cost_steps',
                        'eval_rewards_quad_impact_cost_steps',
                        'eval_rewards_forward_steps',
                        'eval_rewards_ctrl_cost_steps',
                        'eval_rewards_contact_cost_steps', 
                        'eval_done_steps',
                        'eval_done_finite_steps',
                        'eval_done_abs_steps',
                        'eval_done_ang_min_steps',
                        'eval_done_ang_max_steps',

                        'eval_done_height_max_steps',
                        'eval_done_height_min_steps',

                        'eval_done_qpos_max_steps',
                        'eval_done_qpos_min_steps',

                        'eval_delta_height_steps',
                        'eval_delta_ang_steps',                                  
                    ]
        results_dict = {i : ch.zeros(shape) for i in dict_list }
        ...
        for t in iterator:
            ...
            if self.params.ATTACK_TRANSITION:
                if (t+1) % self.params.TEST_ATTACK_FREQ == 0:

                    maybe_attacked_last_states = self.apply_attack(last_states)
                    self.reset_state(self.envs[0].state_filter.scale_back(maybe_attacked_last_states -last_states))
            ...
            current_state = envs[0].env.sim.get_state()
            posbefore = current_state.qpos[0]
            ret = self.multi_actor_step(next_actions, envs)

            # done_info = List of (length, reward) pairs for each completed trajectory
            # (next_rewards, next_states, next_dones) act like multi-actor env.step()
            done_info, next_rewards, next_states, next_not_dones, posafter, height, ang, s, info = ret
            eval_done = len(done_info)

            # Reset the policy (if the policy has memory if we are done)
            if next_not_dones.item() == 0:
                self.policy_model.reset()
                self.val_model.reset()
            if self.params.GAME == 'Hopper-v2':
                results_dict['eval_rewards_steps'][:, t] = info['true_reward']
                results_dict['eval_rewards_sqr_steps'][:, t] = 1e-3 * np.square(next_actions[0].cpu().numpy()).sum() 
                results_dict['eval_rewards_posa_steps'][:, t] =  (posafter - posbefore) /envs[0].env.dt 
                results_dict['eval_rewards_alive_steps'][:, t] =1 
                
                
                eval_done_ang_min = not(((ang) > -0.2))
                eval_done_height_min = not((height > 0.7))
                eval_done_ang_max = not(((ang) < 0.2))
                eval_done_abs = not((np.abs(s[2:]) < 100).all())
                eval_done_finite = not(np.isfinite(s).all())

                results_dict['eval_done_steps'][:, t] = eval_done

                results_dict['eval_done_height_min_steps'][:, t] =  eval_done_height_min 
                results_dict['eval_done_ang_min_steps'][:, t] = eval_done_ang_min 
                results_dict['eval_done_ang_max_steps'][:, t] = eval_done_ang_max
                results_dict['eval_done_abs_steps'][:, t] = eval_done_abs 
                results_dict['eval_done_finite_steps'][:, t] = eval_done_finite
                
            
            
            if self.params.GAME == 'Walker2d-v2':
                results_dict['eval_rewards_steps'][:, t] = info['true_reward'] 
                results_dict['eval_rewards_sqr_steps'][:, t] =  1e-3 * np.square(next_actions[0].cpu().numpy()[0]).sum()
                results_dict['eval_rewards_posa_steps'][:, t] =  (posafter - posbefore) /envs[0].env.dt 
                results_dict['eval_rewards_alive_steps'][:, t] =  1 


                eval_done_height_min = not(height > 0.8)
                eval_done_height_max = not(height < 2.0)
                eval_done_ang_min = not(ang > -1.0)
                eval_done_ang_max = not(ang < 1.0)

                results_dict['eval_done_steps'][:, t] = eval_done 

                results_dict['eval_done_height_min_steps'][:, t] = eval_done_height_min 
                results_dict['eval_done_height_max_steps'][:, t] = eval_done_height_max 
                results_dict['eval_done_ang_min_steps'][:, t] =  eval_done_ang_min
                results_dict['eval_done_ang_max_steps'][:, t] = eval_done_ang_max 
            
            if self.params.GAME == 'Ant-v2':
                results_dict['eval_rewards_steps'][:, t] = info['true_reward'] 
                results_dict['eval_rewards_ctrl_cost_steps'][:, t] = info['reward_ctrl'] 
                results_dict['eval_rewards_contact_cost_steps'][:, t] =  info['reward_contact'] 
                results_dict['eval_rewards_forward_steps'][:, t] = info['reward_forward']
                results_dict['eval_rewards_alive_steps'][:, t] =  info['reward_survive'] 

                eval_done_qpos_min = not(s[2] >= 0.2)
                eval_done_qpos_max = not(s[2] <= 1.0)

                results_dict['eval_done_steps'][:, t] =  eval_done 

                results_dict['eval_done_qpos_min_steps'][:, t] = eval_done_qpos_min
                results_dict['eval_done_qpos_max_steps'][:, t] = eval_done_qpos_max 

            if self.params.GAME == 'Humanoid-v2':
                results_dict['eval_rewards_steps'][:, t] = info['true_reward'] 
                results_dict['eval_rewards_ctrl_cost_steps'][:, t] = info['reward_quadctrl'] 
                results_dict['eval_rewards_quad_impact_cost_steps'][:, t] = info['reward_impact']
                results_dict['eval_rewards_posa_steps'][:, t] = info['reward_linvel']
                results_dict['eval_rewards_alive_steps'][:, t] = info['reward_alive']
                #print(len(self.envs))
                #print(info['reward_alive'] + info['reward_linvel'] + info['reward_impact']+ info['reward_quadctrl'])
                #print(self.envs[0].total_true_reward , (eval_rewards_ctrl_cost + eval_rewards_quad_impact_cost + eval_rewards_posa + eval_rewards_alive))                

                eval_done_qpos_min = not(ang > 1)
                eval_done_qpos_max = not(ang < 2.0)
            ...
            if self.params.GAME == 'HalfCheetah-v2':
                results_dict['eval_rewards_steps'][:, t] = info['true_reward'] 
                results_dict['eval_rewards_ctrl_cost_steps'][:, t] = info['reward_ctrl']
                results_dict['eval_rewards_posa_steps'][:, t] = info['reward_run']
            ...
            if self.params.GAME == 'Swimmer-v2':
                results_dict['eval_rewards_steps'][:, t] = info['true_reward'] 
                results_dict['eval_rewards_ctrl_cost_steps'][:, t] = info['reward_ctrl']
                results_dict['eval_rewards_posa_steps'][:, t] = info['reward_fwd']
        ...
        to_ret = (ep_length, ep_reward, actions, action_means, states , results_dict)


        return to_ret

    def reset_state(self, state_delta):
        names = {'Hopper-v2': 1 , 'Humanoid-v2':2 , 'Walker2d-v2': 1, 'Ant-v2':2, 'HalfCheetah-v2':1, 'Swimmer-v2':2}
        start = names[self.params.GAME]

        current_state = self.envs[0].env.sim.get_state()

        if self.params.GAME == 'Hopper-v2' or self.params.GAME == 'Walker2d-v2' or self.params.GAME == 'HalfCheetah-v2' or self.params.GAME == 'Swimmer-v2':
            true_obs = np.concatenate(
                            [self.envs[0].env.sim.data.qpos.flat[start:], self.envs[0].env.sim.data.qvel.flat]
                        )
            adv_state = (state_delta + true_obs)[0]
            
            current_state.qpos[start:] = adv_state[0:len(current_state.qpos)-start].cpu()
            current_state.qvel[:] = adv_state[len(current_state.qpos)-start:len(current_state.qpos)-start+len(current_state.qvel)].cpu()
        if self.params.GAME == 'Humanoid-v2':
            true_obs = np.concatenate(
                                [
                                    self.envs[0].env.sim.data.qpos.flat[start:],
                                    self.envs[0].env.sim.data.qvel.flat,
                                    self.envs[0].env.sim.data.cinert.flat,
                                    self.envs[0].env.sim.data.cvel.flat,
                                    self.envs[0].env.sim.data.qfrc_actuator.flat,
                                    self.envs[0].env.sim.data.cfrc_ext.flat,
                                ]
                            )
            adv_state = (state_delta + true_obs)[0]
            
            current_state.qpos[start:] = adv_state[0:len(current_state.qpos)-start].cpu()
            start = len(current_state.qpos)-start
            current_state.qvel[:] = adv_state[start :start+len(current_state.qvel)].cpu()
        ...
        if self.params.GAME == 'Ant-v2':
            ...
            start = len(current_state.qpos)-start
            current_state.qvel[:] = adv_state[start :start+len(current_state.qvel)].cpu()
            #start = start + len(current_state.qvel)
            #current_state.cfrc_ext[:] = adv_state[start:start+len(current_state.cfrc_ext)].cpu()

        #print("1.",current_state)
        #print("1.5 denormal",env.state_filter.de_norm(adv_state))
        #if (self.attack_val is None) or (self.attack_model == 'sappo'):
        #print('reset_state, state', adv_state)
        #name = env.unwrapped.spec.id
        #print("name", name)
        
        
        #print(env.env.sim.get_state())
        self.envs[0].env.sim.set_state(current_state)
        #print("2.",env.env.sim.get_state())
        self.envs[0].env.sim.forward()
        #print("after", env.env.sim.get_state())
