...
def run():
    ...
    for dataset_name, dataset in dataset.items():
        ...
        #         break
        while True:
            i = input("Conversation ID >>> ")
            dialog = dataset[int(i)]
            print('================Conversation {}================'.format(i))
            personality = dialog['personality']

            for persona_idx, persona in enumerate(personality):
                persona_decoded = tokenizer.decode(persona, skip_special_tokens=True)
                print('Persona {}: {}\n'.format(persona_idx, persona_decoded))
            ...
            for j, utterance in enumerate(dialog['utterances']):
                ...
                # history_ = utterance['history']
                speaking_partner = utterance['history'][-1]
                keywords = utterance['key']

                # history_ = tokenizer.encode(history_)
                speaking_partner_decode = tokenizer.decode(speaking_partner, skip_special_tokens=True)
                reply = tokenizer.decode(reply, skip_special_tokens=True)
                keywords = [tokenizer.decode(keyword, skip_special_tokens=True) for keyword in keywords]

                print('Speaking Partner >>> {}'.format(speaking_partner_decode))
                print('Golden Reply >>> {}'.format(reply))
                print('BERT Keywords >>> {}'.format(keywords))

                reference = re.findall(r"[\w']+|[.,!?;]", reply)


                raw_text_key = input("Your Key Words >>> ")
                key_phrase = [tokenizer.encode(raw_text_key)]

                history.append(speaking_partner)
                ...
                for seed in range(args.num_suggestions):
                    ...
                    with torch.no_grad():
                        out_ids = sample_sequence(personality, history, tokenizer, key_phrase, model, args)

                    out_text = tokenizer.decode(out_ids, skip_special_tokens=True)
                    print("Suggested Reply {} >>> {}".format(seed, out_text))
                    candidate = re.findall(r"[\w']+|[.,!?;]", out_text)
                    score_bleu = sentence_bleu([reference], candidate)
                    print("BLEU >>> {}".format(score_bleu))
