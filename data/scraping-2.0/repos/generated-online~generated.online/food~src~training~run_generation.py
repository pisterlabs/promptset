...
def prepare_ctrl_input(args, _, tokenizer, prompt_text):
    ...
    if not any(encoded_prompt[0] == x for x in tokenizer.control_codes.values()):
        logger.info(
            "WARNING! You are not starting your generation from a control code so you "
...
def main():
    ...
    try:
        ...
        raise KeyError(
            "the model {} you specified is not supported. You are welcome to add it and "
            "open a PR :)"
        )
    ...
    for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
        ...
        # Add the prompt at the beginning of the sequence. Remove the excess text that
