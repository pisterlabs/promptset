...
assistant_id = 'asst_9yJxQXDqFpdTKgH76fWWXNjY'
...
st.set_page_config(page_title="Chat with GPT about Rare Diseases", page_icon=":speech_balloon:")
...
# Uncomment the following lines to create a sidebar for API key configuration and additional features
...
st.sidebar.header("Add Your Features")
...
st.title("Chat with GPT about Rare Diseases")
st.write("This is a chat application that uses OpenAI to answer questions about rare diseases. ")
...
if st.session_state.start_chat:
    ...
    if prompt := st.chat_input("What is up?"):
        ...
        run = client.beta.threads.runs.create(
            thread_id=st.session_state.thread_id,
            assistant_id=assistant_id,
            instructions="You are a specialist in medical genetics and are here to answer questions related to genetics and rare diseases only. Strictly answer only questions related to medical or healthcare related matters specifically rare or genetic diseases.  If anyone asks any question not related to these topics, you must refuse to reply. Provide references to retrieved files where possible and add a link to the document. When adding other information mark it clearly as such.with a different color"
        )
