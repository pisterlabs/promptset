...
class Baseline():
    ...
    def generate_goal_prompt(self, objects):
        ...
        The goal should not contain any objects that are not in the environment, and different from the previous goal.
        You have to move all the blocks in the environment, not any bowls.
        Skills available: pick block place in next to other block, pick block place in bowl, pick block place on block
        format: 
            preference =
            goal1 = 
        if you think you are sure about the preference, please type 'goal1 = done'
        """.format(str(objects))

    def generate_obj_prompt(self):
        self.obj_prompt = """
        Now, you have to build an new environment that helps to find out the user's preference.
    ...
    def scene_generation(self,  flag = False):
        ...
        self.messages.append({'role':'user','content':template+self.obj_prompt})
        while objects == None:
            f_src = self.ask_vision()
            lines = f_src.split('\n')
            print(f_src)
            for l in lines:
                l = l.lower()
    ...
    def user_feedback_append(self, image_path = './temp/stop.png', flag = True,\
        ...
        if flag:
            ...
            if prev:
                ...
                content.append(image_message)
            question = "But the user stopped me here. The previous goal is not correct. \
                Please generate totally differet goal. The user did not perfer the current scene."
            content.append({"type": "text", "text": question})
        ...
        #     Please remeber that the preference more related to the semantic meaning, not the exact precision of placing.\
