...
class Baseline():
    def __init__(self, key_file_path = './key/key.txt', VERBOSE=True):
        ...
        self.retrive_messages = [{'role': 'system', 'content': self.system_prompt}]
    ...
    def inner_thought(self, scene, goal, fail=True):
        question = "I am trying to infer the user's preference from the scene, with goal: {}.".format(goal)
        if fail:
            question += " but I failed to infer the preference from the scene."
        else:
            question += " and I successfully inferred the preference from the scene."
        question += "scene: {}".format(scene)
        question += "can you extract some usuful informations from the past scenes to infer the user's preference?"
        self.retrive_messages.append({"role": "user", "content": question})
        f_src = openai.ChatCompletion.create(
                        messages=self.retrive_messages,
                        model = 'gpt-4-1106-preview'
                    )['choices'][0]['message']['content'].strip()
        self.retrive_messages.append({"role": "assistant", "content": f_src})
        # self.messages.append({"role": "user", "content": f_src})
        return f_src
        
    def append_in(self, scene):
        question = "I have retrieved some information from the past scenes."
        question += "scene: {}".format(scene)
        question += "Use this information to infer the user's preference."
        self.messages.append({"role": "user", "content": question})
