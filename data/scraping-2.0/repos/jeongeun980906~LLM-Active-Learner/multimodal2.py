...
from llm.vision import Baseline
import openai
...
def main(args):
    ...
    while num_iter < 5:
        ...
        if gui.label ==3:
            continue
        time.sleep(3)
        logger.user_feedback_append(flag=flag, goal = goal_1, prev = prev)
        num_iter += 1
    ...
    #save video
    rendered_clip.write_videofile(f'./temp/{name}_v.mp4')


    #save log
    with open(f'./temp/{name}_v.txt', 'w') as f:
