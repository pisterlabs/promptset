...
def download(args, url, filename = None, root = CACHE_PATH):
    
    print(f"args.local_rank : {args.local_rank}")

    if (
        not distributed_utils.is_distributed
        or args.local_rank == 0
...
class OpenAIDiscreteVAE(nn.Module):
    def __init__(self, args):
        ...
        self.enc = load_model(download(args, OPENAI_VAE_ENCODER_PATH))
        self.dec = load_model(download(args, OPENAI_VAE_DECODER_PATH))
...
class VQGanVAE1024(nn.Module):
    def __init__(self, args):
        ...
        print(f" ************** VQGanVAE1024 **************** ")
        model_filename = 'vqgan.1024.model.ckpt'
        config_filename = 'vqgan.1024.config.yml'


        download(args, VQGAN_VAE_CONFIG_PATH, config_filename)
        download(args, VQGAN_VAE_PATH, model_filename)
