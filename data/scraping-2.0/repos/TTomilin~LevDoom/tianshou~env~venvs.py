...
class BaseVectorEnv(object):
    ...
    All of the VectorEnv must inherit :class:`~LevDoomLab.env.BaseVectorEnv`.
    ...
    def normalize_obs(self, obs: np.ndarray) -> np.ndarray:
        ...
        if self.obs_rms and self.norm_obs:
            ...
            obs = obs / 255. * 2 - 1
            obs = (obs - np.mean(obs)) / (np.std(obs) + self.__eps)
...
class DummyVectorEnv(BaseVectorEnv):
    ...
    .. seealso::
        ...
        Please refer to :class:`~LevDoomLab.env.BaseVectorEnv` for other APIs' usage.
    """
