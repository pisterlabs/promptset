"""Diese Datei dient dazu, das Sprachmodell für die Stufe 3 zu initialisieren."""
...
def get_chain():
    ...
    # Lädt die synthetischen Dokumente mithilfe von einem DirectoryLoader ein
    loader = DirectoryLoader(r'C:\Users\meine\PycharmProjects\BA-Chatbasiertes_Datenextraktionstool\step3\docs_data', glob="**/*.txt", loader_cls=TextLoader)
    docs = loader.load()

    # Teilt die Dokumente zur besseren Verarbeitung in kleinere Texte auf
    ...
    prompt_template = """Given the following directory that contains information about an ifc-file which is a building information model
                ...
                Below the short answer you should add a long answer that contains the document that you got the information from
                and reasons for your answer. You should also explain, why you dont know the answer. (Max. 2 sentences)
    ...
    chain = RetrievalQA.from_chain_type(llm=llm, chain_type="stuff", retriever=docsearch.as_retriever(search_kwargs={"k": 1}), chain_type_kwargs=chain_type_kwargs)
