...
from rl_app import video_recorder
...
class Monitor(Wrapper):
  ...
  def step(self, action):
    ...
    self._capture_if_required(done)
    return observation, reward, done, info

  def _capture_if_required(self, done=False):
    # this code captures one frame only after 1/fps seconds.
    curr_time = time.time()
    while curr_time - self._start_time >= self._n_frames / float(self.fps):
      if (curr_time - self._start_time >=
          (self._n_frames + 1) / float(self.fps)) and not done:
        # use last frame
        self.video_recorder.capture_frame(last_frame=True)
      else:
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''
  ...
  def _set_mode(self, mode):
    if mode == 'evaluation':
      ...
      raise error.Error('Invalid mode {}: must be "training" or "evaluation"',
                        mode)
...
def detect_training_manifests(training_dir, files=None):
  ...
  return [
      os.path.join(training_dir, f) for f in files
...
def collapse_env_infos(env_infos, training_dir):
  ...
  for key in ['env_id', 'gym_version']:
    if key not in first:
      raise error.Error(
          "env_info {} from training directory {} is missing expected key {}. This is unexpected and likely indicates a bug in gym."
