...
seeds.extend(get_datas('data/samples_bitna.jsonl'))
seeds.extend(get_datas('data/samples_dial_state_heum.jsonl'))
augmented_dials = get_datas('data/augmented_dial_v2.1_6shots_temp1.0_pp0.3_gpt-4.jsonl')
# augmented_dials = []

answers = []
for sample in tqdm(augmented_dials, desc=f"Completing..."):
    rand_seeds_indies = []
    seeds_pool = []
    while len(rand_seeds_indies) < 3:
    ...
    for dial in splited_dialogue[1:]:
        ...
        # 본인 프롬프트에 맞게 수정
        prompt = f'''### Instruction: Please creat a current dialogue state base on the last bot response. Please be mindful our dialogues have chariteristic that user ask a food or travel recommendation bot. Previous State denotes the dialogue state that is based on except that last utterance of user and bot. You should create a dialogue state that helps to request additional information to user for better recommendation. You should create a state following key-value format; Please diversely create a dialogue state so that bot can request new information to users. You should update Current State from Previous State and creat new states to request additional information to user. You should follow the structure of given samples; \n ### Input: [Sample 1] [Previous State] 'prev_state': {samples_pool[0]['prev_state']} [Dialogue] {samples_pool[0]['dialogue']} [Current State] 'current_state' : {samples_pool[0]['cur_state']} \n [Sample 2] [Previous State] 'prev_state': {samples_pool[1]['prev_state']} [Dialogue] {samples_pool[1]['dialogue']} [Current State] 'current_state' : {samples_pool[1]['cur_state']} \n [Sample 3] [Previous State] 'prev_state': {samples_pool[2]['prev_state']} [Dialogue] {samples_pool[2]['dialogue']} [Current State] 'current_state' : {samples_pool[2]['cur_state']} \n ### Output: [Sample 4] [Previous State] 'prev_state': {prev_state} [Dialogue] {restore_dial} [Current State] 'current_state': '''
...
with open('data/augmented_dial_state_v2_temp1.0_pp0.3.jsonl', 'w', encoding='utf-8') as fw:
    for data in answers:
