...
async def list_guild_events(self, ctx: commands.Context) -> list:
    events = await fetch_guild_events()  # Some hypothetical function to fetch events

    if not events:
        # If there are no events, you can display a message to the user
        await ctx.send("There are no events currently.")
