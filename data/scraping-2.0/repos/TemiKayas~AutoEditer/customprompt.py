...
# Load your API key from an environment file
...
def custompromptcall(custom_prompt):
    ...
    # Extract key ideas using GPT-3.5 with a custom prompt
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a helpful assistant."},
            {"role": "user", "content": custom_prompt.format(combined_text)}
        ]
    )
