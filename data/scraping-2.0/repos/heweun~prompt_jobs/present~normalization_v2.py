...
## 크롤링 병합 결과 출력하기
new = pd.read_csv(path+'/data/0725master.csv')
...
def generate_job_name(Text):
    ...
    Step 3: If the answer is true, select only one job name from the following list '{Job}' that describes the job information.
...
new.to_csv(path+"/data/0725master_v3.csv", index=False)