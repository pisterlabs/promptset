...
# Function to convert Excel sheets to Markdown tables
def excel_to_markdown(file_path: str) -> str:
    markdown_tables = ""
    xls = pd.ExcelFile(file_path)
    for sheet_name in xls.sheet_names:
        df = pd.read_excel(xls, sheet_name)
        markdown_tables += f"### {sheet_name}\n"
        markdown_tables += df.to_markdown() + "\n\n"
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--import-excel", type=str, help="Path to the Excel file to be imported")
...
def main():
    ...
    if args.import_excel:
        file_content += excel_to_markdown(args.import_excel)
