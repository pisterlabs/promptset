...
def main(argv):
    prompt_ids = [21, 22, 23, 24, 25]

    for prompt_id in prompt_ids:
        prompt_responses = list(db.query("""SELECT id FROM prompt_response WHERE prompt_id = %s""", [prompt_id]))
        ...
        db.query("""DELETE FROM prompt_response WHERE prompt_id = %s""", [prompt_id])
