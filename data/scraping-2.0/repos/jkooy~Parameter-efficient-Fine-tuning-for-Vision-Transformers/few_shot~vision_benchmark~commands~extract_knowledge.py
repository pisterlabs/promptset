...
def add_zero_shot_args(parser):
    ...
    parser.add_argument('--n_shot', type=int, default=5, help="number of shots")
...
def write_to_yaml_file(out, yaml_file):
    with open(yaml_file, 'w') as fp:
        yaml.dump(out, fp, default_flow_style=False, encoding='utf-8', allow_unicode=True)
...
def extract_konwledge_for_odinw35(config, args):
    ...
    dict_category_and_query = load_from_yaml_file('resources/datasets/od/odinw_benchmark35.yaml')
    ...
    name2wiki = {}
    wiki_answer_list = []
    dict_category_and_query_output = {}
    for classname, items in tqdm(dict_category_and_query.items(), f'Extracting wiki knowledge with Wikitionary.'):
        ...
        # Knowledge source 1: Wiki Definition
        try:
            classname_query = clean_name(classname)
            # print(classname)
            if classname_query.split('-')[0] in ['dog', 'cat']:
                
                classname_query = classname_query.split('-')[-1]

            wiki_knowledge_text = resolve_meaning( classname_query , wik_dict)
        except:
            continue
        ...
        items['path_wn3'] = items['path_wn'][:min(len(items['path_wn']), 3)]
        items['path_wn10'] = items['path_wn'][:min(len(items['path_wn']), 10)]
        # pdb.set_trace()

        dict_category_and_query_output[classname] = items


    # Using a JSON string
    write_to_yaml_file(dict_category_and_query_output, f'resources/datasets/od/odinw_benchmark35_knowledge.yaml')  
...
def extract_gpt3_konwledge_for_odinw35(config, args):
    ...
    dict_category_and_query = load_from_yaml_file('resources/datasets/od/odinw_benchmark35_knowledge.yaml')
    ...
    for classname, items in tqdm(dict_category_and_query.items(), f'Extracting wiki knowledge with Wikitionary.'):
        if items['def_wiki']:
            count_has_wiki_knowledge += 1
            name2wiki_exist.append( (classname, items['def_wiki']) )
        else: 
            print(f'Missing wiki knowledge: {classname}')

    print(f'The wiki knowledge coverage is {count_has_wiki_knowledge}/{len(dict_category_and_query)} ')
    ...
    wiki_answer_list = []
    dict_category_and_query_output = {}
    for classname, items in tqdm(dict_category_and_query.items(), f'Extracting GPT3 knowledge.'):


        pred_answer_list, pred_prob_list = ask_gpt3(args, classname, name2wiki_exist)
        print(f'GPT3 | {classname}: {pred_answer_list}')
                
        items['gpt3'] = pred_answer_list

        dict_category_and_query_output[classname] = items
        logging.info(f'=> GPT3 knowledge extraction duration time: {time.time() - start:.2f}')

    write_to_yaml_file(dict_category_and_query_output, f'resources/datasets/od/odinw_benchmark35_knowledge_and_gpt3.yaml')  
...
def main():
    ...
    # if args.dataset_file_name == 'odinw_benchmark35':
    #     # extract_konwledge_for_odinw35(config, args)
    #     extract_gpt3_konwledge_for_odinw35(config, args)
