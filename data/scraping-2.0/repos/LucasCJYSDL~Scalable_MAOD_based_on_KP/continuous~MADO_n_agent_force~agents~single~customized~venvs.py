...
from agents.single.customized.dummy import DummyEnvWorker, EnvWorker
...
class BaseVectorEnv(gym.Env):
    ...
    def reset(
        self, id: Optional[Union[int, List[int], np.ndarray]] = None, random_init: bool=False, is_sample: bool=False
    ) -> np.ndarray:
        ...
        if self.is_async:
            self._assert_id(id)
        obs_list = [self.workers[i].reset(random_init=random_init, is_sample=is_sample) for i in id]
        # print("Here: ", obs_list)
        try:
