...
class database_methods():
    ...
    def fetch_user(self,user_name):
        try:
            ...
            return 'Exception'   
    def add_user(self,username,password,recording_name,user_tier):
        try:
            conn,cursor=self.connect_db()
            sql_insert = "INSERT INTO user_data (username, password, recording_name, user_tier, recording_id) VALUES (%s, %s, %s, %s, %s)"
            restaurant_id = self.generate_id()
            record = (username, password, recording_name, user_tier, recording_name)
    ...
    def query_pinecone(self,query,recording_id):
        index=self.pinecone_init()
        openai.api_key = os.getenv('open_api_key')
        MODEL=os.getenv('Embedding_Model')
        xq = openai.Embedding.create(input=query, engine=MODEL)['data'][0]['embedding']
        res = index.query([xq], top_k=20, include_metadata=True,namespace=recording_id)
        #return res['matches']
        recording=[]
