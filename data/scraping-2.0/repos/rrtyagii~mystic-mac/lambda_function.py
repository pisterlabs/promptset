...
def webhook_verification(event, context):
    ...
    if len(query_params) > 0:
        if query_params['hub.mode'] == 'subscribe':
            if query_params['hub.verify_token'] == '':
                response = {
                    'statusCode': 200,
...
def send_message(phone_number, message, phone_number_id, permanent_token):
...
def read_reciepts(phone_number_id, message_ID, permanent_token):
