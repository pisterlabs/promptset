...
# optimizer
optimizer = model.configure_optimizers_back(weight_decay, learning_rate, (beta1, beta2))
if init_from == 'resume':
...
def get_lr(iter):
    # return torch.maximum(torch.tensor( 6e-4*(1 + math.cos(math.pi*iter/5000))/2),torch.tensor(0.000001))
    if iter > lr_decay_iters:
        return min_lr    
    decay_ratio = (iter) / (lr_decay_iters)
    assert 0 <= decay_ratio <= 1
    coeff = 0.5 * (1.0 + math.cos(math.pi * decay_ratio)) # coeff ranges 0..1
    return min_lr + coeff * (learning_rate - min_lr)    
# logging
