...
from sky_langchain.log import logger
...
class RecursiveUrlLoader(BaseLoader):
    ...
    def _get_child_links_recursive(
        ...
        visited.add(url)
        logger.info(f"scraping: {len(visited)} links")
        try:
        ...
        sub_links = extract_sub_links(
            response.text,
            url,
            base_url=self.domain or self.url,
        ...
        for i, link in enumerate(sub_links):
            if depth == 0:
                logger.info(f"progress: {i}/{len(sub_links)} links")
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        if self.use_async:
            ...
            data = self._get_child_links_recursive(self.url, visited)
            return data
