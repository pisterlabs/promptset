...
def predict(input, history=[]):
    ...
    pdreplace = populate_dataframe_from_csv('word replacements.txt')

    input = replace_text_in_first_column_with_2nd_column(input, pdreplace)
...
def replace_text_in_first_column_with_2nd_column(text, df):
    for index, row in df.iterrows():
        text = text.replace(row[0], row[1])
    return text

def populate_dataframe_from_csv(csvfile):
...
if __name__ == "__main__":    
    ...
    gr.Interface(fn=predict,
             ...
             ["Can you list the steps to printing a label in imanage records manager?"],
