    ...
    def get_example(index):
        ...
        if len(example_idx) > 0:
            ...
            for key, value in DIc_.items():
                ...
                # print("value",value)
                # print("-")
                if count_["DiseaseClass"] < 6 and count_["SpecificDisease"] < 6 and count_["CompositeMention"] < 6 and count_["Modifier"] < 6:
                    for label in value:
                        label_text = label[0]
                        type_ = label[1]
                        if type_ == "DiseaseClass":
                            context = context.replace(label_text, "</D>" + label_text+"|True|as it is DiseaseClass" + "<D>")
                            count_["DiseaseClass"] = count_["DiseaseClass"] + 1

                        if type_ == "SpecificDisease":
                            context = context.replace(label_text, "</S>" + label_text+"|True|as it is SpecificDisease" + "<S>")
                            count_["SpecificDisease"] = count_["SpecificDisease"] + 1
                        if type_ == "CompositeMention":
                            context = context.replace(label_text, "</C>" + label_text+"|True|as it is CompositeMention" + "<C>")
                            count_["CompositeMention"] = count_["CompositeMention"] + 1
                        if type_ == "Modifier":
                            context = context.replace(label_text, "</M>" + label_text+"|True|as it is Modifier" + "<M>")
...
if __name__ == '__main__':
    ...
    example_idx = read_idx("data/NCBI/5shot_test.jsonl")

    last_results = None
    # if args.last_results != "None":
    #     last_results = read_results(dir_=args.last_results)
    data_name = "NCBI"
    prompts = mrc2prompt(mrc_data=ner_test, data_name=data_name, example_idx=example_idx, train_mrc_data=mrc_train,
                         example_num=args.example_num, last_results=last_results)

    inputfile = "1shot_NCBI.json"
    out_putfile = "data/NCBI/our_method_NCBI_shot5.json"  #
