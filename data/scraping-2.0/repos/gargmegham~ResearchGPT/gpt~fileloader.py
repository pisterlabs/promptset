...
class UnstructuredFileIOLoader(UnstructuredBaseLoader):
    ...
    def __init__(
        self, file: IO, filename: str, mode: str = "single", **unstructured_kwargs: Any
    ):
    ...
    def _get_elements(self) -> list:
        return partition(
            file=self.file, file_filename=self.filename, **self.unstructured_kwargs
        )
...
def read_bytes_to_text(file: bytes, filename: str) -> str:
    return "\n\n".join(
        [
            doc.page_content
            for doc in read_bytes_to_documents(file=file, filename=filename)
        ]
    )
