import openai, os, uuid, subprocess
import configparser
from utils.logger import Logger
from utils.db import save_data_to_db, load_data_from_db
from utils.json_validator import parse_json_string

config = configparser.ConfigParser()
config.read('config.ini')

openai.api_key = config.get('ai', 'api_key')

class Agent:
  def __init__(self, name_prefix='AI-', log_level=Logger.INFO, print_tokens=True, model='gpt-3.5-turbo'):
    self.uuid = str(uuid.uuid4())
    self.logger = Logger('{}:{}'.format(name_prefix, self.uuid.split('-')[0]), log_level=log_level)
    self.model = model
    self.show_token_use = print_tokens

  def print_tokens(self, response):
    if self.show_token_use:
      usage = response.usage
      if usage['total_tokens'] <= 2000:
        self.logger.data(f'Used {usage["total_tokens"]} tokens.')
      elif usage['total_tokens'] <= 3000:
        self.logger.warn(f'Used {usage["total_tokens"]} tokens.')
      elif usage['total_tokens'] <= 3500:
        self.logger.error(f'Used {usage["total_tokens"]} tokens.')


  def get_response(self, p):
    global model
    m = [{"role": 'system', "content": p}]
    response = openai.ChatCompletion.create(
      model=self.model,
      messages=m,
      temperature=0.2,
    )
    self.print_tokens(response)
    txt = response.choices[0].message.content.strip()
    return txt
  

  def process_request(self, req_uuid, p):
    response_text = self.get_response(p)
    self.logger.info(f'Response received: {response_text}')
    data = parse_json_string(response_text)
    if data:
      save_data_to_db("blackboards", req_uuid, "response", data)
      self.logger.data(f'Response written to database with UUID {req_uuid}')


  def check_response_status(self, req_uuid):
    response_text = load_data_from_db("blackboards", req_uuid, "response")
    if response_text:
      return parse_json_string(response_text)
    else:
      return False


  def submit_request(self, p, req_uuid):
    response_text = load_data_from_db("blackboards", req_uuid, "response")
    if not response_text:
      save_data_to_db("blackboards", req_uuid, "response", {'uuid': 'pending'})
    subprocess.Popen(['python', '-c', f'''from utils.async_ai import Agent; agent = Agent(print_tokens=True); agent.process_request("{req_uuid}", """{p}""")'''])
    return True
