    ...
    for i in range(48, 255):
        ...
        if chunk:
            ...
            if character_range is None:
                pass
            elif is_unicode_range_secondary(character_range) is False:
...
def mb_encoding_languages(iana_name: str) -> List[str]:
    ...
    This function does the correspondence.
    """
    if iana_name.startswith('shift_') or iana_name.startswith('iso2022_jp') or iana_name.startswith('euc_j') or iana_name in frozenset({'cp932'}):
        return ['Japanese']
    else:
        if iana_name.startswith('gb') or iana_name in frozenset({'big5hkscs', 'big5', 'cp950'}):
            return [
             'Chinese', 'Classical Chinese']
        if iana_name.startswith('iso2022_kr') or iana_name in frozenset({'euc_kr', 'johab', 'cp949'}):
            return [
             'Korean']
        return []
...
def characters_popularity_compare(language: str, ordered_characters: List[str]) -> float:
    ...
    for character in ordered_characters:
        if character not in FREQUENCIES[language]:
            ...
            if len(characters_before_source) == 0:
                if before_match_count <= 4:
                    character_approved_count += 1
                    continue
            if len(characters_after_source) == 0:
                if after_match_count <= 4:
                    character_approved_count += 1
                    continue
...
def merge_coherence_ratios(results: List[CoherenceMatches]) -> CoherenceMatches:
    ...
    for language in per_language_ratios:
        merge.append((
         language,
         round(sum(per_language_ratios[language]) / len(per_language_ratios[language]), 4)))
...
def coherence_ratio(decoded_sequence: str, threshold: float=0.1, lg_inclusion: Optional[str]=None) -> CoherenceMatches:
    ...
    if lg_inclusion is not None:
        if 'Latin Based' in lg_inclusion:
