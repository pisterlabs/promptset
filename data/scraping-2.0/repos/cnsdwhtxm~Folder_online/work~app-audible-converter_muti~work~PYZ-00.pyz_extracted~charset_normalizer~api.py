...
import logging
from charset_normalizer.utils import any_specified_encoding, is_multi_byte_encoding, identify_sig_or_bom, should_strip_sig_or_bom, is_cp_similar, iana_name
...
def from_bytes(sequences: bytes, steps: int=5, chunk_size: int=512, threshold: float=0.2, cp_isolation: List[str]=None, cp_exclusion: List[str]=None, preemptive_behaviour: bool=True, explain: bool=False) -> CharsetMatches:
    """
    ...
    if length == 0:
        ...
        if cp_isolation is not None:
            logger.warning('cp_isolation is set. use this flag for debugging purpose. limited list of encoding allowed : %s.', ', '.join(cp_isolation))
            cp_isolation = [iana_name(cp, False) for cp in cp_isolation]
        else:
            cp_isolation = []
        if cp_exclusion is not None:
            logger.warning('cp_exclusion is set. use this flag for debugging purpose. limited list of encoding excluded : %s.', ', '.join(cp_exclusion))
            cp_exclusion = [iana_name(cp, False) for cp in cp_exclusion]
        else:
            cp_exclusion = []
        if length <= chunk_size * steps:
            logger.warning('override steps (%i) and chunk_size (%i) as content does not fit (%i byte(s) given) parameters.', steps, chunk_size, length)
            steps = 1
            chunk_size = length
        if steps > 1:
            if length / steps < chunk_size:
                chunk_size = int(length / steps)
        is_too_small_sequence = len(sequences) < TOO_SMALL_SEQUENCE
        is_too_large_sequence = len(sequences) >= TOO_BIG_SEQUENCE
        ...
        single_byte_hard_failure_count = 0
        single_byte_soft_failure_count = 0
        ...
        for encoding_iana in prioritized_encodings + IANA_SUPPORTED:
            if cp_isolation:
                if encoding_iana not in cp_isolation:
                    continue
                else:
                    if cp_exclusion:
                        if encoding_iana in cp_exclusion:
                            continue
                        else:
                            ...
                            bom_or_sig_available = sig_encoding == encoding_iana
                            strip_sig_or_bom = bom_or_sig_available and should_strip_sig_or_bom(encoding_iana)
                            if encoding_iana in frozenset({'utf_32', 'utf_16'}):
                                if bom_or_sig_available is False:
                    ...
                    try:
                        if is_too_large_sequence:
                            if is_multi_byte_decoder is False:
                                str((sequences[:int(500000.0)] if strip_sig_or_bom is False else sequences[len(sig_payload):int(500000.0)]),
                                  encoding=encoding_iana)
                        else:
                            decoded_payload = str((sequences if strip_sig_or_bom is False else sequences[len(sig_payload):]),
                              encoding=encoding_iana)
                    ...
                    similar_soft_failure_test = False
                    ...
                    if similar_soft_failure_test:
                        ...
                        r_ = range(0 if bom_or_sig_available is False else len(sig_payload), length, int(length / steps))
                        multi_byte_bonus = is_multi_byte_decoder and decoded_payload is not None and len(decoded_payload) < length
                        if multi_byte_bonus:
                        ...
                        if md_ratios:
                            mean_mess_ratio = sum(md_ratios) / len(md_ratios)
                        else:
                            mean_mess_ratio = 0.0
                if mean_mess_ratio >= threshold or early_stop_count >= max_chunk_gave_up:
                    ...
                    if not is_multi_byte_decoder:
                        ...
                        logger.warning('%s was excluded because of initial chaos probing. Gave up %i time(s). Computed mean chaos is %f %%.', encoding_iana, early_stop_count, round((mean_mess_ratio * 100), ndigits=3))
                    ...
                    logger.info('%s passed initial chaos probing. Mean measured chaos is %f %%', encoding_iana, round((mean_mess_ratio * 100), ndigits=3))
                    if not is_multi_byte_decoder:
                        target_languages = encoding_languages(encoding_iana)
                    else:
                    ...
                    results.append(CharsetMatch(sequences, encoding_iana, mean_mess_ratio, bom_or_sig_available, cd_ratios_merged, decoded_payload))
                    ...
                    if encoding_iana == sig_encoding:
                        logger.info('%s is most likely the one as we detected a BOM or SIG within the beginning of the sequence.', encoding_iana)
                        return CharsetMatches([
                         results[encoding_iana]])
                    if results[(-1)].languages:
                        logger.info('Using %s code page we detected the following languages: %s', encoding_iana, results[encoding_iana]._languages)
...
def from_path(path: PathLike, steps: int=5, chunk_size: int=512, threshold: float=0.2, cp_isolation: List[str]=None, cp_exclusion: List[str]=None, preemptive_behaviour: bool=True, explain: bool=False) -> CharsetMatches:
    """
...
def normalize(path: PathLike, steps: int=5, chunk_size: int=512, threshold: float=0.2, cp_isolation: List[str]=None, cp_exclusion: List[str]=None, preemptive_behaviour: bool=True) -> CharsetMatch:
    """
    Take a (text-based) file path and try to create another file next to it, this time using UTF-8.
    """
    results = from_path(path, steps, chunk_size, threshold, cp_isolation, cp_exclusion, preemptive_behaviour)
    ...
    result = results.best()
    target_extensions[0] += '-' + result.encoding
