...
### PARAMETERS ###
activationWords = ['jarvis', 'calcutron', 'shodan', 'showdown','computer']
...
def speak(text, rate = 120):
    ...
    try:     
        ...
        if tts_type == 'google':
            ...
            speech_length = int(pygame.math.ceil(pygame.mixer.Sound.get_length(speech_sound)))
            speech_sound.play()
...
def query_openai(prompt = ""):
    openai.api_key = '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'
...
if __name__ == '__main__': 
    speak('Welcome back Shinato23.', 120)
    ...
    while True:
        ...
        if query[0] in activationWords and len(query) > 1:
            ...
            if query[0] == 'speak':
                ...
                speak("Ok")
                print(Fore.GREEN + Style.BRIGHT + speech)
                speak(speech) 
                
                
            # Navigation
            if len(query) >= 2 and query[0] == 'go' and query[1] == 'to':
                speak('Opening...')
            ...
            # Wikipedia
            if len(query) >= 2 and query[0] == 'wikipedia':
                query = ' '.join(query[1:])
            ...
            # Wolfram Alpha
            if len(query) >= 2 and query[0] == 'compute' or len(query) >= 2 and query[0] == 'computer':
                query = ' '.join(query[1:])
                try:
            ...
            # Note taking
            if len(query) >= 2 and query[0] == 'take note':
            ...
            if len(query) >= 2 and query[0] == 'search':
                speak(f'Ready to {query}')
                pywhatkit.search(query)
            
            if len(query) >= 2 and query[0] == 'play':
                speak(f'Ready to {query}')
                pywhatkit.playonyt(query)


            if len(query) >= 2 and query[0] == 'goodbye':
                speak('Goodbye')
                break
