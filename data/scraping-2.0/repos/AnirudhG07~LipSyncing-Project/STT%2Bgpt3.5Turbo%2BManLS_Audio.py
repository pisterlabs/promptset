...
# Make sure you do $pip install openai==0.28 or anything <1.0.0 
prompt=input_sent

completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=[{"role": "user", "content": prompt}])
# response_format={ "type": "json_object" }, can be added inside create. 
...
def play_audio_whisper(text:str):
    ...
    response.stream_to_file("whisper_ManLS_AI(SentimentAn).mp3")
    # playsound "whisper_ManLS_AI(SentimentAn).mp3"
    playsound.playsound("whisper_ManLS_AI(SentimentAn).mp3")
    os.remove("whisper_ManLS_AI(SentimentAn).mp3")
