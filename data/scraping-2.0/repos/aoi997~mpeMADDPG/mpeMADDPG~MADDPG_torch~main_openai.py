...
import time, torch, pickle, argparse, os
import numpy as np
...
def get_trainers(env, num_adversaries, obs_shape_n, action_shape_n, arglist):
    ...
    if arglist.restore == True:         # restore the model
        for idx in range(arglist.restore_idxs):
    ...
    if arglist.display == True:         # restore the model
        for idx in range(arglist.restore_idxs):
            actors_cur[idx] = torch.load(arglist.old_model_name+"a_c_{}.pt".format(idx))
            actors_tar[idx] = torch.load(arglist.old_model_name+"a_t_{}.pt".format(idx))
            critics_cur[idx] = torch.load(arglist.old_model_name+"c_c_{}.pt".format(idx))
...
def agents_train(arglist, game_step, update_cnt, memory, obs_size, action_size, \
    ...
    if game_step > arglist.learning_start_step and \
        ...
        if update_cnt > arglist.start_save_model and update_cnt % arglist.fre4save_model == 0:
            ...
            #     arglist.scenario_name, time_now, game_step))   os.mkdir
            # if not os.path.exists(model_file_dir): # make the path
            modelDir = "./models/"+arglist.scenario_name+"/"+str(int(time.time()))+"/"
            os.makedirs(modelDir, exist_ok=True)
            for agent_idx, (a_c, a_t, c_c, c_t) in \
                enumerate(zip(actors_cur, actors_tar, critics_cur, critics_tar)):
                torch.save(a_c, os.path.join(modelDir, 'a_c_{}.pt'.format(agent_idx)))
                torch.save(a_t, os.path.join(modelDir, 'a_t_{}.pt'.format(agent_idx)))
                torch.save(c_c, os.path.join(modelDir, 'c_c_{}.pt'.format(agent_idx)))
                torch.save(c_t, os.path.join(modelDir, 'c_t_{}.pt'.format(agent_idx)))
...
def train(arglist):
    ...
    # display newest model
    if arglist.display:
        modelList = os.listdir("./models/"+arglist.scenario_name+"/")
        arglist.old_model_name = "./models/"+arglist.scenario_name+"/"+modelList[-1]+"/"
    ...
    for episode_gone in range(arglist.max_episode):
        ...
        if game_step > 1 and game_step % 20000 == 0:
            ...
            # print(" "*43 + 'episode reward:{} agents mean reward:{} ##########'.format(\
            #     mean_ep_r, mean_agents_r), end='\r')
            print('==========================================================')

            print('=Training: steps:{} episode:{} episode reward:{} agents mean reward:{}'.format(
                game_step, episode_gone, mean_ep_r, mean_agents_r
            ))
