...
kwargs={"engine":"text-davinci-002","temperature":0.7,"max_tokens":5,"top_p":1,"frequency_penalty":0,"presence_penalty":0,"logprobs":10}
...
pencil_outcome="drew lines on the white piece of paper"
...
Answer (respond with yes or no): no
...
prompt+=question_generator(ncf_num,"pencil",pencil_outcome)
prompt+="""
Answer (respond with yes or no):"""



raw_response = openai.Completion.create(prompt=prompt, **kwargs)

logprobs_table=pd.DataFrame(raw_response["choices"][0]["logprobs"])

percentages = pd.DataFrame([raw_response["choices"][0]["logprobs"]["top_logprobs"][0]]).T
percentages.columns = ["Log_Prob"]
percentages["Pct"] = percentages["Log_Prob"].apply(lambda x: 100*np.e**x)

percentages = percentages.rename_axis('Response').reset_index()
percentages["Yes_bool"] = percentages["Response"].apply(lambda x:"y" if "yes" in x.lower() else "n")

sum_yes = percentages.loc[percentages['Yes_bool'] == "y", 'Pct'].sum()


total_points=[0,0,0,0,0] #history, goal, action, structure, uncompromised

total_points[4]=sum_yes #uncompromised
...
Answer (respond with yes or no):"""
    ...
    total_points[option_nums.pop()]=sum_yes #uncompromised
