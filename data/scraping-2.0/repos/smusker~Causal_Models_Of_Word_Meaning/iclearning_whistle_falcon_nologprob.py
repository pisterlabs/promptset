...
api_engine = "falcon_40B"
api_url = "https://api.textsynth.com"
api_key = ""

def textsynth_completion(prompt, max_tokens, temperature):
    response = requests.post(api_url + "/v1/engines/" + api_engine + "/completions", headers = { "Authorization": "Bearer " + api_key }, json = { "prompt": prompt, "max_tokens": max_tokens, "temperature": temperature})
    resp = response.json()
    if "text" in resp: 
        return resp["text"]
    else:
...
for n in range(samples_per_q):
    word_response = textsynth_completion(prompt=prompt, max_tokens=5, temperature=0.7)
