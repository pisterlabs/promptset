...
@app.route("/generate-ad-poster/", methods=["POST"])
...
def generate_ad_poster():
    try:
        ...
        image_url1 = data["url1"]
        image_url2 = data["url2"]
        ...
        if removebg_response1.status_code == 200:
            ...
            if removebg_response2.status_code == 200:
                ...
                return jsonify({"error": f"Error removing background for the second image. Status Code: {removebg_response2.status_code}"})

        else:
            print(f"Error removing background for the first image. Status Code: {removebg_response1.status_code}")
            return jsonify({"error": f"Error removing background for the first image. Status Code: {removebg_response1.status_code}"})
