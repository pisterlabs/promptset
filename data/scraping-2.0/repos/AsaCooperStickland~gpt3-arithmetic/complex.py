...
def eval_one_length_complex(outputs, answers, preds):
    total_count = 0.0
    total_correct = 0.0
    for i, (output, answer) in enumerate(zip(outputs, answers)):

        if FLAGS.output_delim == ",":
            digit_regex = r"([0-9]+(,[0-9]+)+\s\+\s[0-9]+(,[0-9]+)+i)"
        elif FLAGS.output_delim == " ":
            digit_regex = r"([0-9]+(\s[0-9]+)+\s\+\s[0-9]+(\s[0-9]+)+i)"
        elif FLAGS.output_delim == "":
            digit_regex = r"([0-9]+\s\+\s[0-9]+i)"
        else:
        ...
        try:
            if FLAGS.output_form == "ordered":
                pred = re.search(
                    r"putting it together " + digit_regex, output
                ).groups()[0]
            ...
            pred_split = [
                ...
                for pred_ in pred.split(" + ")
            ]
            preds.append(format_complex(pred_split[0], pred_split[1]))
            total_correct += int(pred_split[0]) == int(
                answer[0].replace(" ", "")
            ) and int(pred_split[1]) == int(answer[1].replace(" ", ""))
            total_count += 1
    ...
    if FLAGS.exp_file is None:
        ...
        for n in range(int(rstart), int(rend)):
            ...
            for _ in range(5):
                current_inputs = []
                for i in range(20):
                    y_real, x1_real, x2_real = addition(rng, n)
                    y_complex, x1_complex, x2_complex = addition(rng, n)
                    x1_str = format_complex(x1_real, x1_complex)
                    x2_str = format_complex(x2_real, x2_complex)
                    y_str = (y_real, y_complex)
            ...
            accuracy = eval_one_length_complex(outputs, answers, preds)
            logging.info(f"Accuracy is {accuracy} for {n} digit numbers")
        ...
        for n, v in exp_data.items():
            ...
            exp_data[n]["accuracy"] = eval_one_length_complex(
