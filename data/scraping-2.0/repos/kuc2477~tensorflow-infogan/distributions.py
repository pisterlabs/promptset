"""
COPIED FROM OpenAI's InfoGAN reference implementation source code.
...
class Distribution(object):
    ...
    def kl_prior(self, dist_info):
        return self.kl(
            dist_info,
            self.prior_dist_info(dist_info.values()[0].get_shape()[0])
        )
    ...
    def prior_dist_info(self, batch_size):
        ...
        :return: a dictionary containing distribution information about the
            standard prior distribution, the shape of which is jointly decided
            by batch_size and self.dim
        """
        raise NotImplementedError
...
class Product(Distribution):
    ...
    def join_dist_infos(self, dist_infos):
        ret = dict()
        for idx, dist, dist_info_i in zip(itertools.count(),
                                          self.dists,
                                          dist_infos):
            for k in dist.dist_info_keys:
    ...
    def split_var(self, x):
        ...
        out = []
        for slice_from, slice_to, dist in zip([0] + cum_dims,
                                              cum_dims,
                                              self.dists):
    ...
    def kl(self, p, q):
        ret = tf.constant(0.)
        for p_i, q_i, dist_i in zip(self.split_dist_info(p),
                                    self.split_dist_info(q),
                                    self.dists):
    ...
    def activate_dist(self, dist_flat):
        ret = dict()
        for idx, dist_flat_i, dist_i in zip(itertools.count(),
                                            self.split_dist_flat(dist_flat),
                                            self.dists):
