...
def generate_response(input_text, conversation_history, model):
    messages = [
...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        with open("voices.json", "r") as f:
            ...
            self.voices = sorted([voice['ShortName'] for voice in voice_data])

        self.current_voice = tk.StringVar(self.master)
        self.current_voice.set(self.voices[0])

        self.gpt_model = tk.StringVar(self.master)
        self.gpt_model.set("gpt-4")

        self.max_tokens = tk.StringVar(self.master)
        self.max_tokens.set(200)
    ...
    def create_widgets(self):
        ...
        self.stop_button["text"] = "Exit"
        self.stop_button["command"] = self.stop
        self.stop_button.pack(side="left")

        self.voice_selector = tk.OptionMenu(buttons_frame, self.current_voice, *self.voices)
        ...
        self.gpt3_radio = tk.Radiobutton(buttons_frame, text="GPT-3.5", variable=self.gpt_model, value="text-davinci-003.5")
        self.gpt3_radio.pack(side="left")
        
        self.gpt4_radio = tk.Radiobutton(buttons_frame, text="GPT-4", variable=self.gpt_model, value="gpt-4")
        self.gpt4_radio.pack(side="left")

        self.max_tokens_entry = tk.Entry(buttons_frame, textvariable=self.max_tokens)
    ...
    def save_conversation(self):
        filename = tk.filedialog.asksaveasfilename(defaultextension=".json", filetypes=[("Text Files", "*.txt"), ("CSV Files", "*.csv"), ("JSON Files", "*.json")])
        if not filename:
            return
    ...
    def main(self):
        ...
        while self.is_running:
            ...
            response_text = generate_response(input_text, self.conversation_history, self.gpt_model.get())
            ...
            try:
                ...
                self.queue.put(f"Error: Failed to play WAV - {e}\n")

            try:
                remove_temp_files(audio_file_path)
            except Exception as e:
                self.queue.put(f"Error: Failed to remove temporary files - {e}\n")
