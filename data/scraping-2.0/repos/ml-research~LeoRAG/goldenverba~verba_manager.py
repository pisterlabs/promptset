...
class VerbaManager:
    ...
    def verify_variables(self) -> None:
        ...
        if os.environ.get("LLAMA2_7B_CHAT_HF", "") == "True":
            self.environment_variables["LLAMA2_7B_CHAT_HF"] = True
        else:
            self.environment_variables["LLAMA2_7B_CHAT_HF"] = False
    ...
    def retrieve_chunks(self, queries: list[str]) -> list[Chunk]:
        try:
            queries = self.generator_manager.selected_generator.translate_de_en(queries)
            msg.info('translation success: ', queries)
        except:
            msg.warn('translation error')
            pass
