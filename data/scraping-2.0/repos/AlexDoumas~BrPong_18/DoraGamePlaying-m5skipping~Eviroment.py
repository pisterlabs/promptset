...
class Enviroment:
  ...
  def step(self, action, oldS):
    ...
    if(self.rewardMode == "raw"):
      ...
      r = self.followReward(action,S, oldS)
  ...
  def getActionBreakout(self, gameState, lastGameState):
    ...
    if len(gameState) == 12:
        ...
        paddlemid = (gameState[1]+gameState[3])/2.0
        ...
        Ycollision1 = ball["Y1"]
        Ycollision2 = ball["Y2"]
        Ycollisionmid = (Ycollision1+Ycollision2)/2.0
        #.set_trace()
        if useTime:
        ...
        #pdb.set_trace()
        if useTime:
            # if you're under the ball, you should have stay put, otherwise, you should move in the direction that minimises the distance to the ball. 
            if (Ycollision == 0):
                action = 0
            elif (paddle['Y']-0.04 < Ycollision) and (paddle['Y']+0.04 > Ycollision):
                action = 0
            elif (paddle["Y"] > Ycollision) and (ball['YVel'] <= 0.00001):
                action = 2
            elif (paddle["Y"] < Ycollision) and (ball['YVel'] >= -0.00001):
                action = 1
            elif (paddle["Y"] > Ycollision) and (ball['YVel'] >= -0.00001):
                if -0.0001 < paddle['YVel'] < 0.0001:
                    action = 2
                else:
                    action = 0
            elif (paddle["Y"] < Ycollision) and (ball['YVel'] <= 0.00001):
            ...
            if(Ycollision == 0):
              action = 0
            elif (paddle['Y']-0.03 < Ycollision) and (paddle['Y']+0.03 > Ycollision):
              action = 0
            elif(paddle["Y"] > Ycollision): # removed subtracting 0.05 from paddle["Y"]. Why was this subtraction done initially? 
              action = 2
            elif(paddle["Y"] < Ycollision): # removed adding 0.05 from paddle["Y"]. Why was this addition done initially? 
              action = 1
