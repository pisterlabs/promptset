...
if len(args) != 2:
    print("Usage: python3 "+args[0]+" <file_path>")
    sys.exit(1)
...
for file in binary_files:
    ...
    trainName = re.match(r'([^_]+_[^_]+_[^_]+)', readFile)

    # 振動測定時刻（タイムスタンプ）
    time_stamp = re.search(r'_(\d{6})_', readFile)

    # センサー名 取得
    input_argument = re.search(r'_(B_Out_Rail)_(\d{8})', readFile)
