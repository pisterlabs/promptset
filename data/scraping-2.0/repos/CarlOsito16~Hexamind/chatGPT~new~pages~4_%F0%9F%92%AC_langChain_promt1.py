...
st.write("## 💬 Conversation with prompt 1")
...
if "history_4" not in st.session_state:
    st.session_state.history_4 = []
    st.session_state.history_4.append({"message": "how can I help you today?", "is_user": False} )
    st.session_state.history_4.append({"message": question, "is_user": True} )
...
def update_input_value(button_text):
    ...
    st.session_state.input_value_4 = button_text
    
    
def send_message():
    global button1_text
    st.session_state.history_4.append({"message": input_value  , "is_user": False})
...
if 'input_value_4' not in st.session_state:
    st.session_state.input_value_4 = ""

row1_col1, row1_col2 = st.columns([2,1])
row2_col1, row2_col2 = st.columns([2,1])

# button1_text, button2_text, button3_text = np.random.choice(a= pre_answers, size=3 ,replace=False)
button1_text = generate_answer(st.session_state.history_4[-1]['message'])
...
with row1_col1:
    ...
    for chat in st.session_state.history_4:
        st_message(**chat)
