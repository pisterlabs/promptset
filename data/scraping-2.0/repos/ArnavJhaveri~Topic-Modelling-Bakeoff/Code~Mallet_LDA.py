...
try:
    os.makedirs('twitter/' + 'Mallet_LDA')
except OSError as e:
...
loc3 = "dlatk/" + dataset_name + "/Mallet_LDA/lda.wordGivenTopic.csv" 
with open(loc3) as f:
  for line in f:
    temp = line.strip().split('"')
    if len(temp) > 1:
...
if logit:
    ...
    accuracies = [np.mean(accuracies), nmf_topic_diversity, nmf_coherence] + accuracies + [np.mean(r2s)]
...
with open(folder + 'Mallet_LDA/topics_' + str(num_topics_list[0]) + '.txt', 'w') as f:
    for sublist in interpettable_topics:
        for i in sublist:
