...
from model_utils import qc_dict, out_topics_docs, check_topic_doc_prob, topn_docs_by_topic, make_bigrams
...
bigram = gensim.models.Phrases(reviews, min_count=3, threshold=5) # higher threshold fewer phrases.
...
dictionary.filter_extremes(no_below=5)  # remove terms that appear in < 3 documents, memory use estimate: 8 bytes * num_terms * num_topics * 3
...
%time lda_model = LDA(corpus=DTM, id2word=dictionary, num_topics=n_topics, alpha = 'auto', eta = 'auto', passes = passes, iterations = iterations, eval_every = 1)
