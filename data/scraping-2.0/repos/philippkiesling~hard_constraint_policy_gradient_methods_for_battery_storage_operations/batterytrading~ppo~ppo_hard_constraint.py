...
class HardConstraintPPO(OnPolicyAlgorithm):
    """
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                values, log_prob, entropy = self.policy.evaluate_actions(rollout_data.observations, actions, action_bounds = action_bounds)
                ...
                action_loss_original = th.mean(((mu_original - actions) ** 2))
                # Clip action loss with clip_range_proj
                action_loss = th.clamp(action_loss_original, -self.clip_range_proj, self.clip_range_proj)

                loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss + action_loss *self.proj_coef
