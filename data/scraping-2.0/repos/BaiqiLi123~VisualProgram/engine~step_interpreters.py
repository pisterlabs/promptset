...
from engine.models import XVMLBaseModel
...
class PropertyInterpreter(XVMLBaseModel):
    step_name = 'verify_property'

    def __init__(self,gpu_number=0,
                 path_checkpoint='/visprog/base_models/xvlm/retrieval_mscoco_checkpoint_9.pth',
                 verfi_thresh=0.55):
        ...
        self.verfi_thresh=verfi_thresh
        ...
        def load_json(path: Union[str, pathlib.Path]):
            ...
            with open(path, 'r') as f:
                data = json.load(f)
            return data
        self.possible_options = load_json('/visprog/base_models/xvlm/possible_options.json')
    ...
    def parse(self,prog_step):
        ...
        categ = eval(parse_result['args']['categ'])
        attribute=eval(parse_result['args']['attribute'])
        try:
            obj_name = eval(parse_result['args']['object_name'])
        except KeyError:
            obj_name = ""
        output_var = parse_result['output_var']
        assert(step_name==self.step_name)
        # print(img_var,obj_name,output_var)
        return img_var,attribute,obj_name,output_var,categ


    def html(self,output_var,img_var,obj_var,categ_var,output):
        step_name = html_step_name(self.step_name)
        output_var = html_var_name(output_var)
        img=html_arg_name("img")
        img_var = html_embed_image(img_var,100)
        obj_name = html_arg_name("objname")
        categ = html_arg_name("categ")
        
        return f"""<div>{output_var}={step_name}({img}={img_var},{obj_name}={obj_var},{categ}={categ_var})={output}</div>"""
    ...
    def binary_score(self, image, text, negative_categories):
        ...
        # print("self.score(image, texts)",self.score(image, texts))
    ...
    def execute(self,prog_step,inspect=False):
        img_var,attribute,object_name,output_var,categ = self.parse(prog_step)

        IMAGE_VAR = prog_step.state[img_var]

        image = IMAGE_VAR.convert("RGB")
        ...
        #     transforms.Resize((self.image_res, self.image_res), interpolation=Image.BICUBIC),
        #     transforms.ToTensor(),
        #     normalize,
        # ])
        ...
        if categ=="":
            score = self.forward(image,name,negative_categories=None)
        else:
            negative_categories = [f"{att} {object_name}" for att in self.possible_options[categ]]
            score = self.forward(image,name,negative_categories=negative_categories)
        htmlstr=self.html(output_var,IMAGE_VAR,name,categ,output=eval(np.array_str(score.numpy()))>self.verfi_thresh)
        prog_step.state[output_var] = eval(np.array_str(score.numpy()))>self.verfi_thresh
        return eval(np.array_str(score.numpy()))>self.verfi_thresh,htmlstr


class BestTextMatchInterpreter(PropertyInterpreter):
    step_name = 'BTM'
    ...
    def parse(self,prog_step):
        ...
        step_name = parse_result['step_name']
        img_var = parse_result['args']['image']
        text = eval(parse_result['args']['text'])
        text = text.split("/")
        output_var = parse_result['output_var']

        assert(step_name==self.step_name)
        return img_var,text,output_var


    def html(self,output_var,img_var,text,output):
        step_name = html_step_name(self.step_name)
        output_var = html_var_name(output_var)
        img=html_arg_name("img")
        img_var = html_embed_image(img_var,100)
        text_name = html_arg_name("text")
        
        return f"""<div>{output_var}={step_name}({img}={img_var},{text_name}={text})={output}</div>"""

    def execute(self,prog_step,inspect=False):
        img_var,text,output_var = self.parse(prog_step)
        ...
        res = self.forward(image,text,negative_categories=None)
        # print("res",res)
        res = res.argmax().item()
        # print("argm.i",res)
        selected = res
        ...
        # print(score)
        prog_step.state[output_var] = text[selected]
        htmlstr=self.html(output_var,IMAGE_VAR,text,output=text[selected])
        return text[selected],htmlstr


class BLIP_BestTextMatchInterpreter():
    step_name = 'BEST_TEXT_MATCH'
    ...
    def parse(self,prog_step):
        ...
        assert(step_name==self.step_name)
        return img_var,text,answer,output_var

    def html(self,output_var,img_var,text,answer_var,scores,output):
        step_name = html_step_name(self.step_name)
        output_var = html_var_name(output_var)
        img = html_arg_name("img")
        img_var = html_embed_image(img_var,100)
        text_name = html_arg_name("text")
        answer_name = html_arg_name("Correct_answer")
        
        return f"""<div>{output_var}={step_name}({img}={img_var},{answer_name}={answer_var},{text_name}={text})={output},{scores}</div>"""
    
    def run(self, image, text):
        image = image.convert("RGB")
        img = self.vis_processors["eval"](image).unsqueeze(0).to(self.device)
        score = 0
        tt = self.text_processors["eval"](text)
        itm_output = self.model({"image": img, "text_input": tt}, match_head="itm")
        itm_score = torch.nn.functional.softmax(itm_output, dim=1)
        score = itm_score[:, 1].item()
        return score


    def execute(self,prog_step,inspect=False):
        img_var,text,answer_var,output_var = self.parse(prog_step)
        answer = False
        IMAGE = prog_step.state[img_var]
        ...
        scores = []
        for t in text:
            tt = self.text_processors["eval"](t)
            itm_output = self.model({"image": img, "text_input": tt}, match_head="itm")
            itm_score = torch.nn.functional.softmax(itm_output, dim=1)
            scores.append(itm_score[:, 1].item())

        selected = scores.index(max(scores))
        if text[selected] == answer_var:
            answer = True
        # prog_step.state[output_var] = text[selected]
        prog_step.state[output_var] = answer
        htmlstr=self.html(output_var,IMAGE,text,answer_var,scores,output=text[selected])
        # htmlstr=""
        # return text[selected],htmlstr
        return answer,htmlstr




class MergeImage():
    step_name = 'MergeImage'
    def __init__(self):
        print(f'Registering {self.step_name} step')
    ...
    def parse(self,prog_step):
        ...
        step_name = parse_result['step_name']
        img_var = parse_result['args']['image']
        boxes0_and_depths0 = parse_result['args']['box0']
        boxes1_and_depths1 = parse_result['args']['box1']
        # text = eval(parse_result['args']['Compare'])
        output_var = parse_result['output_var']

        assert(step_name==self.step_name)
        return img_var,step_name,boxes0_and_depths0,boxes1_and_depths1,output_var

    def html(self,output_var,step_name,Merge_img):
        step_name = html_step_name(self.step_name)
        output_var = html_var_name(output_var)
        img = html_arg_name("Merge_img")
        img_0 = html_embed_image(Merge_img,100)

        return f"""<div>{output_var}={step_name}()={img}={img_0}</div>"""
    
    def execute(self,prog_step,inspect=False):

        img_var,step_name,boxes_0,boxes_1,output_var = self.parse(prog_step)
        img = prog_step.state[img_var]
        boxes0_and_depths0 = prog_step.state[boxes_0]
        boxes1_and_depths1 = prog_step.state[boxes_1]
        W,H = img.size

        if len(boxes0_and_depths0)>0 and len(boxes1_and_depths1)>0:
            x = []
            y = []
            for idx in range(len(boxes0_and_depths0)):
                [x1,y1,x2,y2] = boxes0_and_depths0[idx]['boxes']
                x.append(x1)
                x.append(x2)
                y.append(y1)
                y.append(y2)
            for idx in range(len(boxes1_and_depths1)):
                [x1,y1,x2,y2] = boxes1_and_depths1[idx]['boxes']
                x.append(x1)
                x.append(x2)
                y.append(y1)
                y.append(y2)
            ...
            Merge_img = img.crop([min(x),min(y),max(x),max(y)])
        else:
            Merge_img = img

        prog_step.state[output_var] = Merge_img

        htmlstr=self.html(output_var,step_name,Merge_img.convert("RGB"))

        return Merge_img,htmlstr   




class MatchScore(PropertyInterpreter):
    step_name = 'MatchScore'
    ...
    def execute(self,prog_step,inspect=False):
        ...
        score = self.forward(image,text,negative_categories=None)
        score = score[0].item()
        prog_step.state[output_var] = score
        htmlstr=self.html(output_var,IMAGE_VAR,text,output=score)

        # verfi_thresh = 0.55
        #return score > verfi_thresh, htmlstr
        return score, htmlstr

class CompareEval():
    step_name = 'CompareEval'
    def __init__(self):
        print(f'Registering {self.step_name} step')
        self.device = "cuda:0" if torch.cuda.is_available() else "cpu"
        # self.model, self.vis_processors, _ = load_model_and_preprocess(name="blip2_t5", model_type="pretrain_flant5xl", is_eval=True, device=self.device)
        self.model, self.vis_processors, self.text_processors = load_model_and_preprocess("blip2_image_text_matching", "pretrain", device=self.device, is_eval=True)
        self.scorer = BLIP_BestTextMatchInterpreter()
    ...
    def parse(self,prog_step):
        ...
        output_var = parse_result['output_var']
        assert(step_name==self.step_name)
        return img_var,step_name,boxes0_and_depths0,boxes1_and_depths1,text,output_var

    def html(self,output_var,step_name,text,correct,img0=None,score0=None,img1=None,score1=None):
        step_name = html_step_name(self.step_name)
        output_var = html_var_name(output_var)
        img = html_arg_name("img")
        text_name = html_arg_name("Compare Properties")
        answer = html_arg_name(correct)
        if img0 and score0 and img1 and score1:
            img_0 = html_embed_image(img0,100)
            img_1 = html_embed_image(img1,100)
            return f"""<div>{output_var}={step_name}({text_name}={text})={answer},{img_0},{score0},{img_1},{score1})</div>"""
        return f"""<div>{output_var}={step_name}(There is no object of comparison)</div>"""
    
    def execute(self,prog_step,inspect=False):

        img_var,step_name,boxes_0,boxes_1,text,output_var = self.parse(prog_step)
        img = prog_step.state[img_var]
        boxes0_and_depths0 = prog_step.state[boxes_0]
        boxes1_and_depths1 = prog_step.state[boxes_1]
        correct = False

        if len(boxes0_and_depths0) > 0 and len(boxes0_and_depths0) > 0:
            boxes0 = []
            boxes0.append(boxes0_and_depths0[0]['boxes'])
            boxes1 = []
            boxes1.append(boxes1_and_depths1[0]['boxes'])
            if text in ['same','big','small']:
                img0 = img.crop(boxes0[0])
                img1 = img.crop(boxes1[0])
                area0 = (boxes0[0][2]-boxes0[0][0])*(boxes0[0][3]-boxes0[0][1])
                area1 = (boxes1[0][2]-boxes1[0][0])*(boxes1[0][3]-boxes1[0][1])
                if area0 == area1 and text == 'same':
                    correct = True
                elif area0 > area1 and text == 'big':
                    correct = True
                elif area0 < area1 and text == 'small':
                    correct = True

                prog_step.state[output_var] = correct
                htmlstr=self.html(output_var,step_name,text,correct,img0,area0,img1,area1)
            else:

                img0 = img.crop(boxes0[0])
                img1 = img.crop(boxes1[0])
                score0 = self.scorer.run(img0, text)
                score1 = self.scorer.run(img1, text)
                if score0 > score1:
                    correct = True
                prog_step.state[output_var] = correct
                htmlstr=self.html(output_var,step_name,text,correct,img0,score0,img1,score1)
                return correct,htmlstr
            
        prog_step.state[output_var] = correct
        htmlstr=self.html(output_var,step_name,text,correct)
        return correct,htmlstr        
    
class SpatialEval():
    step_name = 'SPATIAL_EVAL'
    ...
    def parse(self,prog_step):
        ...
        step_name = parse_result['step_name']
        img_var = parse_result['args']['image']
        object1 = parse_result['args']['box0']
        object2 = parse_result['args']['box1']
        objects_relation = eval(parse_result['args']['objects_relation'])
        output_var = parse_result['output_var']

        assert(step_name==self.step_name)
        return img_var,object1,object2,objects_relation,output_var

    def html(self,img,relation,correct,output_var):
        step_name = html_step_name(self.step_name)
        img_str = html_embed_image(img)
        correct = html_output(correct)
        output_var = html_var_name(output_var)
        image_arg = html_arg_name('image')
        # relation_arg = html_arg_name('relation')
        # Box1 = html_arg_name('Box1')
        # Box2 = html_arg_name('Box2')
        relation_arg = html_arg_name('relation')
        # return f"""<div>{output_var}={step_name}({image_arg}={img_str},&nbsp;{relation_arg}='{relation}')={correct}</div>"""
        return f"""<div>{output_var}={step_name}({image_arg}={img_str},&nbsp;{relation_arg}='{relation}')={correct}</div>"""
    ...
    def overlap(self, bbox1, bbox2):
        ...
        if max(bbox1[2],bbox2[2]) - min(bbox1[0], bbox2[0]) < bbox1_W +  bbox2_W and max(bbox1[3],bbox2[3]) - min(bbox1[1], bbox2[1]) < bbox1_H +  bbox2_H:
            return True
        return False

    def execute(self,prog_step,inspect=False):
        img_var,box1_var,box2_var,objects_relation,output_var = self.parse(prog_step)
        boxes1_and_depths1 = prog_step.state[box1_var]
        boxes2_and_depths2 = prog_step.state[box2_var]
        img = prog_step.state[img_var]
        ...
        for item in boxes1_and_depths1:
            boxes1.append(item['boxes'])
            depths1.append(item['depths'])
        for item in boxes2_and_depths2:
            boxes2.append(item['boxes'])
            depths2.append(item['depths'])

        correct = False
        if objects_relation in ['left', 'right', 'above', 'below', 'front', 'behind', "next", "on", "beneath", "front", "behind"]:
            relation = objects_relation
            diff_threshold = 5
            ...
            for box in boxes1:
                ...
                center_points_1.append((x, y))

            for box in boxes2:
                x, y = (box[0] + box[2]) / 2, (box[1] + box[3]) / 2
                center_points_2.append((x, y))        

            for idx1 in range(len(boxes1_and_depths1)):
                (x1, y1) = center_points_1[idx1]
                depths1 = boxes1_and_depths1[idx1]['depths']
                for idx2 in range(len(boxes2_and_depths2)):
                    (x2, y2) = center_points_2[idx2]
                    depths2 = boxes2_and_depths2[idx2]['depths']
                    ...
                    if relation == "next":
                        ...
                        if diff < -diff_threshold:
                            correct = True
                    elif relation == "below" or relation == "beneath":
                        diff = y1 - y2
                        if diff > diff_threshold:
                            correct = True
                    elif relation == 'on':
                        if self.overlap(boxes1_and_depths1[idx1]['boxes'], boxes2_and_depths2[idx2]['boxes']):
                            if y1 < y2 or depths1 > depths2:
                                correct = True
                    elif relation == 'front':
                        if depths1 > depths2:
                            correct = True
        ...
        prog_step.state[output_var] = correct
        if inspect:
            html_str = self.html(img, relation, correct, output_var)
            return correct, html_str
        return correct
...
class GroundingDinoInterpreter():
    step_name = 'DINO'

    def __init__(self,thresh=0.4,nms_thresh=0.5):
        print(f'Registering {self.step_name} step')

        self.model = load_model("/visprog/GroundingDINO/groundingdino/config/GroundingDINO_SwinB.cfg.py", "/visprog/GroundingDINO/weights/groundingdino_swinb_cogcoor.pth")
        self.thresh = thresh
    ...
    def parse(self,prog_step):
        ...
        step_name = parse_result['step_name']
        img_var = parse_result['args']['image']
        thresh = eval(parse_result['args']['thresh'])
        obj_name = eval(parse_result['args']['object'])
        output_var = parse_result['output_var']
        assert(step_name==self.step_name)
        # print(img_var,obj_name,output_var)
        return img_var,obj_name,thresh,output_var



    def html(self,img,box_img,output_var,obj_name,selected_scores,final_boxes):
        step_name=html_step_name(self.step_name)
        obj_arg=html_arg_name('object')
        img_arg=html_arg_name('image')
        output_var=html_var_name(output_var)
        img=html_embed_image(img)
        box_img=html_embed_image(box_img,100)
        return f"<div>{output_var}={step_name}({img_arg}={img}, {obj_arg}='{obj_name}')={box_img},{final_boxes},{selected_scores}</div>"
    
    def normalize_coord_cxcywh2xyxy(self,bbox,img_size):
        w,h = img_size
        cx_float,cy_float,w_float,h_float=[v for v in bbox]
    ...
    def get_object_depth(self, detection_boxes, depth_maps):
        bbox_depths = []
        image = np.array(depth_maps).copy()
    ...
    def extract_depth(self, image, device='cpu'):
    ...
    def execute(self,prog_step,inspect=False):
        img_var,obj_name,thresh,output_var = self.parse(prog_step)
        ...
        image_source = IMAGE_VAR.convert("RGB")
        image = np.asarray(image_source)
        image_transformed, _ = transform(image_source, None)
        image_source, image = image, image_transformed
        ...
        if len(boxes)==0:
            ...
            for i in range(len(scores)):
                ...
                if scores[i] > thresh:
                    coord = self.normalize_coord_cxcywh2xyxy(boxes[i],IMAGE_VAR.convert("RGB").size)
        ...
        box_img = self.box_image(IMAGE_VAR.convert("RGB"), selected_boxes)
        # prog_step.state[output_var] = selected_boxes
        # prog_step.state[output_var+'_IMAGE'] = box_img


        #compute depth

        depths = self.extract_depth(IMAGE_VAR.convert("RGB"))
        bbox_depths = self.get_object_depth(selected_boxes, depths)
        final_boxes = []
        for idx in range(len(selected_boxes)):
            temp = {'boxes': selected_boxes[idx], 'depths':bbox_depths[idx]} 
            final_boxes.append(temp)
            # selected_boxes[idx].append(bbox_depths[idx])

        # prog_step.state[output_var+'_DEPTH'] = bbox_depths 

        prog_step.state[output_var] = final_boxes
        prog_step.state[output_var+'_IMAGE'] = box_img

        if inspect:
            #html_str = self.html(IMAGE_VAR.convert("RGB"), box_img, output_var, obj_name)
            ##########
            html_str = self.html(IMAGE_VAR.convert("RGB"), box_img, output_var, obj_name, selected_scores, final_boxes)
            return selected_boxes, html_str
        return selected_boxes

        # annotated_frame = annotate(image_source=image_source, boxes=boxes, logits=logits, phrases=phrases)
        # htmlstr=self.html(img = Image.fromarray(annotated_frame))
...
class ResultInterpreter():
    ...
    def html(self,output,output_var):
        ...
        if isinstance(output, Image.Image):
            output = html_embed_image(output,100)
        else:
...
class BLQAInterpreter():
    ...
    def parse(self,prog_step):
        ...
        img_var = args['image']
        question = eval(args['question'])
        answer_choices=eval(args['answer_choices'])
        output_var = parse_result['output_var']
        assert(step_name==self.step_name)
        return img_var,question,answer_choices,output_var
    ...
    def execute(self,prog_step,inspect=False):
        img_var,question,answer_choices,output_var = self.parse(prog_step)
        img = prog_step.state[img_var]
        # print(answer_choices)
        image, question, answer_choices = img, question, answer_choices
        
        assert len(answer_choices.split(",")) > 1, answer_choices

        question = f"Question: {question} Choices: {answer_choices.replace(',', ', ')} Answer:"

        image = self.vis_processors["eval"](image.convert("RGB")).unsqueeze(0).to(self.device)
...
class CropInterpreter():
    ...
    def expand_box(self,box,img_size,factor=1.2):#1.5
        W,H = img_size
        x1,y1,x2,y2 = box
    ...
    def html(self,img,out_img,output_var,box_img):
        img = html_embed_image(img)
        out_img = html_embed_image(out_img,100)
        box_img = html_embed_image(box_img)
        output_var = html_var_name(output_var)
        step_name = html_step_name(self.step_name)
        box_arg = html_arg_name('bbox')
    ...
    def execute(self,prog_step,inspect=False):
        ...
        img = prog_step.state[img_var]
        boxes_and_depths = prog_step.state[box_var]
        boxes = []
        for item in boxes_and_depths:
...
class TagInterpreter():
    ...
    def html(self,img_var,tagged_img,obj_var,output_var):
        ...
        img_var = html_var_name(img_var)
        obj_var = html_var_name(obj_var)
        tagged_img = html_embed_image(tagged_img,100)
        img_arg = html_arg_name('image')
...
def register_step_interpreters(dataset='nlvr'):
    if dataset=='nlvr':
        ...
        return dict(
            DINO=GroundingDinoInterpreter(),
            verify_property=PropertyInterpreter(),
            BTM=BestTextMatchInterpreter(),
            # BLIP_VQA=BLQAInterpreter(),
            # LOC=LocInterpreter(),
            ...
            EVAL=EvalInterpreter(),
            RESULT=ResultInterpreter(),
            BEST_TEXT_MATCH=BLIP_BestTextMatchInterpreter(),
            SPATIAL_EVAL=SpatialEval(),
