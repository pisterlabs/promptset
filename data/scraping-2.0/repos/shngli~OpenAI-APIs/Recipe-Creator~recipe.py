"""
ask_for_ingredients() will do a while loop asking for an ingredient eg. cucumbers, turkey, 
tomatoes, etc. Then we'll just say done when we're actually finished. It will append each 
of those items to a list and it will report back our ingredients. 

generate_recipe() will use create_recipe_prompt() to create a detailed recipe prompt that 
will also assign a recipe title, and generate the recipe. We will also let the user verify 
the prompt. 

After we generate the recipe, we will write the recipe file.
"""
