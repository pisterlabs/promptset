...
class Optimizer:
    @abc.abstractmethod
    def complete(self, system_prompt, prompt, **kwargs):
        """
        Runs a completion using the string system_prompt+prompt.  Returns an optional training closure to use if the 
        caller decides that the completion was particularly good.
        
        This method first checks if a model exists for the system_prompt. If it does, it uses the model to complete the prompt. 
        ...
        This can not handle cases where either system_prompt or prompt are just whitespace!

        Parameters:
        - system_prompt (str): Stable contextual data to use as a basis for training.
        - prompt (str): The dynamic data to generate a completion for and potentially add to training data.
...
class HostedOptimizer(Optimizer):
    def __init__(self, anarchy_key):
        """
        Initialize a completion data logger.

        Parameters:
        - anarchy_key (str): the API key for your account on chat.dev.
        """
        self.anarchy_key = anarchy_key

    def complete(self, prompt, system_prompt = "", tags = [], ground_truth_model = "gpt4", **kwargs):
        """
        Runs the optimizing completion process on anarchy's hosted server with persistence.

        Parameters:
        - prompt (str): The dynamic data to generate a completion for and potentially add to training data.
        - system_prompt (str): The static data to append to the beginning of every query. Does not actually use the system-prompt style of gpt3.5 - just appended.
        - tags (lst of str): Optionally label each completion and this usage with these extra tags.  A list of strings.
        - ground_truth_model (str): in the absence of rules (based on kwargs, tags, system_prompt, prompt, and ground_truth_model...) to override, this is the model used to generate completions.
...
class LocalOptimizer(Optimizer):
    ...
    def complete(self, system_prompt, prompt, **kwargs):
        completion, train = self.complete_delay_train(system_prompt, prompt, **kwargs)
        train()
        return completion

    def complete_delay_train(self, system_prompt, prompt, c_id = None, **kwargs):
        """
        Runs a completion using the string system_prompt+prompt.  Returns an optional training closure to use if the 
        caller decides that the completion was particularly good.
        ...
        Parameters:
        - system_prompt (str): Stable contextual prompt to use as a basis for training.
        - prompt (str): The dynamic data to generate a completion for and potentially add to training data.
        - c_id (str): To be used if multiple users could share the same system_prompts so that we don't leak data.  If its None, defaults to all possible context.
        - **kwargs: Additional arguments to be passed to the `call_small` and `call_big` methods.
        ...
        c_id = str({'system_prompt' : system_prompt, 
                    'args' : kwargs, 
        ...
        if model is not None:
            ...
            completion = self.call_small(prompt = prompt.strip(), model=model, **kwargs)
