...
class StableDiffusionGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        unet = UNet2DConditionModel.from_pretrained(
            self.cfg.model_key, subfolder="unet_ema"
    ...
    def compute_grad_anpg(
        ...
        add_time_ids = (
            torch.tensor(
                [
                    list(
                        (self.cfg.original_size, self.cfg.original_size)
                        + (0, 0)
                        + (self.cfg.target_size, self.cfg.target_size)
                    )
                ]
            )
            .to(self.weights_dtype)
        ...
        with torch.no_grad():
            ...
            midas_depth_noise = torch.randn_like(midas_depth_latents)
            midas_depth_latents_noisy = self.scheduler.add_noise(
                midas_depth_latents, midas_depth_noise, t
            )
            midas_depth_latent_model_input = torch.cat(
                [midas_depth_latents_noisy] * 3, dim=0
            )
            ...
            noisy_latents_list = [
                torch.cat(
                    [midas_depth_latent_model_input, whole_latents_input], dim=1
                ).to(self.weights_dtype)
            ]
            ...
            delta_d = mask * noise_pred_null + (1 - mask) * (
                noise_pred_null - noise_pred_neg
            )
    ...
    def compute_grad_sds(
        ...
        with torch.no_grad():
            ...
            midas_depth_latent_model_input = torch.cat(
                [midas_depth_latents_noisy] * 2, dim=0
            )
        ...
        if self.cfg.guidance_rescale > 0.0:
            ...
            noise_pred = rescale_noise_cfg(
                noise_pred, noise_pred_text, guidance_rescale=self.cfg.guidance_rescale
            )
    ...
    def compute_grad_sjc(
        ...
        Ds = (
            torch.cat([zs, midas_depth_zs], dim=1)
            - torch.cat([sigma] * 2, dim=1) * noise_pred
        )

        if self.cfg.var_red:
            grad = -(Ds - torch.cat([y, midas_depth_y], dim=1)) / torch.cat(
                [sigma] * 2, dim=1
            )
        else:
            grad = -(Ds - torch.cat([zs, midas_depth_zs], dim=1)) / torch.cat(
                [sigma] * 2, dim=1
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if rgb_as_latents:
            ...
            midas_depth_latents = self.encode_images(
                midas_depth_BCHW_512.to(self.weights_dtype)
            )
            midas_depth_latents = (
                midas_depth_latents - depth_mean
            ) / depth_std * rgb_std + rgb_mean
        ...
        midas_depth_sds = (
            self.cfg.lw_depth
            * F.mse_loss(midas_depth_latents, midas_depth_target, reduction="sum")
            / batch_size
        )
        ...
        if guidance_eval:
            guidance_eval_out = self.guidance_eval(
                **guidance_eval_utils, res=rgb_BCHW.shape[-1]
            )
            texts = []
            for n, e, a, c in zip(
    ...
    def get_noise_pred(
        ...
        noisy_latents_list = [
            torch.cat([midas_depth_latent_model_input, whole_latents_input], dim=1).to(
                self.weights_dtype
            )
        ]
    ...
    def guidance_eval(
        ...
        midas_depth_latents_noisy_tmp = (
            midas_depth_latents_noisy - rgb_mean
        ) / rgb_std * depth_std + depth_mean
        midas_depth_imgs_noisy = self.decode_latents(
            midas_depth_latents_noisy_tmp[:bs]
        ).permute(0, 2, 3, 1)
        ...
        for b in range(bs):
            midas_depth_step_output = self.scheduler.step(
                midas_depth_pred[b : b + 1],
                t[b],
                midas_depth_latents_noisy[b : b + 1],
                eta=1,
            )
            midas_depth_latents_1step.append(midas_depth_step_output["prev_sample"])
            midas_depth_pred_1orig.append(
                midas_depth_step_output["pred_original_sample"]
            )
        midas_depth_latents_1step = torch.cat(midas_depth_latents_1step)
        midas_depth_pred_1orig = torch.cat(midas_depth_pred_1orig)
        midas_depth_latents_1step = (
            midas_depth_latents_1step - rgb_mean
        ) / rgb_std * depth_std + depth_mean
        midas_depth_imgs_1step = self.decode_latents(midas_depth_latents_1step).permute(
            0, 2, 3, 1
        )
        midas_depth_pred_1orig = (
            midas_depth_pred_1orig - rgb_mean
        ) / rgb_std * depth_std + depth_mean
        midas_depth_imgs_1orig = self.decode_latents(midas_depth_pred_1orig).permute(
            0, 2, 3, 1
        )
        ...
        for b, i in enumerate(idxs):
            ...
            for t in tqdm(self.scheduler.timesteps[i + 1 :], leave=False):
                ...
                midas_depth_latents = self.scheduler.step(
                    midas_depth_pred, t, midas_depth_latents, eta=1
                )["prev_sample"]
        ...
        midas_depth_latents_final = (
            midas_depth_latents_final - rgb_mean
        ) / rgb_std * depth_std + depth_mean
        imgs_final = self.decode_latents(latents_final).permute(0, 2, 3, 1)
        midas_depth_imgs_final = self.decode_latents(midas_depth_latents_final).permute(
            0, 2, 3, 1
        )
