...
#from breakPointRecovery import pointCache, pointRecovery
...
def step_candidates_generator(state, num_candidates): #states:{"topic":, "question":, "1th step":, "2th step":, ...} every elememt is in string# 
    if len(list(state.keys())) > 2:
        step_candidates = []
        while len(step_candidates) != num_candidates:
            chain = chain_initializer(state, "")
            chain.append({"role":"user", "content":kick_start_prompt})
...
def demo_sub(initial_state, breadth_of_search, num_first_round, num_second_round, num_search, num_max_retry):
    global retry_count
    #Initialization#
    count = 1
    states = initial_state
    while count <= num_search:
        indicator = False
        start_time = time.time()
        new_states = []
        try:
            for state in states:
                step_candidates = step_candidates_generator(state,num_first_round)
    ...
    if indicator and retry_count<=num_max_retry:
        ...
        start_count = count
        retry_count+=1
        demo_sub(initial_state, breadth_of_search, num_first_round, num_second_round, start_count, num_max_retry)
    if retry_count==num_max_retry:
        print("Reach max num of retry!")
    else:    
        return states 

def output(initial_state, breadth_of_search=1, num_first_round=4, num_second_round=2, num_search=6, num_max_retry=50):
    global retry_count
    retry_count = 0
    demo_sub(initial_state, breadth_of_search, num_first_round, num_second_round, num_search, num_max_retry)
