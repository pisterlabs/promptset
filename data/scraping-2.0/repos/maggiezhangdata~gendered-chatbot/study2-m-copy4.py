...
assistant_id = 'asst_yyTquAPsrHstGxZm5y6djaRL'
...
def sub_wuyan_with_qiyan(text):
    ...
    def regex_replace_five_word_poem(poem, pool):
        # Modified regular expression pattern to match Han characters (using a range that includes common Chinese characters)
        pattern = r'([\u4e00-\u9fff]{5}，[\u4e00-\u9fff]{5}。[\u4e00-\u9fff]{5}，[\u4e00-\u9fff]{5}。)'

        # Search for the pattern in the poem
        if re.search(pattern, poem):
            # Replace the detected part with a randomly selected poem from the pool
            return re.sub(pattern, random.choice(pool), poem)
        else:
            # Return the original poem if the pattern is not found
            return poem
    ...
    if contain_season(text, '春'):
        ...
        return regex_replace_five_word_poem(text, spring_pool) 
    elif contain_season(text, '夏'):
        return regex_replace_five_word_poem(text, summer_pool)
    elif contain_season(text, '秋'):
        return regex_replace_five_word_poem(text, fall_pool)
    elif contain_season(text, '冬'):
        return regex_replace_five_word_poem(text, winter_pool)
    else:
        return text
