...
#        print('Tom observation : ',tom_discretized_observations)
    ...
    def _compute_reward(self, observations, done):
        ...
        j_x=self.get_jerry_odom().pose.pose.position.x
        j_y=self.get_jerry_odom().pose.pose.position.y
        t_x=self.get_tom_odom().pose.pose.position.x
        t_y=self.get_tom_odom().pose.pose.position.y
        ...
        if not done:
            ...
            elif jerry_distance_moved_towards_target < 0:
            ...
            #Tom crashed
                ...
                print('tom crashing reward setting')
            #Jerry crashed
            elif self.jerry_crashed and not self.success:
                print('jerry crashing reward setting')
                jerry_reward = self.reward_crashing
                # tom_reward = 0
        ...
        for i, item in enumerate(data.ranges):
            if (i % mod == 0):
                ...
                if (self.min_range > item > 0):
                    ...
                    self._episode_done = True
                    if self.distance_between_tom_jerry<0.9:
                        print("Both collided")
