...
import amari.openai
from amari.openai import error, util, version
from amari.openai.openai_response import OpenAIResponse
from amari.openai.util import ApiType
...
def _make_session() -> requests.Session:
    if amari.openai.requestssession:
        if isinstance(amari.openai.requestssession, requests.Session):
            return amari.openai.requestssession
        return amari.openai.requestssession()
    if not amari.openai.verify_ssl_certs:
        warnings.warn("verify_ssl_certs is ignored; openai always verifies.")
    s = requests.Session()
    proxies = _requests_proxies_arg(amari.openai.proxy)
    if proxies:
        s.proxies = proxies
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or amari.openai.api_base
        self.api_key = key or util.default_api_key()
        self.amari_api_key = amari_key or util.default_amari_api_key()
        self.api_type = (
            ApiType.from_str(api_type)
            if api_type
            else ApiType.from_str(amari.openai.api_type)
        )
        self.api_version = api_version or amari.openai.api_version
        self.organization = organization or amari.openai.organization
    ...
    def request_headers(
        ...
        if amari.openai.app_info:
            user_agent += " " + self.format_app_info(amari.openai.app_info)
        ...
        if amari.openai.app_info:
            ua["application"] = amari.openai.app_info

        headers = {
            "X-OpenAI-Client-User-Agent": json.dumps(ua),
            "User-Agent": user_agent,
            "X-OpenAI-API-Key": self.api_key,
            "Authorization": f"Bearer {self.amari_api_key}",
        }
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_kwargs = {
            ...
            "headers": headers,
            "data": data,
            "proxy": _aiohttp_proxies_arg(amari.openai.proxy),
            "timeout": timeout,
        }
        try:
...
class AioHTTPSession(AsyncContextManager):
    ...
    async def __aenter__(self):
        self._session = amari.openai.aiosession.get()
