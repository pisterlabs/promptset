...
async def _acheck_index_exists(client, index_name: str) -> bool:
    """Check if Redis index exists."""
    try:
...
class AsyncRedis(VectorStore):
    ...
    async def _acreate_index(
        ...
        existed = False
        try:
            existed = await _acheck_index_exists(self.client, self.index_name)
        ...
        **kwargs: Any,
        ...
        async with self.client.pipeline(transaction=False) as pipeline:
            for i, text in enumerate(texts):
                ...
                # vectorestore expire time is 1 day
                await pipeline.expire(key, 60 * 60 * 24)
                ids.append(key)
    ...
    async def afrom_texts(
        ...
        **kwargs: Any,
        ...
        # Create the search index
        await instance._acreate_index(
            dim=len(embeddings[0]), distance_metric=distance_metric
        )

        # Add data to Redis
        await instance.aadd_texts(texts, metadatas, embeddings)
        return instance
