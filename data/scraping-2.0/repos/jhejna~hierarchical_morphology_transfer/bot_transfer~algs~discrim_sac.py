...
class DSAC(OffPolicyRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=250000,
                 ...
                 discrim_buffer_size=250000,
                 discrim_layers=[32, 32],
                 discrim_learning_rate=0.0005,
                 discrim_start=200,
                 discrim_weight=0.5,
                 discrim_clip=10,
                 discrim_batch_size=64,
                 discrim_train_freq=1,
                 discrim_stop=0.5,
                 discrim_coef=2,
                 discrim_decay=False,
                 discrim_include_skill=True,
        ...
        self.discrim_test_data_collection_freq = 10
        ...
        self.discrim_dim = None
        size_of_discrim_dim = 1
        if self.discrim_include_next_state:
            size_of_discrim_dim += 1
        if self.discrim_include_skill:
            size_of_discrim_dim += 1
        try:
            self.discrim_dim = size_of_discrim_dim * env.env.env.skill_space().shape[0]
        except:
            try:
                self.discrim_dim = size_of_discrim_dim * env.envs[0].env.env.skill_space().shape[0]
            except:
                try:
                    self.discrim_dim = size_of_discrim_dim *env.envs[0].env.env.env.skill_space().shape[0]
                except:
                    pass
        print("DISCRIM DIM", self.discrim_dim)
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                self.test_discrim_replay_buffer = ReplayBuffer(int(0.5 * self.discrim_buffer_size))
                ...
                with tf.variable_scope("discrim_loss", reuse=False):
                    self.discrim_loss = tf.losses.mean_squared_error(self.discrim_output, self.discrim_labels_ph)
                    self.discrim_train_op = tf.train.AdamOptimizer(learning_rate=self.discrim_lr_ph).minimize(self.discrim_loss)
                
                self.disciminator_params = get_vars("discriminator")
                ...
                self.discrim_params = get_vars("discrim_model")
                self.summary = tf.summary.merge_all()

    def _get_discrim_reward(self, obs):
        if len(obs.shape)>1:
            observation = obs
        else:
            observation = obs[None]
        discrim_preds, discrim_loss = self.sess.run([self.discrim_output, self.discrim_loss], feed_dict={self.discrim_input_ph : observation, 
                                                                            self.discrim_labels_ph: 0.95*np.ones(shape=(obs.shape[0],1))})
        return np.clip(-self.discrim_coef * np.log(discrim_preds + 1e-6), 0, self.discrim_clip), discrim_loss
    ...
    def _discrim_train_step(self, step, learning_rate):
        ...
        feed_dict = {
            ...
            self.discrim_labels_ph : batch_labels,
            self.discrim_lr_ph : learning_rate
        }
        discrim_loss, _ = self.sess.run([self.discrim_loss, self.discrim_train_op], feed_dict=feed_dict)
        return discrim_loss

    def _get_discrim_ac(self, test=True):
        if test:
            batch_obs, _, _, batch_labels, _ = self.test_discrim_replay_buffer.sample(self.discrim_batch_size)
        else:
            batch_obs, _, _, batch_labels, _ = self.discrim_replay_buffer.sample(self.discrim_batch_size)     
        preds = self.sess.run(self.discrim_output, feed_dict={self.discrim_input_ph : batch_obs})
        preds = np.array([0.05 if pred < 0.5 else 0.95 for pred in preds])
        discrim_ac = (preds == batch_labels.flat).sum() / len(preds)
        return discrim_ac
    ...
    def _train_step(self, step, writer, learning_rate, discrim_weight):
        ...
        batch_discrim_input = batch_obs[:, -self.discrim_dim:]
        batch_obs = batch_obs[:, :-self.discrim_dim]
        
        batch_rewards = batch_rewards.reshape(self.batch_size, -1)

        batch_rewards *= (1 - discrim_weight)
        
        # modify the reward to use the discirminator if we're past discriminator start
        if step > self.discrim_start:
            discrim_reward, _ = self._get_discrim_reward(batch_discrim_input)
    ...
    def get_parameter_list(self):
        return (self.params +
                self.target_params + self.discrim_params)
    ...
    def save(self, save_path, cloudpickle=False):
        data = {
            ...
            "discrim_grad_steps" : self.discrim_grad_steps,
            "discrim_stop" : self.discrim_stop,
            "discrim_train-freq" : self.discrim_train_freq,
