...
#                chosen_pair['segment1'],chosen_pair['segment2'])
    ...
    def train_predictor(self):
        ...
        # fill out the pools 
        new_good_segs, new_bad_segs = self.comparison_collector.label_unlabeled_comparisons_with_returning_pools()
        good_segs_pool.extend(new_good_segs)
        bad_segs_pool.extend(new_good_segs)

        # data augmentation
        for i in range( min(n_generated_pairs, len(good_segs_pool) ) ):
...
def main():
    ...
    if args.predictor == "rl":
        ...
        while len(comparison_collector.labeled_comparisons) < int(pretrain_labels * 0.75):
            ...
            new_good_segs, new_bad_segs = comparison_collector.label_unlabeled_comparisons_with_returning_pools()
            good_segs_pool.extend(new_good_segs)
            bad_segs_pool.extend(new_good_segs)
