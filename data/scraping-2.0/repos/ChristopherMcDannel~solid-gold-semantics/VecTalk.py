...
messages = [
    {
        "role": "system",
        "content": "You are a financial advisor. Respond to all input in 50 words or less. Speak in the first person. Do not use the $ sign, write out dollar amounts with the full word dollars. Do not use quotation marks. Do not say you are an AI language model.",
    }
]
...
def transcribe(audio):
    ...
    question_df["similarities"] = question_df["embedding"].apply(
    ...
    tts = gTTS(system_message["content"], lang="en")
    tts.save("output.mp3")
...
with gr.Blocks(theme=theme) as ui:
    ...
    advisor = gr.Image(value=config.ADVISOR_IMAGE).style(
        width=config.ADVISOR_IMAGE_WIDTH, height=config.ADVISOR_IMAGE_HEIGHT
    )
