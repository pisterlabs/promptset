...
import sys
sys.path.append("..")
#from lm_models.parser_info import Parser
from parser_info import Parser
import file_utils
...
def pre_process_datasets(encoded_datasets, input_len, cap_length, start_token, delimiter_token, clf_token):
    ...
    for dataset in encoded_datasets:
        ...
        for i, sent, in enumerate(dataset):
            with_cont = [start_token] + sent[:cap_length] + [clf_token]
            input_ids[i, :len(with_cont)] = with_cont
            # lm_labels[i, 0, :len(with_cont)-1] = with_cont[1:]
            lm_labels[i, :len(with_cont)] = with_cont
...
def main():
    ...
    args = parser.parse_args([#'--do_train',
                              '--do_eval',
                              '--dataset=../data/convai2/train_both_original.txt',
    ...
    tokenizer = OpenAIGPTTokenizer.from_pretrained(args.model_name, cache_dir = "./cache/",  special_tokens=special_tokens)
    special_tokens_ids = list(tokenizer.convert_tokens_to_ids(token) for token in special_tokens)
    model = OpenAIGPTLMHeadModel.from_pretrained(args.model_name,cache_dir = "./cache/",
