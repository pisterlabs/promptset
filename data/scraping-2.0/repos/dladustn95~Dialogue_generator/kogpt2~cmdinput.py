...
def sample_sequence(topic, source, vocab, tokenizer, model, args, current_output=None):
    ...
    for i in range(args.max_length):
        ...
        if i < args.min_length and prev.item() in [bos, eos, speaker1, speaker2]:
...
def run():
    ...
    parser.add_argument("--temperature", type=int, default=0.5, help="Sampling softmax temperature")
    parser.add_argument("--top_k", type=int, default=0, help="Filter top-k tokens before sampling (<=0: no filtering)")
    parser.add_argument("--top_p", type=float, default=0.91,
    ...
    while 1:
        topicList = input("Input Keyword 'ex) key1,key2' : ")
        topicList = topicList.split(',')
        topics = []
        qInput = input("input Question : ")
        for topic in topicList:
            topics += vocab[tokenizer(topic)]
        source = vocab[tokenizer(qInput)]
