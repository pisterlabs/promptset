...
def create_prompt_commonsense_abstract(input, zeroshot=True):
  ...
  '''
    ...
    if zeroshot:
        ...
        prompt_hint = 'The goal is to arrange a dining table with objects that have only one of the following relationships: above, to the right of, below, to the left of, above and to the right of, below and to the right of, below and to the left of, above and to the left of, on top of, under, and center.\n\n'
    else:
    ...
    if zero_shot:
        ...
        promot_format = 'Note each action should be on a separate line starting with Place. The answer cannot include other objects.'
        prompt_question = prompt_question + promot_format
    else:
        prompt_question = 'What is a typical way of positioning ' + utensils + 'on a table? '
        promot_format = 'Note each action should be on a separate line starting with Place. The answer cannot include other objects.'
        prompt_question = prompt_question + promot_format
...
def create_prompt_commonsense_concrete(input):
  ...
  '''
    ...
    if input[2] == 'above' or input[2] == 'to the right of' or input[
        ...
        '''
    above and to the left of --> above xxx centimeter and to the left of xxx centimeter
    '''
        # temp = input[2].split(" and ")
        # prompt_question_1 = 'how many centimeters ' + temp[0] + ' the ' + item[0] + ' should the ' + item[1] + ' be placed' + '? '
        # prompt_question_2 = 'how many centimeters ' + temp[1] + ' the ' + item[0] + ' should the ' + item[1] + ' be placed' + '?'
        '''
    above and to the left of xxx centimeter
    '''
...
def extract_distance(commonsense_concrete, type):
  ...
  '''
    ...
    #     print("!Warning: Expected 2-4 values but found {}".format(len(res)))
    #     return 5, 5
    #   result = []
...
def process_response(response_commonsense_abstract, zero_shot):
    ...
    for index in range(len(response_commonsense_abstract)):
        if 'plate' in response_commonsense_abstract[
                index] and 'dinner' not in response_commonsense_abstract[
                    index] and 'bread' not in response_commonsense_abstract[
                        index]:
            response_commonsense_abstract[
                index] = response_commonsense_abstract[index].replace(
                    "plate", "dinner plate")
...
def is_outside(u1_x, u1_y, u1_width, u1_length, table_x, table_y, table_width,
               table_length):
    left1 = u1_x - u1_width / 2
    right1 = u1_x + u1_width / 2
    top1 = u1_y + u1_length / 2
...
print('-' * 20 + '\nFull prompt (including {} tokens, cost {}$):\n{}\n'.format(
    count_tokens(prompt), fee_tokens(prompt), textwrap.fill(prompt, width=150))
      + '-' * 20)
if not chatgpt:
    response_commonsense_abstract = llm(prompt)
else:
    ...
    response_commonsense_abstract = response_from_ChatGPT
...
for commonsense in response_commonsense_abstract:  # extract relationship
...
for item in points_set:
    ...
    if is_outside(u1_x, u1_y, u1_width, u1_length, table_x, table_y,
                  table_width, table_length):
        signal_7 = True
        counter['signal_7'] += 1
    if is_outside(u2_x, u2_y, u2_width, u2_length, table_x, table_y,
                  table_width, table_length):
        signal_8 = True
        counter['signal_8'] += 1
    if is_outside(u3_x, u3_y, u3_width, u3_length, table_x, table_y,
                  table_width, table_length):
        signal_9 = True
        counter['signal_9'] += 1
