...
class DeepFollowEnv(singleagentSim_env.FireflyMultiAgentGTEnv):
    ...
    def _get_obs(self):
        ...
        observations_est =  [scale*delta_target1_est.pose.pose.position.x, scale*delta_target1_est.pose.pose.position.y, scale*delta_target1_est.pose.pose.position.z,\
                        relative_vel_est[0],relative_vel_est[1],relative_vel_est[2],np.cos(theta1_est), np.sin(theta1_est),np.cos(delta_orient), np.sin(delta_orient)]+value_input

        rospy.loginfo("GTObservations==>"+str(observations))
        rospy.loginfo("END Get Observation ==>")
        rospy.loginfo("ESObservations==>"+str(observations_est))
