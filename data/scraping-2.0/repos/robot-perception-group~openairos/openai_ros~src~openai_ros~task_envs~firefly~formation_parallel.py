...
class DeepFollowEnv_Parallel(firefly_env.FireflyEnv):
    def __init__(self, **kwargs):
        ...
        super(DeepFollowEnv_Parallel, self).__init__(ros_ws_abspath, **kwargs)
    ...
    #               'actor::RightHand', 'actor::RightFingerBase', 'actor::RightHandIndex1', 'actor::RThumb'
        ...
        # low = np.array([])
        # joints_low = np.array([-self.image_width,-self.image_height])
    ...
    def _get_obs(self):
        ...
        # joint_detections[:,0] = joint_detections[:,0]/self.image_width
        # joint_detections[:,1] = joint_detections[:,1]/self.image_height
        ...
        observations =  [\
                        detection.data*xcenter/self.image_width, detection.data*ycenter/self.image_height, \
                        detection.data*abs(bbox_coordinates[3]-bbox_coordinates[1])/self.image_height, \
                        detection.data*(xcenter/self.image_width - self.prev_center[0]),  detection.data*(ycenter/self.image_height - self.prev_center[1])
                        ]
                        # detection.data, \
                        # abs(pose.position.z/15)
        #                 # feedback.xcenter - self.prev_feedback.xcenter, \
        #                 # feedback.ycenter - self.prev_feedback.ycenter \
                        # list(joints_flattened) + list(joint_probabilities)
    ...
    def _is_done(self, observations):
        ...
        if self._episode_done:
            rospy.logerr("MAV moved out of workspace or is in desired position==>")
        else:
    ...
    def _compute_reward(self, observations, done):
        ...
        if not done:
            ...
            [flag,height] = self.is_in_MAV_desired_altitude(observations,epsilon_up=7,epsilon_down=3.5)
            if flag:
                reward += 0.16*self.detection_data
            else:
                reward += 0.01*self.detection_data
            ...
            if flag:
                reward += 0.16*self.detection.data
            else:
            ...
            # if flag:
            #     reward += 0.5*detection.data
            # else:
            #     reward += 0.01*detection.data
            #
            ...
            if delta_error < 0.0 and rad_distance_from_target > 6:
                reward += 0.16*self.detection_data
...
#                reward+=-20*(self.step_limit - self.cumulated_steps)
    ...
    def is_in_desired_distance(self,observations,  epsilon_up=7, epsilon_down=3):
        """
        It returns True if the current position is similar to the desired position
        """
        target = self.get_gt_target()
    ...
    def is_in_MAV_desired_altitude(self,observations,epsilon_up=7,epsilon_down=3.5):
        """
        ...
        is_in_desired_pos = False
        if height <= epsilon_up and height >= epsilon_down:
            is_in_desired_pos = True
            self.good_behavior+=1
