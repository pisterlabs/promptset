...
class DeepFollowEnv(multiagentSim_env.FireflyMultiAgentGTEnv):
    ...
    def _get_obs(self):
        ...
        observations_est = [scale*delta_target1_est.pose.pose.position.x, scale*delta_target1_est.pose.pose.position.y, scale*delta_target1_est.pose.pose.position.z,\
                        scale*delta_neighbor_est.pose.pose.position.x, scale*delta_neighbor_est.pose.pose.position.y, scale*delta_neighbor_est.pose.pose.position.z, \
                        np.cos(delta_orient1), np.sin(delta_orient1), \
                        np.cos(delta_orient2), np.sin(delta_orient2), \
                        np.cos(theta1_est), np.sin(theta1_est)]+value_input

        rospy.loginfo("Observations==>"+str(observations))        
        rospy.loginfo("END Get Observation ==>")
        rospy.loginfo("Observations==>"+str(observations_est))
        return observations
