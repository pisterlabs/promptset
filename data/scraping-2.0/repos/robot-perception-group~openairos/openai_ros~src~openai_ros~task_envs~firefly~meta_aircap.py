...
from openai_ros.robot_envs import metaSim_aircap_env
...
class DeepFollowEnv(metaSim_aircap_env.FireflyMultiAgentGTEnv):
    def __init__(self,**kwargs):
        """
        This Task Env is designed for MAVs to track a human target
        """
        rospy.logwarn('Entered Multi Agent Training ENV')
    ...
    ############################################################################################################################
        ...
        low = np.concatenate((low,np.array([self.min_v,self.min_v,self.min_v,\
        ...
        high = np.concatenate((high,np.array([self.max_v,self.max_v,self.max_v,\
        ...
        if self.ACTION_TYPE=="discrete":
            ...
            # Velocity along X, Y and yawrate
            self.action_space = spaces.Box(np.array([self.control_x_min,self.control_y_min,-1]),np.array([self.control_x_max,self.control_y_max,1]))
    ...
    def _get_obs(self):
        ...
        pose_est = self.get_odom(stamp)
        pose_neighbor_est = self.get_neighbor_odom(stamp) 
        # Velocity of the MAV
        vel = self.get_velocity(stamp)
        #Velocity of Neighbor
        vel_neighbor = self.get_velocity_neighbor(stamp)
        # Position of the Target
        target = self.get_gt_target(stamp)
        target_est = self.get_target(stamp)
        target_velocity_est = self.get_target_velocity(stamp)
        ...
        '''Estimated MAV orientation'''
        (r,p,y) = tf.transformations.euler_from_quaternion([pose_est.orientation.x,pose_est.orientation.y,pose_est.orientation.z,pose_est.orientation.w])
        yaw1_est = y
        ...
        '''Estimated Neigbor orientation'''
        (r,p,y) = tf.transformations.euler_from_quaternion([pose_neighbor_est.orientation.x,pose_neighbor_est.orientation.y,pose_neighbor_est.orientation.z,pose_neighbor_est.orientation.w])
        yaw2_est = y
        ...
        '''Relative Estimated Target Position Computation'''
        delta_target1_est = PoseWithCovarianceStamped()        
        homogeneous_matrix_est_1 = tf.transformations.euler_matrix(0,0,yaw1_est,axes='sxyz')
        d_target1_est = np.dot(np.linalg.inv(homogeneous_matrix_est_1),np.array([target_est.pose.pose.position.x - pose_est.position.x,
                                                                    target_est.pose.pose.position.y - pose_est.position.y,
                                                                    target_est.pose.pose.position.z - pose_est.position.z,1]))
        delta_target1_est.pose.pose.position.x = d_target1_est[0]
        delta_target1_est.pose.pose.position.y = -d_target1_est[1]
        delta_target1_est.pose.pose.position.z = -d_target1_est[2]
                                                     
        theta1_est = np.arctan2(delta_target1_est.pose.pose.position.y,delta_target1_est.pose.pose.position.x)        
        ...
        '''Estimated Neighbor base link in Estimated Mav base link frame '''
        delta_neighbor_est = PoseWithCovarianceStamped()        
        d_neighbor_est = np.dot(np.linalg.inv(homogeneous_matrix_est_1),np.array([pose_neighbor_est.position.x - pose_est.position.x,
                                                                                pose_neighbor_est.position.y - pose_est.position.y,
                                                                                pose_neighbor_est.position.z - pose_est.position.z,1]))
        delta_neighbor_est.pose.pose.position.x = d_neighbor_est[0]
        delta_neighbor_est.pose.pose.position.y = -d_neighbor_est[1]
        delta_neighbor_est.pose.pose.position.z = -d_neighbor_est[2]      
        ...
        relative_vel_est = np.dot(np.linalg.inv(homogeneous_matrix_est_1), np.array([vel.twist.twist.linear.y - target_velocity_est.twist.twist.linear.x, 
                                        vel.twist.twist.linear.x - target_velocity_est.twist.twist.linear.y,\
                                       -vel.twist.twist.linear.z - target_velocity_est.twist.twist.linear.z,1]))

        # #Dummy zero inputs to value network during testing.
        # value_input_sa = [0]*len(list(gt)+[pose.position.x,pose.position.y,pose.position.z,\
        #                                    target.twist.twist.linear.x,target.twist.twist.linear.y,target.twist.twist.linear.z,\
        #                                    vel.twist.twist.linear.x,vel.twist.twist.linear.y,-vel.twist.twist.linear.z,\
        #                                    np.cos(yaw1), np.sin(yaw1),np.cos(person_theta), np.sin(person_theta)])

        # Policy Inputs
        scale = 1
        observations_sa =  [scale*delta_target1.pose.pose.position.x, scale*delta_target1.pose.pose.position.y, scale*delta_target1.pose.pose.position.z,\
                        relative_vel[0],relative_vel[1],relative_vel[2]]

        observations_est_sa =  [scale*delta_target1_est.pose.pose.position.x, scale*delta_target1_est.pose.pose.position.y, scale*delta_target1_est.pose.pose.position.z,\
                        relative_vel_est[0],relative_vel_est[1],relative_vel_est[2]]


        observations_ma = [delta_neighbor.pose.pose.position.x, delta_neighbor.pose.pose.position.y, delta_neighbor.pose.pose.position.z, \
                        np.cos(delta_orient1), np.sin(delta_orient1), \
                        np.cos(delta_orient2), np.sin(delta_orient2), \
                        np.cos(theta1), np.sin(theta1)]

        observations_est_ma = [delta_neighbor_est.pose.pose.position.x, delta_neighbor_est.pose.pose.position.y, delta_neighbor_est.pose.pose.position.z, \
                        np.cos(delta_orient1), np.sin(delta_orient1), \
                        np.cos(delta_orient2), np.sin(delta_orient2), \
                        np.cos(theta1_est), np.sin(theta1_est)]

        #Value Inputs
        value_input_ma = list(gt)+[target.twist.twist.linear.x,target.twist.twist.linear.y,target.twist.twist.linear.z,\
        pose.position.x, pose.position.y, pose.position.z, \
        vel.twist.twist.linear.x,vel.twist.twist.linear.y,-vel.twist.twist.linear.z, \
        pose_neighbor.position.x, pose_neighbor.position.y, pose_neighbor.position.z, \
        vel_neighbor.twist.twist.linear.x,vel_neighbor.twist.twist.linear.y, -vel_neighbor.twist.twist.linear.z, \
        np.cos(yaw1), np.sin(yaw1), \
        np.cos(yaw2), np.sin(yaw2), \
        np.cos(person_theta), np.sin(person_theta)]



        observations = observations_sa+observations_ma+value_input_ma
        observations_est = observations_est_sa+observations_est_ma+value_input_ma

        rospy.loginfo("Observations==>"+str(observations[0:15]))        
        rospy.loginfo("END Get Observation ==>")
        rospy.loginfo("Observations==>"+str(observations_est[0:15]))
        return observations
