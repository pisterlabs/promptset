...
from openai_ros.robot_envs import tello_env
...
class DeepFollowEnv(tello_env.FireflyMultiAgentGTEnv):
    ...
    def _get_obs(self):
        ...
        TARGET_orientation=np.array([target.pose.pose.orientation.x,target.pose.pose.orientation.y,target.pose.pose.orientation.z,target.pose.pose.orientation.w])
        MAV_orientation=np.array([pose.orientation.x,pose.orientation.y,pose.orientation.z,pose.orientation.w])

        (r,p,y) =  tf.transformations.euler_from_quaternion(TARGET_orientation)
        TARGET_yaw = y
        ...
        delta_target1.pose.pose.position.x = delta_target_local[0]; delta_target1.pose.pose.position.y = delta_target_local[1]; delta_target1.pose.pose.position.z = delta_target_local[2];    
    ...
    def get_hmr_error(self):
        ...
        for k in range(len(gt_joints)):
            try:
                ...
                diff = Point()
                diff.x = trans[0]-hmr_joints[k].poses.position.x
                diff.y = trans[1]-hmr_joints[k].poses.position.y
                diff.z = trans[2]-hmr_joints[k].poses.position.z                
                error += (self.get_distance_from_point(diff,null_point))
            except:
                diff.x = 10;diff.y=10;diff.z=10
                error = 10
            err_cov  = PoseWithCovarianceStamped()
            err_cov.pose.pose.position = hmr_joints[k].poses.position
