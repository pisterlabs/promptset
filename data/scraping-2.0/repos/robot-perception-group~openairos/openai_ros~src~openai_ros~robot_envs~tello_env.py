...
import time
import os
from openai_ros import robot_tello_env
...
from geometry_msgs.msg import PoseArray, PoseStamped, Twist,TwistStamped, PoseWithCovarianceStamped, TwistWithCovarianceStamped
...
class FireflyMultiAgentGTEnv(robot_tello_env.RobotTelloEnv):
    ...
    def __init__(self,**kwargs):
        ...
        * /command, publishes the desired waypoints and velocity for the robot. Type: uav_msgs
        ...
        self.gt_pose_topic = "/tello1/pose"
        ...
        self.tello_topic = "/solo/cmd_vel"

        self.neighbor_command_topic = neighbor_name+"/command"

        self.destination_topic = "/solo/cmd_pos"
        ...
        self.hmr_topic = machine_name+"/hmr_joints"
        ...
        self._tello_cmd_pub = rospy.Publisher(self.tello_topic, Twist, queue_size=1)
        self._tello_cmd_pub_dup = rospy.Publisher('/solo/dup_cmd_vel', Twist, queue_size=1)
        ...
        self._vel_pub = rospy.Publisher('/tello/obs_vel', Twist, queue_size=1)
    ...
    def _check_hmr_ready(self):
        ...
        while self.hmr is None and not rospy.is_shutdown():
            try:
                self.hmr = rospy.wait_for_message(self.hmr_topic, PoseArray, timeout=30.0)
    ...
    def _check_publishers_connection(self):
        ...
        while self._tello_cmd_pub.get_num_connections() == 0 and not rospy.is_shutdown():
            rospy.logdebug("No susbribers to _cmd_vel_pub yet so we wait and try again")
            try:
                rate.sleep()
            except rospy.ROSInterruptException:
                # This is to avoid error when world is rested, time when backwards.
                pass            
        rospy.logdebug("_tello_cmd_pub Publisher Connected")
    ...
    def move_base(self, action, horizon, update_rate=100, init= False):
        ...
        if init:
            ...
            rot = np.array([self.gt_odom.orientation.x,self.gt_odom.orientation.y,self.gt_odom.orientation.z,self.gt_odom.orientation.w])
            trans = np.array([self.gt_odom.position.y,self.gt_odom.position.x,-self.gt_odom.position.z])
            ...
            orientation = np.array([self.gt_odom.orientation.x,self.gt_odom.orientation.y,self.gt_odom.orientation.z,self.gt_odom.orientation.w])
            (r,p,y) =  tf.transformations.euler_from_quaternion(orientation)       
            
            
            target = self.get_gt_target()
            desired_angle = np.arctan2(self.gt_odom.position.y - target.pose.pose.position.y,self.gt_odom.position.x-target.pose.pose.position.x)
    ...
    def pid(self,z,desired_z,p=1,i=0,d=0):
        error = z-desired_z
        u = p*(error)
        return u

    def pid_yawrate(self,angle,desired_angle,p=2,i=0,d=0):
        delta_orient =  desired_angle - angle 
    ...
    def get_tello_velocity(self):
        import tf
        orientation = np.array([self.gt_odom.orientation.x,self.gt_odom.orientation.y,self.gt_odom.orientation.z,self.gt_odom.orientation.w])
        (r,p,y) =  tf.transformations.euler_from_quaternion(orientation)
        homogeneous_matrix = tf.transformations.euler_matrix(0,0,y,axes='sxyz')
        inv_homogeneous_matrix=np.linalg.inv(homogeneous_matrix[:3,:3])        
        self.tello_vel = np.dot(inv_homogeneous_matrix,np.array([self.posdiff[1],self.posdiff[0],-self.posdiff[2]]))
        self.tello_vel_NED = np.array([self.tello_vel[1],self.tello_vel[0],-self.tello_vel[2]])
        
        vel = Twist()
        vel.linear.x = self.tello_vel_NED[0]
        vel.linear.y = self.tello_vel_NED[1]
        vel.linear.z = self.tello_vel_NED[2]
        self._vel_pub.publish(vel)
        return np.array([self.tello_vel_NED[0],self.tello_vel_NED[1],self.tello_vel_NED[2]])
    ...
    def get_gt_odom(self):
        self.posdiff = (np.array([self.gt_odom.position.x,self.gt_odom.position.y,self.gt_odom.position.z])-\
            np.array([self.prev_odom.position.x,self.prev_odom.position.y,self.prev_odom.position.z]))
        
        if len(self.pos_queuex)==100:
            self.pos_queuex.pop(0);
            self.pos_queuey.pop(0)
            self.pos_queuez.pop(0)
            self.pos_queuet.pop(0)
            
        self.pos_queuex.append(self.gt_odom.position.x)
        self.pos_queuey.append(self.gt_odom.position.y)
        self.pos_queuez.append(self.gt_odom.position.z)
        self.pos_queuet.append(self.gt_odom.header.stamp.to_nsec())
        vx = self.regression(self.pos_queuex,self.pos_queuet)
        vy = self.regression(self.pos_queuey,self.pos_queuet)
        vz = self.regression(self.pos_queuez,self.pos_queuet)
        
        self.prev_odom = self.gt_odom
        self.posdiff = np.array([vx,vy,vz])
        return self.gt_odom

    def regression(self,x,t):        
        mean_x = np.mean(x)
        mean_t = np.mean(t)        
        var_t = np.sum(np.square(np.array(t)-mean_t))        
        std_x = np.array(x) - mean_x
        std_t = np.array(t) - mean_t                
        sum_std_xt = np.sum(std_x*std_t)        
        regressed_value = 10**9*sum_std_xt/var_t     
        if np.isnan(float(regressed_value)):
            regressed_value = 0
        if abs(regressed_value) > 1:
            regressed_value = regressed_value/abs(regressed_value)
