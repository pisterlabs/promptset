...
class FireflyMultiAgentSyncEnv(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self, ros_ws_abspath,**kwargs):
        ...
        * /command, publishes the desired waypoints and velocity for the robot. Type: uav_msgs
        ...
        super(FireflyMultiAgentSyncEnv, self).__init__(controllers_list=self.controllers_list,
        ...
        self._sub1 =rospy.Subscriber(self.gt_neighbor_pose_topic, uav_pose, self._gt_neighbor_pose_callback)
        self._sub2 =rospy.Subscriber(self.alphapose_neighbor_topic, AlphaRes, self._alphapose_neighbor_callback)
        self._sub3 =rospy.Subscriber(self.alphapose_bbox_neighbor_topic, AlphaRes, self._alphapose_bbox_neighbor_callback)
        self._sub4 =rospy.Subscriber(self.camera_info_neighbor, CameraInfo, self._camera_info_neighbor_callback)


        gazebo_pose_sub = message_filters.Subscriber(self.firefly_pose_topic,PoseWithCovarianceStamped)
        gazebo_vel_sub = message_filters.Subscriber(self.velocity_topic, Odometry)
        machine_pose_sub = message_filters.Subscriber(self.gt_pose_topic, uav_pose)
        machine_neighbor_pose_sub = message_filters.Subscriber(self.gt_neighbor_pose_topic, uav_pose)
        target_gt_sub = message_filters.Subscriber(self.gt_target_topic, Odometry)
        alphapose_sub = message_filters.Subscriber(self.alphapose_topic, AlphaRes)
        alphapose_neighbor_sub = message_filters.Subscriber(self.alphapose_neighbor_topic, AlphaRes)
        alphapose_bbox_sub = message_filters.Subscriber(self.alphapose_bbox_topic, AlphaRes)
        alphapose_bbox_neighbor_sub =  message_filters.Subscriber(self.alphapose_bbox_neighbor_topic, AlphaRes)
        camera_info_sub = message_filters.Subscriber(self.camera_info, CameraInfo)
        camera_info_neighbor_sub = message_filters.Subscriber(self.camera_info_neighbor, CameraInfo)
        joints_gt_sub = message_filters.Subscriber(self.joints_gt, PoseArray)

        mpc_command_sub = rospy.Subscriber(self.mpc_command, uav_pose,self._command_callback)
        command_sub = rospy.Subscriber(self.command_topic, uav_pose,self._mpc_command_callback)
        ...
        #subscriber_list = [gazebo_pose_sub,gazebo_vel_sub,machine_pose_sub,machine_neighbor_pose_sub,\
                #target_gt_sub,alphapose_sub,alphapose_neighbor_sub,alphapose_bbox_sub,\
                #alphapose_bbox_neighbor_sub,camera_info_sub,camera_info_neighbor_sub,\
                #joints_gt_sub]
            
        subscriber_list = [gazebo_pose_sub,gazebo_vel_sub,machine_pose_sub,machine_neighbor_pose_sub,\
                target_gt_sub,alphapose_sub,camera_info_sub,camera_info_neighbor_sub,\
                joints_gt_sub]
        ...
        rospy.logwarn("INITIALIZE TIME SYNCHRONIZATION...")
        ts = message_filters.TimeSynchronizer(subscriber_list, 1000)
        ts.registerCallback(self._callback)
        self._check_all_sensors_ready()
        rospy.logwarn("ALL SUBSCRIBERS OK!...")
        self._check_publishers_connection()
        rospy.logwarn("ALL PUBLISHERS OK!...")

    # Methods needed by the RobotGazeboEnv
    # ----------------------------
    def create_circle(self, radius=5):
        self.theta = [k for k in np.arange(1.57,2,0.1)]
    ...
    def _check_all_sensors_ready(self):
        ...
        self._check_gt_odom_ready()
        rospy.logwarn("...")
        self._check_vel_ready()
        rospy.logwarn("...")
        self._check_gt_target_ready()
        rospy.logwarn("...")
        self._check_gt_joints_ready()
        rospy.logwarn("...")
    ...
    def _check_gt_odom_ready(self):
        self.gt_odom = None
        rospy.logdebug("Waiting for /gt_pose to be READY...")
        while self.gt_odom is None and not rospy.is_shutdown():
            # try:
                self.gt_odom = rospy.wait_for_message(self.gt_pose_topic, uav_pose, timeout=5.0)
                rospy.logdebug("Current /gt_pose READY=>")

            # except:
                # rospy.logerr("Current /pose not ready yet, retrying for getting odom")
        print(self.gt_odom.header.stamp.secs)
        return self.gt_odom
    ...
    def _check_gt_joints_ready(self):
        self.joints_gt_data = None
        rospy.logdebug("Waiting for /target to be READY...")
        while self.joints_gt_data is None and not rospy.is_shutdown():
            try:
                self.joints_gt_data = rospy.wait_for_message(self.joints_gt, PoseArray, timeout=5.0)
                rospy.logdebug("Current /target READY=>")
    ...
    def _check_vel_ready(self):
        self.velocity = None
        rospy.logdebug("Waiting for /velocity to be READY...")
        while self.velocity is None and not rospy.is_shutdown():
            # try:
                self.velocity = rospy.wait_for_message(self.velocity_topic, Odometry, timeout=5.0)
                rospy.logdebug("Current /velocity READY=>")
            # except:
                # rospy.logerr("Current /pose not ready yet, retrying for getting odom")
    ...
    def _check_neighbor_alphapose_ready(self):
        ...
        rospy.logerr("Waiting for neighbor /alphapose to be READY...")
        ...
        rospy.logerr("Current /alphapose READY=>")
        self._sub2.unregister()
        print(self.alphapose_neighbor.header.stamp.secs)
    ...
    def _check_neighbor_ready(self):
        ...
        print(self.gt_neighbor_odom.header.stamp.secs)
   ...
   #def _callback(self,firefly_odom,velocity,gt_odom,gt_neighbor_odom,gt_target,alphapose,alphapose_neighbor,bbox,bbox_neighbor,caminfo,caminfo_neighbor,joints_gt_data):
        #self.firefly_odom = firefly_odom
        #self.velocity = velocity
        #self.gt_odom = gt_odom
        #print(self.gt_odom.header.stamp.secs)
        #self.gt_neighbor_odom = gt_neighbor_odom
        #print(self.gt_neighbor_odom.header.stamp.secs)
        #self.gt_target = gt_target
        #self.alphapose = alphapose
        #self.alphapose_neighbor = alphapose_neighbor
        #self.bbox = bbox
        #self.bbox_neighbor = bbox_neighbor
        #self.caminfo = caminfo
        #self.P1 = np.array([self.caminfo.P[0:4],self.caminfo.P[4:8],self.caminfo.P[8:12]])
        #self.caminfo_neighbor = caminfo_neighbor
        #self.P2 = np.array([self.caminfo_neighbor.P[0:4],self.caminfo_neighbor.P[4:8],self.caminfo_neighbor.P[8:12]])
        #self.joints_gt_data = joints_gt_data


    #def _callback(self,gt_odom,gt_neighbor_odom,gt_target):
        #self.gt_odom = gt_odom
        #self.gt_neighbor_odom = gt_neighbor_odom
        #print(self.gt_odom.header.stamp.secs)
        #self.gt_target = gt_target
    ...
    def _callback(self,firefly_odom,velocity,gt_odom,gt_neighbor_odom,gt_target,alphapose,caminfo,caminfo_neighbor,joints_gt_data):
            self.firefly_odom = firefly_odom
            self.velocity = velocity
            self.gt_odom = gt_odom
            print(self.gt_odom.header.stamp.secs)
    ...
    def move_base(self, action, horizon, update_rate=100, init= False):
        ...
        if init:
            ...
            if self.robotID==1:
                goal = homogeneous_matrix.dot(np.concatenate((np.array(action),np.array([1]))))
