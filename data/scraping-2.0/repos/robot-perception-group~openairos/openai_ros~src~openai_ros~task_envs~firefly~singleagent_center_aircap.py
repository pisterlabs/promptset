...
class DeepFollowEnv(singleagentSim_env.FireflyMultiAgentGTEnv):
    def __init__(self,**kwargs):
        ...
        low = np.concatenate((low,np.array([self.work_space_min,self.work_space_min,self.work_space_min,\
                                            self.min_v,self.min_v,self.min_v,-1,-1]))) 
        '''
        ...
        high = np.concatenate((high,np.array([self.work_space_max,self.work_space_max,self.work_space_max,\
                                              self.max_v,self.max_v,self.max_v,1,1]))) 
        '''
    ...
    def _get_obs(self):
        ...
        homogeneous_matrix_est = tf.transformations.euler_matrix(0,0,yaw1_est,axes='sxyz')
        d_target1_est = np.dot(np.linalg.inv(homogeneous_matrix_est),np.array([target_est.pose.pose.position.x - pose_est.position.x,
        ...
        relative_vel_est = np.dot(np.linalg.inv(homogeneous_matrix_est), np.array([vel.twist.twist.linear.y - target_velocity_est.twist.twist.linear.x, 
        ...
        value_input = [0]*len(list(gt)+[pose.position.x,pose.position.y,pose.position.z,target.twist.twist.linear.x,target.twist.twist.linear.y,target.twist.twist.linear.z,vel.twist.twist.linear.x,vel.twist.twist.linear.y,-vel.twist.twist.linear.z,np.cos(yaw1), np.sin(yaw1)])
        ...
        observations =  [scale*delta_target1.pose.pose.position.x, scale*delta_target1.pose.pose.position.y, scale*delta_target1.pose.pose.position.z,\
                        relative_vel[0],relative_vel[1],relative_vel[2],np.cos(theta1), np.sin(theta1)]+value_input
        ...
        observations_est =  [scale*delta_target1_est.pose.pose.position.x, scale*delta_target1_est.pose.pose.position.y, scale*delta_target1_est.pose.pose.position.z,\
                        relative_vel_est[0],relative_vel_est[1],relative_vel_est[2],np.cos(theta1_est), np.sin(theta1_est)]+value_input

        rospy.loginfo("Observations==>"+str(observations))
        rospy.loginfo("END Get Observation ==>")

        return observations_est
