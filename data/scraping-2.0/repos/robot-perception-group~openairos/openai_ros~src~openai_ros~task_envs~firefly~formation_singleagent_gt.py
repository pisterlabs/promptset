...
from openai_ros.robot_envs import firefly_env, firefly_multiagent_gt_inf_env,firefly_singleagentcartesian_env
...
class DeepFollowEnv(firefly_singleagentcartesian_env.FireflyMultiAgentGTEnv):
    ...
    #               'actor::RightHand', 'actor::RightFingerBase', 'actor::RightHandIndex1', 'actor::RThumb'
        ...
        low = np.concatenate((low,np.array([self.work_space_min,self.work_space_min,self.work_space_min,\
                                            self.min_v,self.min_v,self.min_v]))) #dx,dy,dz,vx,vy,vz
        # GT INFORMATION
        ...
        low = np.concatenate((low,np.array([self.work_space_min,self.work_space_min,self.work_space_min,
                                            ...
                                            self.min_v,self.min_v,self.min_v]))) #x,y,z,px,py,pz,vx,vy,vz,pvx,pvy,pvz
        ...
        high = np.concatenate((high,np.array([self.work_space_max,self.work_space_max,self.work_space_max,\
                                              self.max_v,self.max_v,self.max_v]))) #dx,dy,dz,vx,vy,vz
        # GT INFORMATION
        ...
        high = np.concatenate((high,np.array([self.work_space_max,self.work_space_max,self.work_space_max,\
                                              ...
                                              self.max_v,self.max_v,self.max_v]))) #x,y,z,vy,vz,pvx,pvy,pvz
        ...
        if self.ACTION_TYPE=="discrete":
            ...
            self.action_space = spaces.Box(np.array([self.control_x_min,self.control_y_min]),\
                np.array([self.control_x_max,self.control_y_max]))
    ...
    def _compute_reward(self, observations, done):
        ...
        if not done:
            ...
            if distance > 5 and distance <10:
                reward+= 0.5*(1-np.tanh(0.01*distance_to_imgcenter))
            else:
                reward+= 0.05*(1-np.tanh(0.01*distance_to_imgcenter))
