...
from openai_ros.robot_envs import firefly_singleagent_env
...
class DeepFollowEnv(firefly_singleagent_env.FireflySingleAgentEnv):
    ...
    def _get_obs(self):
        ...
        # self.act_pretrained, _states = self.pretrained_model.predict(observation=observations[0:5], deterministic = True)
        return observations
...
#                reward+=-20*(self.step_limit - self.cumulated_steps)
        ...
        if reward_new == 1:
                ...
                rospy.logerr("")
        rospy.logwarn("reward=" + str(reward_new))
        rospy.logdebug("reward=" + str(reward))
        self.reward_msg.data = reward_new
        self._reward_pub.publish(self.reward_msg)
        self.cumulated_reward += reward_new
