...
from openai_ros.robot_envs import firefly_env, firefly_multiagent_gt_inf_env,firefly_multiagent_sync_env
...
class DeepFollowEnv(firefly_multiagent_gt_inf_env.FireflyMultiAgentGTEnv):
    ...
    def _get_obs(self):
        ...
        theta1 = np.arctan2(target.pose.pose.position.y - pose.position.y, target.pose.pose.position.x - pose.position.x)
        theta2 = np.arctan2(target.pose.pose.position.y - pose_neighbor.position.y, target.pose.pose.position.x - pose_neighbor.position.x)
        ...
        observations =  [\
                        ...
                        [pose.position.x - target.pose.pose.position.x, pose.position.y - target.pose.pose.position.y, pose.position.z - target.pose.pose.position.z,\
                        pose.position.x - pose_neighbor.position.x, pose.position.y - pose_neighbor.position.y, pose.position.z - pose_neighbor.position.z, \
                        np.cos(theta1), np.sin(theta1),np.cos(theta2),np.sin(theta2),np.cos(person_theta), np.sin(person_theta)]+\
    ...
    def _compute_reward(self, observations, done):
        ...
        if not done:
            ...
            reward += 0.5*(1-np.tanh(0.01*error))
