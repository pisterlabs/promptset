"""
This module is to calculate the consistency of the test data fetched from wandb.
"""
...
wandb.init(
        ...
        job_type='consistency_evaluation'
        )
...
for run_id, group_df in tqdm(grouped):
    ...
    for _, row in tqdm(group_df.iterrows()):
        ...
        try:
            ...
            # format the prompt
            prompt_gpt = "Decide if the following summary is consistent with the corresponding dialogue. Note that " \
                         "consistency means all information in the summary is supported by the dialogue.\nDialogue:\n"
            ...
            # check if 'yes' is in response_text
            is_consistent = 1 if 'yes' in response_text.lower() else 0
