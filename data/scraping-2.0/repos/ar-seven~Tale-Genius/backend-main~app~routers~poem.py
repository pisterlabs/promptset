...
def generate_story(prompt: str):
    completion = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"Generate a delightful 8 stanza children's poem with an enchanting title about {prompt} that shares an important moral lesson. After each stanza add double newlines to separate the stanzas. Remember, our poems are meant to inspire and educate, so please choose a topic that's fun, safe, and suitable for kids. If the topic is irrelevant or inappropriate, share a special poem with another topic instead. add double newlines to separate the stanzas."
            }
        ]
    )
...
def get_followup_response(session_id: int, story_id: int, question: str,db):
    ...
    system_msg = f"You are an assistant that answers the questions to the children's "\
                 "poem given below. You should answer the questions descriptively in a "\
