...
from algo.skew_explore import SkewExploreKDE
...
class SAC(OffPolicyRLModel):
    ...
    def __init__(self, policy, env, args, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 learning_starts=200, train_freq=1, batch_size=64,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                if isinstance(self.env, HERGoalEnvWrapper):
                    skew_explore_obs_dict = self.env.convert_obs_to_dict(skew_explore_obs)
                    skew_explore_obs = np.array([skew_explore_obs_dict['observation']])
                    tra_obs.append(skew_explore_obs[0])
                    if selected_goal is None:
                        selected_goal = np.array(skew_explore_obs_dict['desired_goal'])
                else:
                    tra_obs.append(skew_explore_obs)

                self.skew_explore.update_history(skew_explore_obs, [done])
                if (step % self.goal_update_frequency == 0 and step!= 0) or step == 2000:
    ...
    def plot_tra(self, t, tra_obs, sampled_goal):
        ...
        if self.args.env == 'maze':
            ...
            ax1.set_xlim([self.skew_explore.x_start-0.05, self.skew_explore.x_end+0.05])
            ax1.set_ylim([self.skew_explore.y_start-0.05, self.skew_explore.y_end+0.05])
        ...
        plt.savefig(self.args.save_path + '/' + self.name + str(int(t%20)) +'.svg')
        logging_info = 'save trajectory plot as: ' + self.args.save_path + '/' + self.name + str(int(t%20)) +'.svg'
    ...
    def load(cls, load_path, env=None, args=None, **kwargs):
        ...
        model = cls(policy=data["policy"], env=env, _init_setup_model=False, args=args)
