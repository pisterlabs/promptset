...
## convert topic model to dataframe  
def topic2df(topic_model, total_topics=1,
                weight_threshold=0.0001,
                display_weights=True,
                num_terms=30):
    """convert topic keywords to dataframe """
    if total_topics:
        assert total_topics <=topic_model.num_topics, "number of topics over total topic size in the model"
    else:
        total_topics = topic_model.num_topics
    topic_dfs = []
    for index in range(total_topics):
        print("\rconverting topic :{}".format(index),end='',flush=True)
        topic = topic_model.show_topic(index,topn=num_terms)
        topic = [(word, round(wt,4)) 
                 for word, wt in topic 
                 if abs(wt) >= weight_threshold]
        ...
        if display_weights:
            topic_df = pd.DataFrame(topic,columns=[index,'{}_w'.format(index)])
            topic_dfs.append(topic_df)
        else:
            tw = [term for term, wt in topic]
            topic_df = pd.DataFrame(tw,columns=[index])
            topic_dfs.append(topic_df)
    
    merged_df = pd.concat(topic_dfs,axis=1)
    return merged_df
