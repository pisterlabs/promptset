...
input_path = "C:\@code\APIMISUSE\data\detection_result_yes_v4_existing.json"
output_path = "C:\@code\APIMISUSE\data\generate_fix_yes_v4_existing.json"
...
for i in range(0, len(data)):
    ...
    code_before_prompt = ""
    for line in code_before:
        code_before_prompt += line
    fix_pattern = data[i]["example"]
    fix_pattern_prompt = ""
    for line in fix_pattern:
        if line!="":
            fix_pattern_prompt += line+"\n"

    prompt_2 = template_2.format(
        code_before=code_before_prompt, fix_rules=fix_pattern_prompt)
