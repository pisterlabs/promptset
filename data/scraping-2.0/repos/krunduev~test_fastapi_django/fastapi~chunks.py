...
class Chunk():
    ...
    def get_answer(self, system:str = default_system, query:str = None):
        ...
        request = f"Ответь на вопрос клиента. Не упоминай документ с информацией для ответа клиенту в ответе. Документ с информацией для ответа клиенту: {message_content}\n\nВопрос клиента: \n{query}"
        messages = [
        ...
        # Формирование ключа в формате 'ГГГГ-ММ-ДД ЧЧ'
        datetime_key = current_datetime.strftime("%Y-%m-%d %H")

        # Инициализация словаря для подсчета запросов, если еще не существует
        if datetime_key not in self.request_count:
            self.request_count[datetime_key] = 0

        # Увеличение счетчика запросов для данного часа и даты
        self.request_count[datetime_key] += 1
