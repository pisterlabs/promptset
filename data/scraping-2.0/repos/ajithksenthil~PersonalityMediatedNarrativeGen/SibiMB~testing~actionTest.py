...
class App:
    ...
    def getUIDataAndChooseAction(self):
        ...
        print("story before next event added ", story)

        print(" next action: ", next_action, " event list ", eventlist)
        if user_suggestion:
            print("user suggested next event")
            cutstory = story[-30:]
            queryuser = f"Given this event representation: {user_suggested_next_action_event}, please add the event to the story, taking into account the following context: {cutstory}."
            nextevent = gpt3(queryuser)
            nextevent = str(nextevent)
            nextevent = nextevent.replace("\n", "")
            print("user suggestion: ", user_suggested_next_action_event)
            print("next event user suggested : ", nextevent)
        else:
            print("generated next event")
        ...
        story = story + str(nextevent)
        story = story.replace("\n", "")
        print("next event ", nextevent)
        print("story after next event added ", story)
