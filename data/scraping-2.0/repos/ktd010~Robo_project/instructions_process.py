...
async def main() -> None:
    ...
    chat_completion = await client.chat.completions.create(
        messages=[
            {
                "role": "user",
                "content": "write this  in to this format only  { object 1, direction, instructions } " + str(prompt3) + "chose direction from, center, north, east, left, right. chose instructions from stack, place only " ,
            }
        ],
        model="gpt-3.5-turbo",
    )
