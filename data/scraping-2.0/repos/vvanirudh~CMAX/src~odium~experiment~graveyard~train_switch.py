...
from odium.agents.her_switch_agent.arguments import get_args
from odium.agents.her_switch_agent.her_switch_agent import her_switch_agent
...
def launch(args):
    assert not args.env_name.startswith('Residual'), 'Residual envs not allowed'
    ...
    if MPI.COMM_WORLD.Get_rank() == 0:
        if args.log_dir or logger.get_dir() is None:
            logger.configure(dir=os.path.join(
                'logs', 'switch_her', args.log_dir), format_strs=['tensorboard', 'log', 'csv', 'json', 'stdout'])
        else:
            logger.configure(dir=os.path.join('logs', 'switch_her', args.env_name), format_strs=[
