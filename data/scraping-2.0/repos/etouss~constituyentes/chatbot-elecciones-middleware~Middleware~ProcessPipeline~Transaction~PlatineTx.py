...
class PlatineTx(GenericTransaction):
    ...
    def process_input(self, input:str) -> str:
        instruction = """We are using an SQL database to store the index and some metadata of a lot of content. Given the database schema your task is to seperate the user input in two.
The first part is a SQL query using the metadata and only the metadata to filter out the id of the tuple irrelevant to the user input without using the content.
