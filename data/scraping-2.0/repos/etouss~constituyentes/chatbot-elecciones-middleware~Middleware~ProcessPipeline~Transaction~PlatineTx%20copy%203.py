...
class PlatineTx(GenericTransaction):
    ...
    def process_input(self, input:str) -> str:
        instruction = """Given a database schema your task is to design an SQL query using only the metadata to filter out irrelevant tuples with respect to the human input.
Do not try to answer the human Question with SQL. 
Do not filter the tuples based on the content of the tuples.\n\n"""
