...
class PlatineTx(GenericTransaction):
    ...
    def process_input(self, input:str) -> str:
        instruction = """Your task is to design an SQL query to filter the tuples of the databases which are relevant to answer a user question. The SQL query should filter based on metadata associated with each tuple, such as the date or name(s).  
...
1. An SQL query that returns the relevant tuple content based on metadata only.
2. A task exploiting the content of the tuples to answer user's question.\n\n"""
