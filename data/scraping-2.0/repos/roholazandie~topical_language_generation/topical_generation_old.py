...
def sample_sequence(model, length, context, num_samples=1, temperature=1, top_k=0, top_p=0.0, repetition_penalty=1.0,
    ...
    topic_probs = torch.tensor(all_topics[0, :])
    ...
    with torch.no_grad():
        for _ in trange(length):
            ...
            # probs1 = F.softmax(next_token_logits, dim=-1).numpy()
            # plt.hist(np.log(probs1), bins=100)
            # plt.show()
            ...
            if temperature == 0:  # greedy sampling:
                ...
                total_probs = token_probs + 0.01*topic_probs

                post_token_ids = (total_probs > 0.001).nonzero().flatten().tolist()

                print(set(post_token_ids).symmetric_difference(set(pre_token_ids)))

                next_token = torch.multinomial(total_probs, num_samples=1)
