...
class TurtleBot3ContinuousNavEnv(turtlebot3_env.TurtleBot3Env):
    def __init__(self):
        """
        ...
        super(TurtleBot3ContinuousNavEnv, self).__init__(ros_ws_abspath)
        ...
        high_action = numpy.asarray([0.2, pi/2])
    ...
    def _get_obs(self):
        ...
        state = numpy.concatenate((self.normalize(self.l1, 0, 3.5), numpy.asarray([self.normalize(self.current_distance2goal, 0, 8), self._get_heading() / pi ])))
    ...
    def _compute_reward(self, observations, action, done):
        return self.setReward(observations, done, action)
        #  [Tri Huynh] ############################3
        reward = 0
        r_towardgoal = 2
        w_oscillatory = 0.05
