...
# 定义处理函数
def get_n_filter(sentence_list,stopkey):
    # 提取语料提取后的名词
    n_filter_list = list()
    for sentence_ in tqdm(sentence_list):
        ...
        for word, flag in words:
            # 过滤长度大于 1 的名词
            if 'n' in flag and len(word) > 1 and word not in stopkey:
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('data/txt/alljd.csv')
    stopkey = [w.strip() for w in codecs.open('data/stopWord.txt', 'r', 'utf-8').readlines()]
    # 对用户评论进行名词标注
    data['comment_n'] = get_n_filter(data['comment'].tolist(), stopkey)
