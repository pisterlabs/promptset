...
from tools import handle_image_history
...
async def process_thread_message(
    ...
    if generated_image:
        ...
        msg = cl.Message(content="This was an image generated by DALL-E 3", elements=elements)
        handle_image_history(msg)
...
async def run_conversation(message_from_ui: cl.Message):
    ...
    if message_from_ui.elements:
        cl.user_session.set("image_id", message_from_ui.elements[0].name)
        handle_image_history(message_from_ui)
        message_from_ui.content = message_from_ui.content + ". image_id: " + message_from_ui.elements[0].name
        print("message_from_ui.element", message_from_ui.content)
                ...
                for tool_call in step_details.tool_calls:
                    ...
                    if tool_call.type == "code_interpreter":
                        ...
                        if not tool_call.id in message_references:
                            ...
                            tool_output = tool_map[function_name].invoke(json.loads(tool_call.function.arguments))
                        ...
                        if not tool_output_id in message_references:
                            message_references[tool_output_id] = cl.Message(
                                author=f"{tool_call.type}_result",
                                content=str(tool_call.function.output) if  hasattr(tool_call.function, "output") else "",
                                language="json",
                                parent_id=context.session.root_message.id,
                            )
                            await message_references[tool_output_id].send()
                            ...
                            message_references[tool_output_id].content = (
                                str(tool_call.function.output) if hasattr(tool_call.function, "output") else ""
                            )
