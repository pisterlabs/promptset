...
def get_def_multi_choice_llm(word):
    ...
    q_template = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content=(
                    "You are a helpfuli english vocab teacher, that helps make multiple choice questions, please provide just the multiple choice question"
                )       
            ),
...
def get_def_answer(word, result):
    A_template = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content=(
                    "You are a helpful english vocab teacher, that helps make multiple choice questions, please provide just the answer letter for the correct answer"
                )       
            ),
    ...
    letter_answer = {"answer": answer.content}
