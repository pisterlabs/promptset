...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == 'step':
            ...
            remote.send((ob, s_ob, reward, done, info, available_actions, state))
        elif cmd == 'reset':
            ob, s_ob, available_actions = env.reset()
            state = env.get_state()
            remote.send((ob, s_ob, available_actions, state))
        elif cmd == 'reset_task':
            ob = env.reset_task()
            remote.send(ob)
        elif cmd == 'render':
            ...
            env.close()
            remote.send(
                (env.observation_space, env.share_observation_space, env.action_space, state_dim))
...
class ShareSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        for remote in self.work_remotes:
            remote.close()
        self.remotes[0].send(('get_spaces', None))
        observation_space, share_observation_space, action_space, self.state_dim = self.remotes[0].recv(
        )
        ShareVecEnv.__init__(self, len(env_fns), observation_space,
                             share_observation_space, action_space)
        self.state = None
    ...
    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        obs, share_obs, rews, dones, infos, available_actions, self.state = zip(*results)
        n_ags = np.stack(obs).shape[1]
        np_s = np.repeat(np.expand_dims(np.stack(self.state), axis=1), n_ags, axis=1)
        share_obs = np.concatenate((np.stack(obs), np_s), axis=-1)
        return np.stack(obs), share_obs, np.stack(rews), np.stack(dones), infos, np.stack(available_actions)

    def reset(self):
        for remote in self.remotes:
            remote.send(('reset', None))
        results = [remote.recv() for remote in self.remotes]
        obs, share_obs, available_actions, self.state = zip(*results)

        n_ags = np.stack(obs).shape[1]
        np_s = np.repeat(np.expand_dims(np.stack(self.state), axis=1), n_ags, axis=1)
        share_obs = np.concatenate((np.stack(obs), np_s), axis=-1)

        return np.stack(obs), share_obs, np.stack(available_actions)
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def get_state_shape(self):
        self.envs[0].reset()
        state_shape = self.envs[0].get_state().shape[-1]
        self.envs[0].close()
        return state_shape
