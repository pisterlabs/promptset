...
import config
from llm.utils import ProgressBar

chat_logger = logging.getLogger(name="ChatLogger")

os.environ["OPENAI_API_KEY"] = config.OPENAI_API_KEY
os.environ["OPENAI_API_ENDPOINT"] = config.OPENAI_API_ENDPOINT
...
class ChatModel:
    ...
    def chat_completion(
        ...
        callback: Callable[[MessagesType, ChatCompletionResponseType], Any]
        | None = None,
        **kwargs,
    ) -> ChatCompletionResponseType:
    ...
    async def async_chat_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            if isinstance(msgs, Message):
                msgs = msgs.messages
            cc = await self.async_client.chat.completions.create(
                messages=msgs, **api_kwargs
            )
            ...
            if did_pass_validation:
                ...
                return await attempt_retry(
                    messages, response, num_retries=num_retries - 1
                )
        except Exception as e:
            warnings.warn(
                f"Failed to create ChatCompletion with arguments: {updated_kwargs.items()}\n"
