...
from forge.sdk.pilot.database.database import get_development_step_from_hash_id, save_development_step, delete_all_subsequent_steps
from forge.sdk.pilot.utils.utils import array_of_objects_to_string
from forge.sdk.pilot.utils.llm_connection import get_prompt, create_gpt_chat_completion
from forge.sdk.pilot.utils.utils import get_sys_message, find_role_from_step, capitalize_first_word_with_underscores
from forge.sdk.pilot.logger.logger import logger
from forge.sdk.pilot.helpers.prompts import ask_user
...
class AgentConvo:
    ...
    def send_message(self, prompt_path=None, prompt_data=None, function_calls=None):
        ...
        # development_step = get_development_step_from_hash_id(
        #     self.agent.project, prompt_path, prompt_data, self.agent.project.llm_req_num)
        development_step = None
        if development_step is not None and self.agent.project.skip_steps:
            # if we do, use it
            print(
                colored(f'Restoring development step with id {development_step.id}', 'yellow'))
            ...
            response = create_gpt_chat_completion(
                self.messages, self.high_level_step, function_calls=function_calls)
            if self.agent.__class__.__name__ == 'Developer':
                development_step = save_development_step(
                    self.agent.project, prompt_path, prompt_data, self.messages, response)
                self.agent.project.checkpoints['last_development_step'] = development_step
    ...
    def log_message(self, content):
        print_msg = capitalize_first_word_with_underscores(
            self.high_level_step)
        if self.log_to_user:
            if self.agent.project.checkpoints['last_development_step'] is not None:
                print(colored("\nDev step ", 'yellow') + colored(
                    self.agent.project.checkpoints['last_development_step'], 'yellow', attrs=['bold']) + '\n', end='')
            print(f"\n{content}\n")
    ...
    def to_playground(self):
        ...
        process.communicate(content.replace(
            '{{messages}}', str(self.messages)).encode('utf-8'))
