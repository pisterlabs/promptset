...
class DeeplengEnv(robot_gazebo_env.RobotGazeboEnv):
    ...
    def _check_auv_ready(self):
        ...
        while self.auv_data is None and not rospy.is_shutdown():
            try:
                ...
                rospy.logerr("/gazebo/model_states not ready yet, retrying")
    ...
    def set_auv_pose(self, x, y, z, roll, pitch, yaw, time_sleep):
        ...
        rot_mat, _ = coordinate_frame_transform(roll,
                                                pitch,
                                                yaw,
                                                frame="world2body")
    ...
    def get_auv_pose(self):
        ...
        rot_mat, _ = coordinate_frame_transform(auv_pose[3],
                                                auv_pose[4],
                                                auv_pose[5],
                                                frame="world2body")
    ...
    def get_auv_velocity(self, frame="body"):
        ...
        roll, pitch, yaw = np.round(quat2euler_angle(self.auv_data.pose[-1]), 2)

        if frame == 'world':
            return auv_vel_world

        if frame == 'body':
            linear_rot_mat, angular_rot_mat = coordinate_frame_transform(roll,
                                                                         pitch,
                                                                         yaw,
                                                                         frame="world2body")
