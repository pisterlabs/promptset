...
class FuncGroups(BaseTool):
    ...
    description = "Input SMILES, return list of functional groups in the molecule. You can use this tool to find the " \
                  "changes of functional group during the reaction."
    dict_fgs: dict = None
...
class rxn_predict(BaseTool):
    def __init__(self):
        self.rxn4chemistry_wrapper = RXN4ChemistryWrapper(api_key=rxnapi_key, base_url='https://some.other.rxn.server')
        self.rxn4chemistry_wrapper.create_project('test_wrapper')

    def _predict(self, input_smiles):
        ...
        return results['response']['payload']['attempts'][0]['smiles']

    def _run(self, input_smiles):
        try:
            pred = self._predict(input_smiles)
            return pred
        except:
            return "Wrong argument. Please input valid reactants molecular SMILES."

    async def _arun(self, input_smiles):
        """Use the tool asynchronously."""
        raise NotImplementedError()