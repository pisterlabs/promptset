...
# Load the dataset
try:
    df_clone = pd.read_csv('../dataset/parallel/processing/sms_predictions_2_in_progress.csv')
    print("Resuming from saved progress.")
except FileNotFoundError:
    df = pd.read_csv('../dataset/parallel/sms_2.csv')
    df_clone = df.copy()
...
for index, row in df_clone.iterrows():
    if pd.isnull(row['Predicted']):
        ...
        df_clone.to_csv('../dataset/parallel/processing/sms_predictions_2_in_progress.csv', index=False)
...
df_clone.to_csv('../dataset/parallel/results/sms_predictions_2_results.csv', index=False)
