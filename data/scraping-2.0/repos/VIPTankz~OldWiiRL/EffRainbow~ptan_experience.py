...
from ptan_agent import BaseAgent
import ptan_utils as utils
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for env_idx, (env, action_n) in enumerate(zip(self.pool, grouped_actions)):
                if self.vectorized:
                    next_state_n, r_n, is_done_n, trun_n, _ = env.step(action_n)
                    #print("Vec shape")
                    #print(len(next_state_n))

                    #if np.random.random() > 0.991:
                    """
                    print(next_state_n[0]._frames[0].shape)
                    #This needs to be just a list of the version below
                    
                    #tester1 = np.swapaxes(next_state_n[0][0], 2, 0)
                    #tester2 = np.swapaxes(next_state_n[0][1], 2, 0)
                    #tester3 = np.swapaxes(next_state_n[0][2], 2, 0)
                    #tester4 = np.swapaxes(next_state_n[0]._frames[3], 2, 0)
                    tester1 = np.swapaxes(next_state_n[0]._frames[0],2,0)
                    #tester2 = np.swapaxes(next_state_n[0]._frames[1],2,0)
                    #tester3 = np.swapaxes(next_state_n[0]._frames[2],2,0)
                    cv2.imwrite("state_test_large" + str(cur_steps[0]) + ".jpg", tester1)
                    #cv2.imwrite("state_test2_large.jpg", tester2)
                    #cv2.imwrite("state_test3_large.jpg", tester3)
                    #cv2.imwrite("state_test4.jpg", tester4)
                    if is_done_n[0]:
                        raise Exception("stop")"""
                else:
                    next_state, r, is_done, trun, _ = env.step(action_n[0])
                    print("Real value")
                    print(next_state._frames[0].shape)
                    #print(next_state.shape)
                    #I ADDED THIS LINE REMOVE FOR NO RENDER
                    #env.render()
                    """if np.random.random() > 0.993:
                        print(next_state._frames[0].shape)
                        tester1 = np.swapaxes(next_state._frames[0], 2, 0)
                        tester2 = np.swapaxes(next_state._frames[1], 2, 0)
                        tester3 = np.swapaxes(next_state._frames[2], 2, 0)
                        #tester4 = np.swapaxes(next_state._frames[3], 2, 0)
                        cv2.imwrite("state_test1.jpg", tester1)
                        cv2.imwrite("state_test2.jpg", tester2)
                        cv2.imwrite("state_test3.jpg", tester3)
                        #cv2.imwrite("state_test4.jpg", tester4)
                        raise Exception("stop")"""
                    
                    next_state_n, r_n, is_done_n, trun_n = [next_state], [r], [is_done], [trun]

                for ofs, (action, next_state, r, is_done,trun) in enumerate(zip(action_n, next_state_n, r_n, is_done_n,trun_n)):
                    idx = global_ofs + ofs
                    state = states[idx]
                    ...
                    cur_rewards[idx] += r
                    cur_steps[idx] += 1
                    if state is not None and not trun:
                        history.append(Experience(state=state, action=action, reward=r, done=is_done))
                    if len(history) == self.steps_count and iter_idx % self.steps_delta == 0 and not trun:
                        yield tuple(history)
                    states[idx] = next_state
                    if is_done or trun:
                        # in case of very short episode (shorter than our steps count), send gathered history
                        if len(history) < self.steps_count and not trun:
                            yield tuple(history)
                        # generate tail of history
                        while len(history) > 1 and not trun:
                            history.popleft()
                            yield tuple(history)
