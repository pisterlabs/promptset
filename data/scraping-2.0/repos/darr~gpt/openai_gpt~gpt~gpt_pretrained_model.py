...
# Modified date : 2019-03-20 14:36
# Author : DARREN
# Describe : not set
...
class OpenAIGPTPreTrainedModel(nn.Module):
    ...
    @classmethod
    def from_pretrained_tf(cls, config, *inputs, **kwargs):
        return cls._load_model_from_tf(config, *inputs, **kwargs)
    ...
    def from_pretrained(cls, config, *inputs, **kwargs):
        ...
        """
        return cls._load_model_from_troch(config, *inputs, **kwargs)

    @classmethod
    def _load_model_from_troch(cls, config, *inputs, **kwargs):
    ...
    @classmethod
    def _load_model_from_tf(cls, config, *inputs, **kwargs):
        config_file = get_model_file_path("config", config)
        model_config = OpenAIGPTConfig.from_json_file(config_file)
        model = cls(model_config, *inputs, **kwargs)
