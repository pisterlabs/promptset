...
def initialize_global_sounds():
    base_path = 'modes/plantony/media/'
...
def get_mode_data():
    try:
        with open('working/config_single.json', 'r') as file:
...
#TODO: reconcile this and load_characters_from_config
...
class ChannelAPI:
    ...
    # def __init__(self):
    #     self.channels = {channel: "Moderator" for channel in CHANNEL_MODERATOR}
    #     self.characters = {}  # Store characters by their default channel

    def __init__(self):
        self.channels = {}  # Initialize an empty dictionary for channels
        self.characters = {}  # Store characters by their default channel
    ...
    def process_text_and_synthesize(self, channel, text, timestamp):
        ...
        # Create an event and a queue
        done_event = threading.Event()
        audio_queue = queue.Queue()

        # Start the worker thread
        threading.Thread(target=self.worker, args=(channel, text, character, timestamp, done_event, audio_queue)).start()
        ...
        while not audio_queue.empty():
            audio_filename = audio_queue.get()
            self.route_to_channel(self, channel, audio_filename, character.max_channels, character.device_id)

    def parallel_processing_thread(self, channel, text, character, timestamp, done_event, audio_queue):
        ...
        # Put the audio_filename into the queue
        audio_queue.put(audio_filename)
        # Signal that this worker is done processing
        done_event.set()

    #TODO: revisit if this is the right way to handle asynchronous loading 

        self.start_threads_staggered(self.parallel_processing_thread, channel, text, character, timestamp)
