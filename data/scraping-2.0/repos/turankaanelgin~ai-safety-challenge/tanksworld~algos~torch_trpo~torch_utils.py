...
def cg_solve(fvp_func, b, nsteps):
    ...
    x = ch.zeros(b.size()).cuda()
    r = b.clone()
    p = b.clone()
    new_rnorm = ch.dot(r, r)
    if torch.any(torch.isnan(new_rnorm)): pdb.set_trace()
    for _ in range(nsteps):
        rnorm = new_rnorm
        fvp = fvp_func(p)
        if torch.any(torch.isnan(fvp)): pdb.set_trace()
        alpha = rnorm / ch.dot(p, fvp)
        if torch.any(torch.isnan(alpha)): alpha = torch.zeros(1).cuda()
        x += alpha * p
        r -= alpha * fvp
        new_rnorm = ch.dot(r, r)
        if torch.any(torch.isnan(new_rnorm)): pdb.set_trace()
        ratio = new_rnorm / rnorm
        if torch.any(torch.isnan(ratio)): ratio = torch.zeros(1).cuda()
        p = r + ratio * p
    return x
