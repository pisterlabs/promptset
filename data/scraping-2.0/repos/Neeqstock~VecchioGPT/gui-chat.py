...
class ChatGUI:
    def __init__(self, root):
        ...
        self.send_button = Button(
            root, text="Send", command=self.send_message, font=entry_font
        )
    ...
    def get_chatgpt_response(self, user_input):
        try:
            ...
            if len(tokenized_chat_text_history) > available_other_tokens:
                self.chat_text_history = self.encoder.decode(
                    tokenized_chat_text_history[-available_other_tokens:]
                )
            truncated_chat_text_history = self.chat_text_history

            input_with_previous_messages = (
            ...
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Continue the conversation below. Pay special attention to the current request.",
                    },
