...
#     # Load your audio file - replace 'path/to/audio/file.mp3' with the actual file path
...
async def TTS_async(input_text: str):
    # API_KEY = os.getenv("OPENAI_API_KEY", None)
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post('https://api.openai.com/v1/engines/tts-1/speech', headers=headers, json=data) as resp:
            if resp.status == 200:
                speech_data = await resp.read()
...
# Replace the call to TTS in your main script with a call to TTS_async
# For example: await TTS_async(st.session_state['text'])
			...
			while st.session_state['run']:
				try:
					...
					if json.loads(result_str)['message_type']=='FinalTranscript':
						...
						await TTS_async(st.session_state['text'])
