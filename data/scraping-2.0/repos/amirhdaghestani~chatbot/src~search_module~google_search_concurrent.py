        ...
        while True:
            try:
                ...
                if ((len(urls) == 0 and len(in_process) == 0)
                    or (search_level==DEEP_SEARCH and ((len(full_text) > 9600) or time.time() - start_time > 45))
...
def search_test(query_phrase):
    ...
    #     url=f"https://zarebin.ir/gse/api/search?q={google_query}?d=[www.mci.ir]"
    #     response = requests.get(url, proxies={"http":"socks5://127.0.0.1:8196",
    #                                           "https":"socks5://127.0.0.1:8196",})
    ...
    # responses = response_json['result']['all']['results']['webs']
    # urls = []
    # for i in range(len(responses)):
    #     url = responses[i]['web_link'].lstrip().rstrip()
    #     urls.append(url)
    
    urls = ["https://hamyab24.ir/blog/how-to-burn-or-block-hamrahe-aval-sim-card/"]
    # urls = ['https://shop.mci.ir/charge', 'https://mci.ir/to-use-charge-card', 'https://mci.ir/ussd', 'https://mizbanfa.net/mag/charging-mci', 'https://mizbanfa.net/mag/buy-charge-from-mci-shop', 'https://esaj.ir/blog/نحوه-شارژ-سیم-کارت-همراه-اول/', 'https://esaj.ir/mci-charge']
    return urls

def search(query_phrase, restriction):
    print(f'***** search {query_phrase}')
    sort = '&sort=date-sdate:d:w'
    for d_indicator in DATE_INDICATOR:
        if d_indicator in query_phrase:
            sort = '&sort=date-sdate:d:s'
            break
    ...
    try:
        start_wall_time = time.time()
        url="https://www.googleapis.com/customsearch/v1?key="+ut.google_key+'&cx='+ut.google_cx+'&num=10'+sort+'&q='+google_query+'&siteSearch='+restriction
...
def response_text_extract(query_phrase, keywords, keyword_weights, url, response, get_time):
    ...
    response_text = ''
    curr = new_curr
    extract_text = extract_text[:3000] # make sure we don't run over token limit
    ...
    google_tldr = ut.ask_gpt_with_retries(ut.MODEL, gpt_tldr_message, tokens=1024, temp=0.15, timeout=t_out, tries=1)
...
def search_google(original_query, search_level, query_phrase, keywords, chat_history, restriction, num_urls):
  start_time = time.time()
  ...
  keyword_weights = {}
  for keyword in keywords:
      zipf = wf.zipf_frequency(keyword, 'fa')
      ...
      if weight > 0:
          ...
          if len(subwds) > 1:
              for subwd in subwds:
                  sub_z = wf.zipf_frequency(subwd, 'fa')
  ...
  try:  # query google for recent info
    sort = ''
    today = " تاریخ امروز " + date.today().strftime("%d %b %Y")
    for word, intial in DATE_MONTH_PERSIAN.items():
        today = today.replace(word, intial)

    for d_indicator in DATE_INDICATOR:
        if d_indicator in original_query:
    ...
    if len(original_query) > 0:
        orig_phrase_urls = search(original_query[:min(len(original_query), 128)], restriction=restriction)
        extract_query = original_query[:min(len(original_query), 128)]
    gpt_phrase_urls = []
    if len(query_phrase) > 0:
        gpt_phrase_urls = search(query_phrase, restriction=restriction)
    ...
    # RRF
    url_score = {}
    for index, url in enumerate(orig_phrase_urls):
        url_score[url] = 1 / (50 + index)
    
    for index, url in enumerate(gpt_phrase_urls):
        score = 1 / (50 + index)
        url_score[url] = url_score[url] + score if url in url_score.keys() \
                                                else score

    url_score = sorted(url_score.items(), key=lambda x: x[1], reverse=True)
    urls = [url[0] for url in url_score]
