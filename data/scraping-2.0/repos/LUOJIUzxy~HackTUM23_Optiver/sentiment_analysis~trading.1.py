...
# Model parameters
    ...
    response1 = requests.post(
        ...
        json={"model": "gpt-4",#ft:davinci-002:personal::8MKo32YZ",#"ft:babbage-002:personal::8MKyXgHp",
        "messages": [
          {
            "role": "system",
            "content": """
...
QUOTED_VOLUME = 10
FIXED_MINIMUM_CREDIT = 0.15 #0.15
...
def interrupt_routine(instrument, price, exchange):
    ...
    maxVolumeBuy = min(100, maxVolumeBuy)
    maxVolumeSell = min(100, maxVolumeSell)
    
    print_positions_and_pnl(exchange)
    
    if price > 0:
        print('BUYING '+ instrument.instrument_id + ", "+  str(maxVolumeBuy) + ", " + str(maxVolumeBuy + min(position, 100)))
        if price * 100 >= 0.1:
        #we buy
            insert_quotes(exchange, instrument, best_bid_price, expected_price, maxVolumeBuy, maxVolumeBuy + min(position, 100))
        elif 0.01 <= price * 100 < 0.1:
        #we buy
            insert_quotes(exchange, instrument, best_bid_price, expected_price, int(maxVolumeBuy/3), int(maxVolumeBuy/3) + min(position, 100))
        elif price * 100 <= 0.01 :
        #we buy
            insert_quotes(exchange, instrument, best_bid_price, expected_price, 0, 0)
    else:
        print('SELLING '+ instrument.instrument_id + ", " + str(maxVolumeSell - min(100,position)) + ": " + str(maxVolumeSell))
        if price * 100 <= -0.1:
        
            insert_quotes(exchange, instrument, expected_price, best_ask_price, maxVolumeSell - min(100,position), maxVolumeSell )
        elif -0.1 < price * 100 <= -0.01:
        
            insert_quotes(exchange, instrument, expected_price, best_ask_price, int(maxVolumeSell/3) - min(100,position), int(maxVolumeSell/3))
        elif price * 100 > -0.01 :
...
while True:
    ...
    for instrument in INSTRUMENTS.values():
        ...
        if not social_feeds:
            ...
            for feed in social_feeds:
                ...
                try:
                    ...
                    senti_instru = values[1]
                    senti_price = float(values[2])
            
                    # Process the values
                    print(values)
                    print(values[1], senti_price)
        ...
        print('Dual listed asset:' + instrumentWithB)
        ...
        if(best_ask_price_DL < best_bid_price):
            ...
            while( (current_ask_price_DL < current_bid_price) and not(current_ask_volume_DL >= max_volume_to_sell_DL and current_bid_volume >= max_volume_to_buy) and dual_listing_index_bid < len(instrument_order_book.bids)-1):
                if(current_bid_volume < max_volume_to_buy-0.1):
                    dual_listing_index_bid += 1
                ...
                #current_bid_volume += instrument_order_book.bids[dual_listing_index_bid].volume
                while((current_ask_price_DL < current_bid_price-0.1) and current_ask_volume_DL < max_volume_to_sell_DL and current_ask_volume_DL < current_bid_volume and dual_listing_index_ask < len(instrument_order_book_DL.asks)-1):
            ...
            while( (current_ask_price < current_bid_price_DL) and not(current_ask_volume >= max_volume_to_sell and current_bid_volume_DL >= max_volume_to_buy_DL) and dual_listing_index_bid < len(instrument_order_book_DL.bids)-1):
                    if(current_bid_volume_DL < max_volume_to_buy_DL-0.1):
                        dual_listing_index_bid += 1
                    ...
                    #current_bid_volume += instrument_order_book.bids[dual_listing_index_bid].volume
                    while((current_ask_price < current_bid_price_DL-0.1) and current_ask_volume < max_volume_to_sell and current_ask_volume < current_bid_volume_DL and dual_listing_index_ask < len(instrument_order_book.asks)-1):
                        dual_listing_index_ask += 1
