...
Q: {QUESTION}
Options:
{options_list}A: Let's think step by step.""".strip() + "\n "

    values = [a for a,o in options]
    
    hf = HFPrompter(args.model, include_bos_token=True)
    
    prompt_with_reasoning = await hf.generate(prompt, max_new_tokens=80, stopping_phrases=["So the answer is", "keep:\n"], step_size=20)

    res = await hf.cond_logprob(prompt_with_reasoning + " So the answer is ", values)
    ...
    return SampleResult(correct_item, result_item, probs, prompt_with_reasoning + " So the answer is " + original_result_item)
...
def result_file(file):
    ...
    return f"results/revised-{tag}{file}-{args.method}{model_name_suffix}.txt"
