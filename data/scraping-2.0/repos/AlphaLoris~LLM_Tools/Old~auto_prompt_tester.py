...
import datetime
import pandas as pd
from pandas import json_normalize  # Used to flatten the JSON
...
https://www.promptingguide.ai/
    ...
    def save_expanded_json_to_excel(self, directory, expanded_json, schema):
        ...
        # If the Excel file doesn't exist, create it and set up the basic structure
        ...
        with pd.ExcelWriter(file_path, engine='openpyxl', mode='a') as writer:
            ...
            new_row = {
                "File Name": "current_file_name",  # Replace with actual file name
            ...
            if not expanded_json["validity_status"]:
                ...
                error_sheet_name = f"Errors_{len(book) + 1}"  # Create a unique sheet name
                df_errors.to_excel(writer, sheet_name=error_sheet_name, index=False)

            # Save the original JSON in a flattened format to "Original_JSONs" sheet
            df_originals = book.get('Original_JSONs', pd.DataFrame())
            flattened_json = json_normalize(expanded_json["original_json"])  # Flatten the JSON
