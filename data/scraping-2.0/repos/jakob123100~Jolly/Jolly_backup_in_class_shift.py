...
RECOGNIZER_LISTEN_PAUSE_THRESHOLD = 2
...
def recognize_start_phrase(phrases, audio, found_phrase):
    try:
        text = recognizer.recognize_google(audio, language="sv-SE", show_all=True)
        print(text)

        for prompt in text["alternative"]:
            if any(phrase in prompt["transcript"] for phrase in phrases):
...
def recognize_start_phrase_test(audio):
    global active
    try:
        text = recognizer.recognize_google(audio, language="sv-SE", show_all=True)
        print(text)
        ...
        for prompt in text["alternative"]:
            if any(phrase in prompt["transcript"] for phrase in ACTIVATION_PHRASES):
                active.value = True
                return
    except sr.UnknownValueError:
        pass  # Ignorera om ljudet inte kunde förstås
...
def callback(recognizer, audio):
    ...
    p = Process(target=recognize_start_phrase_test, args=(audio,))
    recognizing_processes.append(p)
    p.start()
    

def wait_for_phrase_test(phrases):
    recognizer.pause_threshold = RECOGNIZER_ACTIVATION_PAUSE_THRESHOLD
    global active
    active.value = False
    stop_listening = recognizer.listen_in_background(sr.Microphone(), callback)

    while not active.value:
        time.sleep(0.1)
...
def extract_audio(amount_of_tries=3):
    ...
    with sr.Microphone() as source:
        ...
        try:
            return recognizer.listen(source, timeout=3, phrase_time_limit=60)
        except sr.WaitTimeoutError:
            print("Timeout")
