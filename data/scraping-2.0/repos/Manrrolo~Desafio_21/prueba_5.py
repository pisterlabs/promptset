...
def main():
    ...
    choice = st.radio("Do you want to:", ('Upload a new PDF', 'Use an existing .pkl file', 'Search all files'))
    ...
    # Acepta la consulta del usuario
    query = st.text_input("Ask questions about your PDF files:")

    # Ejecuta la búsqueda y respuesta si se ingresó una consulta
    if query:
        with st.spinner('Obteniendo la respuesta...'):
            if choice == 'Search all files':
                # Aquí manejas la nueva opción "Search all files"
                all_responses = []
                ...
                for response_info in all_responses:
                    st.write(f"Respuesta de {response_info['store_name']}: {response_info['response']}")
                    if use_audio:  # Si la opción de audio está activada, generar y mostrar audio
                        audio_file = get_voice_audio(response_info['response'])
                        st.audio(audio_file)
            else:
                # Aquí manejas las opciones existentes "Upload a new PDF" y "Use an existing .pkl file"
                docs = VectorStore.similarity_search(query=query, k=3)
