...
client = OpenAI(api_key=config.get('ai', 'api_key'))
...
class Agent:
    def __init__(self, name_prefix='AI-', log_level=Logger.ERROR, print_tokens=True, model='gpt-3.5-turbo-16k'):
        ...
        self.logger = Logger('{}:{}'.format(
            name_prefix, self.uuid.split('-')[0]), log_level=log_level)
        self.model = model
    ...
    def process_request(self, req_uuid, p):
        ...
        self.db.save_data("blackboards", req_uuid,
    ...
    def submit_request(self, p, req_uuid):
        ...
        if not response_text:
            self.db.save_data("blackboards", req_uuid,
                              "response", json.dumps({'uuid': 'pending'}))
        subprocess.Popen(
            ['python', '-c', f'''from utils.async_ai import Agent; agent = Agent(print_tokens=True); agent.process_request("{req_uuid}", {p})'''])
        return True
