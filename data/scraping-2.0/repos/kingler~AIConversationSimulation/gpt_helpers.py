...
# System messages for programmer GPT assistants
gpt_a_system_message = "You are a creative and innovatative programming assistant aiming to add new and exciting features, provide guidance and improve programs fix potential errors. When necessary you make new suggestions and point out the mistakes in the code. You are free to respectfully disagree with the other assistant"
gpt_b_system_message = "You are a traditional, proressional and critical programming assistant focusing on keeping the code clear, concise, efficient and error handling to the program. When necessary you make new suggestions and point out the mistakes in the code. You are free to respectfully disagree with the other assistant"

# Initiating an alternating conversation between two GPT assistants
#tke multiline user input
user_input = ''
...
def reached_conclusion(messages):
    ...
    response = openai.ChatCompletion.create(
      model="gpt-3.5-turbo",
      stream=True,
      messages=[{"role": "user", "content": f"""has the assistants agreed and reached a conclusion? only yes or no answers are valid.
                 end of convo so far:
                 {chat_messages}"""}])
...
while not is_solved:
    ...
    # gpt_a_messages.append({"role": "user", "content": gpt_a_prompt})
    ...
    # gpt_b_messages.append({"role": "user", "content": gpt_b_prompt})
    gpt_b_messages.append({"role": "assistant", "content": gpt_a_response})
    # conversation.append(("GPT-B:", gpt_b_prompt))
    conversation.append(("GPT-A:", gpt_a_response))

    gpt_a = gpt_a_response

    # continuoally save the conversation to a file
...
# Print and keep track of the conversation
