...
TEMPLATE_MULTISTEP_ARITHEMTIC_UPDATE = """
{examples}
Q: {question}
...
Feedback: {feedback}
Updated Answer: Let's think step by step.Recall that the order of operations in mathematics is as follows: (1) Parentheses, (2) exponents, (3) multiplication and division (from left to right), (4) addition and multiplication (from left to right). So, remember to always compute the expressions inside parentheses or brackets first.{prev_partial_answer}
""".strip()


class LLMwVerifier_MultistepArithmetic(LLMwVerifier):
    def __init__(
            self,
            init_ans_model,
            improve_model,
        ...
        self.prompt_update = PromptTemplate(
            ...
            template=TEMPLATE_MULTISTEP_ARITHEMTIC_UPDATE,
        )
    ...
    def get_verifier_feedback(self, llm_output):
        ...
        if not self.is_eval and float(correct_answer) == float(final_num):
...
class SelfImprove_GPT_MultistepArithmetic(SelfImprove_GPT_QA):
    def __init__(self, model, tokenizer, manual_prompt=False, input_max_length=1024, max_new_tokens=1024, additional_info='', gen_kwargs={}):
