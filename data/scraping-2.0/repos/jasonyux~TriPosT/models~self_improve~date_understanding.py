...
TEMPLATE_DATE_UNDERSTANDING_UPDATE = """
{examples}
Q: {question}
...
class LLMwVerifier_DateUnderstanding(LLMwVerifier):
    def __init__(
            self, 
            init_ans_model,
            improve_model,
        ...
        self.prompt_update = PromptTemplate(
            ...
            template=TEMPLATE_DATE_UNDERSTANDING_UPDATE,
        )
    ...
    def get_verifier_feedback(self, llm_output):
        ...
        if not self.is_eval and correct_answer.lower() == final_chioce.lower():
            if num_steps == 1:
                # can happen for date understanding
                return "Step (1) is correct. The final response is also correct."
            else:
...
class SelfImprove_GPT_DateUnderstanding(SelfImprove_GPT_QA):
