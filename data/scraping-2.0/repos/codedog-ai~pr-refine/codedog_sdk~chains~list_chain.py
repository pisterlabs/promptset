...
bool_template = """Given {k} points to answer a question: {question}.
...
class ListChain(Chain):
    ...
    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, question: str, k: int):
        """Load chain from llm."""
        question_chain = LLMChain(
            llm=llm,
            prompt=PromptTemplate.from_template(
                bool_template.format(k=str(k), question=question)
            ),
        )
        fix_chain = LLMChain(llm=llm, prompt=PromptTemplate.from_template(fix_template))
        return cls(question_chain=question_chain, fix_chain=fix_chain)
...
if __name__ == "__main__":
    ...
    chain = ListChain.from_llm(load_gpt35_llm(), "Cutie name for a dog", 3)
    print(chain.run(a="1"))
