...
def calc_topic_coherence(topic_words,docs,dictionary,emb_path=None,taskname=None,sents4emb=None,calc4each=False):
    ...
    try:
        # Assume taskname is model_taskname e.g. lda_taskname
        w2v_model_path = os.path.join(os.getcwd(),'data',f'{taskname.split("_",1)[1]}','w2v_weight_kv.txt')
...
def evaluate_topic_quality(topic_words, test_data, taskname=None, calc4each=False, w2file=None):
    ...
    if w2file:
        with open(w2file, 'w') as f:
            f.write('\n'.join([f'#{i} {lst}' for i, lst in enumerate(topic_words)]))
            f.write('='*30)
            f.write(f'topic diversity:{td_score}')
            f.write(f'c_v:{c_v}, c_w2v:{c_w2v}, c_uci:{c_uci}, c_npmi:{c_npmi}')
            f.write(f'mimno topic coherence:{mimno_tc}')
            f.close()
...
def show_topic_dist_in_corpus(topic_dist, topk=10):
    dist = {}
    for k in set(topic_dist):
        dist[int(k)] = topic_dist.count(k)/len(topic_dist)
    # Re-label topic #s based on dominance
    dist = dict(sorted(dist.items(), key=lambda item: item[1], reverse=True))
    return dist.keys(), yaml.dump(dist, sort_keys=False)

def create_wordclouds(sorted_topics, topics_dict, directory):
    for i, k in enumerate(sorted_topics):
        topic_freqs = dict(topics_dict[k])
        wordcloud = WordCloud(background_color='white')
        img = wordcloud.generate_from_frequencies(topic_freqs)
        img_name = f'/wcloud{i}_topic{k}.png'
        wordcloud.to_file(''.join([directory,img_name]))
