...
class DialogueTemplate(ModelHubMixin):
    ...
    def get_code_training_prompt(self,messages) -> str:
        if messages is None:
            raise ValueError("Dialogue template must have at least one message.")
        return message['content'] + self.eos_token + "\n"


    def get_dialogue_training_prompt(self) -> str:
...
def prepare_data(example, dialogue_template, is_train=True):
    ...
    if "type" in example.keys() and example["type"] is not None:
        if example["type"] == 'code':
            example['text'] =  dialogue_template.get_code_training_prompt({'content':example['content']})
            return example
        if example["type"] == 'dialogue':
            if "messages" in example.keys() and example["messages"] is not None:
                ...
                example["text"] = dialogue_template.get_dialogue_training_prompt()
            else:
                raise ValueError(f"Could not format example as dialogue! Require either `messages` or `[prompt, completion]` or `[prompt]` keys but found {list(example.keys())}"
            return example
    else:
        raise ValueError(
            f"Could not find example type! Require either etc `code` or `dialogue` keys but found {list(example.keys())}"
    return example
