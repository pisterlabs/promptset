...
class Root(ctk.CTk):
    ...
    def __init__(self):
        ...
        geometry = self.root.geometry("{} x {}".format(screen_width, screen_height))

        self.root.title("Projects Suggestions Generator")

        #ctk.set_appearance_mode("dark")

        title_label = ctk.CTkLabel(self.root, text="Projects Suggestions Generator",
        ...
        theme_button = ctk.CTkSwitch(theme_frame, text= "Change Theme", 
                            font=ctk.CTkFont(size= 15, weight="bold"), command= change_theme,
                            onvalue = 1, offvalue = 0)
        theme_button.pack(side = "right", padx=10, pady =(5, 5), fill = "both", expand = True)
