                ...
                with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
                    ...
                    for cyc in range(total_cycles):
                        ...
                        for update in range(1, n_updates+1):
                            ...
                            # Unpack
                            obs, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward, actual_reward, rew_frac = rollout
                            self.values = values
                            ...
                            if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                                ...
                                f.write(str(np.mean(true_reward)) + "," + str(np.mean(actual_reward)) + "," + str(rew_frac) + "\n")
                                f.close()
...
class Runner(AbstractEnvRunner):
    ...
    def _run(self):
        ...
        mb_obs, mb_rewards, mb_actions, mb_values, mb_dones, mb_neglogpacs, mb_actual_rew = [], [], [], [], [], [], []
        ...
        for step in range(self.n_steps):
            ...
            actual_rew = rewards
            shaped_rew = self.env.get_attr("shaped_rew")[0]
            frac = self.env.get_attr("rew_frac")[0]
            self.rew_frac = min(self.rew_frac, frac)

            rewards = actual_rew + shaped_rew
            ...
            mb_actual_rew.append(actual_rew)

        # batch of steps to batch of rollouts
        mb_obs = np.asarray(mb_obs, dtype=self.obs.dtype)
        mb_rewards = np.asarray(mb_rewards, dtype=np.float32)
        mb_actual_rew = np.asarray(mb_actual_rew, dtype=np.float32)
        ...
        mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, true_reward, mb_actual_rew= \
            map(swap_and_flatten, (mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, true_reward, mb_actual_rew))

        return mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, mb_states, ep_infos, true_reward, mb_actual_rew, self.rew_frac
