...
async def key(message: types.Message):
    ...
    try:
        ...
        for _ in range(num_keys):
            key = f"Drago-{generate_random_key()}"
            expiration_time = datetime.now() + timedelta(days=validity_days)
            keys.append((key, expiration_time))
            generated_keys[key] = {
                "expiration_time": expiration_time,
                "used_by": set()
            }

        keys_info = "\n".join([f"{key} (Valid until {expiration.strftime('%Y-%m-%d %H:%M:%S')})" for key, expiration in keys])
        await bot.send_message(message.chat.id, f"Here are your generated keys:\n{keys_info}")
