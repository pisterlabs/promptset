...
if __name__ == "__main__":
    ...
    info = {
        ...
        "image_prompt":[],
        # "image_wo_object":[],
        # "image_wo_object_prompt":[], # move first object
        "object_image_prompt": [],
        "object_image": [],
        "bounding_box": [],
        "mask":[],
    ...
    image_wo_object_ls = []
    image_wo_object_prompt_ls = []
    info = load_from_disk("/share/sd/sam/Grounded-Segment-Anything-main/outputs/results/debug_intermediate_results")
    read_dir = "/share/sd/sam/Grounded-Segment-Anything-main/outputs/lama_out"
    for d in tqdm(info):
        # load image
        whole_image = None
        image_id = d['image_id']
        read_img_pth = os.path.join(read_dir,str(image_id)+".png")
        image_wo_object_ls.append(read_img_pth)
        # generate caption and tags
        img_wo_obj = Image.open(read_img_pth)
        ori_img = Image.open(d['image'])
        ori_prompt = d['image_prompt']
        
        caption = generate_caption(img_wo_obj, device=device)
        print(f"Caption: {caption}")
        image_wo_object_prompt_ls.append(caption)
        
        whole_image = image_Splicing_2(ori_img,img_wo_obj)
        draw = ImageDraw.Draw(whole_image)
        w,h = ori_img.size
        draw.text((w/2,h/2),f"{ori_prompt}",fontsize=64)
        draw.text((w+w/2,h/2),f"{caption}",fontsize=64)
        whole_image.save(f"debug_post_{image_id}.png")
    pdb.set_trace()
    info = info.add_column("image_wo_object",image_wo_object_ls)
    info = info.add_column("image_wo_object_prompt",image_wo_object_prompt_ls)

    # info['image_wo_object'] = image_wo_object_ls
    # info['image_wo_object_prompt'] = image_wo_object_prompt_ls
    

info.save_to_disk("/share/sd/sam/Grounded-Segment-Anything-main/outputs/results/debug_post_results")
pdb.set_trace()