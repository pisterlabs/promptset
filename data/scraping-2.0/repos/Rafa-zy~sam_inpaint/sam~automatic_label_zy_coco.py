...
def convert_img2base64(image):
    image_stream = io.BytesIO() # in-memory system
...
if __name__ == "__main__":
    ...
    dataset = dset.CocoCaptions(root = '/share/sd/sam/Grounded-Segment-Anything-main/data/train2014',
                            annFile = '/share/sd/sam/Grounded-Segment-Anything-main/data/annotations/captions_train2014.json',
                            )
    # pdb.set_trace()
    # load model
    ...
    for i,d in tqdm(enumerate(dataset)):
        ...
        # visualize raw image
        raw_img_pth = os.path.join(output_dir, "lama", f"{i}.png")
        image_pil, image = process_image(image_pil)

        # Convert image to base64
        # encoded_image = image_to_base64(image_pil)
        ...
        # import io
        # image_stream = io.BytesIO() # in-memory system
        # image_pil_ = resize_image(image_pil, 512)
        # image_pil_.save(image_stream, format='PNG')
        # image_bytes = image_stream.getvalue()
        # encoded_image = base64.b64encode(image_bytes)
        image_pil.save(raw_img_pth) # change to encoded image
        
        encoded_image = convert_img2base64(image_pil)
        with open(os.path.join(output_dir,"base64_raw_img",f"{i}.jpg"), "wb") as f:
        ...
        crop_img_pth = f"/share/sd/sam/Grounded-Segment-Anything-main/outputs_coco/objects/object-{img_id}-{obj}.png"
        mask_img_pth = f"/share/sd/sam/Grounded-Segment-Anything-main/outputs_coco/lama/{img_id}_mask.png" # only take the first mask
        ...
        # you can convert img to base64 by calling convert_img2base64 function
        
        mask_img.save(mask_img_pth)
        crop_img.save(crop_img_pth) # crop the object
        ...
        if INPAINT:
            ...
            whole_image.save(f"/share/sd/sam/Grounded-Segment-Anything-main/outputs_coco/debug/debug_ori_inpaint_{img_id}.jpg")
            w,h = image_pil.size
            # draw.text((w/2,h/2),f"{caption}",fontsize=256)
            # draw.text((w+w/2,h/2),f"{remove_caption}",fontsize=256)
            time.sleep(20)
        cnt += 1
        # save info dict once every 1000 images
        if not(cnt % 1000):
            iddd = int(cnt // 1000)
            info = Dataset.from_dict(info)
            info.save_to_disk(f"/share/sd/sam/Grounded-Segment-Anything-main/outputs/results/debug_results_{iddd}")
end = time.time()
