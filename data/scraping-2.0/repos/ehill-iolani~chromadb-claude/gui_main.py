...
def send_message():
    """
    This function sends the user's message to the Claude model and displays the response.
    """
    ...
    # Display the conversation history in the chat box
    chat_box.config(state=NORMAL)
    chat_box.insert(END, f"You: {input_text}\n")
    chat_box.insert(END, f"Claude: {response['completion']}\n")
...
def main():
    ...
    root = Tk()
    root.title("Claude Chatbot")

    # Create the chat box
    global chat_box
    chat_box = ScrolledText(root, width=50, height=20, state=DISABLED)
    chat_box.pack(padx=10, pady=10)

    # Create the input box
    global input_box
    input_box = Text(root, width=50, height=3)
