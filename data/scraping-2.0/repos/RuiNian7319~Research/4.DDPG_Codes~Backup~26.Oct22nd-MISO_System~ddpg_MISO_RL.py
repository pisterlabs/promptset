...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        ...
        # scaled_out = out
        shift = tf.constant([4.5, 7], dtype=tf.float32)
        scaled_out = tf.add(tf.multiply(out, self.action_bound), shift)
...
def train(sess, env, args, actor, critic, actor_noise):
    ...
    # Restore the old computational graph
    if args['restore'] is True:
        saver.restore(sess, args['ckpt_dir'])
        replay_buffer.load_replay(args['replay_dir'])
        ...
        for j in range(1, int(args['max_episode_len']) + 1):
            ...
            if j % int(args['sampling_time']) == 0:
                ...
                # Add Ornstein-Ulhenbeck exploration noise to the action
                    ...
                    # Decay the actor noise, complete decay once ~100% of the episodes are finished
            ...
            if j == eval_time:
                ...
                # Reward for RL
                r = env.reward_function(j, obj_function=args['obj_func'], delta_u=args['delta_u'])
                # print(r)
            ...
            if replay_buffer.size() > int(args['minibatch_size'] * 5):
                ...
                # If the model was restored, start the minibatch at a higher value
                if args['restore'] is True:
                    # Make sure the previous simulation was ran for long enough (at least 200 episodes).
                    if replay_buffer.size() > 10000:
                        mini_batch_multiplier = 68
                        args['restore'] = False
                ...
                actor.batch_size = min(mini_batch_size, 4800)
            ...
            if terminal:
                ...
                if i % 500 == 0 and i != 0:
                    save_path = saver.save(sess, args['ckpt_dir'])
...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        env = MISOSystem(nsim=args['max_episode_len'], x0=np.array([3.]), u0=np.array([3, 6]), xs=np.array([4.5]),
                         us=np.array([5.5, 8]), step_size=0.2, control=False, q_cost=1, r_cost=0.5, s_cost=0.1,
                         random_seed=1)
        ...
        actor_noise = OrnsteinUhlenbeckActionNoise(mu=np.zeros(action_dim), dt=env.step_size,
                                                   random_seed=int(args['random_seed']), sigma=1.5)

        # Train the Actor-Critic Model
        replay_buffer, action_list = train(sess, env, args, actor, critic, actor_noise)

        # Save replay memory experiences
        replay_buffer.save_replay(args['replay_dir'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-episodes', help='max num of episodes to do while training', default=5001)
    parser.add_argument('--max-episode-len', help='max length of 1 episode', default=50)
    parser.add_argument('--sampling-time', help='Sampling time by RL controller', default=1)
    parser.add_argument('--obj-func', help='Objective function used for reward', default='MPC')
    parser.add_argument('--delta-u', help='Penalize delta u to avoid oscillations', default="l1")
    parser.add_argument('--render-env', help='render the gym env', action='store_true')
    parser.add_argument('--use-gym-monitor', help='record gym results', action='store_true')
    parser.add_argument('--monitor-dir', help='directory for storing gym results', default='./results/gym_ddpg')
    parser.add_argument('--summary-dir', help='directory for storing tensorboard info', default='./results/tf_ddpg')
    parser.add_argument('--ckpt-dir', help='directory for saved models', default='./results/checkpoints/model.ckpt')
    parser.add_argument('--replay-dir', help='directory for replay buffers', default='./results/replay/replay.pickle')
    parser.add_argument('--restore', help='restore model from previous iteration', default=False)
