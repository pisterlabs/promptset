...
from priority_replay_buffer_v2_MaxHeap import ReplayBuffer
    ...
    for i in range(int(args['max_episodes'])):
        ...
        for j in range(1, int(args['max_episode_len']) + 1):
            ...
            if j == eval_time:
                ...
                # Reward for RL.  Output is list of a list... lul
                r = env.reward_function(j, a[0][0])

                # Next state for RL
                s2 = deepcopy(env.x[j, :])

                # Temporal Difference Error from NN.  TD Error = abs([r + gamma*argmax(a) Q(s', a')] - Q(s, a))
                td_error = abs((r + critic.gamma * critic.predict(np.reshape(s2, (1, critic.s_dim)),
                                                                  actor.predict(np.reshape(s2, (1, actor.s_dim))))) -
                               critic.predict(np.reshape(s, (1, critic.s_dim)), a))

                # Add the latest states, action, reward, terminal, and new state to the replay memory
                replay_buffer.add(np.reshape(s, (actor.s_dim,)), np.reshape(a, (actor.a_dim,)), r,
                                  terminal, np.reshape(s2, (actor.s_dim,)), np.reshape(td_error, (1,)))
            ...
            # Batch Training area
            if replay_buffer.size() > int(args['minibatch_size'] * 5) and j % 8 == 0:

                # mini-batch size
                mini_batch_size = int(args['minibatch_size'])

                # Obtain a batch of data from replay buffer
                s_batch, a_batch, r_batch, t_batch, s2_batch = replay_buffer.priority_sample_batch(
                    int(mini_batch_size), type='rank')
                ...
                # Recalculate the TD error for the batch, and re-add into the network
                for k in range(int(mini_batch_size)):
                    td_error = abs((r_batch[k] + critic.gamma * critic.predict(
                        np.reshape(s2_batch[k], (1, critic.s_dim)),
                        actor.predict(np.reshape(s2_batch[k], (1, actor.s_dim))))) -
                                   critic.predict(np.reshape(s_batch[k], (1, critic.s_dim)), np.reshape(a_batch[k], (1, actor.a_dim))))

                    # Re-add into sequence dictionary
                    replay_buffer.add(np.reshape(s_batch[k], (actor.s_dim,)), np.reshape(a_batch[k], (actor.a_dim,)),
                                      r_batch[k], t_batch[k], np.reshape(s2_batch[k], (actor.s_dim,)),
                                      np.reshape(td_error, (1,)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-episodes', help='max num of episodes to do while training', default=251)
