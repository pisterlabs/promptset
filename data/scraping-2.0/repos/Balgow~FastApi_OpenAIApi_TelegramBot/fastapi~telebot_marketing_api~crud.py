...
def get_statistics(db: Session):
    agreed = []
    # answered = []
    # select leads that have been contacted in the last week
    leads =  db.query(Chat_messages.client_id).distinct(Chat_messages.client_id).all()
    leads = [lead[0] for lead in leads]
    last_week_leads = []
    for l in leads:
        lead = db.query(Leads).filter((Leads.tg == l) & (Leads.ls_start > datetime.datetime.now() - datetime.timedelta(days=7))).one_or_none()
        if lead:
            last_week_leads.append(lead.tg)
            if lead.ls_agree:
                agreed.append(lead.tg)
        # if lead and lead.ls_answer:
        #     answered.append(lead.tg)

    report = {
        "report_date": str(datetime.datetime.now().date()),
        "client_engagement": {
            "total_clients_contacted": str(len(last_week_leads)),
            # "clients_interested": str(len(answered)),
            "clients_agreed_for_meeting": str(len(agreed)),
            # "interest_rate": str(round(len(answered)/len(leads)*100, 2)) + '%',
            "meeting_conversion_rate": str(round(len(agreed)/len(leads)*100, 2)) + '%'
    }}       
    # print(leads)
    # print(answered)
    # print(agreed)
