...
def delete_messages_by_client_id(client_id: ChatMessageID, db: Session):

    rows_to_delete = db.query(Chat_messages).filter(Chat_messages.client_id == client_id.client_id)
    rows_to_delete.delete(synchronize_session=False)
    db.commit()

def add_client_if_not_exists(message: Client, db: Session):
    return {'message': 'ok'}
    existing_record = db.query(Clients).filter(Clients.client_id == message.client_id).one_or_none()
    ...
    if existing_record:
        ...
        new_record = Clients(**message.dict())
        db.add(new_record)
        db.commit()
