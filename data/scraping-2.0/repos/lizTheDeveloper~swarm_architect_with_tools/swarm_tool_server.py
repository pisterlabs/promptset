...
class SwarmArchitect:
    ...
    def create_tool(self, tool_name, code_str):
        ...
        with open("./proposed_tools/" + tool_name + ".py", "w") as f:
            f.write(code_str)
...
# swarm_architect.determine_tool_needs_for_goal("Please create a suite of tools that will allow a language model to check out repositiories from github, run the code, and then commit the results to a new branch, file a pull request, and merge the pull request.")
swarm_architect.determine_tool_needs_for_goal("Please create a suite of tools that will provide the same functionality as Code Interpreter to an LLM on my local machine.")
