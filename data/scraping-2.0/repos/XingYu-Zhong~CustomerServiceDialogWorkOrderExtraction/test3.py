...
if __name__ == '__main__':
    ...
    llm2 = OpenAI(model_name="text-davinci-002", temperature=0.1)
    ...
    for record in simple_progress_bar(data, total_length=len(data)):
        ...
        print(f'llm1result=>{llmresult}')
        outputkeylist = extract_keys_final(str(record['output']))
        strresult = ''
        listresult = []
        for output in outputkeylist:
            prompt_tmp = """\
                    你是一个文本提取专家，请你根据我的要求，正确的提取内容。
                    ...
                    """
            prompt = PromptTemplate.from_template(prompt_tmp)
            promptall = prompt.format(outputkey=output, sessionscontext=llmresult)
            llmresult = llm(promptall).replace('\n', '')
            print(f'llm2result=>{llmresult}')
    ...
    output_file_path = "./data/session_testA_result3.json"
