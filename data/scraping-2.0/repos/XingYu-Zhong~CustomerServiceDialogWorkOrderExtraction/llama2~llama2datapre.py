...
if __name__ == '__main__':
    ...
    # 打开一个新文件用于写入
    with open("llama2ftdata.json", "w", encoding="UTF-8") as outfile:
        sessions = []
        outputs = []
        count = 0
        for record in simple_progress_bar(data, total_length=len(data)):
            ...
            strresult = ''
            listresult = []
            prompt_tmp = """\你是一个文本提取专家，请你根据我的要求，正确的提取内容。需求1.请你精确的提取这段对话中的{outputkey}信息。需求2.如果是卡号，身份证号，手机号码则都必须是纯数字，名字是纯中文。需求3.按照指定格式提取信息，keyvalue格式.不需要回复多余的信息，只需要提取出来的信息。请解析以下对应的{outputkey}信息:"""
