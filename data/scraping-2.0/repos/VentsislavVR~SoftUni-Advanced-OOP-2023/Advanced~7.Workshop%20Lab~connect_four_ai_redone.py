...
def place_circle():
    ...
    if player_number == "AI":
        board[row - 1][player_col] = player_two_symbol  # Use the AI's symbol
    else:
...
turns = deque([[player_one_name, player_one_symbol], ["AI", player_two_symbol]])
...
while not win:
    ...
    if player_number == "AI":
        user_input = input("Your turn, please enter your move: ")
        assistant_response = get_assistant_response(user_input)
        print("Assistant:", assistant_response)
        row = place_circle()  # Use the AI's symbol in place_circle()
        counter_for_draw += 1
    else:
        try:
            user_input = input(f"Player {player_number}, please choose a column: ")
            player_col = int(user_input) - 1
        except ValueError:
            print(Fore.RED + "Select a valid number in the range (1-{COLS}) !" + Fore.RESET)
            continue
        ...
        if board[0][player_col] != "0":
            print(Fore.RED + "No empty space on this position, choose another one" + Fore.RESET)
            continue
        row = place_circle()
        counter_for_draw += 1
