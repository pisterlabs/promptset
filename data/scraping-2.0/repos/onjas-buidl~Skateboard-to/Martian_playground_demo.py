...
source_repo_dir = os.path.join(base_dir, "repo_raw", repo_name)  # old repo, unskated
target_repo_dir = os.path.join(base_dir, "repo_skated", repo_name)  # new repo, which will be skated

# detect if the target repo already exists
...
def extract_code_from_llm_output(llm_output):
    code_segments = re.findall(r"```(?:Python|python)?\s*([\s\S]*?)```", llm_output)
    if len(code_segments) == 0:
        raise Exception("No code segments found in the text.")
    return code_segments


def detect_openai_api_usage(code_segment):
...
def skate_file_with_llm(file_path):
    """
    :param file_path:
    ...
    if not detect_openai_api_usage(codebase):
        return False
...
\n```Python\n{code}\n```\nPlease output the complete file of the reformatted code and nothing else.""".format(
    ...
    cleaned_file_content = extract_code_from_llm_output(output)[0]
...
if __name__ == '__main__':
    ...
    new_branch_name = 'Skateboard-LangChain_update-' + random_string
    ...
    print("Pushed the skated files changes to Github. Link: {}".format(repo_URL + '/tree/' + new_branch_name))
