...
def generate_response(model, system_prompt, user_prompt, *args):
    def reportTokens(prompt):
        encoding = llm.tokenizer.encode(prompt)
        # print number of tokens in light gray, with first 50 characters of prompt in green. if truncated, show that it is truncated
        print("\033[37m" + str(len(encoding)) + " tokens\033[0m" + " in prompt: " + "\033[92m" + prompt[:50] + "\033[0m" + ("..." if len(prompt) > 50 else ""))
    ...
    role = "assistant"
    for value in args:
        messages.append(HumanMessage(content=value) if role == "user" else AIMessage(content=value))
        reportTokens(value)
    ...
    try:
        ...
        if file is not None:
            ...
            for filename, filecode in generate_file.map(
                    ...
                    kwargs=dict(model=model, filepaths_string=filepaths_string,
                                shared_dependencies=shared_dependencies, prompt=prompt)
            ):
                write_file(filename, filecode, directory)
