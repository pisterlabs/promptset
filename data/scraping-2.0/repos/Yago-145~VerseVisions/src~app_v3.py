...
def main():
    ...
    def process(tfile, selected_option, output_dir):
        ...
        frames_cada_frase = frames_resto / len(final_lyrics[1:6])

        animation_prompts = {}
        
        if generacion_automatica:
            cont = 1
            animation_prompts = {0: 'black background, deep black, nothingness, void, empty, dark, spherical movements',
                                 start_frame: auto_prompt(f"""From the following lyrics, find the prompt that whose style matches the best.
                                                ...
                                                Here are the lyrics that you're going to replace in the template: {final_lyrics[0]}""").strip()}
            
            for i in final_lyrics[1:6]:
                animation_prompts[int(start_frame + (frames_cada_frase * cont))] = auto_prompt(f"""From the following lyrics, find the prompt that whose style matches the best.
                                                Don't try to understand the lyrics, just find the most accurate style prompt.
                                                Don't modify the lyrics passed, just find the prompt that matches the style the best.
                                                Don't add any additional comments or modifications to the prompt.
                                                Here are the lyrics that you're going to replace in the template: {i}""").strip()
                cont += 1
        else:
            animation_prompts = {0: 'black background, deep black, nothingness, void, empty, dark, spherical movements',
                                 start_frame: prompt_templates[selected_option].replace('{i}', final_lyrics[0])}
            cont = 1
            for i in final_lyrics[1:6]:
                animation_prompts[int(start_frame + (frames_cada_frase * cont))] = prompt_templates[selected_option].replace('{i}', i)
                cont += 1

        frames_list = list(animation_prompts.keys()) 
        frames_list = frames_list[1:]

        st.write('Los frames de las frases son: ', frames_list)
        st.write('Los prompts de las frases son: ', animation_prompts)
                                                               # CAMBIAR FPS AQUÍ
        create_animation(animation_prompts, max_frames = int(float(duration) * 12), out_dir = f'{output_dir}/frames', video_name=f'{output_dir}/video.mp4')

        add_music_text(video_path = f'{output_dir}/video.mp4', audio_path = tfile.name,lyrics=final_lyrics, lyric_frames = frames_list, out_dir= output_dir)
    ...
    if uploaded_file is not None and output_dir:
        if not generacion_automatica and selected_option or generacion_automatica:
            if st.button("Iniciar procesamiento"):
                ...
                fin = time.time()  # Al final de tu función
                tiempo_total = fin - inicio
                minutos = tiempo_total // 60
                segundos = tiempo_total % 60
                st.write('Tiempo de ejecución: {} minutos, {} segundos'.format(int(minutos), int(segundos)))
        else:
