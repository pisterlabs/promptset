...
from .prompts import get_dialog_grade_prompt
...
async def transcribe_to_gpt(
    ...
    await check_tokens_buffer(state, session_maker)
    msg = await SendMessage(
        text='Запрос к gpt-4 отправлен',
        chat_id=query.from_user.id,
    )
    await message_handle_fn(state, msg)

    content = get_dialog_grade_prompt(dialog_text)
    ...
    await msg.edit_text(text=chat_response,
                        reply_markup=ukb.get_look_transcribe_bt())
    await db.create_message(
        cache_data['dialogue_id'],
        role='assistant',
        text=chat_response,
...
async def look_transcribe_callback(message):
    pass