...
async def react1(ctx):
    ...
    with open("self_role_config.json", "w") as w:
...
async def react2(ctx):
    ...
    with open("self_role_config.json", "w") as f:
...
async def statute(ctx):
    with open("./bot_database/channel_config.json", "r") as f:
        channel_config = json.load(f)
    with open("self_role_config.json", "r") as r:
        self_config = json.load(r)
...
@client.command()
async def oavatar(ctx, member: discord.Member=None):
    if member==None:
        member=ctx.author
