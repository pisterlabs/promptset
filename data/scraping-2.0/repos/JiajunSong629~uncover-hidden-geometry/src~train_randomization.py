...
def get_batch(split, noise_level):
    ...
    if noise_level == 1:
        # fill second half and x and y with random idx
        rand = torch.randint(low=0, high=50257, size=(batch_size, x.size(1) // 2 + 1))
        x[:, x.size(1) // 2 :] = rand[:, :-1]
        y[:, y.size(1) // 2 :] = rand[:, 1:]

    elif noise_level == 2:
        # fill all x and y with random idx
        rand = torch.randint(low=0, high=50257, size=(batch_size, block_size + 1))
        x = rand[:, :-1]
        y = rand[:, 1:]
...
def estimate_loss():
    ...
    for split in ["train", "val"]:
        ...
        for k in range(eval_iters):
            X, Y = get_batch(split, noise_level)
            with ctx:
...
# training loop
X, Y = get_batch("train", noise_level)  # fetch the very first batch
t0 = time.time()
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        X, Y = get_batch("train", noise_level)
