...
def rate_limit():
    ...
    # Remove timestamps older than RATE_LIMIT seconds
    timestamps = [t for t in timestamps if current_time - t < RATE_LIMIT]

    # Wait if the rate limit is reached
    while len(timestamps) >= MAX_CALLS_PER_MINUTE:
        print("Rate limit reached. Waiting...")
        time.sleep(1)  # Sleep for 1 second
        current_time = time.time()
        timestamps = [t for t in timestamps if current_time - t < RATE_LIMIT]

    # When exiting the function, we're clear to make an API call,
    # so record the current time in the timestamps
    timestamps.append(current_time)


def process_text(text):
    # Call rate limiting before making the API call
    rate_limit()
...
if __name__ == "__main__":
    ...
    # Initialize a list to keep track of timestamps for API calls
    timestamps = []
