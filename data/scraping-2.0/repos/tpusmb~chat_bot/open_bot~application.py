...
@app.route('/open_bot/interact', methods=["GET", "POST"])
def domain_dia():
    SEM.acquire()
    print(request.json["msg"])
    try:
        IDS.setdefault(request.json["user_id"], []).append(tokenizer.encode(request.json["msg"]))
    except Exception as e:
        print("Error: {}".format(e))
        return jsonify({"result": "error"})
    history = IDS[request.json["user_id"]]
    with torch.no_grad():
        out_ids = sample_sequence(personality, history, tokenizer, model, args)
    history.append(out_ids)
    history = history[-(2 * args.max_history + 1):]
    out_text = tokenizer.decode(out_ids, skip_special_tokens=True)
    IDS[request.json["user_id"]] = history
    SEM.release()
...
if __name__ == "__main__":
    port = 8081
    print("Start server to the port {}".format(port))
