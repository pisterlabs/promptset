...
from grid_score_base_rectangular_network_project.multi_agent_dispatching.common_utils.model import *
...
class QLearningPolicy(nn.Module):
    ...
    def __init__(
        ...
        #     share_params=True,
        # )
        # self.extractor = CNN_splitting_CNN(
        #     conv_params=conv_params,
        #     add_BN=add_BN,
        #     output_dim=output_dim,
        #     share_params=True,
        # )
        # self.extractor = CNN_DNN_splitting_CNN_DNN(
        #     conv_params=conv_params,
        #     add_BN=add_BN,
        #     output_dim=output_dim,
        #     share_params=True,
        # )
        # self.extractor = DNN_splitting_CNN_DNN(
        #     conv_params=conv_params,
        #     add_BN=add_BN,
        #     output_dim=output_dim,
        #     share_params=True,
        # )
        # self.extractor = ID_splitting_CNN_DNN(
        #     conv_params=conv_params,
        #     add_BN=add_BN,
        #     output_dim=output_dim,
        #     share_params=True,
        # )
        # self.extractor = ID_allID_splitting_CNN_DNN(
        #     conv_params=conv_params,
        #     add_BN=add_BN,
        #     output_dim=output_dim,
        #     share_params=True,
        # )
        # self.extractor = ID_allID_splitting_DNN(
        #     conv_params=conv_params,
        #     add_BN=add_BN,
        #     output_dim=output_dim,
        #     share_params=True,
        # )
        self.extractor = ID_allID_DNN_structure2_b(
            conv_params=conv_params,
            add_BN=add_BN,
            output_dim=output_dim,
            share_params=True,
    ...
    def forward(self, input_features: torch.Tensor, loc_ID=None):
        """
        ...
        :return:
        """
        latent_vf, _ = self.extractor(input_features, loc_ID)
        value = self.value_net(latent_vf)
        return value
...
class multi_agent_QLP():
    ...
    def forward(self, loc_features: np.ndarray, weight_features: np.ndarray):
        ...
        :return:
        """
        loc_features_ = copy.deepcopy(loc_features)
        loc_features = loc_features.reshape((loc_features.shape[0], -1, 1))
        index1 = np.repeat(np.array(range(loc_features.shape[0])), self.vehicle_num * 2, axis=0)
        index2 = np.array([0] * len(index1))
        index3 = loc_features[:, :, 0].flatten()
        ...
        for i in range(self.vehicle_num):
            ...
            input_loc_features[vehicle_index] = 1
            input_features = torch.cat((input_loc_features, input_all_loc_features, weight_features), dim=1)
            ############## for ID_splitting_CNN_DNN ##############
            # loc_ID = loc_features_[:, i * 2: (i + 1) * 2]
            ############## ID_splitting_CNN_DNN end ##############
            ############## for ID_allID_splitting_CNN_DNN ##############
            individual_loc = loc_features_[:, i * 2: (i + 1) * 2]
            loc_ID = np.hstack((individual_loc.copy(), loc_features_.copy()))
            ############## ID_allID_splitting_CNN_DNN end ##############
            value = self.QLP[i](input_features, loc_ID).unsqueeze(1)
            values.append(value.clone())
        values = torch.cat(values, dim=1)
        return values
