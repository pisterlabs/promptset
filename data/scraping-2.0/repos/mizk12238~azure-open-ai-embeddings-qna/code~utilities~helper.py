...
class LLMHelper:
    def __init__(self,
        ...
        self.deployment_name: str = os.getenv("OPENAI_ENGINE", os.getenv("OPENAI_ENGINES", "text-davinci-002"))
        ...
        self.k : int = 10 if k is None else k
    ...
    def get_semantic_answer_lang_chain(self, question, chat_history):
        ...
        sources = "\n".join(set(map(lambda x: x.metadata["source"].replace('converted/', '').replace('.txt', ''), result['source_documents'])))
