...
template = """ YANG to XML Converter
Act as a network engineer and convert YANG to XML format.
Some considerations:
- There should be a line per each YANG statement.
- In mandatory statements, you must have the parameter in the XML.
- In cases where the parameter is optional, you can leave it empty.
- In cases where the parameter default value is not given, put a [VALUE_HOLDER] instead.
- Make sure the XML is in a valid format.
- Don't include 'type', 'range', 'default', 'length', and 'units' in the XML.  
For instance, you can rely on the following YANG to XML convertion example below:

Example 1:
YANG: {yang}
XML: {xml}

Now it's your turn to convert the following YANG to XML:
YANG: {yang_test}
XML:
"""
prompt = PromptTemplate(template=template, input_variables=["yang", "xml", "yang_test"])
prompt_len = tokenizer.encode(prompt.format_prompt(yang=yang_file, xml=xml_file, yang_test=yang_file_test).text, return_tensors="pt").shape[1]
...
# output_lc = chain.invoke({"yang": yang_file, "xml": xml_file, "yang_test": yang_file_test})
# end_time = time.time()
# logger.info(f"Chain took {int(end_time - start_time)} seconds")

# # remove the text after "</config>"
# output_lc = output_lc.split("</config>")[0] + "</config>"
...
# inference model
prompt = prompt.format_prompt(yang=yang_file, xml=xml_file, yang_test=yang_file_test).text
...
# remove the text after "</config>"
output_text = [seq.split("</config>")[0] + "</config>" for seq in output_text]
...
# # remove the text after "</config>"
# seq_text = [seq.split("</config>")[0] + "</config>" for seq in seq_text]
