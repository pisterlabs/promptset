...
class ActorAgent(Agent):
    def __init__(self, sess, node_input_dim, job_input_dim, hid_dims, output_dim,
        ...
        self.gsn = GraphSNN(
            ...
            tf.concat([self.node_inputs, self.gcn.outputs_sparse], axis=1),
        ...
        self.node_act_probs_sparse, self.node_act_logits_sparse, self.job_act_probs_sparse, self.job_act_logits_sparse = self.actor_network(
            self.node_inputs, self.gcn.outputs_sparse, self.job_inputs,
            self.gsn.summaries_sparse[0], self.gsn.summaries_sparse[1],
        ...
        self.node_acts = tf.argmax(logits, 1)
        self.node_acts_sparse = tf.argmax(self.node_act_probs_sparse, 1)

        # job_acts [batch_size, num_jobs, 1]
        logits = tf.log(self.job_act_probs)
        # noise = tf.random_uniform(tf.shape(logits))
        # self.job_acts = tf.argmax(logits - tf.log(-tf.log(noise)), 2)
        self.job_acts = tf.argmax(logits, 2)
        self.job_acts_sparse = tf.argmax(self.job_act_probs_sparse, 2)
    ...
    def predict(self, node_inputs, job_inputs,
        ...
        if not sparse:
            if logits:
                return self.sess.run([self.node_act_logits, self.job_act_logits], \
                    feed_dict={i: d for i, d in zip(
                        [self.node_inputs] + [self.job_inputs] + \
                        [self.node_valid_mask] + [self.job_valid_mask] + \
                        self.gcn.adj_mats + self.gcn.masks + self.gsn.summ_mats + \
            ...
            return self.sess.run([self.node_act_probs_sparse, self.job_act_probs_sparse,
                self.node_acts_sparse, self.job_acts_sparse], \
                feed_dict={i: d for i, d in zip(
                    [self.node_inputs] + [self.job_inputs] + \
                    [self.node_valid_mask] + [self.job_valid_mask] + \
                    self.gcn.adj_mats_sparse + self.gcn.masks + self.gsn.summ_mats_sparse + \
                    [self.dag_summ_backward_map], \
                    [node_inputs] + [job_inputs] + \
                    [node_valid_mask] + [job_valid_mask] +  \
                    gcn_mats + gcn_masks + \
    ...
    def translate_state(self, obs, noLocality=False):
        """
    ...
    def invoke_model(self, obs, sparse=False, noLocality=False):
        ...
        node_inputs, job_inputs, \
            ...
            frontier_nodes, executor_limits, \
            exec_commit, moving_executors, \
            exec_map, action_map = self.translate_state(obs, noLocality=noLocality)
        ...
        node_act_probs, job_act_probs, node_acts, job_acts = \
            self.predict(node_inputs, job_inputs,
                node_valid_mask, job_valid_mask, \
                gcn_mats, gcn_masks, summ_mats, \
                running_dags_mat, dag_summ_backward_map, sparse=sparse)
    ...
    def get_action(self, obs, rich_output=False, sparse=False, noLocality=False):
        ...
        #print(f" node {node_act[0]}, exec {use_exec}")
