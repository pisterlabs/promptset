...
def printNumpy(x):
    return "[" + sumStr([decimal.Decimal(float(v)).__format__("f") + ", " for v in x.data])[:-2]+"]"
...
class ActorAgent(Agent):
    def __init__(self, sess, node_input_dim, job_input_dim, hid_dims, output_dim,
        ...
        self.node_act_probs, self.node_act_logits, self.job_act_probs, self.job_act_logits = self.actor_network(
        ...
        #print("dag summ backward map ", self.dag_summ_backward_map)
        ...
        self.prob_each_job = tf.reshape(
            tf.matmul(self.gsn.summ_mats[0],
        ...
        if args.saved_model is not None:
            ...
            graph_def = tf.graph_util.remove_training_nodes(input_graph=graph_def)
            tf.import_graph_def(graph_def)
            #f = open("gcn.tf", "w")
            #ignore = ["LeakyRelu_5/Maximum:0", "LeakyRelu_11/Maximum:0"]
            #consider_bias = ["MatMul:0", "MatMul:0]
            #last_op_type = None
            ...
            #        right  = inputs[1]
                    #matrix=None
                    #print("INPUTS ", left.name, left.shape, right.name, right.shape, op.outputs[0].shape)
            #        if left.op.type == "Const":
            #            matrix = sess.run(left) if not op.get_attr("transpose_a") else sess.run(left).transpose()
                    #    print("Matrix1 ", matrix.shape, matrix)
            #            print(printListsNumpy(list(matrix)),file=f)
            #        elif right.name not in ignore:
            #            matrix = sess.run(right).transpose() if not op.get_attr("transpose_b") else sess.run(right)
            #            print(printListsNumpy(list(matrix)),file=f)
                    #print(matrix)
            #    elif  (last_op_type == "MatMul" and op.type == "Add") or (op.type=="BiasAdd"):
            #        inputs = op.inputs
            #        left   = inputs[0]
            #        right  = inputs[1]
            ...
            #        else:
            #            bias = sess.run(right)
                    #print("BIAS ", bias, type(bias))
                        #if not np.isscalar(bias):
            #        print(printNumpy(bias), file=f)
    ...
    def actor_network(self, node_inputs, gcn_outputs, job_inputs,
        ...
        #print("node inputs shape ", node_inputs_reshape.shape, gcn_outputs_reshape, gsn_dag_summ_reshape, gsn_global_summ_reshape)
        ...
        with tf.variable_scope(self.scope):
            ...
            self.extended_node_features = merge_node
            ...
            node_outputs_logits = node_outputs
            ...
            job_outputs_logits = tf.reshape(job_outputs, [batch_size, -1, len(self.executor_levels)])
            ...
            #print("expanded ", job_outputs)
            # do masked softmax over jobs
            job_outputs = tf.nn.softmax(job_outputs, dim=-1)
            
            #print("NODE OUTPUTS ", node_outputs, node_outputs_logits, job_outputs)
            return node_outputs, node_outputs_logits, job_outputs, job_outputs_logits
    ...
    def predict(self, node_inputs, job_inputs,
        ...
        for gcn_mat in gcn_mats:
            if isinstance(gcn_mat, tf.SparseTensorValue):
                if gcn_mat.values.shape[0]==0:
                    adj_mats.append(np.zeros(gcn_mat.dense_shape))
                else:
                    adj_mats.append(self.sess.run(tf.sparse.to_dense(tf.sparse.reorder(gcn_mat))))
            else:
                adj_mats.append(gcn_mat)

        if isinstance(summ_mats, tf.SparseTensorValue):
            summ_mats = self.sess.run(tf.sparse.to_dense(tf.sparse.reorder(summ_mats)))
    ...
    def invoke_model(self, obs):
        ...
        #print("OBS ", obs, self.postman)
        node_inputs, job_inputs, \
            job_dags, source_job, num_source_exec, \
            frontier_nodes, executor_limits, \
            exec_commit, moving_executors, \
            exec_map, action_map = self.translate_state(obs)
        #print("NODE INPUTS ", list(node_inputs))
        ...
        node_act_probs, job_act_probs, node_acts, job_acts = \
            self.predict(node_inputs, job_inputs,
                ...
                gcn_mats, gcn_masks, summ_mats, \
                running_dags_mat, dag_summ_backward_map)
        #print("NODE PROBABILITIES ", list(node_act_probs))
        #print("JOB PROBABILITIES ", list(job_act_probs))
        return node_acts, job_acts, \
               ...
               gcn_mats, gcn_masks, summ_mats, \
               running_dags_mat, dag_summ_backward_map, \
               exec_map, job_dags_changed, feed_dict

    def get_action(self, obs, rich_output=False):
        # parse observation
        job_dags, source_job, num_source_exec, \
        frontier_nodes, executor_limits, \
        exec_commit, moving_executors, action_map = obs
        #print("OBSERVATION ", job_dags.to_list()[0].nodes, job_dags.to_list()[0].adj_mat, source_job, num_source_exec, frontier_nodes.to_list(), executor_limits,exec_commit, moving_executors, action_map )

        if len(frontier_nodes) == 0:
            # no action to take
            if rich_output:
                return None, num_source_exec, None, None
        ...
        node_act, job_act, \
            ...
            gcn_mats, gcn_masks, summ_mats, \
            running_dags_mat, dag_summ_backward_map, \
            exec_map, job_dags_changed, feed_dict = self.invoke_model(obs)
        ...
        if rich_output:
            return node, use_exec, feed_dict, node_act_probs
