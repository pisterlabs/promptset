    ...
    with torch.no_grad():
        for split, dataloader in [('train', train_dataloader), ('val', val_dataloader)]:
            ...
            for k in range(eval_iters):
                ...
                if is_nan.any():
                    print(f"NaN detected in X at index {is_nan.nonzero(as_tuple=True)}")
...
for e in range(epochs):
    for i, (X, Y) in enumerate(train_dataloader):
        ...
        Y = Y.to(device, dtype=torch.float16, non_blocking=True)
        # forward
        with ctx:
            logits, loss = model(X, Y)
        # backprop
        ...
        scaler.update()
        # log
        print(f"epoch {e} iter {i} train loss {loss.item():.3E}")
exit()
