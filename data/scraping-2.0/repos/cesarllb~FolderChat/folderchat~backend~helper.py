...
<<<<<<<< HEAD:datachad/streamlit/helper.py
...
========
from folderchat.monitoring_dir import WatchdogThread
from folderchat.chain import get_chain
from folderchat.constants import (
>>>>>>>> bb38fef (initial commit):folderchat/helper.py
    ACTIVELOOP_HELP,
    AUTHENTICATION_HELP,
    CHUNK_OVERLAP_PCT,
    CHUNK_SIZE,
...
<<<<<<<< HEAD:datachad/streamlit/helper.py
========
from folderchat.io import delete_files, save_files
from folderchat.logging import logger
from folderchat.models import MODELS, MODES
>>>>>>>> bb38fef (initial commit):folderchat/helper.py
...
def update_chain_with_monitoring() -> None:
    
    event_queue = Queue()
    watchdog_thread = WatchdogThread(data_source=st.session_state["data_source"], event_queue=event_queue)
    watchdog_thread.start()
    
    def process_events() -> None:
        while st.session_state["event_loop"]:
            events: List = []
            try:
                while len(events) < 10:
