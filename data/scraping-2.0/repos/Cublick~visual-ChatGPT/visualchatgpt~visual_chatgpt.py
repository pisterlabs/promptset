...
os.environ["OPENAI_API_KEY"] = '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'
openai.api_key = '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'

dir_path = r'C:\Users\com\PycharmProjects\pythonProject4\visual_chatgpt\image'
...
class ImageEditing:
    ...
    def inference_remove(self, inputs):
        ...
        inputs = inputs.replace('/image/image/', '/image/')
        inputs = os.path.normpath(inputs)
...
class ImageCaptioning:
    ...
    def inference(self, image_path):
        ...
        image_path = image_path.replace('/image/image/', '/image/')
...
load_dict = {'Text2Image': 'cuda:0', 'ImageCaptioning': 'cuda:0'} #, 'Image2Canny': 'cpu', 'InstructPix2Pix': 'cuda:0', 'ImageEditing': 'cuda:0',  'Image2Line': 'cuda:0'}
...
def run_text():
    text = request.args.get('text', 'cat')

    text = f"Translate korean to english: " + f"{text}"
    text = translateGPT(text)

    state = session.get('state', [])
    # text: 입력 문장 / state: 대화 기록 저장
    ...
    # canny edge
    if re.search(r'(\w+_edge_\w+\.png)', response):
        Image_name = re.search(r'(\w+_edge_\w+\.png)', response).group(1)
        send_file_dir = os.path.join(dir_path, Image_name)
        res = send_file(send_file_dir, as_attachment=True)

    # text2image
    elif re.search(r'image/([^\s]+)\.png', response):
        Image_name = re.search(r'image/([^\s]+)\.png', response).group(1)
        Image_name = re.search(r'([^\s]+)\.png', Image_name).group(1) + '.png'
        send_file_dir = os.path.join(dir_path, Image_name)
        res = send_file(send_file_dir, as_attachment=True)

    # replace image object
    elif re.search(r'(\w+_replace-something_\w+\.png)', response):
        Image_name = re.search(r'(\w+_replace-something_\w+\.png)', response).group(1)
        Image_name = re.search(r'([^\s]+)\.png', Image_name).group(1) + '.png'
        send_file_dir = os.path.join(dir_path, Image_name)
        res = send_file(send_file_dir, as_attachment=True)

    # painting (pix2pix)
    elif re.search(r'(\w+_pix2pix_\w+\.png)', response):
        Image_name = re.search(r'(\w+_pix2pix_\w+\.png)', response).group(1)
        Image_name = re.search(r'([^\s]+)\.png', Image_name).group(1) + '.png'
        send_file_dir = os.path.join(dir_path, Image_name)
        res = send_file(send_file_dir, as_attachment=True)
    ...
    # state 리스트에 새로운 튜플을 추가합니다.
    state.append((text, response))

    # 새로운 state 리스트를 세션에 저장합니다.
    session['state'] = state
...
def run_image():
    ...
    # NER Check
    ner = request.form.get('NER', False)
    ...
    # Save Image, But Change the extension to .png
    image = request.files['image']
    image_filename = f"{str(uuid.uuid4())[:8]}.png"
    with tempfile.TemporaryDirectory() as temp_dir:
        temp_image_path = os.path.join(temp_dir, 'temp_image.png')
        image.save(temp_image_path)
        shutil.move(temp_image_path, os.path.join(dir_path, image_filename))

    # Make description and prompt
    description = models['ImageCaptioning'].inference(os.path.join(dir_path, image_filename))
    Human_prompt = f'\nHuman: provide a figure named {image_filename}. The description is: {description}. This information helps you to understand this image, but you should use tools to finish following tasks, rather than directly imagine from my description. If you understand, say \"Received\". \n'
    AI_prompt = "Received.  "
    agent.memory.buffer = agent.memory.buffer + Human_prompt + 'AI: ' + AI_prompt
    ...
    if ner:
        ...
        text = f"please extract the nouns from this sentence: " + f"{description}"
        noun_list = translateGPT(text)
        pattern = re.compile("[^ \na-zA-Z]+")
        noun_list = pattern.sub("", noun_list)
        noun_list = re.sub('[ \n\t]', ',', noun_list).replace(',,', ',')
        mood = 'bright'
        # res['noun_list'] = noun_list

        res['filtering'] = translateGPT(f"""
            Filter out sexually explicit, political, and violent content from the sentence '{description}' and replace them with '*' characters. 
            If there are no corresponding words, return the original sentence. Slang terms will not be considered.
            """).strip()

        # res['replace'] = translateGPT(f"""
        #     After understanding the intent of the sentence and Words that violate openai policy, such as sensational, political, and abusive language, are replaced by other synonyms. {description}.
        #     If a substitute phrase is violated of openai policy, a warning phrase is printed.
        #     """).strip()

        # if not re.findall(r'and \*{2,}|\*{2,}', res['filtering']):
        if res['filtering'].lower() == description:
            res['triple'] = translateGPT(
                    f"""
                    Extract RDF triple from the {description} and separate the subject, verb, and object but, All words are considered in their base form.
                    Always return the output in JSON format as '-Subject: , -Verb: , -Compound Object: '. 
                    """).strip()

        # res['test'] = translateGPT(
        #     f"""
        #     Please create a natural sentence by adding {mood} atmosphere to {description}.
        #     """
        # )
        return res
    else:
        return send_file(os.path.join(dir_path, image_filename), as_attachment=True)
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=2728, threaded=True, debug=True, use_reloader=False)
