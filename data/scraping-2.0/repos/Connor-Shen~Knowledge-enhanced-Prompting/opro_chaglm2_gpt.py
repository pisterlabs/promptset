from Prompts_analysis_experts import meta_prompt, scorer_prompt, system_prompt
import pandas as pd
import openai
import re
import os
...
def save_sample_answers_with_index_to_excel(iter, sample_answers_with_index, file_count):
    # 创建一个DataFrame来存储数据
    df = pd.DataFrame(sample_answers_with_index, columns=['Index', 'prediction'])
    
    # 将数据保存到Excel文件
    excel_file_name = f"saved_data/iter{iter}_dm2_sample_answers_batch_{file_count}.xlsx"
    df.to_excel(excel_file_name, index=False)

# modify the score_prompts function
def score_evaluates(iter, model, scorer_prompt, prompts, training_examples, performance_df):
    ...
    for prompt in prompts:
        ...
        save_sample_answers_with_index_to_excel(iter, sample_answers_with_index, file_count)
        sample_answers_with_index = []
        file_count += 1
        ...
        performance_df.to_excel(f"saved_performance/newprompt_dm2_performance_exp{iter}.xlsx", index=False)

    return performance_df

def opro(performance_df, training_exemplar_df, system_prompt, meta_prompt, model, n_scores=20, n_exemplars=3, n_prompts=8, n_training_samples=10, max_iterations=3):
    """
    ...
    Args:
        ...
        n_exemplars (int): Number of exemplars to sample.s
        n_prompts (int): Number of prompts to generate.
    ...
    for i in range(max_iterations):
        ...
        performance_df = score_evaluates(iter, model, scorer_prompt, prompts, training_examples, performance_df)
        performance_df = rank_instructions(performance_df, n_scores)
        # save the checkpoint
        performance_df.to_excel(f"saved_performance/ranked_performance_dm2_iter{iter}.xlsx", index=False)
    return performance_df
...
if __name__ == '__main__':
    ...
    performance_df = pd.read_excel("/root/autodl-tmp/LLM as Optimizer/llm_as_optimizer/Optimization-by-PROmpting/performance_dm2_expert.xlsx")

    training_exemplar_df = pd.read_excel("data/training_data.xlsx")

    output = opro(performance_df, training_exemplar_df,  system_prompt, meta_prompt, model, n_scores=7, n_exemplars=3, n_prompts=3, n_training_samples=200, max_iterations=2)
    print(output)
    output.to_excel("saved_performance/performance_final_dm2.xlsx")
