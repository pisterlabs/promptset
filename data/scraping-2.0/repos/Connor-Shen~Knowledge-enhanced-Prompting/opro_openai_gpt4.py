...
def save_sample_answers_with_index_to_excel(sample_answers_with_index, file_count):
    ...
    # 将数据保存到Excel文件
    excel_file_name = f"dm0/gpt4/opro_dm0_sample_answers_batch_{file_count}.xlsx"
    df.to_excel(excel_file_name, index=False)

def score_evaluates(iter, prompts, training_examples, performance_df):
    ...
    messages=[
        ...
        performance_df.to_excel(f"dm0/gpt4/newprompt_dm0_performance_exp{iter}.xlsx", index=False)
...
def opro(performance_df, training_exemplar_df, system_prompt, meta_prompt, n_scores=20, n_exemplars=3, n_prompts=8, n_training_samples=10, max_iterations=3):
    ...
    for i in range(max_iterations):
        ...
        iter = i
        performance_df = score_evaluates(iter, prompts, training_examples, performance_df)
        performance_df = rank_instructions(performance_df, n_scores)
        # save the checkpoint
        performance_df.to_excel(f"dm0/gpt4/oprp_performance_dm0_iter{i}.xlsx", index=False)
    return performance_df
...
if __name__ == '__main__':
    ...
    performance_df = pd.read_excel("data/dm0_baselines.xlsx")
    ...
    print(output)
    output.to_excel("dm0/gpt4/performance_final_dm0.xlsx")
