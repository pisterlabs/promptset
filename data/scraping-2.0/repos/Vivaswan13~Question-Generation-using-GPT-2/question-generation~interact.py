...
def run():
    ...
    parser.add_argument("--model_checkpoint", type=str, default="D:\squash-generation-master\question-generation\gpt2_corefs_question_generation", help="Path, url or short name of the model")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu",
                        help="Device (cuda or cpu)")
    parser.add_argument("--filename", type=str, default="D:/squash-generation-master/squash/temp/quac_869/input.pkl", help="File to use for decoding")
    ...
    if args.key is not None:
        ...
        args.filename = "D:/squash-generation-master/squash/temp/%s/input.pkl" % args.key
    ...
    for inst in tqdm.tqdm(data):
        with torch.no_grad():
            para_index = inst["para_index"]# "answer_position"
        ...
        if len(final_output_dict["data"][0]["paragraphs"]) > para_index:
            ...
            print(final_output_dict["data"][0]["paragraphs"][para_index]['context'])
    ...
    #with open("D:/squash-generation-master/squash/temp/generated_questions.json", "w") as f:
        #f.write(json.dumps(final_output_dict))
    print(final_output_dict)
    for i in range(0,question_number):
        with open("D:/squash-generation-master/squash/temp/generated_questions.json", "a") as f:
            f.write(json.dumps(final_output_dict["data"][0]["paragraphs"][i]["qas"][0]["question"]))
            f.write("\n")
