...
def main():
    ...
    # read in data
    # df = pd.read_csv('../results/errors_v1.csv')
    # df["patent_ids"] = df["patent_ids"].apply(lambda x: [x])
    
    

    df = pd.read_csv("errors_reran.csv")
    df["patent_ids"] = df["patent_ids"].apply(literal_eval)
    df["summarization_sources"] = df["summarization_sources"].apply(literal_eval)
    # only keep rows with "API REQUEST ERROR" in the value

    df = df[df["summarization_sources"].apply(lambda x: "API REQUEST ERROR" in x.values())]
    print(df)
    ...
    df[["patent_ids", "summarization_sources", "summarizations"]].to_csv(f"{save_path_template}.csv", index=False)
