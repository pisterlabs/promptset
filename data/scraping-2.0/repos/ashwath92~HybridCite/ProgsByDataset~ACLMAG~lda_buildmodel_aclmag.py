""" Trains an LDA Mallet model on the Acl data set (with bigrams, snowball stemming)."""
...
try:
    ...
    del data_bigrams
try:
    id2word_dictionary = corpora.Dictionary.load('aclmag.dict')
except FileNotFoundError:
    id2word_dictionary = corpora.Dictionary(data_stemmed)
    id2word_dictionary.save('aclmag.dict')  # save dict to disk
...
try:
    corpus = corpora.MmCorpus('aclmag_bow_corpus.mm')
except FileNotFoundError:
    ...
    try:
        corpora.MmCorpus.serialize('aclmag_bow_corpus.mm', corpus)  # save corpus to disk
    except OverflowError:
        # Don't save corpus, call LDA directly
        print("Overflow while saving corpus, skip and train.")
        ldamallet = LdaMallet(mallet_path, corpus=corpus, num_topics=200, id2word=id2word_dictionary)
...
try:
    ldamallet.save('lda_model.model')
except OverflowError:
    print("Trying to pickle model using protocol 4")
    with open('ldamallet_model.model', 'wb') as pick:
        pick.dump(ldamallet_model, pick, protocol=pickle.HIGHEST_PROTOCOL)
