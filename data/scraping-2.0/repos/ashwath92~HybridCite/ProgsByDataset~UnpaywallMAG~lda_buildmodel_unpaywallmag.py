""" Trains an LDA Mallet model on the Unpaywall mag data set (with bigrams, snowball stemming)."""
...
try:
    ...
    with open(filename, 'r') as file:
        ...
        # Form Bigrams
        with open('inputtext_without_stops.pickle', 'wb') as dpick:
            pickle.dump(data, dpick)
    ...
    with open('processedtext.pickle', 'wb') as opick:
        pickle.dump(data_stemmed, opick, protocol=pickle.HIGHEST_PROTOCOL)
     # Save some memory
    del data
    del data_nostops
    del data_bigrams
try:
    id2word_dictionary = corpora.Dictionary.load('unpaywallmag.dict')
except FileNotFoundError:
    id2word_dictionary = corpora.Dictionary(data_stemmed)
    id2word_dictionary.save('unpaywallmag.dict')  # save dict to disk
