              ...
              reset_num_timesteps=True):
                ...
                with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
                    ...
                    #for update in range(1, n_updates + 1):
                        ...
                        # Unpack
                        obs, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward, AI_used, policy_prob = rollout
...
class Runner(AbstractEnvRunner):
    ...
    def _run(self):
        ...
        for step in range(self.n_steps):
            ...
            if(step%4 == 0):
                exp_policy_select = self.policy_decide(self.policy_prob)
            
            #Choose between current policy or other policy and find likelihood update
            if(exp_policy_select):
                control_actions = self.env.env_method("goToGoal")
                AI_used.append(1)
            else:
            ...
            if(AI_used[-1] == 1):
                ...
                action_probs = np.array(self.env.action_space.high)
            ...
            if isinstance(self.env.action_space, gym.spaces.Box):
                clipped_actions = np.clip(control_actions, self.env.action_space.low, self.env.action_space.high)
        ...
        self.ep_reward.append(np.mean(true_reward))
        if(len(self.ep_reward) >= 2):
            if(self.ep_reward[-1] > self.ep_reward[-2]):
                self.norm_w = max(self.norm_w/10.0, 1e-5)
            else:
                self.norm_w = min(self.norm_w*10.0, 10.0)
                    
        update_window = 50
        mean_reward = np.mean(self.ep_reward[-update_window:])
        print(self.prev_mean_reward, mean_reward, self.policy_prob, self.norm_w, self.num_eps)
        
        if((self.num_eps - self.last_trust_update) > update_window and self.prev_mean_reward <= mean_reward):
            self.policy_prob = min(self.policy_prob+self.get_phase_step(), 1.0)
            self.prev_mean_reward = 0.1#mean_reward
            self.last_trust_update = self.num_eps
            self.ep_reward = []
        ...
        return mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, mb_states, ep_infos, true_reward, AI_used, self.policy_prob
