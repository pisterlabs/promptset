...
Tool 2: A news search database which you can ask targeted questions at to learn about the current real estate market news/trends.
Tool 3: A mortgage payments calculator.
Tool 4: A land transfer tax calculator.
Tool 5: The ability to send emails to clients.

Tips:
- If a client asks to meet in person, talk on the phone, or book a showing, use the send_email_to_client tool. 
"""
...
class Agent:
        ...
        while True:
            ...
            if finish_reason == 'stop' or len(self.internal_thoughts) > 3:
                ...
                final_res = self._create_chat_completion(
                    self.chat_history + ([final_thought] if final_thought else []),
                    use_functions=False
                )
                return final_res
    ...
    def _final_thought_answer(self):
        if len(self.internal_thoughts) == 0:
            return None # no internal thoughts
