...
class ExperienceSource:
    ...
    def __init__(self, env, agent, steps_count=2, steps_delta=1, vectorized=False):
        ...
        :param agent: callable to convert batch of states to actions
        :param steps_count: count of steps to track for exp chain
        :param steps_delta: how many steps between exp items
    ...
    def __iter__(self):
        ...
        for env in self.pool:
            obs = env.reset()
            # If env is vectorized, all outputs are list of results
            # More details at https://github.com/openai/universe/blob/master/doc/env_semantics.rst
            if self.vectorized:
                obs_len = len(obs)
                states.extend(obs)
            else:
        ...
        while True:
            ...
            for idx, state in enumerate(states):
                if state is None:
                    actions[idx] = self.pool[0].action_space.sample() # assume all envs from same fam
                else:
...
def _group_list(items, lens):
    ...
    :return: list of list of items grouped by lens
    """
    res = []
    cur_ofs = 0
    for g_len in lens:
...
# Those entries are emitted from ExperienceSourceFirstLast.
# Reward is discounted over the trajectory piece
...
class ExperienceSourceFirstLast(ExperienceSource):
    ...
    in replay buffer when we need only first and last states. For every 
    trajectory piece it calculates discounted reward and emits only first and
    last states and action taken in first state. If we have partial trajectory
    at end of episode, last_state will be None.
    """
...
class ExperienceSourceRollouts:
    ...
    used with agent, keeping value in its state (for example agent.ActorCriticAgent).

    Yields batches of num_envs * n_steps samples with following arrays:
    1. observations
    2. actions
    3. discounted rewards, with values approx
    4. values
    """
    def __init__(self, env, agent, gamma, steps_count=5):
        """
        Constructs the rollout experience source
        :param env: environment or list ov environments to be used
        :param agent: callable to convert batch of states to actions
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for env_idx, (e, action) in enumerate(zip(self.pool, actions)):
                ...
                dones.append(done)
                rewards.append(r)
            # need extra step to get values approx for rollouts
            if step_idx == self.steps_count:
                # calc rollout rewards
...
class ExperiencSourceBuffer:
    """
    Same as ExperienceSource, but takes episodes from the buffer
    """
    def __init__(self, exp_buffer, steps_count=1):
        """
        Create buffered experience source
        :param exp_buffer: list of episodes, each is list of Experience obj
        :param steps_count: count of steps in every entry
        """
        self.update_buffer(exp_buffer)
        self.steps_count = steps_count

    def update_buffer(self, exp_buffer):
        self.buffer = exp_buffer
        self.lens = list(map(len, exp_buffer))

    def __iter__(self):
        """
        Infinitely sample episode from exp buffer and sample item offset
        """
        while True:
            episode = random.randrange(len(self.buffer))
            ofs = random.randrange(self.lens[episode] - self.steps_count - 1)
            yield self.buffer[episode][ofs:ofs+self.step_count]
...
class ExperienceReplayBuffer:
    ...
    def sample(self, batch_size):
        """
        Get one random batch from exp replay
        :param batch_size:
        :return:
        """
...
class PrioritizedReplayBuffer(ExperienceReplayBuffer):
    ...
    def _sample_proportional(self, batch_size):
        ...
        for _ in range(batch_size):
            mass = random.random() * self._it_sum(0, len(self) - 1)
    ...
    def update_priorities(self, idxes, priorities):
        ...
        for idx, priority, in zip(indxes, priorities):
            assert priority > 0
            assert 0 <= idx < len(self)
            self._it_sum[idx] = priority ** self._alpha
            self._it_min[idx] = priority ** self._alpha

            self.max_priority = max(self._max_priority, priority)


class BatchPreprocessor:
    """
    Abstract preprocessor class descendants to which converts
    experience batch to form suitable to learning
    """
...
class QLearningPreprocessor(BatchPreprocessor):
    ...
    TD-error back to exp replay buffer
    """
    ...
    def _calc_Q(self, states_first, states_last):
        """
        Calculates appropriate q vals for first and last states. Way
        of calc depends on settings.
        ...
        """
        # Here we need both first and last vals calculated using main model,
        # so we can combine both states into one batch for efficiency and 
        ...
        # so we should calc first_q and last_q using diff models
    ...
    def _calc_target_rewards(self, states_last, q_last):
        """
        Calculate rewards from final states according to variants
        from construction:
        ...
        """
        # In this case we handle simple and target DQN
    ...
    def preprocess(self, batch):
        ...
        :param batch: list of lists of Experience objs
        :return: tuple of numpy arrays:
            1. states --- observations
            2. target Q-values
            3. vector of td errors fro every batch entry
        """
        ...
        for idx, (total_reward, exps) in enumerate(zip(rewards, batch)):
            ...
            # update total reward and calc td error
            act = exps[0].action
