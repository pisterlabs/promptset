...
from .code_brookie import CodeInterpreter
...
**My Brookie** will require approval before running code. Use `brookie -y` to bypass this.
...
class Interpreter:
  ...
  def __init__(self):
    ...
    self.api_key = "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"
    self.auto_run = False
    self.local = False
    self.model = "gpt-4"
    ...
    # Note: While My Brookie can use Llama, we will prioritize gpt-4.
    # gpt-4 is faster, smarter, can call functions, and is all-around easier to use.
    # This makes gpt-4 better aligned with My Brookies priority to be easy to use.
      ...
      if self.llama_instance == None:
        ...
        try:
          ...
          print(Markdown("".join([
            ...
            "\n\n*( Running language models locally is a difficult task!*. )",
  ...
  def verify_api_key(self):
    ...
    if self.api_key == None:
      ...
      if 'OPENAI_API_KEY' in os.environ:
        ...
        print('', Markdown("**Welcome to My Brookie.**"), '')
        time.sleep(1)
        ...
        if response == "":
            ...
            print(Markdown("> Switching to `Code-Llama`...\n\n**Tip:** Run `brookie --local` to automatically use `Code-Llama`."), '')
            time.sleep(2)
            print(Rule(style="white"))
            return
    ...
    for chunk in response:
      ...
      if chunk["choices"][0]["finish_reason"] or llama_function_call_finished:
        if chunk["choices"][
          ...
          # (Because this is My Brookie, we only have one function.)
          ...
          if not self.local and "parsed_arguments" not in self.messages[-1]["function_call"]:
            ...
            # This is the most common failure pattern: https://github.com/zwpseudo/my-brookie/issues/41
            
            # print("> Function call could not be parsed.\n\nPlease open an issue on Github (openbrookie.com, click Github) and paste the following:")
            # print("\n", self.messages[-1]["function_call"], "\n")
            # time.sleep(2)
          ...
          language = self.messages[-1]["function_call"]["parsed_arguments"][
            "language"]
          if language not in self.code_brookies:
            self.code_brookies[language] = CodeInterpreter(language, self.debug_mode)
          code_brookie = self.code_brookies[language]

          # Let this Code Interpreter control the active_block
          code_brookie.active_block = self.active_block
          code_brookie.run()
