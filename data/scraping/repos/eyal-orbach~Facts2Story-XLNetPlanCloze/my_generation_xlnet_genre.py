...
RESULTS_OUT_PATH = "/home/nlp/eyalo/tmp/pycharm_project_711/customxlnet/examples/generated_text/full_plots_genres_blind/"
...
def main():
    ...
    test_dataset=MaskedPlotGenreDataset(tokenizer, args, args.test_file_path, block_size=512)
    ...
    for batch in test_dataloader:
        batch_counter+=1
        with torch.no_grad():
            inputs_raw = torch.stack([seq.to(args.device) for seq, mask, genre in batch])
            masks_raw = torch.stack([mask.to(args.device) for seq, mask, genre in batch])
            ...
            from MaskedPlotGenreDataset import GENRES_LIST
            genre_text = GENRES_LIST[genres[0]]
            print(genre_text +":\n")
