...
def impl_guidanceresponse_2(inst):
    ...
    assert inst.dataRequirement[0].codeFilter[0].code[0].display == "Hemoglobin A1c/Hemoglobin.total in Blood"
...
def test_guidanceresponse_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "guidanceresponse-additional-data-example.json"
    )
    inst = guidanceresponse.GuidanceResponse.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
