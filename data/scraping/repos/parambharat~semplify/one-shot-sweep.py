...
oneshot_table = cleaned_artifacts.get("oneshot_dataset")
oneshot_df = pd.DataFrame(oneshot_table.data, columns=oneshot_table.columns)
...
def main():
    ...
    predictions_artifact.add(predictions_table, f"oneshot_predictions-{wandb.run.name}")
    wandb.log_artifact(predictions_artifact)
    wandb.log({"oneshot_predictions": predictions_table})
    return df
