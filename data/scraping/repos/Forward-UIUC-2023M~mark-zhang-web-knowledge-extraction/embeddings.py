...
QUESTION = """Which faculties have the title "Undergraduate Academic Advisor"?"""


with open(f"data/html/{FILE}", "r", encoding="utf-8") as f:
    html = f.read()


utils.flatten(html)
get_embedding("", engine='text-embedding-ada-002')
...
case, Make sure not to leave anything out. If there is no relevant information \
to compose the answer, say so.
...
{{#assistant~}}
{{gen 'answer' temperature=0.1 max_tokens=300}}
{{~/assistant}}
""")
