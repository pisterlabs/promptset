...
def process_text_with_gpt3(text):
    ...
    max_tokens_input = 16000 - instruction_token_count - CONFIG["max_tokens_output"]
...
def call_openai_with_retry(text, max_retries=5, base_wait_time=3):
    ...
    for retry in range(max_retries):
        try:
            return process_text_with_gpt3(text)
...
def write_to_json(file_path, file_name, data):
    output_data = {
        "file_name": file_name,
        "gpt_3.5_predictions": json.loads(data + "}")
    }
    ...
    for filename in os.listdir(input_folder):
        ...
        if filename.endswith('.txt'):
            ...
            try:
                ...
                instruction_token_count = count_tokens_in_text(instruction)
                max_tokens_input = 12000 - instruction_token_count - CONFIG["max_tokens_output"]
