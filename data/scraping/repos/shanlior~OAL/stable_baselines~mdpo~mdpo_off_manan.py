...
from stable_baselines.mdpo.policies import SACPolicy
...
class MDPO_OFF_MANAN(OffPolicyRLModel):
    """
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 ...
                 tsallis_q=1, reparameterize=True, klconst=1.0):

        super(MDPO_OFF_MANAN, self).__init__(policy=policy, env=env, replay_buffer=None, verbose=verbose,
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                #with tf.variable_scope("old", reuse=False):
                    #_, _, logp_pi_old, _ = self.old_policy.make_actor(self.processed_obs_ph, action=policy_out, log_type=self.log_type, q_prime=self.tsallis_q, reparameterize=self.reparameterize)
                    #_, old_policy_out, logp_old, old_policy_out_unsquashed = self.old_policy.make_actor(self.processed_obs_ph, log_type=self.log_type, q_prime=self.tsallis_q, reparameterize=self.reparameterize)#, reuse=True)
                ...
                with tf.variable_scope("model", reuse=False):
                    ...
                    #self.deterministic_action, _, logp_pi_old_action, _ = self.policy_tf.make_actor(self.processed_obs_ph, action=old_policy_out, action_unsquashed=old_policy_out_unsquashed, log_type=self.log_type, q_prime=self.tsallis_q, reparameterize=self.reparameterize)
                    #_, _, logp = self.policy_tf.make_actor(self.processed_obs_ph, action=self.actions_ph, log_type=self.log_type, q_prime=self.tsallis_q, reuse=True)
                ...
                with tf.variable_scope("target", reuse=False):
                    ...
                    #_, target_policy_out, _ = self.target_policy.make_actor(self.processed_next_obs_ph)
                    #qf_target, _, _ = self.target_policy.make_critics(self.processed_next_obs_ph, target_policy_out,
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    if self.reparameterize:
                        policy_kl_loss = tf.reduce_mean(logp_pi * self.ent_coef + (self.lam - self.ent_coef) * logp_pi_old - qf1_pi_old)
                        ...
                        target_policy = tf.clip_by_value(tf.exp((-logp_old * self.lam + qf1_pi - value_fn) / (self.ent_coef + self.lam)), 0, 0.9)
                    ...
                    #policy_grads = tf.gradients(policy_loss, get_vars('model/pi'))
                    #print("policy grads shape", policy_grads)
                    #print("mult shape", tf.exp((-logp_pi_old * self.lam + qf1_pi - value_fn) / (self.ent_coef + self.lam)))
                    #policy_grads *= tf.exp((-logp_pi_old * self.lam + qf1_pi - value_fn) / (self.ent_coef + self.lam))
                    #policy_grads, _ = tf.clip_by_global_norm(policy_grads, 0.5)
                    #policy_grads = list(zip(policy_grads, get_vars('model/pi')))

                    # Target for value fn regression
                    # We update the vf towards the min of two Q-functions in order to
                    # reduce overestimation bias from function approximation error.
                    v_backup = tf.stop_gradient(min_qf_pi - self.lam * logp_pi)
                    #v_backup = tf.stop_gradient(min_qf_pi - self.lam * logp_old)
                    ...
                    with tf.control_dependencies([policy_train_op]):
                        ...
                        #self.multistep_ops = [policy_train_op]
    ...
    def _train_step(self, step, learning_rate, ent_coef):
        ...
        #    out = self.sess.run([self.summary] + self.multistep_ops, feed_dict)
        #    summary = out.pop(0)
        #    writer.add_summary(summary, step)
        #else:
        #    _ = self.sess.run(self.multistep_ops, feed_dict)
        #else: 
        #    break
        #print("policy grads", np.mean(policy_grads[0]))
        ...
        #print("policy out", self.sess.run(self.policy_out, feed_dict))
        #print("policy out after tanh", self.sess.run(tf.tanh(self.policy_out), feed_dict))
        #print("policy out unsquashed", self.sess.run(self.policy_out_unsquashed, feed_dict))
        #print("policy out unsquashed again", self.sess.run(self.policy_out_unsquashed, feed_dict))
        #print("policy old out", self.sess.run(self.policy_old_out, feed_dict))
        #print("policy old out unsquashed", self.sess.run(self.policy_old_out_unsquashed, feed_dict))
        #self.sess.run(self.target_update_op)
        # Unpack to monitor losses and entropy
        policy_loss, qf1_loss, qf2_loss, value_loss, *values = out
        #print("policy loss", policy_loss, value_loss, qf1_loss, qf2_loss)
        # qf1, qf2, value_fn, logp_pi, entropy, *_ = values
        entropy = values[4]
        #print("logpi", np.mean(values[3]), np.mean(values[5]))
    ...
    def tf_log_q(x,q):
        ...
        log_q_x = tf.cond(tf.equal(q,1.),true_fn=lam: tf.log(safe_x),false_fn=lam: (tf.pow(safe_x,1-q)-1)/(1-q))
        return log_q_x
