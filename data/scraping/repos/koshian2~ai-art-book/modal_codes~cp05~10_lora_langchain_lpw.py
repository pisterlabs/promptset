...
def run_lpw_with_langchain_with_lora(theme, idx, is_gpt_4=True):
    model_name = "gpt-4-0613" if is_gpt_4 else "gpt-3.5-turbo"
    ...
    with open(f"{CACHE_DIR}/output/10_gpt_lpw_{idx:03}.txt", "w", encoding="utf-8") as fp:
        fp.write(f"{model_name}\n\n{concept}\n\n{prompt}")
    for i, image in enumerate(images):
        image.save(f"{CACHE_DIR}/output/10_gpt_lpw_{idx:03}_{i:02}.jpg", quality=92)
...
if __name__ == "__main__":
    with stub.run():
        main.call()
    subprocess.run(
        f'modal nfs get model-cache-vol output/10_gpt_lpw_* .', shell=True)
