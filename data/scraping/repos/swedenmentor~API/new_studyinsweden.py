...
class Crawler:
    ...
    def crawl_website(self, url, output_file, depth=5):
        ...
        try:
            ...
            cookies = {'citizenship':'vn','relatedCitizenship':'noeu'}
            ...
            for link in links:
                href = link.get('href')
                if (href == None) or (href == '/') or (len(href)==0) or ('#' in href): continue
                if href and href.startswith('https'):
                    new_url = href
                else:
                    new_url = urljoin(url, href)
                if (not new_url.startswith('https://studyinsweden.se/') \
                    or (new_url.startswith('https://studyinsweden.se/blogs'))):
                    continue
    ...
    def extract_universities(self, url, output_file):
        ...
        if script_tag:
            ...
            script_content = script_tag.contents[0] if script_tag.contents else None
            
            # Load the JSON content
            obj_json = json.loads(script_content) if script_content else {}

            # Access the desired data
            universities_data = obj_json.get('props', {}).get('pageProps', {}).get('universities', [])

            # Extract URLs from universities_data
            university_urls = [university.get('link', '').replace('cms.','') for university in universities_data]

            # Crawl the extracted URLs
            for university_url in university_urls:
                self.crawl_website(university_url, output_file, depth=1)
...
if __name__ == '__main__':
    ...
    universities = 'https://studyinsweden.se/universities/'
    output_file = 'new_studyinsweden_vn.jsonl'
    # Clear existing content in the JSONL file
    with open(output_file, 'w'):pass
    # Crawling
    crawler.crawl_website(url, output_file=output_file, depth=max_depth)
    crawler.extract_universities(url = universities, output_file=output_file)
