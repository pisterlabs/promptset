...
class RandomGoalAnt(DirAntEnv):
    ...
    def reset(self, direction=None):
        if direction is None:
            # direction = np.random.choice([0, 90, 180, 270])
            direction = np.random.choice([0, 90, 180, 270])
        self.set_direction(direction)
        obs = super().reset()
        return self.add_goal(obs)
...
class MCPHiddenLayers(nn.Module):
    ...
    def __init__(
        ...
        self.primitives = nn.ModuleList([nn.Sequential(nn.Linear(64, 64), nn.ReLU(), nn.Linear(64, self.action_dim * 2)) for _ in range(num_primitives)])
    ...
    def freeze_primitives(self):
        self.primitives =  [freeze_model(mod) for mod in self.primitives]
    ...
    def forward_actor(self, features: th.Tensor) -> th.Tensor:
        ...
        outs = [self.forward_primitive(i, prim_embed) for i in range(self.num_primitives)]
        mus, sigmas = zip(*outs)
...
class MCPNaive(ActorCriticPolicy):
    ...
    def _build_mlp_extractor(self) -> None:
        self.mlp_extractor = MCPHiddenLayers(self.mcp_state_dim, self.mcp_goal_dim, int(np.prod(self.action_space.shape)), self.num_primitives, self.learn_log_std)
...
# env = make_vec_env('NewAnt-v2', n_envs=1, env_kwargs={'direction': env_direction[direction]})
...
learn_log_std=False
num_primitives=8
run_id = f"mcp_naive_np_{num_primitives}_sb3log_std_{learn_log_std}"
direction = direction
algo = "PPO"
logdir = "logs"
seed = 0
...
mcp_model = PPO(MCPNaive, env, verbose=1, policy_kwargs=policy_kwargs, tensorboard_log=tbdir, seed=seed)
...
pbar = tqdm(total=1000)
while not done:
    action, _ = mcp_model.predict(obs, deterministic=True)
    weight = mcp_model.policy.predict_weights(obs)
    weights.append(weight)
...
plt.xlim(0, ep_len)
plt.ylim(0, 1)
plt.title("Weights assigned to PPO primitives")
plt.tight_layout()
plt.legend()
fname=os.path.join("weights.jpg")
plt.savefig(fname, bbox_inches="tight", dpi=120)
plt.close()
write_gif_to_disk(np.array(imgs), "temp.gif")
...
mcp_model.policy.predict_values(torch.tensor(obs))