...
distributions from openai/baselines modified for MADDPG
...
class SoftMultiCategoricalPdType(PdType):
    ...
    def __init__(self, nvec):
        self.ncats = nvec.astype('float32')
        assert (self.ncats > 0.0).all()
...
class CategoricalPd(Pd):
    ...
    # TODO: Check dimensions for MADDPG, original axis=1 is second dimension
    # TODO: where new axis=-1 is the last dimension
    def mode(self):
...
# TODO: Doesn't work yet - Is this an old comment?
...
class MultiCategoricalPd(Pd):
    ...
    # TODO: Check if needed
    def logp(self, x):
        return tf.add_n([p.logp(px) for p, px in zip(self.categoricals, tf.unstack(x, axis=-1))])
...
class SoftMultiCategoricalPd(Pd):
    ...
    def __init__(self, nvec, flat):
        self.flat = flat
        self.categoricals = list(map(SoftCategoricalPd,
                                     tf.split(flat, np.array(nvec, dtype=np.float32), axis=-1)))

    def flatparam(self):
        return self.flat

    def mode(self):
        return tf.cast(tf.stack([p.mode() for p in self.categoricals], axis=-1), tf.float32)
...
class BernoulliPd(Pd):
    ...
    def kl(self, other):
        return tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=other.logits, labels=self.ps),
                             axis=-1) - tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits,
                                                                                              labels=self.ps), axis=-1)
...
def make_pdtype(ac_space):
    ...
    if isinstance(ac_space, spaces.Box):
        ...
        return SoftMultiCategoricalPdType(ac_space.nvec)
...
def validate_probtype(probtype, pdparam):
    ...
    calcloglik = tf_util.function([X, M], pd.logp(X))
    calcent = tf_util.function([M], pd.entropy())
    ...
    calckl = tf_util.function([M, M2], pd.kl(pd2))
...
# ================================================================
# Functions from openai/baselines/a2c.utils.py
# ================================================================
