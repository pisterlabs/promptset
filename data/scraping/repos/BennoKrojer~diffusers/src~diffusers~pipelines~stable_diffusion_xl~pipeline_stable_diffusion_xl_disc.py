...
class StableDiffusionXLDiscPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        if os.path.exists(os.path.expanduser("~/diffusers/noise_samples_xl.pkl")):
            with open(os.path.expanduser("~/diffusers/noise_samples_xl.pkl"), "rb") as f:
            ...
            self.noise_samples = torch.randn(1000,4, 128, 128)
            with open(os.path.expanduser("~/diffusers/noise_samples_xl.pkl"), "wb") as f:
    ...
    #     """
        ...
        do_classifier_free_guidance = unconditional
        ...
        # 4. Preprocess image
        image = PIL.Image.open(image[0]).convert("RGB")
        image = self.image_processor.preprocess(image, height=1024, width=1024)

        dists = []
        # sample N strengths between 0.1 and 0.9
        rate = int(1000 / sampling_steps)
        self.timesteps = list(range(10, 990, rate))
        # self.timesteps = list(range(300, 600, rate))
