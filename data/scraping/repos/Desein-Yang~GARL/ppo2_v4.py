...
def learn(*,sess, policy, env, nsteps, ent_coef, lr,
    ...
    for update in range(1, nupdates+1):
        ...
        if update % log_interval == 0 or (start_timesteps == 0 and update == 1):
            ...
            success_rate_train = np.sum(
                np.array([epinfo['r'] for epinfo in active_ep_buf])==10
            ) / large_bufsize
            ...
            mpi_print('len_epi / total_timesteps'.ljust(25),
            ...
            if can_save:
                ...
                checkpoint = int(step_elapsed // 1e6)
                if checkpoint % 8 == 0:
                    save_model(sess,datapoints,str(checkpoint) + 'M')

        update += 1
        if update > nupdates + 1:
            should_continue = False
