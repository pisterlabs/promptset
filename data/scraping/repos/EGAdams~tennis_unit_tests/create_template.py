...
def main(prompt, model=TURBO_MODEL ):
    ...
    prompt = (
        "Please make a unit test template for the .h, .cpp, and Makefile files that I have provided.  My files are as follows: \n```"
        + context
        + "```\n\n"
        + "Please write a template for a Makefile that will include the paths and the build patterns using the provided PinInterface project as a guide.  Please also create test templates for the .cpp and .h files using the provided PinInterface object as a guide.  The templates will be used by another AI to generate unit tests for a different object to test which will be provided later." )
