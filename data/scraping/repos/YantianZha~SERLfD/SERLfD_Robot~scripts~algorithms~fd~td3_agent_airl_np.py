...
class Agent(TD3Agent):
    ...
    def augment_experience(self, experiences, predicates, next_predicates, is_to_tensor=True):
        ...
        with torch.no_grad():
            states, augmented_rewards, next_states, predicate_util_vectors = self.np2torchTensor([states, augmented_rewards, next_states, predicate_util_vectors])
            # compute energy (energy is negative utility)
            energy = self.eval_explainer(states, predicate_vectors, augmented_rewards, next_states,
            ...
            if self.hyper_params["STATES_WITH_UTILITIES"]:
                batch_log_q = self.evaluate_state_action(torch.cat((states_s, predicate_util_vectors), dim=-1),
                                                         actions).detach()
            elif not self.hyper_params["STATES_WITH_PREDICATES"]:
                states_s = states_s[:][:-self.hyper_params["NUM_PREDICATES"]]
    ...
    def update_model(self, experiences):
        ...
        if self.hyper_params["STATES_WITH_UTILITIES"]:
            ...
            states, next_states = img2simpleStates(states, end=self.hyper_params[
                                                                         "SIMPLE_STATES_SIZE"] - self.hyper_params[
        ...
        states, actions, next_states, dones, weights = self.np2torchTensor([states, actions, next_states, dones, weights])
        ...
        if self.use_n_step:
            ...
            if self.hyper_params["STATES_WITH_UTILITIES"]:
                ...
                states_n, next_states_n = img2simpleStates(states_n, end=self.hyper_params[
                    "SIMPLE_STATES_SIZE"]-self.hyper_params["NUM_PREDICATES"]), img2simpleStates(
                    next_states_n, end=self.hyper_params["SIMPLE_STATES_SIZE"]-self.hyper_params["NUM_PREDICATES"])
            else:
            ...
            states_n, actions_n, next_states_n, dones_n = self.np2torchTensor([states_n, actions_n, next_states_n, dones_n])
                ...
                if self.hyper_params["STATES_WITH_UTILITIES"]:
                    ...
                    states_n, next_states_n = img2simpleStates(states_n, end=self.hyper_params[
                                                                             ...
                                                                             self.hyper_params[
                                                                                 "NUM_PREDICATES"]), img2simpleStates(
                        next_states_n,
                        end=self.hyper_params["SIMPLE_STATES_SIZE"] - self.hyper_params["NUM_PREDICATES"])
                else:
        ...
        for i_episode in range(1, self.args.episode_num + 1):
            ...
            while not done:
                ...
                if self.hyper_params["STATES_WITH_UTILITIES"]:
                    ...
                    state = state[:-self.hyper_params["NUM_PREDICATES"]]
