...
src_path = "../../../datasets/pre_datasets/KoAlpaca_v1.1a_textonly.json"
dst_path = "../../../datasets/pre_datasets/KoAlpaca_v1.1a_textonly_orca_style_comma.json"
...
def get_existing_line_count(filepath):
...
def process_line(line):
    ...
    try:
        ...
        print(f"Error calling the GPT API: {e}")
        gpt_response = "API Error. Unable to generate response."
...
def main(cfg):
    ...
    # Get number of existing lines
    existing_lines_count = get_existing_line_count(cfg.dst_path)

    with open(cfg.src_path, 'r') as f:
        data = f.readlines()

    # Remove already processed lines based on existing_lines_count
    data = data[existing_lines_count:]

    # Define number of processes
    n_processes = 10  # Adjust this based on your system's capabilities
    with Pool(processes=n_processes) as pool:
        results = list(tqdm(pool.imap(process_line, data), total=len(data)))
    ...
    with open(cfg.dst_path, "rb+") as k:
        ...
        k.truncate()
        k.write(b"\n]")
    # Since we are saving line by line, the saving portion in the main function can be removed.
