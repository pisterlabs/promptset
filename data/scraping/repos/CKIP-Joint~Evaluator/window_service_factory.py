...
from .codegen_window_service import CodegenWindowService
...
class WindowServiceFactory:
    ...
    def get_window_service(model_name: str, service: TokenizerService) -> WindowService:
        ...
        elif model_name in ["huggingface/bloom_1b1", "huggingface/bloom_3b", "huggingface/bloom_7b1"]:
            window_service = BloomWindowService(service)
        elif model_name in ["huggingface/bloomz_1b1", "huggingface/bloomz_3b", "huggingface/bloomz_7b1"]:
            window_service = BloomWindowService(service)
        elif model_name == "huggingface/codegen_2b_nl":
            window_service = CodegenWindowService(service)
        elif model_name == "huggingface/to_be_evaluated": # This will load model in $EVALUATED_MODEL_PATH
            window_service = BloomWindowService(service)
