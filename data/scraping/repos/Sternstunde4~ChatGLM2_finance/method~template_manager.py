    ...
    def __init__(self):
        self.templates = {
            "open_input": PromptTemplate(
                ...
                template="你是一个金融专家，请简洁和专业地回答用户的问题，在回答涉及数字的回复时，请用纯数字，若问题有要求小数点请遵守。"
                          "如果无法从中得到答案，可改写问题回复。答案请使用中文。 问题是：{question}"
            ),
            "basic_input": PromptTemplate(
                input_variables=["context", "question"],
                template="已知信息：\n{context}\n"
                         "请根据已知信息，简洁和专业地回答用户的问题，在回答涉及数字的回复时，请用纯数字，若问题有要求小数点请遵守。"
                         "如果无法从中得到答案，可改写问题回复。答案请使用中文。 问题是：{question}"
            ),
            "ratio_input": PromptTemplate(
                input_variables=["context","question"],
                template="已知信息：\n{context}\n" \
                        "请根据已知信息，简洁和专业地回答用户的问题，在回答涉及数字的回复时，请用纯数字，若问题有要求小数点请遵守，增长率请列出公式和过程。"
                         "如果无法从中得到答案，可改写问题回复。答案请使用中文。 问题是：{question}"
            ),
