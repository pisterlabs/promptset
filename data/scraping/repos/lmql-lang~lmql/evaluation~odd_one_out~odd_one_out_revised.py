...
excluded_samples = [
    "dress, jacket, pen, skirt",
    "England, France, German, Singapore, Spain"
]
...
async def process_lmql(d):
    ...
    return SampleResult(correct_item, result_item, result_probs, prompt, module.lmql_code)
...
Pick the odd word out: {options}.""".strip() + "\n"
    ...
    hf = HFPrompter(args.model, include_bos_token=True)
    
    prompt_with_reasoning = await hf.generate(prompt, max_new_tokens=50, stopping_phrases=["So the odd one is", "Pick the odd word out", "keep:.", "\n"], step_size=20)
    ...
    trace = prompt_with_reasoning.strip() + "|".join(options.split(", "))
    return SampleResult(correct_item, result_item, probs, trace)
...
async def process_sample_task(d, sem):
    options = ", ".join(sorted(d["target_scores"].keys()))
    if options in excluded_samples:
...
def result_file(file):
    ...
    return f"results/revised-{file}-{args.method}{model_name_suffix}.txt"
...
async def run_eval():
    ...
    if module is None and model.endswith(".lmql"):
        if model == "query.lmql":
            # use query_revised.lmql for this revised eval
            model = "query_revised.lmql"
