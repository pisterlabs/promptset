...
class KnowboxTool(BaseTool):
    ...
    def _run(self, query: str) -> str:
        ...
        dbClient = ChromaDb().get_client(index="knowbox", doc_path=Config.DATA_PATH)
        genie = ChromaDb().getRetrievalQAStream(dbClient)
        response = genie.run(query=query, top_k=1)
