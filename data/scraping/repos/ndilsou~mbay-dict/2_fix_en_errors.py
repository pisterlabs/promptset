...
def main(filename: str):
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        results = []
        fn = partial(fix_error, verbose=True)
        n = 10
        for i, batch in enumerate(batched(raw_mbay_content, n)):
            ...
            if i % 10 == 0:
                ...
                save_results(results, filepath.parent / "checkpoints", FILENAME, suffix=f"{run_id}_batch_{i}")
        end = time.perf_counter()
        duration = dt.timedelta(seconds=end - start)
        rprint(f"Finished in {duration}")


    save_results(results, filepath.parent, FILENAME) 


def save_results(results: Iterable[Record], dir: Path, filename:str, suffix: str = ""):
...
def fix_error(record: Record, verbose=False, progress: Callable[[], None] | None = None) -> Record:
    entry = record["entry"]
    try:
        ...
        # _input = HumanMessage(**record["input"])
        output = chat([_input, SystemMessage(content="remember to extract ALL examples and focus ONLY on the last input provided.")])
