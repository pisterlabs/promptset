...
Env_config = namedtuple('Env_config', [
    ...
    'leg_l_w', 'leg_l_h', 'leg_r_w', 'leg_r_h',
    'lower_l_w', 'lower_l_h', 'lower_r_w', 'lower_r_h'
])
...
class BipedalWalkerCustom(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        self.leg_l_w, self.leg_l_h = self.config.leg_l_w / SCALE, self.config.leg_l_h / SCALE
        self.leg_r_w, self.leg_r_h = self.config.leg_r_w / SCALE, self.config.leg_r_h / SCALE
        self.lower_l_w, self.lower_l_h = self.config.lower_l_w / SCALE, self.config.lower_l_h / SCALE
        self.lower_r_w, self.lower_r_h = self.config.lower_r_w / SCALE, self.config.lower_r_h / SCALE

        self.leg_l_fd, self.leg_r_fd, self.lower_l_fd, self.lower_r_fd = self.define_leg_parts()
    ...
    def define_leg_parts(self):
        leg_l_fd = fixtureDef(
            shape=polygonShape(box=(self.leg_l_w / 2, self.leg_l_h / 2)),
            density=1.0,
            restitution=0.0,
            categoryBits=0x0020,
            maskBits=0x001)
        leg_r_fd = fixtureDef(
            shape=polygonShape(box=(self.leg_r_w / 2, self.leg_r_h / 2)),
            density=1.0,
            restitution=0.0,
            categoryBits=0x0020,
            maskBits=0x001)
        ...
        lower_l_fd = fixtureDef(
            shape=polygonShape(box=(0.8 * self.lower_l_w / 2, self.lower_l_h / 2)),
            density=1.0,
            restitution=0.0,
            categoryBits=0x0020,
            maskBits=0x001)
        lower_r_fd = fixtureDef(
            shape=polygonShape(box=(0.8 * self.lower_r_w / 2, self.lower_r_h / 2)),
            density=1.0,
            restitution=0.0,
            categoryBits=0x0020,
            maskBits=0x001)
       
        return leg_l_fd, leg_r_fd, lower_l_fd, lower_r_fd
    ...
    def _reset(self):
        ...
        init_y = TERRAIN_HEIGHT + max([self.leg_l_h+self.lower_l_h*1.5, self.leg_r_h+self.lower_r_h*1.5])
        ...
        if True: #remove
            ...
            leg = self.world.CreateDynamicBody(
                position=(init_x, init_y - self.leg_l_h / 2 - LEG_DOWN),
                angle=(-1 * 0.05),
                fixtures=self.leg_l_fd
            )
            leg.color1 = (0.6 + 1 / 10., 0.3 + 1 / 10., 0.5 + 1 / 10.)
            leg.color2 = (0.4 + 1 / 10., 0.2 + 1 / 10., 0.3 + 1 / 10.)
            rjd = revoluteJointDef(
                bodyA=self.hull,
                bodyB=leg,
                localAnchorA=(0, LEG_DOWN),
                localAnchorB=(0, self.leg_l_h / 2),
                enableMotor=True,
                enableLimit=True,
            ...
            lower = self.world.CreateDynamicBody(
                position=(init_x, init_y - self.lower_l_h * 3 / 2 - LEG_DOWN),
                angle=(-1 * 0.05),
                fixtures=self.lower_l_fd
            )
            lower.color1 = (0.6 + 1 / 10., 0.3 + 1 / 10., 0.5 + 1 / 10.)
            lower.color2 = (0.4 + 1 / 10., 0.2 + 1 / 10., 0.3 + 1 / 10.)
            rjd = revoluteJointDef(
                bodyA=leg,
                bodyB=lower,
                localAnchorA=(0, -self.leg_l_h / 2), # may be wrong
                localAnchorB=(0, self.lower_l_h / 2),
                enableMotor=True,
                enableLimit=True,
                maxMotorTorque=MOTORS_TORQUE,
                motorSpeed=1,
            ...
            #right leg
            leg = self.world.CreateDynamicBody(
                position=(init_x, init_y - self.leg_r_h / 2 - LEG_DOWN),
                angle=(1 * 0.05),
                fixtures=self.leg_r_fd
            )
            leg.color1 = (0.6 - 1 / 10., 0.3 - 1 / 10., 0.5 - 1 / 10.)
            leg.color2 = (0.4 - 1 / 10., 0.2 - 1 / 10., 0.3 - 1 / 10.)
            rjd = revoluteJointDef(
            ...
            lower = self.world.CreateDynamicBody(
                position=(init_x, init_y - self.lower_r_h * 3 / 2 - LEG_DOWN),
                angle=(1 * 0.05),
                fixtures=self.lower_r_fd
            )
            lower.color1 = (0.6 - 1 / 10., 0.3 - 1 / 10., 0.5 - 1 / 10.)
            lower.color2 = (0.4 - 1 / 10., 0.2 - 1 / 10., 0.3 - 1 / 10.)
            rjd = revoluteJointDef(
                bodyA=leg,
                bodyB=lower,
                localAnchorA=(0, -self.leg_r_h / 2), # may be wrong
                localAnchorB=(0, self.lower_r_h / 2),
                enableMotor=True,
                enableLimit=True,
                maxMotorTorque=MOTORS_TORQUE,
                motorSpeed=1,
