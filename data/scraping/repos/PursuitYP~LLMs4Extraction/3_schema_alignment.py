""" LLMs for DeepShovel: 结构化数据抽取 - 实体对齐和关系对齐（输入是预处理好的段落文件） """
# 将三元组（例如用openie抽取的）与知识图谱模式对齐：1. 实体链接到本体；2. 关系对齐
# 输出是对齐后的三元组，格式为：[head, head_label, relation_aligned, old_relation, tail, tail_label]
import os
import openai
...
def get_completion(prompt, model="gpt-3.5-turbo"):
    for i in range(5):  # Retry the API call up to 5 times
        try:
            messages = [
                {"role": "system", "content": "I want you to act as an entity and relation alignment tool to help me align the triples to the Knowledge Graph schema."}, 
...
def read_relations_from_txt(input_rel_file):
...
def align_relations(input_rel_file, write_csv_file):
    # 从txt文件中读取关系，并进行 <预处理> re过滤提取内容和筛除一些项
    # sentence_relations_pack也可以是保存好的 <sentence - relations>
    sentence_relations_pack = read_relations_from_txt(input_rel_file)
...
if __name__ == "__main__":
    ...
    ref_unclear_keys = ['patient', 'medication', 'drug', 'test', 'gene', 'symptom', 'scale']
    ...
    align_relations(input_rel_file, write_csv_file)
