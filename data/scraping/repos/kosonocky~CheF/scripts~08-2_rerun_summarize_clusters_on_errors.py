...
def main():
    ...
    # read in data
    df = pd.read_csv(f"../results/schembl_summs_v4_vocab_gpt_cleaned_eps_0.340_diff_20030_clusters.csv")
    error_df = df[df["gpt_cleaned_labels"] == "API REQUEST ERROR"][["original_clustered_labels"]]
    clusters = error_df["original_clustered_labels"].tolist()
    ...
    error_df["gpt_cleaned_labels"] = new_labels
    
    # replace API REQUEST ERROR with corrected label
    df.loc[df["gpt_cleaned_labels"] == "API REQUEST ERROR", "gpt_cleaned_labels"] = error_df["gpt_cleaned_labels"].tolist()

    df[["original_clustered_labels", "gpt_cleaned_labels"]].to_csv(f"{save_path_template}_errors_fixed.csv", index=False)
