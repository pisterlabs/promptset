...
def main():
    ...
    print(f"INFO: Using {(n_cpus:=16)} CPUs")

    output_dir = f"../results/surechembl_smiles_canon_chiral_randomized_patents_l10p_summarizations/{head_n}"
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    save_path_template = f"{output_dir}/surechembl_canon_chiral_rand_l10p_summarizations_{gpt_model}_desc-{desc_len}_temp-{gpt_temperature}"
    ...
    for i in range(0, len(df), chunk_size):
        ...
        df_chunk["summarization_sources"] = [s for s in summarization_sources]
        df_chunk["summarizations"] = [s for s in summarizations]
        df_chunk[["smiles", "cid", "patent_ids", "summarization_sources", "summarizations"]].to_csv(f"{save_path_template}_i{str(i).zfill(9)}.csv", index=False)
