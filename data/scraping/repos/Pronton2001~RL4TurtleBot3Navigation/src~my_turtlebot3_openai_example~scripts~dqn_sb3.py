...
#########################################################################
model_name = "/sb3_dqn_s2_v2/model"
pretrained_model_name= "/sb3_dqn_s2_v1/model"
enjoy = True
load = False
load_policy = False
env = None
...
if __name__ == '__main__':
    ...
    if enjoy:
        agent = DQN.load(outdir + pretrained_model_name, env=env)
        obs = env.reset()
        for i in range(1000):
    ...
    if load:
        ...
        oldAgent = DQN.load(outdir + pretrained_model_name, env=env)
    ...
    agent.learn(total_timesteps=100000, reset_num_timesteps=False, callback=callback)
    # agent.predict(total_timesteps=100000, reset_num_timesteps=False, callback=callback)
