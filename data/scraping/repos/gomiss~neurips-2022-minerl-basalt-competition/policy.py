...
class MinecraftAgentPolicy(nn.Module):
    ...
    @th.no_grad()
    def act(self, obs, first, state_in, stochastic: bool = True, taken_action=None, return_pd=False, legal_action=None):
        # We need to add a fictitious time dimension everywhere
        obs = tree_map(lambda x: x.unsqueeze(1), obs)
        first = first.unsqueeze(1)
        ...
        if legal_action is not None:
            pd['buttons'][th.from_numpy(legal_action != 1).reshape_as(pd['buttons'])] = -1e6
