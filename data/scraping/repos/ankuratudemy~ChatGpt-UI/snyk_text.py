...
def chat_completion():
    ...
    try:
        loader = WebBaseLoader(
            [f"https://cwe.mitre.org/data/definitions/{cwe}.html", f"https://security.snyk.io/vuln/{snyk_code}"])
        data = loader.load()
        ...
        vectorstore = FAISS.from_texts(context, embeddings)
        
        prompt_template = """
            Your a a senior software developer. Please use the following context to answer question on CVE or CWE vulnerabilities.
            Context: {context}
            - -
            Question: {question}
        ...
        qa = RetrievalQA.from_chain_type(llm=OpenAI(temperature=0.7, max_tokens=2048),
                                         chain_type="stuff",
