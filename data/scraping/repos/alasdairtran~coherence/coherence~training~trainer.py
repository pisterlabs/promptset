"""Changes from AllenNLP:
- Support half-precision training.
- Avoid recording nan losses in tensorboard: `if loss.item() < float('inf')`.
"""
...
from coherence.utils import seed_environment

logger = logging.getLogger(__name__)  # pylint: disable=invalid-name

class TrainerFP16(TrainerBase):
    def __init__(self,
                 model: Model,
        ...
        fp16: ``bool``, (default = False)
            If True, then run with half precision.  This option requires apex
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_group in train_generator_tqdm:
            ...
            if self._batch_num_total % self.statistics_interval == 0:
            ...
            if loss.item() < float('inf') and self._tensorboard.should_log_this_batch():
                self._tensorboard.add_train_scalar(
                    "loss/loss_train", metrics["loss"])
                self._tensorboard.log_metrics(
                    {"epoch_metrics/" + k: v for k, v in metrics.items()})

            if loss.item() < float('inf') and self._tensorboard.should_log_histograms_this_batch():
    ...
    def train(self) -> Dict[str, Any]:
        ...
        this_epoch_val_metric: Optinal[float] = None
        ...
        # We run validation once at the beginning
        if self.validate_first and self._validation_data is not None:
            with torch.no_grad():
                # We have a validation set, so compute all the metrics on it.
                val_loss, num_batches = self._validation_loss()
                val_metrics = training_util.get_metrics(
                    self.model, val_loss, num_batches, reset=True)
                histograms = self.model.get_json_metrics(reset=True)

            self._tensorboard.log_metrics(
                {}, val_metrics=val_metrics, log_to_console=True)

            if self._serialization_dir:
                metrics_json = json.dumps(histograms, indent=2)
                file_path = os.path.join(self._serialization_dir,
                                    f'histograms_epoch_{epochs_trained}.json')
        ...
        for epoch in range(epoch_counter, self._num_epochs):
            ...
            if self._serialization_dir:
                metrics_json = json.dumps(histograms, indent=2)
                file_path = os.path.join(self._serialization_dir,
                                    f'histograms_epoch_{epoch + 1}.json')
                with open(file_path, "w") as metrics_file:
                    metrics_file.write(metrics_json)
    ...
    def from_params(cls,  # type: ignore
        ...
        statistics_interval = params.pop_int(
            "statistics_interval", 5000)
...
class TrainerPieces(NamedTuple):
    ...
    # test_dataset: Optional[Iterable[Instance]]
    ...
    def from_params(params: Params, serialization_dir: str, recover: bool = False) -> 'TrainerPieces':
        ...
        if vocab_type == 'default' and os.path.exists(os.path.join(serialization_dir, "vocabulary")):
        ...
        return TrainerPieces(model, iterator, corpus,
                             validation_iterator, batch_weight_key, trainer_params)
