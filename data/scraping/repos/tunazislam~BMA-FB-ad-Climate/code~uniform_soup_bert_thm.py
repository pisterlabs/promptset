...
def uniform(state_dicts, model):
    with torch.no_grad():
        merged = {}
        for key in state_dicts[0]:
            ###print("key", key) #key bert.embeddings.position_ids, key bert.embeddings.word_embeddings.weigh,key bert.embeddings.position_embeddings.weight
            if state_dicts[0][key].dtype == torch.int64 or state_dicts[0][key].dtype == torch.uint8:
                ...
                merged[key] = torch.mean(torch.stack(
                    [sd[key] for idx, sd in enumerate(state_dicts) if idx in range(len(state_dicts))]), axis=0)
        ###print(merged)
        #return merged
...
model = uniform(state_dicts, model)



#test_results.append(test(model))
torch.save(model.state_dict(), 'output_point_estimate_thm/uniform_soup_model.pt')

### Load the soup model
model.load_state_dict(torch.load("output_point_estimate_thm/uniform_soup_model.pt"))
...
df_b.to_csv("output_point_estimate_thm/test_bert_stance_uniform_soup.csv")
...
df_b = df_train[['id', 'ad_creative_body', 'funding_entity', 'pred_theme', 'time', 'imp',
       'spend', 'region', 'demo', 'avg_imp', 'avg_spend', 'pred_stance', 'stance']] #extracted 13 columns
df_b.to_csv("output_point_estimate_thm/train_bert_stance_uniform_soup.csv")
...
df_b = df_valid[['id', 'ad_creative_body', 'funding_entity', 'pred_theme', 'time', 'imp',
       'spend', 'region', 'demo', 'avg_imp', 'avg_spend', 'pred_stance', 'stance']] #extracted 13 columns
df_b.to_csv("output_point_estimate_thm/valid_bert_stance_uniform_soup.csv")
