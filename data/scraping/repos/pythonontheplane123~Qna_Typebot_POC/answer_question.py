...
prompt_1 = """
Let's find the answer to '{question}' using the file extracts: {files_string}. If no answer or the question isn't valid, respond appropriately. Follow these steps:

Analyze the question and files.
Note the answer and source filenames.
Explain how you found the answer.
Format the response in markdown as: 'Answer: <answer>'."
"""
...
def get_answer_from_files(question, session_id, pinecone_index):
    ...
    try:
        ...
        prompt_2 = f"Given a question, try to answer it using the content of the file extracts below, and if you cannot answer, or find " \
        ...
        prompt_1 = f"find the answer to Question : {question} using the file extracts: {files_string}. If no answer or the question isn't valid, respond appropriately. Follow these steps:"\
        f"Analyze the question and files."\
        f"Note the answer and source filenames."\
        f"Explain how you found the answer."\
        f"Format the response in markdown as: '<answer>'."\
