...
def initialise_music():
    ...
    app_directory = os.path.dirname(current_file)

    Cmaj.export(app_directory + "/store/synth/Cmaj_default.mp3", format="mp3")
    Gmaj.export(app_directory + "/store/synth/Gmaj_default.mp3", format="mp3")
    Amin.export(app_directory + "/store/synth/Amin_default.mp3", format="mp3")
    Fmaj.export(app_directory + "/store/synth/Fmaj_default.mp3", format="mp3")
    ...
    kick.export(app_directory + "/store/drums/kick_default.mp3", format="mp3")
    snare.export(app_directory + "/store/drums/snare_default.mp3", format="mp3")
    kick_half.export(app_directory + "/store/drums/kickhf_default.mp3", format="mp3")
...
def create_music(words,melody,chords,beat,output_path="final_song.mp3"):    
    ...
    #         print(SENTS[i])
            tts = gTTS(SENTS[i])
            tts.save(app_directory + "/store/sents/" + str(i) + ".mp3")

        #Get chords.
        chordict = {}
        
        Cmaj = AudioSegment.from_mp3(app_directory + "/store/synth/Cmaj_default.mp3")
        Gmaj = AudioSegment.from_mp3(app_directory + "/store/synth/Gmaj_default.mp3")
        Amin = AudioSegment.from_mp3(app_directory + "/store/synth/Amin_default.mp3")
        Fmaj = AudioSegment.from_mp3(app_directory + "/store/synth/Fmaj_default.mp3")
        ...
        kick = AudioSegment.from_mp3(app_directory + "/store/drums/kick_default.mp3")
        snare = AudioSegment.from_mp3(app_directory + "/store/drums/snare_default.mp3")
        kick_half = AudioSegment.from_mp3(app_directory + "/store/drums/kickhf_default.mp3")
        ...
        for num in range(0,len(SENTS)):
            ...
            y, sr = librosa.load(app_directory + "/store/sents/" + str(num) + ".mp3", sr=None, mono=False)
            ...
            write(app_directory + "/store/pitch_sents/" + str(num) + ".wav", samplerate, data)

        #Join all the chords, melody(vocals) and beat into one file
        num = 0

        voice_path = app_directory + "/store/pitch_sents/" + str(num) + ".wav"
        ...
        # print(len(Vocal),len(chordict[CHORDS[num]]),len(drumdict[BEAT[num]]),len(song_part))
        ...
        #     print(len(Vocal),len(chordict[CHORDS[num]]),len(drumdict[BEAT[num]]),len(song_part))
            ...
            SONG = SONG + song_part
        print("output_path from sing.py : ", output_path)
...
def generate_melody_from_prompt(desc_user="A fun melody"):
    ...
    ## Set OpenAI API key - this has been changed for security reasons
    os.environ['OPENAI_API_KEY']= "asdaqwe"
...
def sing(words="", output_file_path="final_song.mp3"):
    ...
    if words == "":
        ...
        You are a stupid.
        we are all stupid.
        we like being stupid."""
    else:
        WORDS = words

    MELODY = generate_melody_from_prompt(desc_user="something interesting and unique")
    ...
    print("Genetraing vocals with given melody")
    is_worked = create_music(WORDS,MELODY,CHORDS,BEAT, output_file_path)
    
    if is_worked == True:
        return True
    else:
        is_worked2 = create_music(WORDS,melody_bkup,CHORDS,BEAT, output_file_path)
        if is_worked == True:
            return True
        else:
            return False
