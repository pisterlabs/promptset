...
class ActorAgent(Agent):
    ...
    def get_action(self, obs):
        ...
        if len(frontier_nodes) == 0:
            # no action to take
            print("actor_agent: No frontier nodes, returning None")
        ...
        if sum(node_valid_mask[0, :]) == 0:
            ...
            print("actor_agent: No node to assign per mask, returning None")
            return None, num_source_exec
        ...
        print(f"actor_agent: selected node {node.idx} of job {node.job_dag.name}")
