...
def main():
    ...
    df_text=df[['Content']]
    print(len(df_text['Content']))
    for idx in range(len(df_text['Content'])):
        dataLine = df_text['Content'][idx].lower()
        dataLine = word_tokenize(dataLine)

        #dataLine = dataLine.apply(spell_check)

        dataLine = remove_stopwords(dataLine)

        text=dataLine
        filtered_chunk = remove_punct(dataLine)
        

        filtered_data=filtered_chunk
        freq_words = frequent_words(filtered_data)
        lst = []
        ...
        def remove_freq_words(text):
            ...
            for item in text:
                if item not in lst:
                    result.append(item)
            return result   
        filtered_data = remove_freq_words(filtered_data)
        filtered_data = lemmatization(filtered_data)
        filtered_data = stemming(filtered_data)
        chunk_list.append(filtered_data)
        print(idx)
        
    model=create_gensim_lsa_model(chunk_list,number_of_topics,words)
