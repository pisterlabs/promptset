...
def parse_args():
    ...
    # user options
    parser.add_argument('--label', type=str, default='exp0learn_policy_autopot_selfask_epsilon99_1000')
...
def eval_rl(args,result_file,pids,logger):
    ...
    with torch.no_grad():
        ...
        for i, pid in enumerate(pids):
            try:
            # if pid not in  [63,40,499,566,667]:
            #     continue
                ...
                print(pid,e)
def dataprepare(args,logFilename):
    ...
    logger = utils.Logger(os.path.join(args.ckpt_path, logFilename))
    return result_file,logger,problems,pids

def run_from_break(logFilename):
    result_file,logger,problems,pids  = dataprepare(args,logFilename)
    with open(result_file,"r") as reader:
        lines = json.loads(reader.read(),parse_int = int)
    out = lines["results"].keys()
    out = [eval(item) for item in list(out) ]
    new_pids = [pid for pid in pids if pid not in out]
    return result_file,logger,problems,new_pids
...
if __name__ == '__main__':
    ...
    state = False
    times = 1
    # result_file = 'promptPGpot/results/gpt3_rl/exp0learn_policy_autopot_selfask_epsilon99_1000_test_TQ-A_2_seed_1.json'
    if state == True:
        result_file,logger,problems,pids = dataprepare(args,'log_eval_random.txt')
        eval_rl(args,result_file,pids,logger)
    else:
        result_file,logger,problems,pids = run_from_break(f'log_eval_break{times}.txt')
        eval_rl(args, result_file, pids, logger)
