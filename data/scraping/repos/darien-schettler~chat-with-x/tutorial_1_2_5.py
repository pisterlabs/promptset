import argparse
from misc.utils import print_colored_output, retrieve_local_api_keys, get_streaming_cb
...
def llm_interaction(model_name="text-davinci-002", temperature=0.0, use_color=True, input_clr="blue",
                    response_clr="red", verbose=True, streaming=True, generate=False, **kwargs):
    """ Interact with an LLM to demonstrate the streaming functionality within LangChain

    Args:
        model_name (str, optional): Model name for the LLM (default: text-davinci-002) to be used in the ChatOpenAI
        instance
        temperature (float, optional): Temperature for randomness in output. Higher values will result in more
        random output
        ...
        generate (bool, optional): Flag to add generate with output check.
    ...
    if use_chat:
        ...
        if generate:
            print("\n... NOT USING `.generate` AS THE MODEL IS A CHAT MODEL ...\n")
    else:
        llm = OpenAI(
            model_name=model_name, temperature=temperature,
            streaming=streaming, verbose=verbose,
...
def main():
    ...
    parser.add_argument("-g", "--generate", action="store_true", help="Flag to add generate with output check.")
