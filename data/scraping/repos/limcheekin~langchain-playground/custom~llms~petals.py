...
class Petals(LLM):
    ...
    """The default torch.type. For CPU, it need to support AVX512"""
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            values["tokenizer"] = AutoTokenizer.from_pretrained(
                model_name, use_auth_token=huggingface_api_key)
            values["client"] = AutoDistributedModelForCausalLM.from_pretrained(
                model_name, torch_dtype=torch_dtype, use_auth_token=huggingface_api_key)
            values["huggingface_api_key"] = huggingface_api_key

        except ImportError:
            raise ValueError(
                "Could not import transformers or petals python package."
                "Please install with `pip install -U transformers petals torch`."
            )
        return values
    ...
    def _identifying_params(self) -> Mapping[str, Any]:
        ...
        return {**{"model_name": self.model_name, "torch_dtype": self.torch_dtype}, **self._default_params}
