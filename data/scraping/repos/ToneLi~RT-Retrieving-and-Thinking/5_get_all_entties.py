...
SYSTEM_PROMPT = "you are an excellent linguist. The task is to verify whether the word is an Disease or Chemical entity extracted from the given sentence."
...
def mrc2prompt(ner_test,gpt_results,transfered_label,sentences):
    ...
    for item_idx in tqdm(range(len(ner_test))):
        ...
        for el in entties:
            ...
            prompt += f"The given sentence: {sentence}\nIs the label of word \"{entity_text}\" indicating the Disease or the Chemical? Please answer with Disease or Chemical.\n"
               ...
               #  print("entity_words",len(entity_words))
                ...
                for j in range(len(GUIDELINES_PROMPTs)):
                    ...
                    if entity_word not in words_flag:
                        ...
                        if "Disease" in predictions  or  "disease"  in predictions:
                            pre_labels = pre_labels.replace(entity_word,"</D>" + entity_word + "<D>")
                        elif "Chemical" in predictions  or  "chemical"  in predictions:
