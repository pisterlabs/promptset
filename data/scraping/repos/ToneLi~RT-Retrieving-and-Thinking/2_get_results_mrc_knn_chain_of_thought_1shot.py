...
SYSTEM_PROMPT = "You are a smart and intelligent Named Entity Recognition (NER) system. the sentence from where your extract the entities and the output format with examples."
...
def mrc2prompt(mrc_data, data_name="CONLL", example_idx=None, train_mrc_data=None, example_num=16, last_results=None,
    ...
    def get_example(index):
        ...
        if len(example_idx) > 0:
            for idx_ in example_idx[index][:example_num]:
                ...
                # print(DIc_)
                # print("--------")
                count_ = {"DiseaseClass": 0, "SpecificDisease": 0,"CompositeMention": 0,"Modifier": 0}
            for key, value in DIc_.items():
                ...
                f = -1
                label_texts = []
                if count_["DiseaseClass"] < 2 and count_["SpecificDisease"] < 2 and count_["CompositeMention"] < 2 and count_["Modifier"] < 2:
                    for label in value:
                        f = f + 1
                        type_ = label[0]
                        label_text = label[1]
                        ...
                        if type_ == "DiseaseClass":
                            context = context + str(f) + "." + label_text + "|" + "True|" + "as it is a DiseaseClass "
                            count_["DiseaseClass"] = count_["DiseaseClass"] + 1
                            # context = context.replace(label_text, "</C>" + label_text + "<C>")
                        if type_ == "SpecificDisease":
                            context = context + str(f) + "." + label_text + "|" + "True|" + "as it is a SpecificDisease "
                            count_["SpecificDisease"] = count_["SpecificDisease"] + 1

                        if type_ == "CompositeMention":
                            context = context + str(
                                f) + "." + label_text + "|" + "True|" + "as it is a CompositeMention "
                            count_["CompositeMention"] = count_["CompositeMention"] + 1

                        if type_ == "Modifier":
                            context = context + str(
                                f) + "." + label_text + "|" + "True|" + "as it is a Modifier "
                            count_["Modifier"] = count_["Modifier"] + 1
                ...
                for f_excampl in false_excamples:
                    fl = fl + 1
                    context = context + str(
                        f + fl) + "." + f_excampl + "|" + "False|" + "as it is not a DiseaseClass or SpecificDisease or CompositeMention or Modifier "
            ...
            exampel_prompt += f"The given sentence:  HFE mutations analysis in 711 [mask1] probands: evidence for S65C implication in mild form of hemochromatosis . [mask2] [mask3] is a common [mask4] [mask5] genetic [mask6] of iron metabolism. The HFE candidate gene encoding an HLA class I-like protein involved in HH was identified in 1996. Two [maskn] have been described C282Y,\n"
            exampel_prompt += f"1. 1996|False|as it is a number 2. mutations|False|as it is a medical word 3 identified|False|as it is a verb\n"
    ...
    for item_idx in tqdm(range(len(mrc_data))):
        ...
        prompt = f"You are an excellent linguist. Within the NCBI dataet, the task is to label {transfered_label} entities that in the given sentence. Below are some examples, and you should make the same prediction as the examples.\n"
...
if __name__ == '__main__':
    ...
    #     best_of=1
    # )
    mrc_train = read_mrc_data(dir_="data/NCBI/dev_NCBI.json")
    mrc_train_data_source = mrc_data(dir_="data/NCBI/dev_NCBI.json")
    ...
    example_idx = read_idx("data/NCBI/1shot_test.jsonl")
    ...
    data_name="NCBI"
    # prompts = mrc2prompt(mrc_data=ner_test, data_name=data_name, example_idx=example_idx, train_mrc_data=mrc_train, example_num=args.example_num, last_results=last_results)
    ...
    out_putfile = "data/NCBI/GPTNER_gpt4_output_shot1_NCBI_our_method_try_1.json"
