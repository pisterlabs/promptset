# reference: https://github.com/JayZeeDesign/researcher-gpt
import os
...
plugin = lightbulb.Plugin("gpt_researcher")
...
@plugin.command
@lightbulb.option("msg", "message to send to the research agent", type=str)
@lightbulb.command("gptr", "GPT Researcher", auto_defer=True)
...
async def gptr(ctx):
    ...
    conent = agent({"input": ctx.options.msg})
    actual_content = conent["output"]
    actual_content = "Query: " + ctx.options.msg + "\n\n" + actual_content
    await ctx.respond(actual_content)
