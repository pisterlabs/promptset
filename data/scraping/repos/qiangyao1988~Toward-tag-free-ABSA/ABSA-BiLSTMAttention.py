...
class BiLSTMAttention(nn.Module):
    ...
    def forward(self, review_indices):
        ...
        outs = torch.split(out, hidden_dim, dim=-1)    
        out = outs[0] + outs[1]
...
mymodel = BiLSTMAttention(embedding_matrix, embed_dim, hidden_dim, polarities_dim, device)
...
def train(model, data_loader, criterion, optimizer, log_step, num_epoch, device):
    ...
    # save model
    torch.save(model, 'RestBiLSTMAttention.pkl')
...
newmodel = torch.load('RestBiLSTMAttention.pkl')
