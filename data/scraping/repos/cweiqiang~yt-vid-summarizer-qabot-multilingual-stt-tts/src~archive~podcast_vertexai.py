...
with cb:
    ...
    loader = YoutubeLoader.from_youtube_url(
        'https://www.youtube.com/watch?v=jlivBvu3Jrc', add_video_info=True)
    data = loader.load()
    ...
    llm_summary = ChatVertexAI(temperature=0.3, credentials=vertexai_creds)
    ...
    llm_question_answer = ChatVertexAI(
        temperature=0.2, credentials=vertexai_creds)

    # Create the vector database and RetrievalQA Chain
    embeddings = VertexAIEmbeddings(credentials=vertexai_creds)
