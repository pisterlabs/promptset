...
def gpt3_completion(prompt, engine='text-davinci-002', temp=1.0, top_p=1.0, tokens=1000, freq_pen=0.0, pres_pen=0.0, stop=['asdfasdf', 'asdasdf']):
    ...
    while True:
        try:
            ...
            save_file('gpt3_logs/plots_complete/%s' % filename, prompt + '\n\n==========\n\n' + text)
            return text
        ...
        for modifier in modifiers:
            for place in places:
                for period in periods:
                    ...
                    save_file('Synthentic_data/plots_complete/prompts/%s' % filename, outprompt)
                    save_file('Synthentic_data/plots_complete/completions/%s' % filename, completion)
                    print('\n\n', outprompt)
                    print('\n\n', completion)
                    if count > 300:
                        exit()
