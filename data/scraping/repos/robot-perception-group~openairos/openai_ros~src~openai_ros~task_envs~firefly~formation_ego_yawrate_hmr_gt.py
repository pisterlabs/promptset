...
from openai_ros.robot_envs import firefly_env, firefly_multiagent_gt_inf_env, firefly_singleagentcartesian_yawrate_env, firefly_ego_yawrate_hmr_env
...
class DeepFollowEnv(firefly_ego_yawrate_hmr_env.FireflyMultiAgentGTEnv):
    def __init__(self,**kwargs):
        ...
        low = np.concatenate((low,np.array([self.work_space_min,self.work_space_min,self.work_space_min,\
                                            self.min_v,self.min_v,self.min_v,-1,-1,-1,-1]))) #dx,dy,dz,vx,vy,vz,bearing,person_orientation
        # GT INFORMATION
        ...
        low = np.concatenate((low,np.array([self.work_space_min,self.work_space_min,self.work_space_min,
                                            ...
                                            self.min_v,self.min_v,self.min_v,\
                                            -1,-1,-1,-1]))) #x,y,z,px,py,pz,vx,vy,vz,pvx,pvy,pvz,yaw,person_orientation
        ...
        high = np.concatenate((high,np.array([self.work_space_max,self.work_space_max,self.work_space_max,\
                                              self.max_v,self.max_v,self.max_v,1,1,1,1]))) #dx,dy,dz,vx,vy,vz,bearing,person_orientation
        # GT INFORMATION
        ...
        high = np.concatenate((high,np.array([self.work_space_max,self.work_space_max,self.work_space_max,\
                                              ...
                                              self.max_v,self.max_v,self.max_v,\
                                              1,1,1,1]))) #x,y,z,vy,vz,pvx,pvy,pvz,bearing,person_orientation
    ...
    def _get_obs(self):
        ...
        '''Translation of Actor Joints w.r.t world: Input for Value Function'''
        gt = []
        ...
        relative_vel = np.dot(np.linalg.inv(homogeneous_matrix),np.array([vel.twist.twist.linear.y - target.twist.twist.linear.x,vel.twist.twist.linear.x - target.twist.twist.linear.y,\
        ...
        observations =  [delta_target1.pose.pose.position.x, delta_target1.pose.pose.position.y, delta_target1.pose.pose.position.z,\
                        relative_vel[0],relative_vel[1],relative_vel[2],np.cos(theta1), np.sin(theta1),np.cos(delta_orient), np.sin(delta_orient)]+\
                        ...
                        vel.twist.twist.linear.x,vel.twist.twist.linear.y,-vel.twist.twist.linear.z,np.cos(yaw1), np.sin(yaw1),np.cos(target_orient), np.sin(target_orient)]
