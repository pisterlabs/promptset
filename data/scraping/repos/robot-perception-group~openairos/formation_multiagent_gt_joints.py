...
from openai_ros.robot_envs import firefly_env, firefly_multiagent_gt_env,firefly_multiagent_sync_env
...
class DeepFollowEnv(firefly_multiagent_gt_env.FireflyMultiAgentGTEnv):
    ...
    #               'actor::RightHand', 'actor::RightFingerBase', 'actor::RightHandIndex1', 'actor::RThumb'
        ...
        # low = np.append(low,0)
        low = np.concatenate((low,np.array([self.work_space_min,self.work_space_min,self.work_space_min,-15,-1,-1,-1,-1]))) #x,y,z,alt,theta1, theta2
        # GT INFORMATION
        low = np.concatenate((low,np.tile(np.array([self.work_space_min,self.work_space_min,self.work_space_min]),14))) #JOINTS
        low = np.concatenate((low,np.array([self.work_space_min,self.work_space_min,self.work_space_min,self.work_space_min,self.work_space_min,self.work_space_min]))) #ROBOT POSITIONS
        ...
        high = np.concatenate((high,np.array([self.work_space_max,self.work_space_max,self.work_space_max,15,1,1,1,1]))) #x,y,z,alt,theta1, theta2
        # GT INFORMATION
        high = np.concatenate((high,np.tile(np.array([self.work_space_max,self.work_space_max,self.work_space_max]),14))) #JOINTS
        high = np.concatenate((high,np.array([self.work_space_max,self.work_space_max,self.work_space_max,self.work_space_max,self.work_space_max,self.work_space_max]))) #ROBOT POSITIONS
    ...
    def _get_obs(self):
        ...
        noisy_bbox = self.get_noisy_bbox()
        noisy_bbox_neighbor = self.get_noisy_bbox_neighbor()
        detection = self.get_noisy_detection()
        detection_neighbor = self.get_noisy_neighbor_detection()
        bbox_coordinates = np.array(noisy_bbox.bbox)
        bbox_coordinates_neighbor = np.array(noisy_bbox_neighbor.bbox)


        if len(noisy_joints.res)!=28 or len(noisy_joints_neighbor.res)!=28:
            joints_flattened = self.normalize_image_coordinates
            joints_neighbor_flattened = self.normalize_image_coordinates
        else:
            joints_flattened = noisy_joints.res + self.normalize_image_coordinates
            joints_neighbor_flattened = noisy_joints_neighbor.res + self.normalize_image_coordinates
        ...
        #joints_flattened = joint_detections[5:,0:2].reshape(24,)
        #joints_neighbor_flattened = joint_detections_neighbor[5:,0:2].reshape(24,)
        ...
        observations =  [\
                        ...
                        list(joints_flattened)+list(joints_neighbor_flattened)+\
                        [pose.position.x - pose_neighbor.position.x, pose.position.y - pose_neighbor.position.y, pose.position.z - pose_neighbor.position.z, abs(float(pose.position.z)) - abs(self.desired_height), \
    ...
    def _compute_reward(self, observations, done):
        ...
        if not done:
            ...
            if flag:
                reward += 0.1*self.detection_data
            else:
                reward += 0.01*self.detection_data
            #
            #
            ...
            [flag,error] = self.get_lsq_triangulation_error_with_noisy_gt(observations,epsilon=5)
            # if flag:
            #     reward += 0.8*self.detection.data
            #     rospy.logwarn("################### 3-D JOINT ESTIMATES: GOOD JOB ###########################")
            # else:
            #     reward += 0.01*self.detection.data
            reward += self.detection.data*0.5*(1-np.tanh(0.01*error))
