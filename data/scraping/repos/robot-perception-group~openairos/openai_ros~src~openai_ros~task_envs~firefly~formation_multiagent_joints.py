...
from openai_ros.robot_envs import firefly_env, firefly_multiagent_env,firefly_multiagent_sync_env
...
class DeepFollowEnv(firefly_multiagent_sync_env.FireflyMultiAgentSyncEnv):
    ...
    #               'actor::RightHand', 'actor::RightFingerBase', 'actor::RightHandIndex1', 'actor::RThumb'
        ...
        low = np.concatenate((low,np.tile(joints_low,24))) # 12 x,y positions excluding actor head
        # low = np.concatenate((low,np.tile(joints_low,4))) # lhip and rhip x,y positions
        # low = np.append(low,0)
        low = np.concatenate((low,np.array([self.work_space_min,self.work_space_min,self.work_space_min,0,-1,-1,-1,-1]))) #x,y,z,alt,theta1, theta2
        ...
        #                 ])
        high=[]
        joints_high = np.array([self.image_width,self.image_height])
        # high = np.concatenate((high,np.tile(joints_high,34))) # 17 x,y positions
        high = np.concatenate((high,np.tile(joints_high,24))) # 12 x,y positions excluding actor head
    ...
    def _get_obs(self):
        ...
        observations =  [\
                        ...
                        [pose.position.x - pose_neighbor.position.x, pose.position.y - pose_neighbor.position.y, pose.position.z - pose_neighbor.position.z, abs(float(pose.position.z)), \
    ...
    def _compute_reward(self, observations, done):
        ...
        if not done:
            ...
            [flag,error] = self.get_lsq_triangulation_error(observations,epsilon=5)
            # if flag:
