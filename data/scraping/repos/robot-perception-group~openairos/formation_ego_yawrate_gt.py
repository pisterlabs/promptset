...
from openai_ros.robot_envs import firefly_env, firefly_multiagent_gt_inf_env, firefly_singleagentcartesian_yawrate_env, firefly_ego_yawrate_env
...
class DeepFollowEnv(firefly_ego_yawrate_env.FireflyMultiAgentGTEnv):
    ...
    def _get_obs(self):
        ...
        for k in range(len(gt_joints)):
            try:
                ...
                self.gt[k] = np.array([trans[0],trans[1],trans[2]])
        ...
        '''Actor Root in Mav base link frame'''
        try:
        ...
        #print(np.linalg.norm(relative_vel))
        #print(np.linalg.norm(np.array([vel.twist.twist.linear.y - target.twist.twist.linear.x,vel.twist.twist.linear.x - target.twist.twist.linear.y,\
                        #-vel.twist.twist.linear.z - target.twist.twist.linear.z,1])))
        ...
        observations =  [delta_target1.pose.pose.position.x, delta_target1.pose.pose.position.y, delta_target1.pose.pose.position.z,\
                        relative_vel[0],relative_vel[1],relative_vel[2],np.cos(theta1), np.sin(theta1)]+\
