...
class MinecraftPolicy(nn.Module):
    ...
    def forward(self, ob, state_in, context):
        print("policy.py def forward")
        first = context["first"]
        print("policy.py def forward: img_preprocess")
        x = self.img_preprocess(ob["img"])
        x = self.img_process(x)
        print("policy.py def forward: after img_preprocess: ", x)

        if self.diff_obs_process:
            print("policy.py def forward: diff_obs_process")
            processed_obs = self.diff_obs_process(ob["diff_goal"])
            x = processed_obs + x

        if self.pre_lstm_ln is not None:
            print("policy.py def forward: pre_lstm_ln")
            x = self.pre_lstm_ln(x)

        if self.recurrent_layer is not None:
            print("policy.py def forward: recurrent_layer")
            x, state_out = self.recurrent_layer(x, first, state_in)
            print("policy.py def forward: after recurrent_layer x: ", x)
            #print("policy.py def forward: after recurrent_layer state_out: ", state_out)
        else:
            state_out = state_in
        ...
        print("policy.py def forward: len(pi_latent[0][0]): ", len(pi_latent[0][0]))
        print("policy.py def forward: pi_latent: ", pi_latent)
        
        #pi_latent =  th.tensor([.0,.0,.0,.0], device='cuda:0')
        #pi_latent = [0]
        #vf_latent = th.tensor([.0,.0,.0,.0], device='cuda:0')
        #state_out = th.tensor([.0,.0,.0,.0], device='cuda:0')
...
class MinecraftAgentPolicy(nn.Module):
    def __init__(self, action_space, policy_kwargs, pi_head_kwargs):
        ...
        print("policy.py MinecraftAgentPolicy")

        self.action_space = action_space
        print("policy.py action_space: ", action_space)
    ...
    def forward(self, obs, first: th.Tensor, state_in):
        ...
        print("policy.py MinecraftAgentPolicy def forward: ", pi_h)
    ...
    def get_logprob_of_action(self, pd, action):
        ...
        """
        print("policy.py MinecraftAgentPolicy: get_logprob_of_action: action:", action)
        print("policy.py MinecraftAgentPolicy: get_logprob_of_action: pd:", pd)
        ac = tree_map(lambda x: x.unsqueeze(1), action)
        print("policy.py MinecraftAgentPolicy: get_logprob_of_action: ac:", ac)
    ...
    def get_output_for_observation(self, obs, state_in, first):
        ...
        print("policy.py def get_out_for_obs before: self(obs=obs")
        print("policy.py self = MinecraftAgentPolicy")
