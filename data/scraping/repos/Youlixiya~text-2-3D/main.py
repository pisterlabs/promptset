...
from blip import load_blip, blip_generate_caption, clear_blip

# torch.autograd.set_detect_anomaly(True)
BLIP_MODEL_KEY = 'Salesforce/blip-image-captioning-base'
if __name__ == '__main__':
    ...
    if opt.blip:
        print(f'[INFO] {load_blip(BLIP_MODEL_KEY)}')
        opt.text = blip_generate_caption(Image.open(opt.image).convert('RGB'))
        print(f'[INFO] {clear_blip()}')
