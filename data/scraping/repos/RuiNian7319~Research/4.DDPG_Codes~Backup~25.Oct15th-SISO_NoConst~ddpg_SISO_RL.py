...
def train(sess, env, args, actor, critic, actor_noise):
    ...
    for i in range(int(args['max_episodes'])):
        ...
        for j in range(1, int(args['max_episode_len']) + 1):
            ...
            if j % int(args['sampling_time']) == 0 or j == 1:
                ...
                # Add Ornstein-Ulhenbeck exploration noise to the action
                    ...
                    actor_noise.noise_decay(int(args['max_episodes']) * int(args['max_episode_len']))
            ...
            env.x[j, :] = odeint(env.ode, env.x[j - 1, :], [env.t[j - 1], env.t[j]], args=([env.u[j, 0]], ))[-1]
...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        env = LinearSystem(nsim=args['max_episode_len'], model_type='SISO', x0=np.array([0.5]), u0=np.array([1]),
                           xs=np.array([5]), us=np.array([10]), step_size=0.2)
        ...
        actor_noise = OrnsteinUhlenbeckActionNoise(mu=np.zeros(action_dim), dt=env.step_size,
                                                   random_seed=int(args['random_seed']), sigma=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--delta-u', help='Penalize delta u to avoid oscillations', default=None)
