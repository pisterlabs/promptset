...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        ...
        # scaled_out = out
        shift = tf.constant([2], dtype=tf.float32)
...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        env = SIMOSystem(nsim=args['max_episode_len'], x0=np.array([2, 2/3]), u0=np.array([2]), xs=np.array([4, 4/3]),
                         us=np.array([4]), step_size=0.2, control=False, q_cost=1, r_cost=0.5, s_cost=0.1,
                         random_seed=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-episodes', help='max num of episodes to do while training', default=8001)
