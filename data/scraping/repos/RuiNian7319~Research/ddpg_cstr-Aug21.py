...
from priority_replay_buffer_v3_MaxHeap import ReplayBuffer
    ...
    for i in range(int(args['max_episodes'])):
        ...
        for j in range(1, int(args['max_episode_len']) + 1):
            ...
            if replay_buffer.size() > int(args['minibatch_size'] * 5):
                ...
                s_batch, a_batch, r_batch, t_batch, s2_batch, index_list = replay_buffer.priority_sample_batch(
                ...
                for k in range(int(mini_batch_size)):
                    td_error = abs((r_batch[k] + critic.gamma * critic.predict(
                        ...
                        actor.predict(np.reshape(s2_batch[k], (1, actor.s_dim))))) -
                                   critic.predict(np.reshape(s_batch[k], (1, critic.s_dim)),
                                                  np.reshape(a_batch[k], (1, actor.a_dim))))

                    # Update replay memory replay_buffer
                    index = index_list[k]
                    replay_buffer.buffer.heap[int(index)][-1][0] = td_error
