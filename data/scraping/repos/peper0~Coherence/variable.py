...
class StateVariable(log.Loggable):
    ...
    def update(self, value):
        self.info("variable check for update: %s=%s service: %s", self.name, value, self.service)
        ...
        if new_value == self.value:
            self.info("variable %s NOT updated, no value change (value=%s)", self.name, self.value)
            return
