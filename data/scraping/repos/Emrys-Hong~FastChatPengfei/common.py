...
openai.api_key="6c4effb959654461a5ce041edd96effe"
...
def run_judge_single(question, answer, judge, ref_answer, multi_turn=False):
    ...
    if model in ["gpt-3.5-turbo", "gpt-4"]:
        judgment = chat_compeletion_openai(model, conv, temperature=0, engine='GPT4', max_tokens=2048)
...
def run_judge_pair(question, answer_a, answer_b, judge, ref_answer, multi_turn=False):
    ...
    if model in ["gpt-3.5-turbo", "gpt-4"]:
        conv.system = system_prompt
        judgment = chat_compeletion_openai(model, conv, engine="GPT4", temperature=0, max_tokens=2048)
    elif model in ["claude-v1", "claude-instant-v1"]:
...
def chat_compeletion_openai(model, conv, engine, temperature, max_tokens):
